...
/**
   ...
   stats/achievements differently on Durango.
        ...
        Using getters like this means we can use different Stats easilly on
   different platforms and still have a convenient identifier to use to award
   them.
*/
class GenericStats {
...
protected:
    ...
    virtual byteArray getParam_itemsUsed(
        std::shared_ptr<Player>       plr,
        std::shared_ptr<ItemInstance> itm
    );
    virtual byteArray getParam_itemsBought(int id, int aux, int count);

    virtual byteArray getParam_mobKill(
        std::shared_ptr<Player> plr,
        std::shared_ptr<Mob>    mob,
        DamageSource*           dmgSrc
    );
