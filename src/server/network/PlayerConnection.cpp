...
#include "network/packet/ClientCommandPacket.h"
#include "network/packet/ContainerAckPacket.h"
#include "network/packet/ContainerButtonClickPacket.h"
#include "network/packet/ContainerClickPacket.h"
...
#include "network/packet/DebugOptionsPacket.h"
...
#include "network/packet/KickPlayerPacket.h"
...
#include "network/packet/PlayerCommandPacket.h"
#include "network/packet/PlayerInfoPacket.h"
#include "network/packet/PlayerInputPacket.h"
#include "network/packet/ServerSettingsChangedPacket.h"
#include "network/packet/SetCarriedItemPacket.h"
#include "network/packet/SetCreativeModeSlotPacket.h"
...
#include "network/packet/TradeItemPacket.h"
...
#include "world/inventory/InventoryMenu.h"
...
PlayerConnection::PlayerConnection(
    ...
    this->player = player;
    //	player->connection = this;		// 4J - moved out as we can't assign in
    // a ctor
...
void PlayerConnection::tick() {
    ...
    if ((tickCount - lastKeepAliveTick) > 20 * 1) {
        ...
        send(
            std::shared_ptr<KeepAlivePacket>(
    ...
    if (getWasKicked()) {
        server->getPlayers()->broadcastAll(
            std::shared_ptr<ChatPacket>(new ChatPacket(
                player->name,
                ChatPacket::e_ChatPlayerKickedFromGame
            ))
        );
    } else {
        server->getPlayers()->broadcastAll(
...
#endif
                    disconnect(
                        DisconnectPacket::eDisconnect_IllegalPosition
                    ); //"Nope!");
                    return;
                }
                // CraftBukkit end
...
//            " + player->z);
        ...
        if (!server->isFlightAllowed() && !player->gameMode->isCreative()
            && !level->containsAnyBlocks(testBox)
...
#ifndef _CONTENT_PACKAGE
                    wprintf(
                        L"%ls was kicked for floating too long!\n",
                        player->name.c_str()
                    );
#endif
...
void PlayerConnection::teleport(
    double x,
    double y,
    double z,
    float  yRot,
        ...
        player->connection->send(
            std::shared_ptr<MovePlayerPacket>(
                new MovePlayerPacket::
                    PosRot(x, y + 1.62f, y, z, yRot, xRot, false, false)
            )
        );
}
...
void PlayerConnection::handlePlayerAction(
    ...
    if (packet->action == PlayerActionPacket::START_DESTROY_BLOCK) {
        ...
        server->getPlayers()->prioritiseTileChanges(
            ...
            z,
            level->dimension->id
        ); // 4J added - make sure that the update packets for this get
           // prioritised over other general world updates
...
void PlayerConnection::onDisconnect(
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"Server received request for custom texture %ls\n",
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"Server received custom texture %ls\n",
            packet->textureName.c_str()
        );
#endif
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"Server received custom texture %ls and geometry\n",
            packet->textureName.c_str()
        );
#endif
        app.AddMemoryTextureFile(
...
#ifndef _CONTENT_PACKAGE
            wprintf(
                L"Adding skin boxes for skin id %X, box count %d\n",
                packet->dwSkinID,
                packet->dwBoxC
            );
#endif
...
void PlayerConnection::handleTextureReceived(const std::wstring& textureName) {
    ...
    // for the data
    auto it = find(
        m_texturesRequested.begin(),
        m_texturesRequested.end(),
        textureName
    );
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"Skin for server player %ls has changed to %ls (%d)\n",
            player->name.c_str(),
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"Cape for server player %ls has changed to %ls\n",
            player->name.c_str(),
    ...
    if (packet->action == ServerSettingsChangedPacket::HOST_IN_GAME_SETTINGS) {
        ...
        if ((networkPlayer != NULL && networkPlayer->IsHost())
            ...
            app.SetGameHostOption(
                ...
                app.GetGameHostOption(packet->data, eGameHostOption_FireSpreads)
            );
            app.SetGameHostOption(
                eGameHostOption_TNT,
    ...
    if (packet->action == ClientCommandPacket::PERFORM_RESPAWN) {
        if (player->wonGame) {
            player = server->getPlayers()->respawn(
                player,
                player->m_enteredEndExitPortal ? 0 : player->dimension,
                true
            );
        }
        ...
        //	{
        //		player.connection.disconnect("You have died. Game over, man,
        // it's game over!"); 		server.selfDestruct();
        //	}
        //	else
        //	{
...
void PlayerConnection::handleSetCreativeModeSlot(
    ...
    if (player->gameMode->isCreative()) {
        ...
        bool validSlot =
            (packet->slotNum >= InventoryMenu::CRAFT_SLOT_START
             && packet->slotNum
        ...
        bool validData = item == NULL
                      || (item->getAuxValue() >= 0 && item->count > 0
                          && item->count <= 64);
