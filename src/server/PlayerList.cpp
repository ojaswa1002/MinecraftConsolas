...
#include "server/level/EntityTracker.h"
#include "server/level/PlayerChunkMap.h"
#include "server/level/ServerChunkCache.h"
#include "server/level/ServerLevel.h"
#include "server/level/ServerPlayer.h"
#include "server/level/ServerPlayerGameMode.h"
#include "server/network/PendingConnection.h"
...
#include "world/level/storage/PlayerIO.h"
...
#include "platform/network/sony/NetworkPlayerSony.h"
#endif
...
void PlayerList::placeNewPlayer(
    ...
    playerConnection->send(
        std::shared_ptr<LoginPacket>(new LoginPacket(
            L"",
            player->entityId,
            level->getLevelData()->getGenerator(),
            level->getSeed(),
            player->gameMode->getGameModeForPlayer()->getId(),
            (byte)level->dimension->id,
            (byte)level->getMaxBuildHeight(),
            (byte)getMaxPlayers(),
            level->difficulty,
    ...
    playerConnection->send(
        std::shared_ptr<SetSpawnPositionPacket>(
    ...
    player->doTick(
        ...
        true,
        false
    ); // 4J - added - force sending of the nearest chunk before the player is
       // teleported, so we have somewhere to arrive on...
    playerConnection
...
void PlayerList::changeDimension(
    std::shared_ptr<ServerPlayer> player,
    ServerLevel*                  from
) {
...
void PlayerList::validatePlayerSpawnPosition(
    ...
    app.DebugPrintf(
        "New pos is %f, %f, %f in dimension %d\n",
        player->x,
        player->y,
        player->z,
        player->dimension
    ...
    app.DebugPrintf(
        "Final pos is %f, %f, %f in dimension %d\n",
        player->x,
        player->y,
        player->z,
        player->dimension
    ...
    if (level->dimension->id == -1 && player->y > 125) {
        ...
        if (bedPosition != NULL) {
            ...
            if (respawnPosition != NULL) {
                player->moveTo(
                    respawnPosition->x + 0.5f,
                    respawnPosition->y + 0.1f,
                    respawnPosition->z + 0.5f,
                    0,
                    0
                );
        ...
        app.DebugPrintf(
            "Updated pos is %f, %f, %f in dimension %d\n",
            player->x,
            player->y,
            player->z,
            player->dimension
...
void PlayerList::add(std::shared_ptr<ServerPlayer> player) {
    ...
    if (level->isAtLeastOnePlayerSleeping()) {
        ...
        player->connection->send(
            std::shared_ptr<ChatPacket>(new ChatPacket(
                firstSleepingPlayer->name,
                ChatPacket::e_ChatBedPlayerSleep
            ))
        );
    }
}
...
std::shared_ptr<ServerPlayer> PlayerList::getPlayerForLogin(
    ...
    std::shared_ptr<ServerPlayer> player =
        std::shared_ptr<ServerPlayer>(new ServerPlayer(
            server,
            server->getLevel(0),
            userName,
...
std::shared_ptr<ServerPlayer> PlayerList::respawn(
    ...
    bool isEmptying =
        (targetDimension != oldDimension); // We're not emptying this dimension
                                           // on this machine if this player is
                                           // going back into the same dimension
    ...
    //		(a) if isEmptying is true, then remove the player from the tracker,
    // and send "remove entity" packets for anything seen (this is the original
    // behaviour of the code) 		(b) if isEmptying is false, then we'll be
        ...
        if (respawnPosition != NULL) {
            ...
            player->connection->send(
                std::shared_ptr<GameEventPacket>(new GameEventPacket(
                    GameEventPacket::NO_RESPAWN_BED_AVAILABLE,
                    0
                ))
            );
        }
    ...
    player->connection->send(
        std::shared_ptr<RespawnPacket>(new RespawnPacket(
            (char)player->dimension,
            player->level->getSeed(),
            player->level->getMaxBuildHeight(),
            player->gameMode->getGameModeForPlayer(),
            level->difficulty,
            level->getLevelData()->getGenerator(),
            player->level->useNewSeaLevel(),
            player->entityId,
            level->getLevelData()->getXZSize(),
            level->getLevelData()->getHellScale()
        ))
...
void PlayerList::toggleDimension(
    std::shared_ptr<ServerPlayer> player,
    int                           targetDimension
) {
    ...
    player->connection->send(
        std::shared_ptr<RespawnPacket>(new RespawnPacket(
            (char)player->dimension,
            newLevel->getSeed(),
            newLevel->getMaxBuildHeight(),
            player->gameMode->getGameModeForPlayer(),
            newLevel->difficulty,
            newLevel->getLevelData()->getGenerator(),
            newLevel->useNewSeaLevel(),
            player->entityId,
...
std::shared_ptr<ServerPlayer> PlayerList::getPlayer(const std::wstring& name) {
    for (unsigned int i = 0; i < players.size(); i++) {
        ...
        if (p->name == name) // 4J - used to be case insensitive (using
                             // equalsIgnoreCase) - imagine we'll be shifting to
                             // XUIDs anyway
        {
            return p;
        }
...
void PlayerList::sendLevelInfo(
    ...
    if ((level->dimension->id == 0)
        ...
        player->connection->send(
            std::shared_ptr<XZPacket>(new XZPacket(
                XZPacket::STRONGHOLD,
                level->getLevelData()->getXStronghold(),
...
void PlayerList::updatePlayerGameMode(
    std::shared_ptr<ServerPlayer> newPlayer,
...
PlayerList::findAlivePlayerOnSystem(std::shared_ptr<ServerPlayer> player) {
...
void PlayerList::removePlayerFromReceiving(
    std::shared_ptr<ServerPlayer> player,
    ...
    auto it = find(
        receiveAllPlayers[dimIndex].begin(),
        receiveAllPlayers[dimIndex].end(),
        player
    );
        ...
        for (auto itP = players.begin(); itP != players.end(); ++itP) {
            ...
            if (checkingPlayer != NULL) {
                ...
                for (auto it = receiveAllPlayers[newPlayerDim].begin();
                     it != receiveAllPlayers[newPlayerDim].end();
                     ++it) {
...
void PlayerList::addPlayerToReceiving(std::shared_ptr<ServerPlayer> player) {
    ...
    if (thisPlayer == NULL) {
        ...
        for (auto it = receiveAllPlayers[playerDim].begin();
             it != receiveAllPlayers[playerDim].end();
             ++it) {
...
#ifndef _CONTENT_PACKAGE
        app.DebugPrintf(
            "Add: Adding player %ls as primary in dimension %d\n",
            player->name.c_str(),
            playerDim
        );
#endif
