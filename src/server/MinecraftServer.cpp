...
#include "server/level/DerivedServerLevel.h"
...
#include "util/java/InputOutputStream/FileOutputStream.h"
...
#include "world/level/storage/McRegionLevelStorage.h"
...
#include "Settings.h"
#ifdef _XBOX
#include "xbox/source files/xui/menu screens/debug/XUI_DebugSetCamera.h"
#endif
#include "platform/telemetry/TelemetryManager.h"
#include "ps3/ShutdownManager.h"
#include "server/commands/ServerCommandDispatcher.h"
...
bool MinecraftServer::initServer(
    std::int64_t         seed,
    ...
    app.DebugPrintf(
        "ServerSettings: game-type is %s\n",
        (app.GetGameHostOption(eGameHostOption_GameType) == 0) ? "Survival Mode"
                                                               : "Creative Mode"
    );
    app.DebugPrintf(
    ...
    setPvpAllowed(
        app.GetGameHostOption(eGameHostOption_PvP) > 0 ? true : false
    ); // settings->getBoolean(L"pvp", true);
    ...
    m_bLoaded = loadLevel(
        new McRegionLevelStorageSource(File(L".")),
        levelName,
        seed,
        pLevelType,
        initData
...
int MinecraftServer::runPostUpdate(void* lpParam) {
    ...
    do {
        ...
        if (server->m_postProcessRequests.size()) {
            ...
            request.chunkSource
                ->postProcess(request.chunkSource, request.x, request.z);
            PIXEndNamedEvent();
        } else {
            LeaveCriticalSection(&server->m_postProcessCS);
        }
...
bool MinecraftServer::loadLevel(
    ...
    ); // LevelSettings::GAMETYPE_SURVIVAL);
    ...
    if (initData->saveData != NULL) {
        ...
        ConsoleSaveFileOriginal oldFormatSave(
            ...
            initData->saveData->data,
            initData->saveData->fileSize,
            false,
            initData->savePlatform
        );
    ...
    for (unsigned int i = 0; i < levels.length; i++) {
        ...
        //        levels[i]));		// 4J - have moved this to the ServerLevel
        //        ctor so that it is set up in time for the first chunk to load,
        //        which might actually happen there
        ...
        app.DebugPrintf(
            "MinecraftServer::loadLevel - Difficulty = %d\n",
            levels[i]->difficulty
        );
        ...
        levels[i]->setSpawnSettings(
            settings->getBoolean(L"spawn-monsters", true),
            animals
        );
#endif
            ...
            for (int x = -r; x <= r && running; x += 16) {
                for (int z = -r; z <= r && running; z += 16) {
                    ...
                    level->cache->create(
                        ...
                        (spawnPos->z + z) >> 4,
                        true
                    ); // 4J - added parameter to disable postprocessing here
                    PIXEndNamedEvent();
    ...
    if (levels[0]->dimension->id == 0) {
        ...
        if (!levels[0]->getLevelData()->getHasStronghold()) {
            int x, z;
            if (app.GetTerrainFeaturePosition(
...
void MinecraftServer::stopServer() {
    ...
    if (ShutdownManager::ShouldRun(
        ...
        )) // This thread will take itself out if we are shutting down
#endif
    {
    ...
    if (initServer(seed, initData, initSettings, findSeed)) {
        ...
        while (running && !s_bServerHalted) {
            ...
            if (MinecraftServer::setTimeAtEndOfTick) {
                ...
                for (unsigned int i = 0; i < levels.length; i++) {
                    //					if (i == 0 ||
                    // settings->getBoolean(L"allow-nether", true))		// 4J
                    // removed - we always have nether
                    {
                        ServerLevel* level = levels[i];
            ...
            for (int i = 0; i < XUSER_MAX_COUNT; i++) {
...
#if defined(_XBOX_ONE) || defined(__ORBIS__)
                {
                    ...
                    for (unsigned int j = 0; j < levels.length; j++) {
                        ...
                        // overwrite the level.dat with the data from the
                        // nethers leveldata. Fix for #7418 - Functional:
                        // Gameplay: Saving after sleeping in a bed will place
                        // player at nighttime when restarting.
                        ServerLevel* level = levels[levels.length - 1 - j];
                        PIXBeginNamedEvent(
                            0,
                ...
                case eXuiServerAction_SpawnMob: {
                    ...
                    mob->setDespawnProtected(); // 4J added, default to being
                ...
                case eXuiServerAction_ExportSchematic:
                    ...
                    if (!s_bServerHalted) {
                        ConsoleSchematicFile::XboxSchematicInitParam* initData =
                            (ConsoleSchematicFile::XboxSchematicInitParam*)
                                param;
#ifdef _XBOX
                        File targetFileDir(
                            File::pathRoot + File::pathSeparator + L"Schematics"
                        );
#else
...
#ifndef _CONTENT_PACKAGE
                {
                    ...
                    // player->absMoveTo(pos->m_camX, pos->m_camY, pos->m_camZ,
...
void MinecraftServer::handleConsoleInput(
...
void MinecraftServer::handleConsoleInputs() {
    while (consoleInput.size() > 0) {
        ...
        //        commands->handleCommand(input);		// 4J - removed - TODO -
        //        do we want equivalent of console commands?
    }
}
