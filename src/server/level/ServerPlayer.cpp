...
#include "world/damagesource/EntityDamageSource.h"
...
#include "world/inventory/BrewingStandMenu.h"
#include "world/inventory/ContainerMenu.h"
#include "world/inventory/CraftingMenu.h"
#include "world/inventory/EnchantmentMenu.h"
#include "world/inventory/FurnaceMenu.h"
#include "world/inventory/MerchantContainer.h"
#include "world/inventory/MerchantMenu.h"
#include "world/inventory/RepairMenu.h"
#include "world/inventory/ResultSlot.h"
#include "world/inventory/Slot.h"
#include "world/inventory/TrapMenu.h"
#include "world/item/ComplexItem.h"
...
void ServerPlayer::flagEntitiesToBeRemoved(
    unsigned int* flags,
    bool*         removedFound
) {
        ...
        if (currentCarried != lastCarried[i]) {
            getLevel()->getTracker()->broadcast(
                ...
                std::shared_ptr<SetEquippedItemPacket>(
                    new SetEquippedItemPacket(this->entityId, i, currentCarried)
                )
            );
...
void ServerPlayer::doTick(
    bool sendChunks,
    bool dontDelayChunks /*=false*/,
...
void ServerPlayer::doTickA() {
    ...
    for (unsigned int i = 0; i < inventory->getContainerSize(); i++) {
        ...
        if (ie != NULL) {
            ...
            if (Item::items[ie->id]
                    ->isComplex()) // && connection->countDelayedPackets() <= 2)
            {
    ...
    if (!chunksToSend.empty()) {
        ...
        if (nearestValid) {
            ...
            if (connection->isLocal()) {
                ...
                //				app.DebugPrintf("%ls: canSendOnSlowQueue %d,
                // countDelayedPackets %d GetSendQueueSizeBytes %d done: %d",
                ...
                if (dontDelayChunks
...
#ifdef _XBOX_ONE
                        // The network manager on xbox one doesn't currently
                        // split data into slow & fast queues - since we can
                        // only measure both together then bytes provides a
                        // better metric than count of data items to determine
                        // if we should avoid queueing too much up
                        (g_NetworkManager.GetHostPlayer()
                             ->GetSendQueueSizeBytes(NULL, true)
                         < 8192)
                        &&
#else
                        (g_NetworkManager.GetHostPlayer()
                             ->GetSendQueueSizeMessages(NULL, true)
                         < 4)
                        &&
#endif
                    ...
                    //					static
                    // std::unordered_map<std::wstring,std::int64_t>
                    // mapLastTime;
                    //					std::int64_t thisTime =
                    // System::currentTimeMillis();
                    //					std::int64_t lastTime =
                    // mapLastTime[connection->getNetworkPlayer()->GetUID().toString()];
                    //					app.DebugPrintf(" - OK to send (%d ms
                    // since last)\n", thisTime - lastTime);
                    //					mapLastTime[connection->getNetworkPlayer()->GetUID().toString()]
                    //= thisTime;
                } else {
...
void ServerPlayer::doTickB(bool ignorePortal) {
    ...
    if (getHealth() != lastSentHealth || lastSentFood != foodData.getFoodLevel()
        || ((foodData.getSaturationLevel() == 0) != lastFoodSaturationZero)) {
        // 4J Stu - Added m_lastDamageSource for telemetry
        connection->send(
            std::shared_ptr<SetHealthPacket>(new SetHealthPacket(
                getHealth(),
...
void ServerPlayer::
    broadcast(std::shared_ptr<TileEntity> te, bool delay /*= false*/) {
    if (te != NULL) {
    ...
    if (!e->removed) {
        ...
        if (e->GetType() == eTYPE_ITEMENTITY) {
            entityTracker->broadcast(
                e,
                std::shared_ptr<TakeItemEntityPacket>(
...
ServerPlayer::startSleepInBed(int x, int y, int z, bool bTestUse) {
        ...
        std::shared_ptr<Packet> p =
            std::shared_ptr<EntityActionAtPositionPacket>(
                new EntityActionAtPositionPacket(
                    shared_from_this(),
                    EntityActionAtPositionPacket::START_SLEEP,
                    x,
                    y,
                    z
                )
...
bool ServerPlayer::startRepairing(int x, int y, int z) {
    if (containerMenu == inventoryMenu) {
        ...
        connection->send(
            std::shared_ptr<ContainerOpenPacket>(new ContainerOpenPacket(
                containerCounter,
                ContainerOpenPacket::REPAIR_TABLE,
                0,
                9
            ))
        );
...
bool ServerPlayer::openBrewingStand(
    ...
    if (containerMenu == inventoryMenu) {
        ...
        connection->send(
            std::shared_ptr<ContainerOpenPacket>(new ContainerOpenPacket(
                containerCounter,
                ContainerOpenPacket::BREWING_STAND,
                0,
...
bool ServerPlayer::openTrading(std::shared_ptr<Merchant> traderTarget) {
    if (containerMenu == inventoryMenu) {
        ...
        containerMenu->containerId = containerCounter;
        containerMenu->addSlotListener(this);
        auto container = ((MerchantMenu*)containerMenu)->getTradeContainer();

        connection->send(
            std::shared_ptr<ContainerOpenPacket>(new ContainerOpenPacket(
                containerCounter,
                ContainerOpenPacket::TRADER_NPC,
                container->getName(),
                container->getContainerSize()
            ))
        );
        ...
        if (offers != NULL) {
            ...
            connection->send(
                std::shared_ptr<CustomPayloadPacket>(new CustomPayloadPacket(
                    CustomPayloadPacket::TRADER_LIST_PACKET,
...
void ServerPlayer::refreshContainer(
    ...
    connection->send(
        std::shared_ptr<ContainerSetContentPacket>(
...
void ServerPlayer::setContainerData(
    ...
    connection->send(
        std::shared_ptr<ContainerSetDataPacket>(
...
void ServerPlayer::setPlayerInput(
    float xa,
    float ya,
    bool  jumping,
    bool  sneaking,
...
void ServerPlayer::awardStat(Stat* stat, byteArray param) {
    ...
    if (!stat->awardLocallyOnly) {
        ...
        while (count > 100) {
            connection->send(
                std::shared_ptr<AwardStatPacket>(
            ...
            if (shared_from_this() != player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(
                        new ChatPacket(name, ChatPacket::e_ChatBedPlayerSleep)
                    )
                );
            } else {
                player->connection->send(
    ...
    case IDS_PLAYER_ENTERED_END:
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() != player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(
                        new ChatPacket(name, ChatPacket::e_ChatPlayerEnteredEnd)
                    )
                );
            }
        }
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxPigsSheepCows
                    ))
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxChickens
                    ))
                );
            }
        }
    ...
    case IDS_MAX_SQUID_SPAWNED:
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(
                        new ChatPacket(name, ChatPacket::e_ChatPlayerMaxSquid)
                    )
                );
            }
        }
    ...
    case IDS_MAX_WOLVES_SPAWNED:
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(
                        new ChatPacket(name, ChatPacket::e_ChatPlayerMaxWolves)
                    )
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxMooshrooms
                    ))
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxVillagers
                    ))
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxBredChickens
                    ))
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxBredMooshrooms
                    ))
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxBredWolves
                    ))
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerCantShearMooshroom
                    ))
                );
            }
        }
        ...
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(new ChatPacket(
                        name,
                        ChatPacket::e_ChatPlayerMaxHangingEntities
                    ))
                );
            }
        }
    ...
    case IDS_MAX_BOATS:
        for (unsigned int i = 0; i < server->getPlayers()->players.size();
            ...
            if (shared_from_this() == player) {
                player->connection->send(
                    std::shared_ptr<ChatPacket>(
                        new ChatPacket(name, ChatPacket::e_ChatPlayerMaxBoats)
                    )
                );
            }
        }
...
void ServerPlayer::startUsingItem(
...
void ServerPlayer::restoreFrom(
    std::shared_ptr<Player> oldPlayer,
    bool                    restoreAll
) {
...
void ServerPlayer::onEffectRemoved(MobEffectInstance* effect) {
    ...
    connection->send(
        std::shared_ptr<RemoveMobEffectPacket>(
...
void ServerPlayer::crit(std::shared_ptr<Entity> entity) {
    getLevel()->getTracker()->broadcastAndSend(
        shared_from_this(),
        std::shared_ptr<AnimatePacket>(
            new AnimatePacket(entity, AnimatePacket::CRITICAL_HIT)
        )
    );
}
...
void ServerPlayer::magicCrit(std::shared_ptr<Entity> entity) {
    getLevel()->getTracker()->broadcastAndSend(
        shared_from_this(),
        std::shared_ptr<AnimatePacket>(
            new AnimatePacket(entity, AnimatePacket::MAGIC_CRITICAL_HIT)
        )
    );
}
...
void ServerPlayer::onUpdateAbilities() {
    ...
    connection->send(
        std::shared_ptr<PlayerAbilitiesPacket>(
...
int ServerPlayer::getFlagIndexForChunk(const ChunkPos& pos, int dimension) {
    ...
    return LevelRenderer::getGlobalIndexForChunk(
               pos.x * 16,
               0,
               pos.z * 16,
               dimension
...
#ifndef _CONTENT_PACKAGE
void ServerPlayer::debug_setPosition(
    double x,
    double y,
    double z,
    double nYRot,
