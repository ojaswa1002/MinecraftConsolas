...
) // 4J - added extra parameter
{
    ...
    if ((chunk == NULL) || (chunk->x != x) || (chunk->z != z)) {
...
#endif // _DURANGO
        {
            ...
            // same) oxooo	ooooo	oooxo	oxPxo	ooxoo ooooo	ooooo	ooooo
            // ooxoo	ooooo
            ...
            if (hasChunk(x, z - 1) && hasChunk(x, z - 2)
                && hasChunk(x - 1, z - 1) && hasChunk(x + 1, z - 1))
                chunk->checkChests(this, x, z - 1);
            if (hasChunk(x - 1, z) && hasChunk(x + 1, z) && hasChunk(x, z - 1)
                && hasChunk(x, z + 1))
...
void ServerChunkCache::dontDrop(int x, int z) {
    ...
    m_toDrop.erase(
        std::remove(m_toDrop.begin(), m_toDrop.end(), chunk),
        m_toDrop.end()
    );
}
#endif
                    ...
                    if (chunk->shouldSave(force)
                        ...
                        if (saveThreads[j] == NULL) {
                            ...
                            if (j == 0)
                                ...
                                ); // XSetThreadProcessor( saveThreads[j], 1);
                            else if (j == 1)
                                saveThreads[j]->SetProcessor(
                                    CPU_CORE_SAVE_THREAD_B
                                ); // XSetThreadProcessor( saveThreads[j], 3);
                            else if (j == 2)
                                saveThreads[j]->SetProcessor(
                                    CPU_CORE_SAVE_THREAD_C
                                ); // XSetThreadProcessor( saveThreads[j], 5);
...
ServerChunkCache::getMobsAt(MobCategory* mobCategory, int x, int y, int z) {
...
int ServerChunkCache::runSaveThreadProc(LPVOID lpParam) {
    ...
    params->wakeEvent->WaitForSignal(
        INFINITE
    ); // WaitForSingleObject(params->wakeEvent,INFINITE);
