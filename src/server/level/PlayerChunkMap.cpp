...
void PlayerChunkMap::flagEntitiesToBeRemoved(
    unsigned int* flags,
...
void PlayerChunkMap::PlayerChunk::
    add(std::shared_ptr<ServerPlayer> player, bool sendPacket /*= true*/) {
    ...
    if (find(players.begin(), players.end(), player) != players.end()) {
        ...
        // assert(false);
        // 4J - was			throw new IllegalStateException("Failed to add
        // player. " + player + " already is in chunk " + x + ", " + z);
    }
    ...
    if (sendPacket)
        player->connection->send(
            std::shared_ptr<ChunkVisibilityPacket>(
...
void PlayerChunkMap::PlayerChunk::remove(std::shared_ptr<ServerPlayer> player) {
    ...
    if (players.size() == 0) {
        std::int64_t id =
            (pos.x + 0x7fffffffLL) | ((pos.z + 0x7fffffffLL) << 32);
...
void PlayerChunkMap::PlayerChunk::broadcast(std::shared_ptr<Packet> packet) {
    ...
    for (int i = 0;
         i < parent->level->getServer()->getPlayers()->players.size();
         i++) {
...
PlayerChunkMap::getChunk(int x, int z, bool create) {
    std::int64_t id = (x + 0x7fffffffLL) | ((z + 0x7fffffffLL) << 32);
...
void PlayerChunkMap::broadcastTileUpdate(
...
void PlayerChunkMap::add(std::shared_ptr<ServerPlayer> player) {
    ...
    player->connection->send(
        std::shared_ptr<ChunkVisibilityAreaPacket>(
...
bool PlayerChunkMap::isPlayerIn(
    std::shared_ptr<ServerPlayer> player,
    int                           xChunk,
    int                           zChunk
) {
    ...
    if (chunk == NULL) {
        ...
        auto it2 = find(
            player->chunksToSend.begin(),
            player->chunksToSend.end(),
            chunk->pos
        );
