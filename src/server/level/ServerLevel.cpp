...
#include "world/entity/npc/Npc.h"
...
#include "world/level/levelgen/feature/BonusChestFeature.h"
...
void ServerLevel::staticCtor() {
    ...
    ); // On Orbis, this core is also used for Matching 2, and that priority of
       // that seems to be always at default no matter what we set it to.
       // Prioritise this below Matching 2.
#endif
    m_updateThread->Run();
...
: Level(
    ...
    server->setLevel(
        dimension,
        this
    ); // The listener needs the server to have the level set up... this will be
       // set up anyway on return of this ctor but setting up early here
...
ServerLevel::~ServerLevel() {
    ...
    for (auto it = m_queuedSendTileUpdates.begin();
         it != m_queuedSendTileUpdates.end();
         ++it) {
        Pos* p = *it;
        delete p;
    }
    ...
    delete this->tracker; // MGH - added, we were losing about 500K going in and
                          // out the menus
...
void ServerLevel::tick() {
    ...
    if (allPlayersAreSleeping()) {
        ...
        if (!somebodyWokeUp) {
            ...
            std::int64_t newTime = levelData->getTime() + TICKS_PER_DAY;
    ...
    // for Minecraft 1.8, spawn friendlies really rarely	- 4J - altered from
    // once every 400 ticks to 40 ticks as we depend on this a more than the
    // original since we don't have chunk post-process spawning
    MobSpawner::tick(
        this,
        spawnEnemies,
...
Biome::MobSpawnerData* ServerLevel::getRandomMobSpawnAt(
    ...
    return (Biome::MobSpawnerData*)WeighedRandom::getRandomItem(
        random,
        (std::vector<WeighedRandomItem*>*)mobList
    );
}
        ...
        if (random->nextInt(16) == 0) {
            ...
            if (isRaining()) {
                ...
                if (b->hasRain()) {
                    ...
                    if (tile != 0) {
                        Tile::tiles[tile]
                            ->handleRain(this, x + xo, yy - 1, z + zo);
                    }
                }
            }
        }
...
ServerLevel::fetchTicksInChunk(LevelChunk* chunk, bool remove) {
    EnterCriticalSection(&m_tickNextTickCS);
    ...
    for (auto it = tickNextTickSet.begin(); it != tickNextTickSet.end();) {
...
void ServerLevel::tick(std::shared_ptr<Entity> e, bool actual) {
    ...
    if (e->rider.lock() == NULL
        || (dynamic_pointer_cast<Player>(e->rider.lock())
...
std::shared_ptr<Explosion> ServerLevel::explode(
    ...
    for (auto it = players.begin(); it != players.end(); ++it) {
        ...
        if (player->distanceToSqr(x, y, z) < 64 * 64) {
            ...
            player->connection->send(
                std::shared_ptr<ExplodePacket>(new ExplodePacket(
                    x,
                    y,
                    z,
                    r,
...
void ServerLevel::tileEvent(int x, int y, int z, int tile, int b0, int b1) {
    ...
    for (auto it = tileEvents[activeTileEventsList].begin();
         it != tileEvents[activeTileEventsList].end();
         ++it) {
...
void ServerLevel::setTimeAndAdjustTileTicks(std::int64_t newTime) {
    std::int64_t delta = newTime - levelData->getTime();
...
void ServerLevel::entityRemovedExtra(std::shared_ptr<Entity> e) {
    if (dynamic_pointer_cast<ItemEntity>(e) != NULL) {
        ...
        //%d\n",m_arrowEntities.size());
        auto it = find(
            m_experienceOrbEntities.begin(),
            m_experienceOrbEntities.end(),
            e
        );
...
int ServerLevel::runUpdate(void* lpParam) {
    ShutdownManager::HasStarted(
        ShutdownManager::eRunUpdateThread,
        m_updateTrigger
    );
        ...
        for (unsigned int iLev = 0; iLev < 3; ++iLev) {
            ...
            for (int i = 0; i < m_updateChunkCount[iLev]; i++) {
                ...
                for (int j = 0; j < 80; j++) {
                    ...
                    if ((id == Tile::grass_Id && grassTicks >= MAX_GRASS_TICKS)
                        || (id == Tile::calmLava_Id
