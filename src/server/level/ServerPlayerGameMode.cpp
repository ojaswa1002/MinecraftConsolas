...
void ServerPlayerGameMode::tick() {
    ...
    if (hasDelayedDestroy) {
        ...
        if (t == 0) {
            ...
            float destroyProgress = tile->getDestroyProgress(
                                        player,
                                        player->level,
                                        delayedDestroyX,
                                        delayedDestroyY,
...
void ServerPlayerGameMode::stopDestroyBlock(int x, int y, int z) {
    if (x == xDestroyBlock && y == yDestroyBlock && z == zDestroyBlock) {
        ...
        if (t != 0) {
            ...
            //			fixes issues with this failing to destroy because of
            // packets bunching up
            ...
            if (clientLevel) {
                ...
                if (lc->sharingTilesAndData) {
                    ...
                    Minecraft::GetInstance()
                        ->levelRenderer->destroyedTileManager
                        ->destroyingTileAt(clientLevel, x, y, z);
...
bool ServerPlayerGameMode::useItem(
...
bool ServerPlayerGameMode::useItemOn(
    std::shared_ptr<Player>       player,
    Level*                        level,
    std::shared_ptr<ItemInstance> item,
    int                           x,
