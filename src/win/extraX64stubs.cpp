...
#include "ps3/source files/sentient/SentientManager.h"
#include "ps3/source files/social/SocialManager.h"
#include "stats/StatsCounter.h"
#elif defined _DURANGO
#include "stats/StatsCounter.h"

#include "Durango\DurangoExtras\xcompress.h"
#include "durango/source files/sentient/DynamicConfigurations.h"
#include "durango/source files/sentient/SentientManager.h"
#include "durango/source files/social/SocialManager.h"
#elif defined _WINDOWS64
#include "win/sentient/DynamicConfigurations.h"
#include "win/sentient/SentientManager.h"
...
#include "psvita/source files/sentient/DynamicConfigurations.h"
#include "psvita/source files/sentient/SentientManager.h"
#include "psvita/source files/social/SocialManager.h"
#else
#include <perf.h>

#include "stats/StatsCounter.h"

#include "orbis/source files/sentient/DynamicConfigurations.h"
#include "orbis/source files/sentient/SentientManager.h"
#include "orbis/source files/social/SocialManager.h"
#endif
...
void IQNetPlayer::SendData(
    ...
    app.DebugPrintf(
        "Sending from 0x%x to 0x%x %d bytes\n",
        this,
        player,
        dwDataSize
    );
...
// #ifndef __PS3__
#if !(                                                                         \
    defined _DURANGO || defined __PS3__ || defined __ORBIS__                   \
    || defined                                     __PSVITA__                  \
)
...
void C_4JProfile::SetTrialAwardText(
...
UINT C_4JProfile::RequestSignInUI(
    bool bFromInvite,
    bool bLocalGame,
    bool bNoGuestsAllowed,
    bool bMultiplayerSignIn,
    bool bAddUser,
...
UINT C_4JProfile::DisplayOfflineProfile(
    int (*Func)(LPVOID, const bool, const int iPad),
    LPVOID lpParam,
    int    iQuadrant
) {
...
UINT C_4JProfile::RequestConvertOfflineToGuestUI(
    int (*Func)(LPVOID, const bool, const int iPad),
    LPVOID lpParam,
    int    iQuadrant
) {
...
bool C_4JProfile::GetChatAndContentRestrictions(
    int   iPad,
...
void C_4JProfile::AllowedPlayerCreatedContent(
    int   iPad,
    bool  thisQuadrantOnly,
    BOOL* allAllowed,
    BOOL* friendsAllowed
) {}
BOOL C_4JProfile::CanViewPlayerCreatedContent(
    int        iPad,
    bool       thisQuadrantOnly,
    PPlayerUID pXuids,
    DWORD      dwXuidCount
...
bool C_4JProfile::GetProfileAvatar(
    int iPad,
...
void C_4JProfile::SetSignInChangeCallback(
    void (*Func)(LPVOID, bool, unsigned int),
    LPVOID lpParam
) {}
void C_4JProfile::SetNotificationsCallback(
    void (*Func)(LPVOID, DWORD, unsigned int),
    LPVOID lpParam
) {}
...
int (*defaultOptionsCallback)(
    LPVOID,
    C_4JProfile::PROFILESETTINGS*,
    const int iPad
)                     = NULL;
...
C_4JProfile::PROFILESETTINGS*
C_4JProfile::GetDashboardProfileSettings(int iPad) {
...
void C_4JProfile::SetRichPresenceContextValue(
    int iPad,
    int iContextID,
    int iVal
) {}
void C_4JProfile::SetCurrentGameActivity(
    int  iPad,
    int  iNewPresence,
    bool bSetOthersToIdle
) {}
void C_4JProfile::DisplayFullVersionPurchase(
    bool bRequired,
    int  iQuadrant,
    int  iUpsellParam
) {}
...
BOOL CSentientManager::RecordLevelStart(
    DWORD              dwUserId,
...
BOOL CSentientManager::RecordLevelExit(
    DWORD                dwUserId,
    ESen_LevelExitStatus levelExitStatus
) {
    return true;
}
BOOL CSentientManager::RecordLevelSaveOrCheckpoint(
    DWORD dwUserId,
...
BOOL CSentientManager::RecordLevelResume(
    DWORD              dwUserId,
    ESen_FriendOrMatch friendsOrMatch,
    ESen_CompeteOrCoop competeOrCoop,
    int                difficulty,
...
BOOL CSentientManager::RecordMenuShown(
    DWORD dwUserId,
    INT   menuID,
...
BOOL CSentientManager::RecordUpsellPresented(
    DWORD         dwUserId,
...
BOOL CSentientManager::RecordUpsellResponded(
    DWORD              dwUserId,
    ESen_UpsellID      upsellId,
    INT                marketplaceOfferID,
