...
typedef enum gdraw_d3d11_resourcetype {
    ...
    GDRAW_D3D11_RESOURCE_dynbuffer, // Streaming buffer for dynamic vertex/index
                                    // data (handle count ignored)

    GDRAW_D3D11_RESOURCE__count,
} gdraw_d3d11_resourcetype;

IDOC extern int gdraw_D3D11_SetResourceLimits(
    gdraw_d3d11_resourcetype type,
    S32                      num_handles,
    S32                      num_bytes
);
...
IDOC extern GDrawFunctions* gdraw_D3D11_CreateContext(
    ID3D11Device*        dev,
...
IDOC extern void
gdraw_D3D11_SetErrorHandler(void(__cdecl* error_handler)(HRESULT hr));
...
IDOC extern void gdraw_D3D11_SetTileOrigin(
    ID3D11RenderTargetView*   main_rt,
    ID3D11DepthStencilView*   main_ds,
    ID3D11ShaderResourceView* non_msaa_rt,
    S32                       x,
    S32                       y
);
...
gdraw_D3D11_WrappedTextureCreate(ID3D11ShaderResourceView* tex_view);
