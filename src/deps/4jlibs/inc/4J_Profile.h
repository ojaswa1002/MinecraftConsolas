...
public:
    ...
    void SetTrialTextStringTable(
        CXuiStringTable* pStringTable,
        int              iAccept,
        int              iReject
    );
    void SetTrialAwardText(
        eAwardType AwardType,
        int        iTitle,
        int        iText
    ); // achievement popup in the trial game
    ...
    bool GetChatAndContentRestrictions(
        int   iPad,
    ...
    void AllowedPlayerCreatedContent(
        int   iPad,
        bool  thisQuadrantOnly,
        BOOL* allAllowed,
        BOOL* friendsAllowed
    );
    BOOL CanViewPlayerCreatedContent(
        int        iPad,
        bool       thisQuadrantOnly,
        PPlayerUID pXuids,
        DWORD      dwXuidCount
    ...
    void RegisterAward(
        ...
        CXuiStringTable* pStringTable         = NULL,
        int              iTitleStr            = -1,
        int              iTextStr             = -1,
        int              iAcceptStr           = -1,
    ...
    // PURCHASE
    void DisplayFullVersionPurchase(
        bool bRequired,
        int  iQuadrant,
        int  iUpsellParam = -1
    );
    ...
    void SetDebugFullOverride(
        bool bVal
    ); // To override the license version (trail/full). Only in debug/release,
       // not ContentPackage
};
