...
public:
    ...
    unsigned int
         GetValue(int iPad, unsigned char ucAction, bool bRepeat = false);
    ...
    // SouthPaw, etc
    void
    SetJoypadStickAxisMap(int iPad, unsigned int uiFrom, unsigned int uiTo);
    void
    SetJoypadStickTriggerMap(int iPad, unsigned int uiFrom, unsigned int uiTo);
    void SetKeyRepeatRate(float fRepeatDelaySecs, float fRepeatRateSecs);
    void SetDebugSequence(
        const char* chSequenceA,
        int (*Func)(LPVOID),
        LPVOID lpParam
    );
    ...
    // *pStringTable=NULL);
    EKeyboardResult RequestKeyboard(
        LPCWSTR Title,
        LPCWSTR Text,
        DWORD   dwPad,
        UINT    uiMaxChars,
    ...
    void GetText(std::uint16_t* UTF16String);
    ...
    //
    // 		Remarks
    // 		This requirement applies to any player-entered string that can be
    // exposed to other players on Xbox LIVE. It includes session names, content
    // descriptions, text messages, tags, team names, mottos, comments, and so
    // on.
    //
    // 		Games may decide to not send the text, blank it out, or use generic
    // text if the text was rejected by the Xbox LIVE service.
    //
    // 		Games verify the text by calling the XStringVerify function.
    //
    // 		Exemption It is not required to use the Xbox LIVE service to verify
    // real-time text communication. An example of real-time text communication
    // is in-game text chat.
    //
