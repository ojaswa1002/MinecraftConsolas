...
public:
    ...
    void SetState(
        ESaveGameControlState eControlState,
        int (*Func)(LPVOID, const bool),
        LPVOID lpParam
    );
    ...
    void         SetSaveImages(
            ...
            ); // Sets the thumbnail & image for the save, optionally setting the
               // metadata in the png
    C4JStorage::ESaveGameState
         SaveSaveData(int (*Func)(LPVOID, const bool), LPVOID lpParam);
    void CopySaveDataToNewSave(
        PBYTE  pbThumbnail,
        DWORD  cbThumbnail,
        WCHAR* wchNewName,
        int (*Func)(LPVOID lpParam, bool),
        LPVOID lpParam
    );
    ...
    void SetSaveMessageVPosition(
        float fY
    ); // The 'Saving' message will display at a default position unless changed
    ...
    C4JStorage::ESaveGameState LoadSaveDataThumbnail(
        ...
        LPVOID lpParam
    ); // Get the thumbnail for an individual save referenced by pSaveInfo
    ...
    void RegisterMarketplaceCountsCallback(
        int (*Func)(LPVOID lpParam, C4JStorage::DLC_TMS_DETAILS*, int),
        LPVOID lpParam
    );
    ...
    DWORD                           InstallOffer(
                                  ...
                                  int (*Func)(LPVOID, int, int),
                                  LPVOID lpParam,
                                  bool   bTrial = false
                              );
    DWORD GetAvailableDLCCount(int iPad);

    C4JStorage::EDLCStatus
    GetInstalledDLC(int iPad, int (*Func)(LPVOID, int, int), LPVOID lpParam);
    XCONTENT_DATA& GetDLC(DWORD dw);
    DWORD          MountInstalledDLC(
                 int   iPad,
                 DWORD dwDLC,
                 int (*Func)(LPVOID, int, DWORD, DWORD),
                 LPVOID lpParam,
                 LPCSTR szMountDrive = NULL
             );
    DWORD UnmountInstalledDLC(LPCSTR szMountDrive = NULL);
    void  GetMountedDLCFileList(
    ...
    C4JStorage::ETMSStatus ReadTMSFile(
        ...
        BYTE**                    ppBuffer,
        DWORD*                    pdwBufferSize,
        int (*Func)(LPVOID, WCHAR*, int, bool, int) = NULL,
        LPVOID lpParam                              = NULL,
        int    iAction                              = 0
    );
    bool WriteTMSFile(
    ...
    C4JStorage::ETMSStatus TMSPP_ReadFile(
        ...
        LPCSTR                       szFilename,
        int (*Func)(LPVOID, int, int, PTMSPP_FILEDATA, LPCSTR) = NULL,
        LPVOID lpParam                                         = NULL,
        int    iUserData                                       = 0
    );
    ...
    // iUserData=0); 	C4JStorage::ETMSStatus				TMSPP_DeleteFile(int
    // iPad,LPCSTR szFilePath,C4JStorage::eTMS_FILETYPEVAL eFileTypeVal,int(
    // *Func)(LPVOID,int,int),LPVOID lpParam=NULL, int iUserData=0); 	bool
