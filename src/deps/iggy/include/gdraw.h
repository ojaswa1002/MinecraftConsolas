...
IDOC typedef enum gdraw_bformat {
    ...
    GDRAW_BFORMAT_vbib_single_format, // Platform uses vertex and index buffers,
                                      // but doesn't support multiple vertex
                                      // formats in a single VB
...
IDOC typedef struct GDrawInfo {
    ...
    rrbool always_mipmap; // if GDraw can generate mipmaps nearly for free, then
                          // set this flag
...
typedef struct GDrawRenderState {
    ...
    U8 stencil_set; // "Set" these stencil bits (note that actual implementation
                    // initializes stencil to 1, and "set" makes them 0)

    U8 reserved[2];  // Currently unused (used to make padding to 4/8-byte
                     // boundary for following pointer explicit)
    S32 blur_passes; // For filters that include blurring, this is the number of
                     // box filter passes to run
                     // align 0 mod 8
    ...
    // 0 mod 8
    gswf_recti scissor_rect; // The rectangle to which rendering will be clipped
                             // if $(GDrawRenderState::scissor) is set
                             // 0 mod 8
    ...
    F32 focal_point[4]; // Data used for
                        ...
                        // 0 mod 8
    F32 blur_x, blur_y; // The size of the box filter, where '1' is the identity
                        // and 2 adds half a pixel on each side
                        // 0 mod 8
...
IDOC typedef void RADLINK gdraw_set_view_size_and_world_scale(
    S32 w,
    S32 h,
    F32 x_world_to_pixel,
    F32 y_world_to_pixel
);
...
IDOC typedef void RADLINK gdraw_render_tile_begin(
    S32         tx0,
    S32         ty0,
    S32         tx1,
    S32         ty1,
...
IDOC typedef void RADLINK gdraw_filter_quad(
    GDrawRenderState* r,
    S32               x0,
    S32               y0,
    S32               x1,
...
IDOC typedef struct GDrawPrimitive {
    F32* vertices; // Pointer to an array of $gswf_vertex_xy, $gswf_vertex_xyst,
                   // or $gswf_vertex_xyoffs
...
IDOC typedef void RADLINK gdraw_draw_indexed_triangles(
    GDrawRenderState*  r,
    GDrawPrimitive*    prim,
...
IDOC typedef enum gdraw_texture_format {
    ...
    GDRAW_TEXTURE_FORMAT_rgba32, // 32bpp RGBA data in platform-preferred byte
                                 // order (returned by $gdraw_make_texture_begin
                                 // as $gdraw_texture_type)
...
IDOC typedef void RADLINK gdraw_set_texture_unique_id(
    GDrawTexture* tex,
    void*         old_unique_id,
    void*         new_unique_id
);
...
IDOC typedef rrbool RADLINK gdraw_make_texture_begin(
    ...
    GDraw_MakeTexture_ProcessingInfo* output_info,
    GDrawStats*                       stats
);
...
IDOC typedef GDrawTexture* RADLINK gdraw_make_texture_end(
    GDraw_MakeTexture_ProcessingInfo* info,
    GDrawStats*                       stats
);
...
IDOC typedef rrbool RADLINK gdraw_update_texture_begin(
    GDrawTexture* tex,
    void*         unique_id,
    GDrawStats*   stats
);
...
gdraw_update_texture_end(GDrawTexture* tex, void* unique_id, GDrawStats* stats);
...
IDOC typedef enum gdraw_vformat {
    GDRAW_vformat_v2, // Indicates vertices of type $gswf_vertex_xy (8 bytes per
                      // vertex)
...
IDOC typedef rrbool RADLINK gdraw_make_vertex_buffer_begin(
    void*                                  unique_id,
    gdraw_vformat                          vformat,
    S32                                    vdata_len_in_bytes,
    S32                                    idata_len_in_bytes,
    GDraw_MakeVertexBuffer_ProcessingInfo* info,
    GDrawStats*                            stats
);
...
IDOC typedef void RADLINK gdraw_describe_vertex_buffer(
    GDrawVertexBuffer*              buffer,
    GDraw_VertexBuffer_Description* desc
);
/* Returns a description for a given GDrawVertexBuffer */


IDOC typedef rrbool RADLINK gdraw_try_to_lock_texture(
    GDrawTexture* tex,
    void*         unique_id,
    GDrawStats*   stats
);
...
IDOC typedef void RADLINK gdraw_free_vertex_buffer(
    GDrawVertexBuffer* vb,
    void*              unique_id,
    GDrawStats*        stats
);
...
IDOC typedef void RADLINK
gdraw_free_texture(GDrawTexture* t, void* unique_id, GDrawStats* stats);
...
IDOC typedef rrbool RADLINK gdraw_texture_draw_buffer_begin(
    gswf_recti*                   region,
    gdraw_texture_format          format,
    gdraw_texturedrawbuffer_flags flags,
    void*                         unique_id,
    GDrawStats*                   stats
);
...
IDOC typedef void RADLINK
gdraw_draw_mask_begin(gswf_recti* region, S32 mask_bit, GDrawStats* stats);
...
IDOC typedef void RADLINK
gdraw_draw_mask_end(gswf_recti* region, S32 mask_bit, GDrawStats* stats);
