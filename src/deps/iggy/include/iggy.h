...
typedef enum IggyResult {
    ...
    IGGY_RESULT_Warning_CannotSustainFrameRate =
        201, // During a call to $IggyPlayerReadyToTick, Iggy detected that its
             // rendering of a Flash file was not keeping up with the frame rate
             // requested.
    ...
    IGGY_RESULT_Error_OutOfMemory =
        1001, // Iggy ran out of memory while processing the SWF. The Iggy
              ...
              // it (except read AS3 variables). Should this happen, you'll want
              // to $IggyPlayerDestroy and reopen the $Iggy.
} IggyResult;
...
typedef enum IggyDatatype {
    ...
    IGGY_DATATYPE_object, // Data of type Object (or a subclass) in AS3 (appears
                          // in datatype query, never as arguments)
    ...
    IGGY_DATATYPE_qname,     // Data of type QName in AS3 (appears in datatype
                             // query, never as arguments)
    IGGY_DATATYPE_function, // Data of type Function in AS3 (appears in datatype
                            // query, never as arguments)
...
typedef struct IggyDataValue {
    S32 type; // an $IggyDatatype which determines which of the union members is
              // valid.
#ifdef __RAD64__
    S32 padding;
#endif
    IggyTempRef temp_ref; // An opaque temporary reference which you can
                          // efficiently turn into an $IggyValueRef; this is
                          // written by Iggy on callbacks but never read by Iggy
    union {
        IggyStringUTF16 string16; // A UTF16 string, valid if type =
        ...
        IggyValuePath*
            valuepath; // A path to an object in the AS3 VM, valid if type =
                       // $(IggyDatatype::IGGY_DATATYPE_valuepath); this is only
...
typedef void* RADLINK Iggy_AllocateFunction(
    void*   alloc_callback_user_data,
    size_t  size_requested,
    size_t* size_returned
);
typedef void RADLINK
Iggy_DeallocateFunction(void* alloc_callback_user_data, void* ptr);
...
typedef enum IggyConfigureBoolName {
    IGGY_CONFIGURE_BOOL_StartupExceptionsAreWarnings, // if true, ActionScript
                                                      // exceptions thrown
                                                      // during startup will not
                                                      // prevent Iggy from being
...
IggyConfigureBool(IggyConfigureBoolName prop, rrbool value);
...
typedef enum {
    ...
    IGGY_VERSIONED_BEHAVIOR_textfield_position =
        129, // This changes the behavior of textfield positioning as reported
             // by AS3 getBounds/getRect and width/height. Values with different
             // behavior: IGGY_VERSION_1_0_24, IGGY_VERSION_default.
...
RADEXPFUNC void RADEXPLINK IggyConfigureVersionedBehavior(
    IggyVersionedBehaviorName prop,
...
RADEXPFUNC void RADEXPLINK
IggyUseTmLite(void* context, IggyTelemetryAmount amount);
RADEXPFUNC void RADEXPLINK
IggyUseTelemetry(void* context, IggyTelemetryAmount amount);
...
typedef void RADLINK
Iggy_TranslationFreeFunction(void* callback_data, void* data, S32 length);
typedef rrbool RADLINK Iggy_TranslateFunctionUTF16(
    void*            callback_data,
    IggyStringUTF16* src,
    IggyStringUTF16* dest
);
typedef rrbool RADLINK Iggy_TranslateFunctionUTF8(
    void*           callback_data,
    IggyStringUTF8* src,
    IggyStringUTF8* dest
);
...
RADEXPFUNC void RADEXPLINK IggySetLoadtimeTranslationFunction(
    Iggy_TranslateFunctionUTF16*  func,
    void*                         callback_data,
    Iggy_TranslationFreeFunction* freefunc,
...
RADEXPFUNC void RADEXPLINK IggySetLoadtimeTranslationFunctionUTF8(
    Iggy_TranslateFunctionUTF8*   func,
    void*                         callback_data,
    Iggy_TranslationFreeFunction* freefunc,
    void*                         free_callback_data
);
...
RADEXPFUNC void RADEXPLINK IggySetTextfieldTranslationFunctionUTF16(
    Iggy_TextfieldTranslateFunctionUTF16* func,
    void*                                 callback_data,
    Iggy_TranslationFreeFunction*         freefunc,
    void*                                 free_callback_data
);
RADEXPFUNC void RADEXPLINK IggySetTextfieldTranslationFunctionUTF8(
    Iggy_TextfieldTranslateFunctionUTF8* func,
    void*                                callback_data,
    Iggy_TranslationFreeFunction*        freefunc,
    void*                                free_callback_data
);
...
typedef void RADLINK Iggy_TraceFunctionUTF16(
    void*            user_callback_data,
    Iggy*            player,
...
RADEXPFUNC void RADEXPLINK IggySetTraceCallbackUTF8(
    Iggy_TraceFunctionUTF8* trace_utf8,
    void*                   user_callback_data
);
RADEXPFUNC void RADEXPLINK IggySetTraceCallbackUTF16(
    Iggy_TraceFunctionUTF16* trace_utf16,
    void*                    user_callback_data
);
...
typedef struct IggyProperties {
    S32 movie_width_in_pixels;  // the width of the "document" specified in the
                                // SWF file
    S32 movie_height_in_pixels; // the height of the "document" specified in the
                                // SWF file
...
RADEXPFUNC void RADEXPLINK
IggyPlayerPause(Iggy* player, IggyAudioPauseMode pause_audio);
RADEXPFUNC void RADEXPLINK IggyPlayerPlay(Iggy* player);
RADEXPFUNC void RADEXPLINK
IggyPlayerSetFrameRate(Iggy* player, F32 frame_rate_in_fps);
RADEXPFUNC void RADEXPLINK
IggyPlayerGotoFrameRS(Iggy* f, S32 frame, rrbool stop);
...
IDOCN typedef struct {
    rrbool(RADLINK* connection_valid)(
        Iggy*    swf,
        HIGGYEXP iggyexp
    ); // Iggy queries this to check if Iggy Explorer is still connected
    S32(RADLINK* poll_command)(
        Iggy*    swf,
        HIGGYEXP iggyexp,
        U8**     buffer
    ); // stores command in *buffer, returns number of bytes
    void(RADLINK* send_command)(
        ...
        void*    buffer,
        S32      len
    ); // writes a command with a payload of buffer:len
    S32(RADLINK* get_storage)(
        Iggy*    swf,
        HIGGYEXP iggyexp,
        U8**     buffer
    ); // returns temporary storage Iggy can use for assembling commands
    rrbool(RADLINK* attach)(
        Iggy*                    swf,
        HIGGYEXP                 iggyexp,
        iggyexp_detach_callback* cb,
        void*                    cbdata,
        IggyForPerfmonFunctions* pmf
    ); // an Iggy file is trying to attach itself to this connection (one at a
       // time)
    rrbool(RADLINK* detach)(
        Iggy*    swf,
        HIGGYEXP iggyexp
    ); // the current Iggy file should be detached (generate callback)
    void(RADLINK* draw_tile_hook)(
        Iggy*           swf,
        HIGGYEXP        iggyexp,
        GDrawFunctions* iggy_gdraw
    ); // only used by perfmon
} IggyExpFunctions;
...
typedef struct {
    ...
    F32 cx,
        cy; // if IGGY_VERTEX_curve, control point on segment; ignored otherwise
...
typedef S32
    RADLINK IggyFontGetCodepointGlyph(void* user_context, U32 codepoint);
...
typedef void RADLINK IggyVectorFontGetGlyphShape(
    void*            user_context,
    S32              glyph,
    IggyVectorShape* shape
);
typedef void RADLINK IggyVectorFontFreeGlyphShape(
    void*            user_context,
    S32              glyph,
    IggyVectorShape* shape
);
...
IggyBitmapFontCanProvideBitmap(void* user_context, S32 glyph, F32 pixel_scale);
...
RADEXPFUNC void RADEXPLINK IggyFontInstallVectorUTF8(
    const IggyVectorFontProvider* vfp,
    const char*                   fontname,
    S32                           namelen_in_bytes,
    U32                           fontflags
);
...
RADEXPFUNC void RADEXPLINK IggyFontInstallBitmapUTF8(
    const IggyBitmapFontProvider* bmf,
    const char*                   fontname,
    S32                           namelen_in_bytes,
    U32                           fontflags
);
...
RADEXPFUNC void RADEXPLINK IggyFontInstallBitmapOverrideUTF8(
    const IggyBitmapFontOverride* bmf,
    const char*                   fontname,
    S32                           namelen_in_bytes,
    U32                           fontflags
);
...
RADEXPFUNC void RADEXPLINK
IggyFontRemoveUTF8(const char* fontname, S32 namelen_in_bytes, U32 fontflags);
RADEXPFUNC void RADEXPLINK IggyFontRemoveUTF16(
    const U16* fontname,
    S32        namelen_in_16bit_quantities,
    U32        fontflags
);
...
RADEXPFUNC void RADEXPLINK IggyFontSetIndirectUTF16(
    const U16* request_name,
    S32        request_namelen,
    U32        request_flags,
    const U16* result_name,
    S32        result_namelen,
    U32        result_flags
);

RADEXPFUNC void RADEXPLINK IggyFontSetFallbackFontUTF8(
    const char* fontname,
    S32         fontname_len,
    U32         fontflags
);
RADEXPFUNC void RADEXPLINK IggyFontSetFallbackFontUTF16(
    const U16* fontname,
    S32        fontname_len,
    U32        fontflags
);
...
IDOCN typedef S32 (*IGGYSND_OPEN_FUNC)(
...
RADEXPFUNC void RADEXPLINK
IggyPlayerSetAudioVolume(Iggy* iggy, F32 attenuation);

#define IGGY_AUDIODEVICE_default   0
#define IGGY_AUDIODEVICE_primary   1
#define IGGY_AUDIODEVICE_secondary 2

IDOCN RADEXPFUNC void
    RADEXPLINK IggyPlayerSetAudioDevice(Iggy* iggy, S32 device);
...
typedef void RADLINK Iggy_CustomDrawCallback(
    void*                         user_callback_data,
    Iggy*                         player,
...
RADEXPFUNC void RADEXPLINK IggySetCustomDrawCallback(
    Iggy_CustomDrawCallback* custom_draw,
    void*                    user_callback_data
);
...
RADEXPFUNC void RADEXPLINK
IggyPlayerGetBackgroundColor(Iggy* player, F32 output_color[3]);
...
RADEXPFUNC void RADEXPLINK
IggyPlayerSetPixelShape(Iggy* swf, F32 pixel_x, F32 pixel_y);
RADEXPFUNC void RADEXPLINK
IggyPlayerSetStageRotation(Iggy* f, Iggy90DegreeRotation rot);
RADEXPFUNC void RADEXPLINK IggyPlayerDraw(Iggy* f);
RADEXPFUNC void RADEXPLINK IggyPlayerSetStageSize(Iggy* f, S32 w, S32 h);
RADEXPFUNC void RADEXPLINK IggyPlayerSetFaux3DStage(
    Iggy* f,
    F32*  top_left,
    F32*  top_right,
    F32*  bottom_left,
...
RADEXPFUNC void RADEXPLINK
IggyPlayerForceMipmaps(Iggy* f, rrbool force_mipmaps);

RADEXPFUNC void RADEXPLINK
IggyPlayerDrawTile(Iggy* f, S32 x0, S32 y0, S32 x1, S32 y1, S32 padding);
RADEXPFUNC void RADEXPLINK IggyPlayerDrawTilesStart(Iggy* f);
RADEXPFUNC void RADEXPLINK IggyPlayerDrawTilesEnd(Iggy* f);
RADEXPFUNC void RADEXPLINK
IggyPlayerSetRootTransform(Iggy* f, F32 mat[4], F32 tx, F32 ty);
...
RADEXPFUNC void RADEXPLINK IggyGenericInstallResourceFile(
    IggyGeneric* g,
    void*        data,
    S32          data_length,
    rrbool*      can_free_now
...
typedef rrbool RADLINK Iggy_AS3ExternalFunctionUTF8(
    void*                         user_callback_data,
    Iggy*                         player,
    IggyExternalFunctionCallUTF8* call
);
...
RADEXPFUNC IggyName RADEXPLINK
IggyPlayerCreateFastName(Iggy* f, IggyUTF16 const* name, S32 len);
RADEXPFUNC IggyName RADEXPLINK
IggyPlayerCreateFastNameUTF8(Iggy* f, char const* name, S32 len);
...
RADEXPFUNC IggyResult RADEXPLINK IggyPlayerCallMethodRS(
    Iggy*          f,
    IggyDataValue* result,
...
typedef enum {
    ...
    IGGY_GC_WARNING_greylist_overflow, // the grey list overflowed, increase the
                                       // size of
...
typedef struct {
    ...
    U32 last_slice_old_allocation_bytes; // the number of bytes that were
                                         ...
                                         // from the old heap since the previous
                                         // GC step
...
RADEXPFUNC void RADEXPLINK IggyPlayerConfigureGCBehavior(
    Iggy*                           player,
    Iggy_GarbageCollectionCallback* notify_callack,
    IggyGarbageCollectorControl*    control
);
RADEXPFUNC void RADEXPLINK
IggyPlayerQueryGCSizes(Iggy* player, IggyPlayerGCSizes* sizes);
...
RADEXPFUNC IggyValueRef RADEXPLINK
IggyValueRefCreateArray(Iggy* f, S32 num_slots);
...
RADEXPFUNC rrbool RADEXPLINK         IggyValuePathMakeNameRef(
            IggyValuePath* result,
            IggyValuePath* parent,
            char const*    text_utf8
        );
RADEXPFUNC void RADEXPLINK
IggyValuePathFromRef(IggyValuePath* result, Iggy* iggy, IggyValueRef ref);

RADEXPFUNC void RADEXPLINK IggyValuePathMakeNameRefFast(
    IggyValuePath* result,
    IggyValuePath* parent,
    IggyName       name
);
RADEXPFUNC void RADEXPLINK IggyValuePathMakeArrayRef(
    IggyValuePath* result,
...
RADEXPFUNC void RADEXPLINK
IggyValuePathSetParent(IggyValuePath* result, IggyValuePath* new_parent);
RADEXPFUNC void RADEXPLINK
IggyValuePathSetArrayIndex(IggyValuePath* result, int new_index);

RADEXPFUNC void RADEXPLINK
IggyValuePathSetName(IggyValuePath* result, IggyName name);
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetTypeRS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    IggyDatatype*  result
...
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetF64RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    F64*           result
);
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetF32RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    F32*           result
);
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetS32RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    S32*           result
);
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetU32RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    U32*           result
...
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetStringUTF16RS(
    ...
    char const*    sub_name_utf8,
    S32            max_result_len,
    IggyUTF16*     utf16_result,
    S32*           result_len
);
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetBooleanRS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    rrbool*        result
);
RADEXPFUNC IggyResult RADEXPLINK IggyValueGetArrayLengthRS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    S32*           result
...
RADEXPFUNC rrbool RADEXPLINK IggyValueSetF64RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    F64            value
);
RADEXPFUNC rrbool RADEXPLINK IggyValueSetF32RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    F32            value
);
RADEXPFUNC rrbool RADEXPLINK IggyValueSetS32RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    S32            value
);
RADEXPFUNC rrbool RADEXPLINK IggyValueSetU32RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    U32            value
);
RADEXPFUNC rrbool RADEXPLINK IggyValueSetStringUTF8RS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    char const*    utf8_string,
...
RADEXPFUNC rrbool RADEXPLINK IggyValueSetBooleanRS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    rrbool         value
);
RADEXPFUNC rrbool RADEXPLINK IggyValueSetValueRefRS(
    IggyValuePath* var,
    IggyName       sub_name,
    char const*    sub_name_utf8,
    IggyValueRef   value_ref
...
RADEXPFUNC rrbool RADEXPLINK
IggyValueSetUserDataRS(IggyValuePath* result, void const* userdata);
RADEXPFUNC IggyResult RADEXPLINK
IggyValueGetUserDataRS(IggyValuePath* result, void** userdata);
...
typedef enum IggyKeyloc {
    ...
    IGGY_KEYLOC_Left =
        1, // Specifies the left-hand-side key for keys with left/right variants
           // (such as $(IggyKeycode::IGGY_KEYCODE_SHIFT),
...
RADEXPFUNC void RADEXPLINK
IggyMakeEventActivate(IggyEvent* event, IggyActivestate event_type);
RADEXPFUNC void RADEXPLINK IggyMakeEventMouseLeave(IggyEvent* event);
RADEXPFUNC void RADEXPLINK
IggyMakeEventMouseMove(IggyEvent* event, S32 x, S32 y);
RADEXPFUNC void RADEXPLINK
IggyMakeEventMouseButton(IggyEvent* event, IggyMousebutton event_type);
RADEXPFUNC void RADEXPLINK
IggyMakeEventMouseWheel(IggyEvent* event, S16 mousewheel_delta);
RADEXPFUNC void RADEXPLINK IggyMakeEventKey(
    IggyEvent*   event,
...
RADEXPFUNC void RADEXPLINK
IggyMakeEventFocusGained(IggyEvent* event, S32 focus_direction);
RADEXPFUNC rrbool RADEXPLINK IggyPlayerDispatchEventRS(
    Iggy*            player,
    IggyEvent*       event,
    IggyEventResult* result
);
RADEXPFUNC void RADEXPLINK IggyPlayerSetShiftState(
    Iggy*  f,
    rrbool shift,
    rrbool control,
    rrbool alt,
...
RADEXPFUNC void RADEXPLINK
IggySetTextCursorFlash(U32 cycle_time_in_ms, U32 visible_time_in_ms);

RADEXPFUNC rrbool RADEXPLINK IggyPlayerHasFocusedEditableTextfield(Iggy* f);
RADEXPFUNC rrbool RADEXPLINK
IggyPlayerPasteUTF16(Iggy* f, U16* string, S32 stringlen);
RADEXPFUNC rrbool RADEXPLINK
IggyPlayerPasteUTF8(Iggy* f, char* string, S32 stringlen);
...
RADEXPFUNC
S32 RADEXPLINK IggyPlayerCopyUTF16(Iggy* f, U16* buffer, S32 bufferlen);
RADEXPFUNC S32 RADEXPLINK
IggyPlayerCopyUTF8(Iggy* f, char* buffer, S32 bufferlen);
...
RADEXPFUNC void RADEXPLINK IggyPlayerSetIMEFontUTF16(
    Iggy*            f,
    const IggyUTF16* font_name_utf16,
    S32              namelen_in_2byte_words
);
...
IDOCN RADEXPFUNC void RADEXPLINK IggyIMEGetTextExtents(
    Iggy*            f,
    U32*             pdw,
    U32*             pdh,
    const IggyUTF16* str,
...
IDOCN RADEXPFUNC void RADEXPLINK IggyIMEDrawString(
    Iggy*            f,
    S32              px,
    S32              py,
    const IggyUTF16* str,
...
IDOCN RADEXPFUNC void RADEXPLINK IggyIMEWin32GetCandidatePosition(
    ...
    F32*  pdx,
    F32*  pdy,
    F32*  pdcomp_str_height
);
IDOCN RADEXPFUNC void* RADEXPLINK IggyIMEGetFocusedTextfield(Iggy* f);
IDOCN RADEXPFUNC void RADEXPLINK
IggyIMEDrawRect(S32 x0, S32 y0, S32 x1, S32 y1, const U8 rgb[3]);
...
RADEXPFUNC rrbool RADEXPLINK IggyPlayerGetFocusableObjects(
    Iggy*                f,
    IggyFocusHandle*     current_focus,
    IggyFocusableObject* objs,
    S32                  max_obj,
    S32*                 num_obj
);
RADEXPFUNC void RADEXPLINK
IggyPlayerSetFocusRS(Iggy* f, IggyFocusHandle object, int focus_key_char);
...
IDOCN RADEXPFUNC void*
    RADEXPLINK IggyGDrawMallocAnnotated(SINTa size, const char* file, int line);

RADEXPFUNC void RADEXPLINK IggyGDrawFree(void* ptr);
RADEXPFUNC void RADEXPLINK
IggyGDrawSendWarning(Iggy* f, char const* message, ...);
RADEXPFUNC void RADEXPLINK IggyWaitOnFence(void* id, U32 fence);
RADEXPFUNC void RADEXPLINK
IggyDiscardVertexBufferCallback(void* owner, void* vertex_buffer);
...
typedef struct {
    ...
    S32 dynamic_allocation_count; // number of freeable allocations for this
                                  // subcategory
    S32 dynamic_allocation_bytes; // estimated bytes of freeable allocations for
                                  // this subcategory
} IggyMemoryUseInfo;
...
RADEXPFUNC rrbool RADEXPLINK IggyDebugGetMemoryUseInfo(
    Iggy*              player,
    IggyLibrary        lib,
    char const*        category_string,
    S32                category_stringlen,
    S32                iteration,
...
IDOCN RADEXPFUNC void RADEXPLINK
iggy_sync_check_todisk(char* filename_or_null, U32 flags);
IDOCN RADEXPFUNC void RADEXPLINK
