...
IDOC RADEXPFUNC HIGGYPERFMON RADEXPLINK IggyPerfmonCreate(
    iggyperfmon_malloc* perf_malloc,
    iggyperfmon_free*   perf_free,
...
#define IggyPerfmonPadFromXInputStatePointer(pad, xis)                         \
    ...
    (pad).field.trigger_left_low =                                             \
        0                                                                      \
        != ((xis)->Gamepad.bLeftTrigger >= XINPUT_GAMEPAD_TRIGGER_THRESHOLD),  \
    (pad).field.trigger_right_low =                                            \
        0                                                                      \
        != ((xis)->Gamepad.bRightTrigger >= XINPUT_GAMEPAD_TRIGGER_THRESHOLD)
...
IDOC RADEXPFUNC void RADEXPLINK
IggyPerfmonDestroy(HIGGYPERFMON p, GDrawFunctions* iggy_draw);
