...
static GDrawTexture * RADLINK gdraw_MakeTextureEnd(GDraw_MakeTexture_ProcessingInfo *p, GDrawStats *stats)
{
   ...
   D3D1X_(TEXTURE2D_DESC) desc = { static_cast<UINT>(w), static_cast<UINT>(h), static_cast<UINT>(nmips), 1, (DXGI_FORMAT) p->i3, { 1, 0 },
...
static void RADLINK gdraw_UpdateTextureRect(GDrawTexture *t, void * /*unique_id*/, S32 x, S32 y, S32 stride, S32 w, S32 h, U8 *samples, gdraw_texture_format /*format*/)
{
   ...
   D3D1X_(BOX) box = { static_cast<UINT>(x), static_cast<UINT>(y), 0, static_cast<UINT>(x+w), static_cast<UINT>(y+h), 1 };
...
static void RADLINK gdraw_SetAntialiasTexture(S32 width, U8 *rgba)
{
   ...
   D3D1X_(TEXTURE2D_DESC) desc = { static_cast<UINT>(width), 1, 1, 1, DXGI_FORMAT_R8G8B8A8_UNORM, { 1, 0 }, D3D1X_(USAGE_IMMUTABLE), D3D1X_(BIND_SHADER_RESOURCE), 0, 0 };
   D3D1X_(SUBRESOURCE_DATA) data = { rgba, static_cast<UINT>(width*4), 0 };
...
static GDrawVertexBuffer * RADLINK gdraw_MakeVertexBufferEnd(GDraw_MakeVertexBuffer_ProcessingInfo *p, GDrawStats * /*stats*/)
{
   ...
   HRESULT hr;
   D3D1X_(BUFFER_DESC) vbdesc = { static_cast<UINT>(p->vertex_data_length), D3D1X_(USAGE_IMMUTABLE), D3D1X_(BIND_VERTEX_BUFFER), 0, 0 };
   D3D1X_(SUBRESOURCE_DATA) vbdata = { p->vertex_data, 0, 0 };
   
   D3D1X_(BUFFER_DESC) ibdesc = { static_cast<UINT>(p->index_data_length), D3D1X_(USAGE_IMMUTABLE), D3D1X_(BIND_INDEX_BUFFER), 0, 0 };
...
static GDrawHandle *get_color_rendertarget(GDrawStats *stats)
{
   ...
   D3D1X_(TEXTURE2D_DESC) desc = { static_cast<UINT>(gdraw->frametex_width), static_cast<UINT>(gdraw->frametex_height), 1, 1, DXGI_FORMAT_R8G8B8A8_UNORM, { 1, 0 },
...
static ID3D1X(DepthStencilView) *get_rendertarget_depthbuffer(GDrawStats *stats)
{
   if (!gdraw->depth_buffer[1]) {
      ...
      D3D1X_(TEXTURE2D_DESC) desc = { static_cast<UINT>(gdraw->frametex_width), static_cast<UINT>(gdraw->frametex_height), 1, 1, DXGI_FORMAT_D24_UNORM_S8_UINT, { 1, 0 },
...
GDrawTexture * RADLINK gdraw_D3D1X_(MakeTextureFromResource)(U8 *resource_file, S32 /*len*/, IggyFileTextureRaw *texture)
{
   ...
   D3D1X_(TEXTURE2D_DESC) desc = { static_cast<UINT>(width), static_cast<UINT>(height), static_cast<UINT>(mipmaps), 1, DXGI_FORMAT_UNKNOWN, { 1, 0 },
