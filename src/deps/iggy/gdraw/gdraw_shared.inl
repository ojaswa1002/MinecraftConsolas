...
struct GDrawHandle {
    ...
    void* owner; // 4/8 // opaque handle used to allow freeing resources without
                 // calling back to owner
    ...
    void* raw_ptr; // 4/8 // pointer to allocation - when you're managing memory
                   // manually
...
struct GDrawHandleCache {
    ...
    GDrawFence prev_frame_start,
        prev_frame_end;    // fence value at start/end of previous frame, for
                           // thrashing detection
    GDrawHandle handle[1]; // the rest of the handles must be stored right after
                           // this in the containing structure
};
...
static void gdraw_HandleTransitionInsertBefore(
    GDrawHandle*     t,
    ...
    ); // sentinels should never get here!
    assert(
        t->state != (U32)new_state
    ); // code should never call "transition" if it's not transitioning!
    ...
    printf(
        "GD %chandle %p %s->%s\n",
        t->cache->is_vertex ? 'v' : 't',
        t,
        gdraw_StateName(t->state),
        gdraw_StateName(new_state)
    );
#endif
    t->state = new_state;
    check_lists(t->cache);
...
static RADINLINE void
gdraw_HandleTransitionTo(GDrawHandle* t, GDrawHandleState new_state) {
    gdraw_HandleTransitionInsertBefore(
        t,
        new_state,
...
static rrbool
gdraw_HandleCacheLockStats(GDrawHandle* t, void* owner, GDrawStats* stats) {
    ...
    // unlock call
    assert(
        t->state == GDRAW_HANDLE_STATE_live
...
static void
gdraw_HandleCacheInit(GDrawHandleCache* c, S32 num_handles, S32 bytes) {
    S32 i;
...
static void gdraw_HandleCacheAllocateEnd(
    GDrawHandle*     t,
    S32              bytes,
    void*            owner,
    GDrawHandleState new_state
    ...
    if (bytes == 0) assert(new_state == GDRAW_HANDLE_STATE_user_owned);
    else
        assert(
            new_state == GDRAW_HANDLE_STATE_locked
...
static void gdraw_HandleCacheFree(GDrawHandle* t) {
    ...
    assert(
        t->state != GDRAW_HANDLE_STATE_alloc
        && t->state != GDRAW_HANDLE_STATE_sentinel
    );
...
static RADINLINE void gdraw_WorldSpace(
    volatile F32* RADRESTRICT vvec,
...
static RADINLINE void gdraw_ObjectSpace(
    volatile F32* RADRESTRICT vvec,
...
static void gdraw_GetObjectSpaceMatrix(
    F32* RADRESTRICT         mat,
    gswf_matrix* RADRESTRICT xform,
    F32* RADRESTRICT         proj,
    F32                      depth,
    int                      out_col_major
) {
...
static void
gdraw_ShiftRect(gswf_recti* out, gswf_recti const* in, S32 dx, S32 dy) {
    out->x0 = in->x0 + dx;
    out->y0 = in->y0 + dy;
    out->x1 = in->x1 + dx;
    out->y1 = in->y1 + dy;
...
static GDrawTexture* gdraw_BlurPass(
    GDrawFunctions*   g,
    GDrawBlurInfo*    c,
    GDrawRenderState* r,
    int               taps,
...
static void gdraw_BlurAxis(
    ...
    if (w > 1) {
        if (w <= MAX_TAPS) {
            ...
            S32 w_mip = (S32)ceil(
                linear_remap(w, MAX_TAPS + 1, MAX_TAPS * MAX_TAPS, 2, MAX_TAPS)
            );
...
static void verify_against_blocks(int num_regions, void* vptr, S32 len) {
    ...
    while (s < e) {
        ...
        if (region[i].begin <= (UINTa)ptr) s = i;
        else e = i - 1;
    ...
    // at this point, s >= e
    assert(
        s < num_regions && region[s].begin == (UINTa)ptr
        && (UINTa)ptr + len <= region[s].end
    );
}

static void
debug_complete_check(gfx_allocator* alloc, void* ptr, S32 len, void* skip) {
...
static void
gfxalloc_split_free(gfx_allocator* alloc, gfx_block_info* b, U32 pos) {
...
static gfx_allocator*
gfxalloc_create(void* mem, U32 mem_size, U32 align, U32 max_allocs) {
    gfx_allocator* a;
    ...
    GFXALLOC_IF_CHECK(GFXALLOC_ASSERT(
                          a->num_blocks
...
static void* gfxalloc_alloc(gfx_allocator* alloc, U32 size_in_bytes) {
    ...
    if (best) {
        ...
        debug_check_overlap(alloc->cache, best->ptr, best->size);
        return best->ptr;
    } else return NULL; // not enough space!
}
...
static void gfxalloc_free(gfx_allocator* alloc, void* ptr) {
    ...
    *prevnext = b->next;
    ...
    if (t->is_free) {
        ...
        b = t;
    } else gfxalloc_add_free(alloc, b);
...
static void
gdraw_gpu_memcpy(GDrawHandleCache* c, void* dst, void* src, U32 num_bytes);

static void gdraw_Defragment_memmove(
    GDrawHandleCache* c,
    U8*               dst,
    U8*               src,
    U32               num_bytes,
    ...
    if ((flags & GDRAW_DEFRAGMENT_may_overlap) || dst + num_bytes <= src
        ...
        if (dst < src) chunk_size = (U32)(src - dst);
        else chunk_size = (U32)(dst - src);
...
static void
gdraw_DefragmentMain(GDrawHandleCache* c, U32 flags, GDrawStats* stats) {
    gfx_allocator*  alloc = c->alloc;
    gfx_block_info *b, *n;
    U8*             p;
    S32             i;
    ...
    for (i = 0; i < c->max_handles; i++)
        if (c->handle[i].raw_ptr) {
            ...
            for (b = alloc->hash
                         [gfxalloc_get_hash_code(alloc, c->handle[i].raw_ptr)];
                 b;
                 b = b->next)
...
static rrbool
gdraw_MigrateAllResources(GDrawHandle* sentinel, GDrawStats* stats) {
    GDrawHandle* h;
...
static rrbool
gdraw_TwoPoolDefragmentMain(GDrawHandleCache* c, GDrawStats* stats) {
    gfx_allocator* t;
...
static void
gdraw_PostDefragmentCleanup(GDrawHandleCache* c, GDrawStats* stats) {
...
static void
gdraw_Downsample_2x2_line(U8* dst, U8* line0, U8* line1, U32 width, U32 bpp) {
    U32 x;
    if (bpp == 4) {
        U32* in0 = (U32*)line0;
        U32* in1 = (U32*)line1;
...
static void gdraw_Downsample_1x2(
    U8* dst,
    S32 dstpitch,
    U8* src,
    S32 srcpitch,
...
static GDRAW_MAYBE_UNUSED void gdraw_Downsample(
    U8* dst,
    S32 dstpitch,
    U32 width,
    U32 height,
    ...
    else if (!width) // non-square texture, width was reduced to 1 in a previous
                     // step
...
static rrbool gdraw_MipmapBegin(
    GDrawMipmapContext* c,
    U32                 width,
    U32                 height,
    U32                 mipmaps,
    U32                 bpp,
...
static void
check_block_alloc(gfx_allocator* alloc, void* ptr, rrbool allocated) {
    int i, n = 0, m = 0;
    for (i = 0; i < GFXALLOC_HASH_SIZE; ++i) {
        gfx_block_info* b = alloc->hash[i];
        while (b) {
...
static void gdraw_bufring_init(
    gdraw_bufring* RADRESTRICT ring,
    void*                      ptr,
    U32                        size,
    U32                        nsegs,
    U32                        align
...
static void*
gdraw_bufring_alloc(gdraw_bufring* RADRESTRICT ring, U32 size, U32 align) {
    U32                align_up;
...
static rrbool gdraw_res_free_lru(GDrawHandleCache* c, GDrawStats* stats) {
    ...
    if (c->prev_frame_end.value < r->fence.value && !c->is_thrashing) {
        IggyGDrawSendWarning(
            NULL,
            c->is_vertex ? "GDraw Thrashing vertex memory"
...
static GDrawHandle* gdraw_res_alloc_outofmem(
    GDrawHandleCache* c,
    GDrawHandle*      t,
...
static GDrawHandle*
gdraw_res_alloc_begin(GDrawHandleCache* c, S32 size, GDrawStats* stats) {
    GDrawHandle* t;
    if (size > c->total_bytes)
        gdraw_res_alloc_outofmem(
            c,
            NULL,
            "memory (single resource larger than entire pool)"
        );
    else {
    ...
    if (size) {
        ...
        if (!ptr) {
            ...
            while (!(ptr = gfxalloc_alloc(c->alloc, size))) {
                if (c->alloc->actual_bytes_free >= 3 * size
                    || // if we should have enough free bytes to satisfy the
                       // request by now
                    (
                        c->alloc->actual_bytes_free >= size
                        && size * 2 >= c->total_bytes
                    )) // or the resource is very big and the alloc doesn't fit
                {
                    ...
                    // possible.
                    if (!c->did_defragment
                        && c->prev_frame_start.value
                               <= c->handle->fence.value) {
                        // defragment.
                    defrag:
                        if (gdraw_CanDefragment(
                                c
                            )) { // only try defrag if it has a chance of
                                 // helping.
                ...
                if (!gdraw_res_free_lru(c, stats)) {
                    if (c->alloc->actual_bytes_free
                        >= size) // nothing left to free but we should be good -
                                 // defrag again, even if it's the second time
                                 // in a frame
                        goto defrag;
