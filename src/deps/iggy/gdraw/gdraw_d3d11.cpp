...
set_pixel_shader(ID3D11DeviceContext* ctx, ID3D11PixelShader* shader) {
    ctx->PSSetShader(shader, NULL, 0);
}

static RADINLINE void
set_vertex_shader(ID3D11DeviceContext* ctx, ID3D11VertexShader* shader) {
    ctx->VSSetShader(shader, NULL, 0);
}

static ID3D11BlendState* create_blend_state(
    ID3D11Device* dev,
    BOOL          blend,
    D3D11_BLEND   src,
    D3D11_BLEND   dst
...
static void create_pixel_shader(
    ProgramWithCachedVariableLocations* p,
    ...
    *p = *src;
    if (p->bytecode) {
        HRESULT hr =
            gdraw->d3d_device
                ->CreatePixelShader(p->bytecode, p->size, NULL, &p->pshader);
        if (FAILED(hr)) {
...
static void create_vertex_shader(
    ProgramWithCachedVariableLocations* p,
    ProgramWithCachedVariableLocations* src
) {
    *p = *src;
    if (p->bytecode) {
        HRESULT hr =
            gdraw->d3d_device
                ->CreateVertexShader(p->bytecode, p->size, NULL, &p->vshader);
        if (FAILED(hr)) {
...
GDrawFunctions* gdraw_D3D11_CreateContext(
    ID3D11Device*        dev,
