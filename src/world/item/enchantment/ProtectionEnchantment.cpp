...
const int ProtectionEnchantment::names[] = {
    IDS_ENCHANTMENT_PROTECT_ALL,
    IDS_ENCHANTMENT_PROTECT_FIRE,
    IDS_ENCHANTMENT_PROTECT_FALL,
    IDS_ENCHANTMENT_PROTECT_EXPLOSION,
...
: Enchantment(id, frequency, EnchantmentCategory::armor),
  type(type) {
    if (type == FALL) {
...
double ProtectionEnchantment::getExplosionKnockbackAfterDampener(
    ...
    int level = EnchantmentHelper::getEnchantmentLevel(
        Enchantment::explosionProtection->id,
        entity->getEquipmentSlots()
    );
