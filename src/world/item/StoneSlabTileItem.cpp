...
unsigned int
StoneSlabTileItem::getDescriptionId(std::shared_ptr<ItemInstance> instance) {
    return halfTile->getAuxName(instance->getAuxValue());
}

bool StoneSlabTileItem::useOn(
    ...
    if (((face == Facing::UP && !isUpper) || (face == Facing::DOWN && isUpper))
...
bool StoneSlabTileItem::mayPlace(
    Level*                        level,
    int                           x,
    int                           y,
    int                           z,
