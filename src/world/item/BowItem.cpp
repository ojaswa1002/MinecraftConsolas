...
const std::wstring BowItem::TEXTURE_PULL[] =
    {L"bow_pull_0", L"bow_pull_1", L"bow_pull_2"};
...
void BowItem::releaseUsing(
    ...
    if (infiniteArrows || player->inventory->hasResource(Item::arrow_Id)) {
        ...
        int damageBonus = EnchantmentHelper::getEnchantmentLevel(
            Enchantment::arrowBonus->id,
            itemInstance
        );
        if (damageBonus > 0) {
            arrow->setBaseDamage(
                arrow->getBaseDamage() + (double)damageBonus * .5 + .5
            );
        }
        int knockbackBonus = EnchantmentHelper::getEnchantmentLevel(
            Enchantment::arrowKnockback->id,
            itemInstance
        );
        if (knockbackBonus > 0) {
