    ...
    if (level->mayPlace(tileId, x, y, z, false, face, player)) {
        if (!bTestUseOnOnly) {
            ...
            if (level->setTileAndData(x, y, z, tileId, dataValue)) {
                ...
                player->awardStat(
                    GenericStats::blocksPlaced(tileId),
                ...
                if (level->getTile(x, y, z) == tileId) {
                    ...
                    Tile::tiles[tileId]
                        ->finalizePlacement(level, x, y, z, dataValue);
                }
...
unsigned int
TileItem::getDescriptionId(std::shared_ptr<ItemInstance> instance) {
...
unsigned int
TileItem::getUseDescriptionId(std::shared_ptr<ItemInstance> instance) {
