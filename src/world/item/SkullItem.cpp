...
const unsigned int SkullItem::NAMES[SKULL_COUNT] = {
    IDS_ITEM_SKULL_SKELETON,
    IDS_ITEM_SKULL_WITHER,
    IDS_ITEM_SKULL_ZOMBIE,
    IDS_ITEM_SKULL_CHARACTER,
    IDS_ITEM_SKULL_CREEPER
};
...
bool SkullItem::mayPlace(
    ...
    if (currentTile == Tile::topSnow_Id) {
        face = Facing::UP;
    } else if (currentTile != Tile::vine_Id && currentTile != Tile::tallgrass_Id
    ...
    return level
        ->mayPlace(Tile::skull_Id, x, y, z, false, face, nullptr); //, item);
}
...
unsigned int
SkullItem::getDescriptionId(std::shared_ptr<ItemInstance> instance) {
...
std::wstring
SkullItem::getHoverName(std::shared_ptr<ItemInstance> itemInstance) {
#if 0
