...
public:
    ...
    virtual float
    getDestroySpeed(std::shared_ptr<ItemInstance> itemInstance, Tile* tile);
    virtual bool TestUse(Level* level, std::shared_ptr<Player> player);
    virtual std::shared_ptr<ItemInstance>
    use(std::shared_ptr<ItemInstance> itemInstance,
        Level*                        level,
        std::shared_ptr<Player>       player);
...
public:
    ...
    virtual bool
    shouldMoveCraftingResultToInventory(std::shared_ptr<ItemInstance> instance);
...
protected:
    virtual Item*
    setPotionBrewingFormula(const std::wstring& potionBrewingFormula);
...
protected:
    HitResult* getPlayerPOVHitResult(
        Level*                  level,
        std::shared_ptr<Player> player,
        bool                    alsoPickLiquid
    );
