...
const std::wstring PotionBrewing::MOD_GUNPOWDER =
    L""; // gunpowder makes them throwable! // gunpowder requires 13 and sets 14
#endif
...
void PotionBrewing::staticCtor() {
    ...
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::regeneration->getId(),
            L"0 & !1 & !2 & !3 & 0+6"
        )
    );
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::movementSpeed->getId(),
            L"!0 & 1 & !2 & !3 & 1+6"
        )
    );
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::fireResistance->getId(),
            L"0 & 1 & !2 & !3 & 0+6"
        )
    );
    potionEffectDuration.insert(
    ...
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::poison->getId(),
            L"!0 & !1 & 2 & !3 & 2+6"
        )
    );
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::weakness->getId(),
            L"!0 & !1 & !2 & 3 & 3+6"
        )
    );
    potionEffectDuration.insert(
    ...
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::movementSlowdown->getId(),
            L"!0 & 1 & !2 & 3 & 3+6"
        )
    );
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::damageBoost->getId(),
            L"0 & !1 & !2 & 3 & 3+6"
        )
    );
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::nightVision->getId(),
            L"!0 & 1 & 2 & !3 & 2+6"
        )
    );
    potionEffectDuration.insert(
        intStringMap::value_type(
            MobEffect::invisibility->getId(),
            L"!0 & 1 & 2 & 3 & 2+6"
        )
    );
    ...
    potionEffectDuration.put(
        fireResistance.getId(),
        "14 & !5 & 6-!1 & 14+13+12"
    );
    potionEffectDuration.put(
        waterBreathing.getId(),
        "0+1+12 & !6 & 10 & !11 & !13"
    );
...
int PotionBrewing::constructParsedValue(
    bool isNot,
    bool hasMultiplier,
    bool isNeg,
    int  countCompare,
...
// simplified brewing
int PotionBrewing::parseEffectFormulaValue(
...
PotionBrewing::getEffects(int brew, bool includeDisabledEffects) {
        ...
        if (duration > 0) {
            ...
            if (effect->isInstantenous()) {
                ...
                duration   = (int)Math::round(
                    (double)duration * effect->getDurationModifier()
                );
...
int PotionBrewing::applyBrewBit(
    int  currentBrew,
    int  bit,
    bool isNeg,
    bool isNot,
...
int PotionBrewing::valueOf(int brew, int p1, int p2, int p3, int p4) {
    return (
        (isLit(brew, p1) ? 0x08 : 0) | (isLit(brew, p2) ? 0x04 : 0)
