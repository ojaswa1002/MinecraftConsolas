...
public:
    ...
    use(Level* level, std::shared_ptr<Player> player);
    std::shared_ptr<ItemInstance>
                 useTimeDepleted(Level* level, std::shared_ptr<Player> player);
    ...
    static bool
    matches(std::shared_ptr<ItemInstance> a, std::shared_ptr<ItemInstance> b);
...
public:
    ...
    bool sameItem_not_shared(
        ItemInstance* b
    ); // 4J Stu - Added this for the one time I need it
    ...
    static std::shared_ptr<ItemInstance>
                 clone(std::shared_ptr<ItemInstance> item);
    ...
    void
    onCraftedBy(Level* level, std::shared_ptr<Player> player, int craftCount);
