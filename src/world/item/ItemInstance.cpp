...
ItemInstance::use(Level* level, std::shared_ptr<Player> player) {
    return getItem()->use(shared_from_this(), level, player);
}

std::shared_ptr<ItemInstance>
ItemInstance::useTimeDepleted(Level* level, std::shared_ptr<Player> player) {
...
std::shared_ptr<ItemInstance>
ItemInstance::clone(std::shared_ptr<ItemInstance> item) {
...
void ItemInstance::inventoryTick(
    ...
    Item::items[id]
        ->inventoryTick(shared_from_this(), level, owner, slot, selected);
}

void ItemInstance::onCraftedBy(
    Level*                  level,
    std::shared_ptr<Player> player,
    int                     craftCount
) {
        ...
        if (list != NULL) {
            for (int i = 0; i < list->size(); i++) {
                ...
                if (Enchantment::enchantments[type] != NULL) {
                    ...
                    lines->push_back(
                        Enchantment::enchantments[type]
                            ->getFullname(level, unformatted)
                    );
