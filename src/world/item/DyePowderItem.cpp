...
unsigned int
DyePowderItem::getDescriptionId(std::shared_ptr<ItemInstance> itemInstance) {
    int colorValue = Mth::clamp(itemInstance->getAuxValue(), 0, 15);
    return COLOR_DESCS[colorValue];
}

unsigned int
DyePowderItem::getUseDescriptionId(std::shared_ptr<ItemInstance> itemInstance) {
    return COLOR_USE_DESCS[itemInstance->getAuxValue()];
}

bool DyePowderItem::useOn(
                ...
                if (!level->isClientSide) {
                    ...
                    for (int j = 0; j < 128; j++) {
                        ...
                        for (int i = 0; i < j / 16; i++) {
                            ...
                            yy += (random->nextInt(3) - 1) * random->nextInt(3)
                                / 2;
...
bool DyePowderItem::interactEnemy(
    std::shared_ptr<ItemInstance> itemInstance,
