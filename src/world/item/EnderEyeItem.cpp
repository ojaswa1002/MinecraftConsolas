...
bool EnderEyeItem::useOn(
    ...
    if (player->mayBuild(x, y, z) && targetType == Tile::endPortalFrameTile_Id
...
bool EnderEyeItem::TestUse(Level* level, std::shared_ptr<Player> player) {
    ...
    // if (!level->isClientSide)
    {
        if ((level->dimension->id == LevelData::DIMENSION_OVERWORLD)
            ...
            //
            // 				app.DebugPrintf("=== FOUND stronghold in terrain
            // features list\n");
            //
