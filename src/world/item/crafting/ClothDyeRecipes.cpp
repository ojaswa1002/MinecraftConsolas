...
void ClothDyeRecipes::addRecipes(Recipes* r) {
    ...
    for (int i = 0; i < 16; i++) {
        r->addShapelessRecipy(
            ...
            new ItemInstance(Item::items[Tile::cloth_Id], 1, 0),
            L'D'
        );
    }
    ...
    // some dye recipes
    r->addShapelessRecipy(
        new ItemInstance(Item::dye_powder, 2, DyePowderItem::YELLOW),
        L"tg",
        Tile::flower,
        L'D'
    );
    ...
    r->addShapelessRecipy(
        new ItemInstance(Item::dye_powder, 2, DyePowderItem::RED),
        L"tg",
        Tile::rose,
        L'D'
    );
    ...
    r->addShapelessRecipy(
        new ItemInstance(Item::dye_powder, 3, DyePowderItem::WHITE),
        L"ig",
        Item::bone,
        L'D'
    );
    ...
    for (int i = 0; i < 16; i++) {
        r->addShapedRecipy(
            new ItemInstance(Tile::woolCarpet, 3, i),
            L"sczg",
            L"##",
            L'#',
            new ItemInstance(Tile::cloth, 1, i),
            L'D'
        );
    }
}
