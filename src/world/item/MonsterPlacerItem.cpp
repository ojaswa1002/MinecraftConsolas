...
std::wstring
MonsterPlacerItem::getHoverName(std::shared_ptr<ItemInstance> itemInstance) {
...
int MonsterPlacerItem::getColor(
...
MonsterPlacerItem::canSpawn(int iAuxVal, Level* level, int* piResult) {
    ...
    if (newEntity != NULL) {
        ...
        case eTYPE_MUSHROOMCOW:
            if (level
                    ->canCreateMore(eTYPE_MUSHROOMCOW, Level::eSpawnType_Egg)) {
                canSpawn = true;
            } else {
            ...
            if ((newEntity->GetType() & eTYPE_ANIMALS_SPAWN_LIMIT_CHECK)
                ...
                if (level->canCreateMore(
                    ...
                    *piResult = eSpawnResult_FailTooManyPigsCowsSheepCats;
                       ...
                       == eTYPE_MONSTER) {
                // 4J-PB - check if the player is trying to spawn an enemy in
                // peaceful mode
...
std::shared_ptr<Entity> MonsterPlacerItem::spawnMobAt(
    Level* level,
    int    mobId,
    double x,
    double y,
    ...
    for (int i = 0; i < SPAWN_COUNT; i++) {
        ...
        if (mob) {
            newEntity->moveTo(
                ...
                y,
                z,
                Mth::wrapDegrees(level->random->nextFloat() * 360),
                0
            );
            newEntity
                ->setDespawnProtected(); // 4J added, default to being protected
                                         // against despawning (has to be done
                                         // after initial position is set)
