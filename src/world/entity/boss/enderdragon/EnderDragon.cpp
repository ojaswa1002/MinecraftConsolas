...
#include "world/entity/boss/BossMobPart.h"
...
#include "world/level/tile/TheEndPortal.h"
...
const int EnderDragon::FLAME_FREQUENCY =
    2; // Every FLAME_FREQUENCY ticks it sets fire to blocks while doing a flame
       // pass
...
EnderDragon::EnderDragon(Level* level) : BossMob(level) {
    ...
    neck = std::shared_ptr<BossMobPart>(
        new BossMobPart(this, L"neck", 6, 6)
    ); // 4J Added
...
void EnderDragon::aiStep() {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        part->tick();
        part->moveTo(
            x - (ss1 * dd1 + ss * dd) * ccTilt,
            y + (p0[1] - p1[1]) * 1 - (dd + dd1) * ssTilt + 1.5f,
            z + (cc1 * dd1 + cc * dd) * ccTilt,
            0,
            0
        );
    }
...
void EnderDragon::hurt(std::vector<std::shared_ptr<Entity>>* entities) {
    ...
    for (auto it = entities->begin(); it != entities->end(); ++it) {
        std::shared_ptr<Mob> e =
            dynamic_pointer_cast<Mob>(*it); // entities.get(i);
        if (e != NULL)                      //(e instanceof Mob)
        {
...
void EnderDragon::findNewTarget() {
    ...
    case e_EnderdragonAction_HoldingPattern: {
        if (!newTarget && m_currentPath != NULL && m_currentPath->isDone()) {
            ...
            // More likely to strafe a player if they are close to the egg, or
            // there are not many crystals remaining
            else if (playerNearestToEgg != NULL
                     && (random->nextInt(abs(dist) + 2) == 0
                         || random->nextInt(m_remainingCrystalsCount + 2)
                                == 0)) {
...
bool EnderDragon::hurt(
    ...
    if (source == DamageSource::explosion
        || (dynamic_pointer_cast<Player>(source->getEntity()) != NULL)) {
...
bool EnderDragon::
    ...
    if (force || validTransition) {
        ...
        app.DebugPrintf(
            "EnderDragon: Invalid state transition from %d to %d\n",
            getSynchedAction(),
            action
        );
    }
...
void EnderDragon::navigateToNextPathNode() {
    if (m_currentPath != NULL && !m_currentPath->isDone()) {
        ...
        zTarget = curr->z;
        app.DebugPrintf(
            "Path node pos is (%f,%f,%f)\n",
            curr->x,
            curr->y,
            curr->z
        );
        app.DebugPrintf(
            "Setting new target to (%f,%f,%f)\n",
            xTarget,
            yTarget,
            zTarget
        );
...
Path* EnderDragon::
    findPath(int startIndex, int endIndex, Node* finalNode /* = NULL */) {
    for (unsigned int i = 0; i < 24; ++i) {
    ...
    app.DebugPrintf(
        "Failed to find path from %d to %d\n",
        startIndex,
        endIndex
    );
    if (finalNode != NULL) {
...
double EnderDragon::getHeadPartYRotDiff(
    int         partIndex,
    doubleArray bodyPos,
    doubleArray partPos
) {
