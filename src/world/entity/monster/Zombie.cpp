...
#include "world/entity/ai/goal/BreakDoorGoal.h"
...
#include "world/entity/ai/goal/MoveThroughVillageGoal.h"
...
Zombie::Zombie(Level* level) : Monster(level) {
    ...
    goalSelector.addGoal(
        2,
        new MeleeAttackGoal(this, eTYPE_PLAYER, runSpeed, false)
    );
    goalSelector.addGoal(
        3,
        new MeleeAttackGoal(this, eTYPE_VILLAGER, runSpeed, true)
    );
    ...
    targetSelector.addGoal(
        2,
        new NearestAttackableTargetGoal(this, typeid(Villager), 16, 0, false)
    );
}
...
void Zombie::killed(std::shared_ptr<Mob> mob) {
    ...
    if (level->difficulty >= Difficulty::NORMAL
        ...
        level->levelEvent(
            nullptr,
            LevelEvent::SOUND_ZOMBIE_INFECTED,
            (int)x,
            (int)y,
            (int)z,
            0
...
bool Zombie::interact(std::shared_ptr<Player> player) {
    ...
    if (item != NULL && item->getItem() == Item::apple_gold
        ...
        if (!level->isClientSide) {
            startConverting(
                random->nextInt(
                    VILLAGER_CONVERSION_WAIT_MAX - VILLAGER_CONVERSION_WAIT_MIN
                    + 1
                )
...
void Zombie::finishConversion() {
    ...
    villager->addEffect(new MobEffectInstance(
        MobEffect::confusion->id,
        SharedConstants::TICKS_PER_SECOND * 10,
        0
    ));
    level->levelEvent(
        nullptr,
        LevelEvent::SOUND_ZOMBIE_CONVERTED,
        (int)x,
        (int)y,
        (int)z,
        0
