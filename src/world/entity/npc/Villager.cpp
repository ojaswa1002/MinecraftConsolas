...
#include "world/entity/ai/goal/InteractGoal.h"
#include "world/entity/ai/goal/LookAtPlayerGoal.h"
#include "world/entity/ai/goal/LookAtTradingPlayerGoal.h"
#include "world/entity/ai/goal/MakeLoveGoal.h"
#include "world/entity/ai/goal/MoveIndoorsGoal.h"
#include "world/entity/ai/goal/MoveTowardsRestrictionGoal.h"
#include "world/entity/ai/goal/OpenDoorGoal.h"
#include "world/entity/ai/goal/PlayGoal.h"
#include "world/entity/ai/goal/RandomStrollGoal.h"
#include "world/entity/ai/goal/RestrictOpenDoorGoal.h"
#include "world/entity/ai/goal/TakeFlowerGoal.h"
#include "world/entity/ai/goal/TradeWithPlayerGoal.h"
...
void Villager::_init(int profession) {
    ...
    goalSelector.addGoal(
        1,
        new AvoidPlayerGoal(this, typeid(Zombie), 8, 0.3f, 0.35f)
    );
...
void Villager::serverAiMobStep() {
    if (--villageUpdateInterval <= 0) {
        level->villages
            ->queryUpdateAround(Mth::floor(x), Mth::floor(y), Mth::floor(z));
...
void Villager::addOffers(int addCount) {
    ...
    case PROFESSION_FARMER:
        ...
        if (random->nextFloat() < .5f) {
            newOffers->push_back(new MerchantRecipe(
                ...
                std::shared_ptr<ItemInstance>(
                    new ItemInstance(Item::flint_Id, 2 + random->nextInt(2), 0)
                )
            ));
        }
        break;
    case PROFESSION_BUTCHER:
        ...
        addItemForPurchase(
            newOffers,
            Item::helmet_cloth_Id,
            random,
            getRecipeChance(.3f)
        );
        addItemForPurchase(
            newOffers,
            Item::leggings_cloth_Id,
            random,
            getRecipeChance(.3f)
        );
        addItemForPurchase(
    ...
    case PROFESSION_SMITH:
        ...
        addItemForPurchase(
            newOffers,
            Item::hatchet_diamond_Id,
            random,
            getRecipeChance(.3f)
        );
        addItemForPurchase(
            newOffers,
            Item::pickAxe_iron_Id,
            random,
            getRecipeChance(.5f)
        );
        addItemForPurchase(
        ...
        addItemForPurchase(
            newOffers,
            Item::helmet_diamond_Id,
            random,
            getRecipeChance(.2f)
        );
        addItemForPurchase(
            newOffers,
            Item::chestplate_iron_Id,
            random,
            getRecipeChance(.2f)
        );
        addItemForPurchase(
            newOffers,
            Item::chestplate_diamond_Id,
            random,
            getRecipeChance(.2f)
        );
        addItemForPurchase(
            newOffers,
            Item::leggings_iron_Id,
            random,
            getRecipeChance(.2f)
        );
        addItemForPurchase(
            newOffers,
            Item::leggings_diamond_Id,
            random,
            getRecipeChance(.2f)
        );
        addItemForPurchase(
        ...
        addItemForPurchase(
            newOffers,
            Item::helmet_chain_Id,
            random,
            getRecipeChance(.1f)
        );
        addItemForPurchase(
            newOffers,
            Item::chestplate_chain_Id,
            random,
            getRecipeChance(.1f)
        );
        addItemForPurchase(
            newOffers,
            Item::leggings_chain_Id,
            random,
            getRecipeChance(.1f)
        );
        break;
 ...
 * @param likelyHood
 */
void Villager::addItemForTradeIn(
    ...
    if (random->nextFloat() < likelyHood) {
        list->push_back(new MerchantRecipe(
            getItemTradeInValue(itemId, random),
            Item::emerald
        ));
    }
}
...
std::shared_ptr<ItemInstance>
Villager::getItemTradeInValue(int itemId, Random* random) {
 ...
 * @param likelyHood
 */
void Villager::addItemForPurchase(
    MerchantRecipeList* list,
    int                 itemId,
    Random*             random,
    float               likelyHood
...
std::shared_ptr<AgableMob>
Villager::getBreedOffspring(std::shared_ptr<AgableMob> target) {
