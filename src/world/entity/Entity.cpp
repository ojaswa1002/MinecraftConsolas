...
#include "world/item/enchantment/ProtectionEnchantment.h"
...
int Entity::entityCounter =
    2048; // 4J - changed initialiser to 2048, as we are using range 0 - 2047 as
          // special unique smaller ids for things that need network tracked
    ...
    if (((size_t)TlsGetValue(tlsIdx) != 0)) {
        ...
        if (server) {
            ...
            if (removedFound) {
                ...
                //				app.DebugPrintf("getSmallId: Removed entities
                // found\n");
            ...
            for (int j = 0; j < 32; j++) {
                ...
                if (puiRemovedFlags) {
                    if (puiRemovedFlags[i] & uiMask) {
                        //						app.DebugPrintf("Avoiding using
...
Entity::Entity(Level* level, bool useSmallId) // 4J - added useSmallId parameter
{
    ...
    entityData->define(
        DATA_AIR_SUPPLY_ID,
        TOTAL_AIR_SUPPLY
    ); // 4J Stu - Brought forward from 1.2.3 to fix 38654 - Gameplay: Player
       // will take damage when air bubbles are present if resuming game from
...
void Entity::setPos(double x, double y, double z) {
    ...
    bb->set(
        x - w,
        y - heightOffset + ySlideOffset,
        z - w,
        x + w,
        y - heightOffset + ySlideOffset + h,
        z + w
    );
}
...
) // 4J - added noEntityCubes parameter
{
    ...
    if (isPlayerSneaking) {
        ...
        while (xa != 0
               && level
                      ->getCubes(shared_from_this(), bb->cloneMove(xa, -1.0, 0))
                      ->empty()) {
            if (xa < d && xa >= -d) xa = 0;
            else if (xa > 0) xa -= d;
            else xa += d;
        ...
        while (za != 0
               && level
                      ->getCubes(shared_from_this(), bb->cloneMove(0, -1.0, za))
                      ->empty()) {
            if (za < d && za >= -d) za = 0;
            else if (za > 0) za -= d;
            else za += d;
        ...
        for (int x = x0; x <= x1; x++)
            for (int y = y0; y <= y1; y++)
                for (int z = z0; z <= z1; z++) {
                    ...
                    if (t > 0) {
                        Tile::tiles[t]
                            ->entityInside(level, x, y, z, shared_from_this());
                    }
                }
    }
}
...
void Entity::playStepSound(int xt, int yt, int zt, int t) {
    ...
    if (GetType() == eTYPE_PLAYER) {
        ...
        unsigned int uiAnimOverrideBitmask =
            getAnimOverrideBitmask(); // this is masked for custom anim off, and
                                      // force anim
...
bool Entity::updateInWaterState() {
    return level->checkAndHandleWater(
        bb->grow(0, -0.4f, 0)->shrink(0.001, 0.001, 0.001),
        Material::water,
        shared_from_this()
    );
}
...
bool Entity::isUnderLiquid(Material* material) {
    ...
    int    yt = Mth::floor(
        yp
    ); // 4J - this used to be a nested pair of floors for some reason
    int zt = Mth::floor(z);
    int t  = level->getTile(xt, yt, zt);
...
ListTag<DoubleTag>*
Entity::newDoubleList(unsigned int number, double firstValue, ...) {
...
ListTag<FloatTag>*
Entity::newFloatList(unsigned int number, float firstValue, float secondValue) {
...
Entity::spawnAtLocation(int resource, int count, float yOffs) {
...
void Entity::positionRider() {
    ...
    if (lockedRider) {
        ...
        lockedRider->setPos(
            x,
            y + getRideHeight() + lockedRider->getRidingHeight(),
            z
        );
    }
}
