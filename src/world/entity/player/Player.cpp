...
#include "world/item/enchantment/ThornsEnchantment.h"
...
void Player::spawnEatParticles(
    ...
    if (useItem->getUseAnimation() == UseAnim_eat) {
        for (int i = 0; i < count; i++) {
            ...
            level->addParticle(
                PARTICLE_ICONCRACK(useItem->getItem()->id, 0),
                p->x,
                p->y,
                p->z,
                d->x,
...
#ifndef _CONTENT_PACKAGE
    wprintf(
        L"Attempting to set skin to %08X for player %ls\n",
        skinId,
        name.c_str()
    );
#endif
...
#ifndef _CONTENT_PACKAGE
    wprintf(
        L"Attempting to set cape to %08X for player %s\n",
        capeId,
        name.c_str()
    );
#endif
...
DWORD Player::getCapeIdFromPath(const std::wstring& cape) {
    ...
    if (cape.size() >= 14) {
        ...
        // hex
        if (dlcCape) ss << std::dec << capeValue.c_str();
        else ss << std::hex << capeValue.c_str();
        ss >> capeId;
...
void Player::die(DamageSource* source) {
    ...
    if (app.isXuidNotch(m_xuid)) {
        drop(
            std::shared_ptr<ItemInstance>(new ItemInstance(Item::apple, 1)),
            true
        );
    }
    inventory->dropAll();
...
Player::drop(std::shared_ptr<ItemInstance> item, bool randomly) {
...
void Player::readAdditionalSaveData(CompoundTag* entityTag) {
    ...
    if (entityTag->contains(L"SpawnX") && entityTag->contains(L"SpawnY")
        ...
        respawnPosition = new Pos(
            entityTag->getInt(L"SpawnX"),
...
void Player::directAllTameWolvesOnTarget(
    std::shared_ptr<Mob> target,
    bool                 skipSitting
) {
    // filter un-attackable mobs
    if ((dynamic_pointer_cast<Creeper>(target) != NULL)
        || (dynamic_pointer_cast<Ghast>(target) != NULL)) {
        return;
    }
    ...
    auto itEnd = nearbyWolves->end();
    for (auto it = nearbyWolves->begin(); it != itEnd; it++) {
...
bool Player::interact(std::shared_ptr<Entity> entity) {
    if (entity->interact(dynamic_pointer_cast<Player>(shared_from_this())))
        return true;
...
void Player::attack(std::shared_ptr<Entity> entity) {
    ...
    if (dmg > 0 || magicBoost > 0) {
        ...
        if (dynamic_pointer_cast<Mob>(entity) && fireAspect > 0
            && !entity->isOnFire()) {
        ...
        if (wasHurt) {
            if (knockback > 0) {
                entity->push(
                    -Mth::sin(yRot * PI / 180) * knockback * .5f,
                    0.1,
...
Player::startSleepInBed(int x, int y, int z, bool bTestUse) {
...
bool Player::checkBed() {
    return (
        level->getTile(bedPosition->x, bedPosition->y, bedPosition->z)
        == Tile::bed_Id
    );
}
...
void Player::checkMovementStatistiscs(double dx, double dy, double dz) {
    ...
    if (isUnderLiquid(Material::water)) {
        ...
        if (distance > 0) {
            ...
            causeFoodExhaustion(
                FoodConstants::EXHAUSTION_SWIM * distance * .01f
            );
        }
    } else if (isInWater()) {
        ...
        if (horizontalDistance > 0) {
            ...
            if (distanceSwim >= 100) {
                ...
                awardStat(
                    GenericStats::swimOneM(),
                    GenericStats::param_swim(newDistance / 100)
                );
            }
            causeFoodExhaustion(
                FoodConstants::EXHAUSTION_SWIM * horizontalDistance * .01f
            );
        }
    } else if (onLadder()) {
        if (dy > 0) {
        ...
        if (horizontalDistance > 0) {
            ...
            if (distanceWalk >= 100) {
                ...
                awardStat(
                    GenericStats::walkOneM(),
                    GenericStats::param_walk(newDistance / 100)
                );
            }
            if (isSprinting()) {
                causeFoodExhaustion(
                    FoodConstants::EXHAUSTION_SPRINT * horizontalDistance * .01f
                );
            } else {
                causeFoodExhaustion(
                    FoodConstants::EXHAUSTION_WALK * horizontalDistance * .01f
                );
            }
        }
    }
...
void Player::killed(std::shared_ptr<Mob> mob) {
    ...
    // Hunter" Achievement.
    if (dynamic_pointer_cast<Monster>(mob) != NULL
        ...
        case eTYPE_SKELETON:
            if (mob->isRiding() && mob->riding->GetType() == eTYPE_SPIDER)
                awardStat(
                    GenericStats::killsSpiderJockey(),
                    GenericStats::param_noArgs()
                );
            else
                awardStat(
                    GenericStats::killsSkeleton(),
                    GenericStats::param_noArgs()
                );
            break;
        case eTYPE_SPIDER:
        ...
        case eTYPE_PIGZOMBIE:
            if (level->dimension->id == 0)
                awardStat(
                    GenericStats::killsZombiePigman(),
                    GenericStats::param_noArgs()
                );
            else
                awardStat(
                    GenericStats::killsNetherZombiePigman(),
                    GenericStats::param_noArgs()
                );
            break;
        case eTYPE_GHAST:
...
void Player::causeFoodExhaustion(float amount) {
    ...
    if (app.GetGameHostOption(eGameHostOption_TrustPlayers) == 0
        && getPlayerGamePrivilege(Player::ePlayerGamePrivilege_CannotBuild)
               != 0)
        return;
...
void Player::restoreFrom(std::shared_ptr<Player> oldPlayer, bool restoreAll) {
    if (restoreAll) {
        ...
        for (auto it = oldPlayer->activeEffects.begin();
             it != oldPlayer->activeEffects.end();
             ++it) {
...
int Player::hash_fnct(const std::shared_ptr<Player> k) {
    ...
    return (int)boost::hash_value(
        k->name
    ); // 4J Stu - Names are completely unique?
#else
    return (int)std::hash<std::wstring>{}(
        k->name
    ); // 4J Stu - Names are completely unique?
#endif //__PS3__
}
...
unsigned int Player::getPlayerGamePrivilege(
...
void Player::setPlayerGamePrivilege(
    EPlayerGamePrivileges privilege,
    unsigned int          value
) {
...
bool Player::isAllowedToTeleport() {
    bool allowed = false;
    if (isModerator()
        && getPlayerGamePrivilege(Player::ePlayerGamePrivilege_CanTeleport)
               != 0) {
        allowed = true;
    }
    return allowed;
...
bool Player::hasInvisiblePrivilege() {
    ...
    if (app.GetGameHostOption(eGameHostOption_HostCanBeInvisible) != 0
        && getPlayerGamePrivilege(Player::ePlayerGamePrivilege_Invisible)
               != 0) {
        enabled = true;
    }
    return enabled;
...
void Player::enableAllPlayerPrivileges(
    unsigned int& uigamePrivileges,
    bool          enable
) {
    Player::setPlayerGamePrivilege(
            ...
            if (pDLCSkinFile != NULL) {
                ...
                if (dwBoxC != 0) {
                    ...
                    m_ppAdditionalModelParts = app.SetAdditionalSkinBoxes(
                        m_dwSkinId,
                        pDLCSkinFile->getAdditionalBoxes()
                    );
                }
