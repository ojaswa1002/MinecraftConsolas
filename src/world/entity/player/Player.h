...
#include "world/inventory/PlayerEnderChestContainer.h"
...
public:
    ...
    drop(std::shared_ptr<ItemInstance> item, bool randomly);
...
protected:
    virtual int
    getDamageAfterMagicAbsorb(DamageSource* damageSource, int damage);
    virtual bool isPlayerVersusPlayer();
    void
    directAllTameWolvesOnTarget(std::shared_ptr<Mob> target, bool skipSitting);
...
public:
    ...
    virtual bool openFurnace(
        std::shared_ptr<FurnaceTileEntity> container
    ); // 4J - added bool return
    ...
    virtual void
    openItemInstanceGui(std::shared_ptr<ItemInstance> itemInstance);
...
public:
    ...
    startSleepInBed(int x, int y, int z, bool bTestUse = false);
...
public:
    ...
    virtual Icon*
    getItemInHandIcon(std::shared_ptr<ItemInstance> item, int layer);
...
public:
    ...
    bool      isHurt();
    virtual void
         startUsingItem(std::shared_ptr<ItemInstance> instance, int duration);
...
public:
    ...
    virtual void
    restoreFrom(std::shared_ptr<Player> oldPlayer, bool restoreAll);
...
public:
    ...
    static bool
    eq_test(const std::shared_ptr<Player> x, const std::shared_ptr<Player> y);
...
public:
    ...
    static unsigned int getPlayerGamePrivilege(
    ...
    static void setPlayerGamePrivilege(
        unsigned int&         uiGamePrivileges,
        EPlayerGamePrivileges privilege,
        unsigned int          value
    );
    ...
    static void
         enableAllPlayerPrivileges(unsigned int& uigamePrivileges, bool enable);
...
public:
    ...
    void
    SetAdditionalModelParts(std::vector<ModelPart*>* ppAdditionalModelParts);
...
private:
    ...
    ePlayerNameValidState
        m_ePlayerNameValidState; // 4J-PB - to ensure we have the characters for
                                 // this name in our font, or display a player
                                 // number instead
#endif
};
