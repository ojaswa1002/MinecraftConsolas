...
Inventory::getResourceItem(int type, int iAuxVal) {
    int slot = getSlot(type, iAuxVal);
    if (slot < 0) return nullptr;
    return getItem(slot);
}
    ...
    if (!item->isDamaged()) {
        ...
        if (item->count < lastSize) {
            player->awardStat(
                ...
                GenericStats::param_itemsCollected(
                    item->id,
                    item->getAuxValue(),
                    count
                )
...
std::shared_ptr<ItemInstance>
Inventory::removeItem(unsigned int slot, int count) {
...
void Inventory::setItem(unsigned int slot, std::shared_ptr<ItemInstance> item) {
    ...
    if (item != NULL) {
        ...
        app.DebugPrintf(
            "Inventory::setItem - slot = %d,\t item = %d ",
            slot,
            item->id
        );
...
void Inventory::hurtArmor(int dmg) {
    ...
    for (unsigned int i = 0; i < armor.length; i++) {
        if (armor[i] != NULL
            ...
            armor[i]->hurt(
                dmg,
                std::dynamic_pointer_cast<Mob>(player->shared_from_this())
            );
            if (armor[i]->count == 0) {
                armor[i] = nullptr;
            }
