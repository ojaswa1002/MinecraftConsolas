...
public:
    ...
    virtual int
    getLightColor(float a); // 4J - change brought forward from 1.8.2
...
protected:
    ListTag<DoubleTag>*
    newDoubleList(unsigned int number, double firstValue, ...);
    ListTag<FloatTag>*
    newFloatList(unsigned int number, float firstValue, float secondValue);

public:
    virtual float               getShadowHeightOffs();
    std::shared_ptr<ItemEntity> spawnAtLocation(int resource, int count);
    std::shared_ptr<ItemEntity>
    spawnAtLocation(int resource, int count, float yOffs);
    std::shared_ptr<ItemEntity>
    spawnAtLocation(std::shared_ptr<ItemInstance> itemInstance, float yOffs);
...
private:
    ...
    static const int EXTRA_WANDER_TICKS =
        20
        * 30; // Number of ticks each extra entity will be allowed to wander
              // for. This should be enough for it to realistically be able to
              // walk further than the biggest enclosure we want to consider
