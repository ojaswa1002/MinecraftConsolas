...
Sheep::Sheep(Level* level) : Animal(level) {
    ...
    container->setItem(
        0,
        std::shared_ptr<ItemInstance>(new ItemInstance(Item::dye_powder, 1, 0))
    );
    container->setItem(
        1,
        std::shared_ptr<ItemInstance>(new ItemInstance(Item::dye_powder, 1, 0))
...
bool Sheep::interact(std::shared_ptr<Player> player) {
    ...
    if (item != NULL && item->id == Item::shears->id && !isSheared()
        ...
        if (!level->isClientSide) {
            ...
            player->awardStat(
                GenericStats::shearedEntity(eTYPE_SHEEP),
...
Sheep::getBreedOffspring(std::shared_ptr<AgableMob> target) {
    ...
    if (level->canCreateMore(GetType(), Level::eSpawnType_Breed)) {
        ...
        int                    color = getOffspringColor(
            std::dynamic_pointer_cast<Animal>(shared_from_this()),
            otherSheep
        );
