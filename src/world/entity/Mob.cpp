...
bool Mob::canSee(std::shared_ptr<Entity> target) {
    HitResult* hres = level->clip(
        ...
        Vec3::newTemp(target->x, target->y + target->getHeadHeight(), target->z)
    );
...
void Mob::baseTick() {
    ...
    if (lastHurtByPlayerTime > 0) lastHurtByPlayerTime--;
        ...
        // shared_ptr and free an old one, when generally this won't be doing
        // anything at all. This is the lightweight but ugly alternative
        if (lastHurtByPlayer) {
...
void Mob::spawnAnim() {
    for (int i = 0; i < 20; i++) {
        ...
        level->addParticle(
            ...
            z + random->nextFloat() * bbWidth * 2 - bbWidth - za * dd,
            xa,
            ya,
            za
        );
...
bool Mob::hurt(DamageSource* source, int dmg) {
    ...
    if (source->isFire() && hasEffect(MobEffect::fireResistance)) {
        ...
        if (plr != NULL && source == DamageSource::lava) // Only award when in
                                                         // lava (not any fire).
        {
            plr->awardStat(
                GenericStats::stayinFrosty(),
        ...
        for (int i = 0; i < effects->size(); i++) {
            ...
            activeEffects.insert(
                std::unordered_map<int, MobEffectInstance*>::value_type(
                    id,
                    new MobEffectInstance(id, duration, amplifier)
                )
            );
        }
    }
...
void Mob::checkDespawn() {
    ...
    if (player != NULL) {
        ...
        if (noActionTime > 20 * 30 && random->nextInt(800) == 0 && sd > 32 * 32
            && removeWhenFarAway()) {
            remove();
        } else if (sd < 32 * 32) {
            noActionTime = 0;
...
float Mob::getWalkingSpeedModifier() {
    ...
    if (hasEffect(MobEffect::movementSlowdown)) {
        speed *= 1.0f
               - .15f
                     * (getEffect(MobEffect::movementSlowdown)->getAmplifier()
                        + 1);
    }
    return speed;
}
...
void Mob::breakItem(std::shared_ptr<ItemInstance> itemInstance) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        level->addParticle(
            PARTICLE_ICONCRACK(itemInstance->getItem()->id, 0),
            p->x,
            p->y,
            p->z,
            d->x,
