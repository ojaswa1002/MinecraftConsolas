...
class MouseInventoryClickHandler {
    /*	public static void handle(int buttonNum, boolean clickedOutside, int
       ...
       slot.mayPlace(inventory.carried)) { int c = buttonNum == 0 ?
       inventory.carried.count : 1; if (c > slot.getMaxStackSize()) c =
       slot.getMaxStackSize(); slot.set(inventory.carried.remove(c)); if
       (inventory.carried.count == 0) inventory.carried = null; } else if
       (clicked != null && inventory.carried != null) {

                    if (slot.mayPlace(inventory.carried)) {
                        if (clicked.id != inventory.carried.id) {
                            if (inventory.carried.count <=
       ...
       inventory.carried = null; clicked.count += c; } else if (buttonNum == 1)
       { int c = 1; if (c > slot.getMaxStackSize() - clicked.count) c =
       slot.getMaxStackSize() - clicked.count; if (c >
       inventory.carried.getMaxStackSize() - clicked.count) c =
       inventory.carried.getMaxStackSize() - clicked.count;
                                inventory.carried.remove(c);
                                if (inventory.carried.count == 0)
       inventory.carried = null; clicked.count += c;
                            }
                        }
                    } else {
                        if (clicked.id == inventory.carried.id &&
       inventory.carried.getMaxStackSize() > 1) { int c = clicked.count; if (c >
       0 && c + inventory.carried.count <= inventory.carried.getMaxStackSize())
       { inventory.carried.count += c; clicked.remove(c); if (clicked.count ==
       0) slot.set(null); slot.onTake();
                ...
                if (clickedOutside) {
                    ...
                    if (buttonNum == 1) {
                        ...
                        if (inventory.carried.count == 0) inventory.carried =
       null;
                    }
                }
            }
