...
#ifdef __PSVITA__
#include "psvita/psvitaextras/CustomSet.h"
#endif
...
protected:
    // This is the only shared_ptr ref to levelStorage - we need to keep this as
    // long as at least one Level references it, to be able to cope with moving
    // from dimension to dimension where the Level(Level *level, Dimension
    // *dimension) ctor is used
...
public:
    ...
    bool
    reallyHasChunksAt(int x0, int y0, int z0, int x1, int y1, int z1); // 4J
                                                                       // added
...
public:
    ...
    virtual bool
    setTileAndDataNoUpdate(int x, int y, int z, int tile, int data);
    virtual bool setTileAndDataNoUpdate(
        int  x,
        int  y,
        int  z,
        int  tile,
...
public:
    ...
    void setBrightnessNoUpdateOnClient(
        LightLayer::variety layer,
        int                 x,
        int                 y,
        int                 z,
...
#endif
    inline void setBrightnessCached(
        lightCache_t*       cache,
    ...
    inline int getBrightnessCached(
        lightCache_t*       cache,
        LightLayer::variety layer,
        int                 x,
        int                 y,
    ...
    inline int
    getEmissionCached(lightCache_t* cache, int ct, int x, int y, int z);
    inline int getBlockingCached(
        lightCache_t*       cache,
        LightLayer::variety layer,
        int*                ct,
        int                 x,
...
public:
    ...
    AABBList* getTileCubes(
        AABB* box,
        bool  blockAtEdge
    ); // 4J Stu - Brought forward from 12w36 to fix #46282 - TU5: Gameplay:
       // Exiting the minecart in a tight corridor damages the player
    ...
    virtual void
    addToTickNextTick(int x, int y, int z, int tileId, int tickDelay);
    virtual void
    forceAddTileTick(int x, int y, int z, int tileId, int tickDelay);
    ...
    // cannons
    std::shared_ptr<Explosion> explode(
        std::shared_ptr<Entity> source,
        double                  x,
        double                  y,
        double                  z,
    ...
    void
    setTileEntity(int x, int y, int z, std::shared_ptr<TileEntity> tileEntity);
    ...
    bool         isSolidBlockingTileInLoadedChunk(
                int  x,
                int  y,
                int  z,
    ...
    virtual bool
    isTopSolidBlocking(int x, int y, int z); // 4J - brought forward from 1.3.2
...
public:
    ...
    void checkLight(
        ...
        int  z,
        bool force            = false,
        bool rootOnlyEmissive = false
    ); // 4J added force, rootOnlySource parameters
private:
    int* toCheckLevel;
    int  getExpectedSkyColor(
         lightCache_t* cache,
         int           oc,
         int           x,
         int           y,
...
public:
    ...
    virtual std::vector<TickNextTickData>*
...
public:
    ...
    getEntities(std::shared_ptr<Entity> except, AABB* bb);
    std::vector<std::shared_ptr<Entity>>*
    ...
    unsigned int countInstanceOfInRange(
        eINSTANCEOF clas,
        bool        singleType,
        int         range,
        int         x,
    ...
    getNearestPlayer(double x, double z, double maxDist);
    std::shared_ptr<Player>
    getNearestAttackablePlayer(std::shared_ptr<Entity> source, double maxDist);
    std::shared_ptr<Player>
    getNearestAttackablePlayer(double x, double y, double z, double maxDist);

    std::shared_ptr<Player> getPlayerByName(const std::wstring& name);
    std::shared_ptr<Player>
              getPlayerByUUID(const std::wstring& name); // 4J Added
    byteArray getBlocksAndData(
        int  x,
        int  y,
        int  z,
        int  xs,
    ...
    void         setOverrideTimeOfDay(
                std::int64_t time
            ); // 4J Added so we can override timeOfDay without changing tick time
    std::int64_t getSeed();
    std::int64_t getTime();
    ...
    std::shared_ptr<SavedData>
         getSavedData(const type_info& clazz, const std::wstring& id);
    ...
    TilePos*
    findNearestMapFeature(const std::wstring& featureName, int x, int y, int z);
