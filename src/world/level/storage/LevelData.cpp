...
LevelData::LevelData(CompoundTag* tag) {
    ...
    newSeaLevel = tag->getBoolean(
        L"newSeaLevel"
    ); // 4J added - only use new sea level for newly created maps. This read
       // defaults to false. (sea level changes in 1.8.2)
    hasBeenInCreative = tag->getBoolean(
        L"hasBeenInCreative"
    ); // 4J added so we can not award achievements to levels modified in
       // creative
...
LevelData::LevelData(LevelData* copy) {
    ...
    //    this->loadedPlayerTag = copy->loadedPlayerTag;		// 4J - we don't
    //    store this anymore
...
CompoundTag*
LevelData::createTag(std::vector<std::shared_ptr<Player>>* players) {
...
std::int64_t LevelData::getSeed() { return seed; }
...
std::int64_t LevelData::getTime() { return time; }

std::int64_t LevelData::getSizeOnDisk() { return sizeOnDisk; }
...
void LevelData::setSeed(std::int64_t seed) { this->seed = seed; }
...
void LevelData::setTime(std::int64_t time) { this->time = time; }

void LevelData::setSizeOnDisk(std::int64_t sizeOnDisk) {
...
std::int64_t LevelData::getLastPlayed() { return lastPlayed; }
