...
void DirectoryLevelStorage::PlayerMappings::addMapping(
    ...
    std::int64_t index = (((std::int64_t)(centreZ & 0x1FFFFFFF)) << 34)
                       | (((std::int64_t)(centreX & 0x1FFFFFFF)) << 5)
...
bool DirectoryLevelStorage::PlayerMappings::getMapping(
    int& id,
    int  centreX,
    int  centreZ,
    int  dimension,
    ...
    // std::int64_t zMasked = centreZ & 0x1FFFFFFF;
    // std::int64_t xMasked = centreX & 0x1FFFFFFF;
    // std::int64_t zShifted = zMasked << 34;
    // std::int64_t xShifted = xMasked << 5;
...
void DirectoryLevelStorage::PlayerMappings::readMappings(DataInputStream* dis) {
    ...
    for (unsigned int i = 0; i < count; ++i) {
        std::int64_t index = dis->readLong();
        int          id    = dis->readInt();
        m_mappings[index]  = id;
...
CompoundTag* DirectoryLevelStorage::loadPlayerDataTag(PlayerUID xuid) {
    ...
    if (it != m_cachedSaveData.end()) {
        ...
        bis.reset();
        app.DebugPrintf(
            "Loaded player data from cached file %ls\n",
            realFile.getName().c_str()
        );
        return tag;
...
int DirectoryLevelStorage::getAuxValueForMap(
    PlayerUID xuid,
    int       dimension,
    int       centreXC,
    int       centreZC,
    ...
    if (!foundMapping) {
        for (unsigned int i = 0; i < m_usedMappings.length; ++i) {
            if (m_usedMappings[i] < 0xFF) {
                ...
                mapId = (i * 8) + offset;
                m_playerMappings[xuid]
                    .addMapping(mapId, centreXC, centreZC, dimension, scale);
    ...
    if (!foundMapping && mapId >= 0 && mapId < MAXIMUM_MAP_SAVE_DATA) {
        ...
        if (m_saveFile->doesFileExist(file)) {
            auto it = find(
                m_mapFilesToDelete.begin(),
                m_mapFilesToDelete.end(),
                mapId
            );
...
void DirectoryLevelStorage::dontSaveMapMappingForPlayer(PlayerUID xuid) {
    ...
    if (it != m_playerMappings.end()) {
        for (auto itMap = it->second.m_mappings.begin();
             itMap != it->second.m_mappings.end();
             ++itMap) {
...
void DirectoryLevelStorage::saveAllCachedData() {
    ...
    for (auto it = m_cachedSaveData.begin(); it != m_cachedSaveData.end();
        ...
        app.DebugPrintf(
            "Actually writing cached file %ls\n",
            it->first.c_str()
        );
