...
public:
    ...
    virtual void playerRemoved(
        std::shared_ptr<Entity> entity
    ) = 0; // 4J added - for when a player is removed from the level's player
           // array, not just the entity storage

    virtual void skyColorChanged() = 0;

    virtual void
    playStreamingMusic(const std::wstring& name, int x, int y, int z) = 0;

    virtual void levelEvent(
    ...
    virtual void
    destroyTileProgress(int id, int x, int y, int z, int progress) = 0;
};
