...
const int
MobSpawner::tick(ServerLevel* level, bool spawnEnemies, bool spawnFriendlies) {
...
#ifdef __PSVITA__
                    chunksToPoll.insert(
                        ChunkPos((xx[i] - r) + l, (zz[i] - r)),
                        false
                    );
                    chunksToPoll.insert(
                        ChunkPos((xx[i] + r), (zz[i] - r) + l),
                        false
                    );
                    chunksToPoll.insert(
                        ChunkPos((xx[i] + r) - l, (zz[i] + r)),
                ...
                for (int ll = 0; ll < 4; ll++) {
                    ...
                    if (isSpawnPositionOk(mobCategory, level, x, y, z)) {
                        ...
                        if (currentMobType == NULL) {
                            currentMobType =
                                level
                                    ->getRandomMobSpawnAt(mobCategory, x, y, z);
                            if (currentMobType == NULL) {
                                break;
                            }
                        }
                        ...
                        //						   catch (exception e)
                        //						   {
                        //							   // TODO 4J We can't print
                        // a stack trace, and the newInstance function doesn't
                        // throw an exception just now anyway
                        //							   //e.printStackTrace();
                        //							   return count;
                        //						   }
...
void MobSpawner::finalizeMobSettings(
    ...
    if (dynamic_pointer_cast<Spider>(mob) != NULL
        ...
        (dynamic_pointer_cast<Sheep>(mob))
        ...
        for (int attemptCount = 0; attemptCount < 20 && !nextPlayer;
...

            {
                ...
                if (y >= (yStart + 16) || y >= Level::maxBuildHeight) {
                    ...
                    mob = dynamic_pointer_cast<Mob>(
                        EntityIO::newByEnumType(bedEnemies[type], level)
                    );
                    //					}
                    //					catch (exception e)
                    //					{
                    //						// TODO 4J Stu - We can't print a
                    // stack trace, and newInstance doesn't currently throw an
                    // exception anyway
                    //						//e.printStackTrace();
...
void MobSpawner::postProcessSpawnMobs(
    Level*  level,
    Biome*  biome,
    int     xo,
    int     zo,
