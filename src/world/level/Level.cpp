...
#include "world/entity/ai/village/VillageSiege.h"
...
void inline Level::setBrightnessCached(
    lightCache_t*       cache,
...
inline int Level::getBrightnessCached(
    lightCache_t*       cache,
    LightLayer::variety layer,
    int                 x,
    int                 y,
    ...
    if (y & 0xffffff00)
        return getBrightness(
            ...
            y,
            z
        ); // Fall back on original method for out-of-bounds y
...
// thread
inline int
Level::getEmissionCached(lightCache_t* cache, int ct, int x, int y, int z) {
...
// thread
inline int Level::getBlockingCached(
    lightCache_t*       cache,
    LightLayer::variety layer,
    int*                ct,
    int                 x,
...
bool Level::setTileAndDataNoUpdate(
    int  x,
    int  y,
    int  z,
    int  tile,
...
void Level::updateLightIfOtherThan(
    LightLayer::variety layer,
    int                 x,
    int                 y,
    int                 z,
...
int Level::getBrightnessPropagate(
    LightLayer::variety layer,
    int                 x,
    int                 y,
    int                 z,
...
void Level::getNeighbourBrightnesses(
    int*                brightnesses,
    LightLayer::variety layer,
    int                 x,
    int                 y,
...
void Level::removeEntity(std::shared_ptr<Entity> e) {
    ...
    if (dynamic_pointer_cast<Player>(e) != NULL) {
        ...
        playerRemoved(
            e
        ); // 4J added - this will let the entity tracker know that we have
           // actually removed the player from the level's player list
    }
}
...
AABBList* Level::getCubes(
    ...
    Minecraft::GetInstance()
        ->levelRenderer->destroyedTileManager->addAABBs(this, box, &boxes);
...
void Level::tickEntities() {
    ...
    for (unsigned int i = 0; i < entities.size();) {
        ...
        if (!e->removed) {
            ...
            if (!(app.DebugSettingsOn() && app.GetMobsDontTickEnabled()
                  && (dynamic_pointer_cast<Mob>(e) != NULL)
    ...
    if (!tileEntitiesToUnload.empty()) {
        ...
        for (auto it = tileEntityList.begin(); it != tileEntityList.end();) {
            bool found = false;
            for (auto it2 = tileEntitiesToUnload.begin();
                 it2 != tileEntitiesToUnload.end();
                 it2++) {
                if ((*it) == (*it2)) {
                    found = true;
                    break;
...
bool Level::checkAndHandleWater(
    AABB*                   box,
    Material*               material,
...
std::shared_ptr<Explosion> Level::explode(
    std::shared_ptr<Entity> source,
    double                  x,
    double                  y,
    double                  z,
...
void Level::removeTileEntity(int x, int y, int z) {
    ...
    if (te != NULL && updatingTileEntities) {
        ...
        if (te != NULL) {
            auto it = find(
                pendingTileEntities.begin(),
                pendingTileEntities.end(),
                te
            );
            if (it != pendingTileEntities.end()) {
                pendingTileEntities.erase(it);
...
bool Level::isSolidBlockingTileInLoadedChunk(
    int  x,
    int  y,
    int  z,
...
void Level::tickWeather() {
    ...
    if (app.DebugSettingsOn()) {
        if (app.GetGameSettingsDebugMask(ProfileManager.GetPrimaryPad())
            ...
            levelData->setThunderTime(
                random->nextInt(TICKS_PER_DAY * 7) + TICKS_PER_DAY / 2
            );
    ...
    if (thunderTime <= 0) {
        if (levelData->isThundering()) {
            levelData->setThunderTime(
                random->nextInt(20 * 60 * 10) + 20 * 60 * 3
            );
        } else {
            levelData->setThunderTime(
                random->nextInt(TICKS_PER_DAY * 7) + TICKS_PER_DAY / 2
    ...
    if (rainTime <= 0) {
        if (levelData->isRaining()) {
            levelData->setRainTime(
                random->nextInt(TICKS_PER_DAY / 2) + TICKS_PER_DAY / 2
            );
        } else {
            levelData->setRainTime(
                random->nextInt(TICKS_PER_DAY * 7) + TICKS_PER_DAY / 2
...
int Level::getExpectedBlockColor(
    lightCache_t* cache,
    int           oc,
    int           x,
    int           y,
...

    {
        ...
        int ex = 0;
        {
            ...
            if (layer == LightLayer::Sky) {
                ...
                expected = getExpectedBlockColor(
                    cache,
                    cc,
                    xc,
                    yc,
        ...
        if (ex > cc) {
            ...
            //			darktcc = tcc;
            /////////////////////////////////////////////////////
            tcc = tcn; // 4J added - we've moved all the edge tiles to the start
                       // of the array, so only need to process these now. The
                       // original processes all tcc tiles again in the next
                       // section
        }
    }
...
Level::fetchTicksInChunk(LevelChunk* chunk, bool remove) {
    return NULL;
}


std::vector<std::shared_ptr<Entity>>*
Level::getEntities(std::shared_ptr<Entity> except, AABB* bb) {
    MemSect(40);
    es.clear();
...
std::vector<std::shared_ptr<Entity>>*
Level::getEntitiesOfClass(const type_info& baseClass, AABB* bb) {
    int                                   xc0 = Mth::floor((bb->x0 - 2) / 16);
    int                                   xc1 = Mth::floor((bb->x1 + 2) / 16);
    int                                   zc0 = Mth::floor((bb->z0 - 2) / 16);
    int                                   zc1 = Mth::floor((bb->z1 + 2) / 16);
...
unsigned int Level::countInstanceOf(
    eINSTANCEOF   clas,
    bool          singleType,
    unsigned int* protectedCount /* = NULL*/,
...
unsigned int Level::countInstanceOfInRange(
    eINSTANCEOF clas,
    bool        singleType,
    int         range,
    int         x,
...
bool Level::mayPlace(
    ...
    bool                    ignoreEntities,
    int                     face,
    std::shared_ptr<Entity> ignoreEntity
) {
    ...
    if (targetTile != NULL
        && (targetTile == Tile::water || targetTile == Tile::calmWater
            ...
            || targetTile == Tile::fire
            || targetTile->material->isReplaceable()))
        targetTile = NULL;
...
std::shared_ptr<Player> Level::getNearestPlayer(
    std::shared_ptr<Entity> source,
    double                  maxDist,
    double                  maxYDist /*= -1*/
) {
...
Level::getNearestPlayer(double x, double z, double maxDist) {
    double                  best   = -1;
    std::shared_ptr<Player> result = nullptr;
    auto                    itEnd  = players.end();
    for (auto it = players.begin(); it != itEnd; it++) {
...
byteArray Level::getBlocksAndData(
    int  x,
    int  y,
    int  z,
    int  xs,
    ...
    for (int xc = xc0; xc <= xc1; xc++) {
        ...
        for (int zc = zc0; zc <= zc1; zc++) {
            ...
            if (z0 < 0) z0 = 0;
            if (z1 > 16) z1 = 16;
            p = getChunk(xc, zc)->getBlocksAndData(
                &result,
                x0,
                y0,
                z0,
...
std::int64_t Level::getSeed() { return levelData->getSeed(); }


std::int64_t Level::getTime() { return levelData->getTime(); }
...
Level::getSavedData(const type_info& clazz, const std::wstring& id) {
...
Random* Level::getRandomFor(int x, int z, int blend) {
    std::int64_t seed = (x * 341873128712l + z * 132897987541l)
...
TilePos* Level::findNearestMapFeature(
