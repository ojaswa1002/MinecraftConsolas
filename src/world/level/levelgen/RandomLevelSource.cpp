...
: generateStructures(generateStructures) {
    ...
    pprandom =
        new Random(seed); // 4J - added, so that we can have a separate random
                          // for doing post-processing in parallel with creation
...
void RandomLevelSource::prepareHeights(int xOffs, int zOffs, byteArray blocks) {
    ...
    level->getBiomeSource()->getRawBiomeBlock(
        biomes,
        xOffs * CHUNK_WIDTH - 2,
        zOffs * CHUNK_WIDTH - 2,
        xSize + 5,
                ...
                for (int y = 0; y < CHUNK_HEIGHT; y++) {
                    ...
                    for (int x = 0; x < CHUNK_WIDTH; x++) {
                        int offs = (x + xc * CHUNK_WIDTH)
                                 ...
                                 | (0 + zc * CHUNK_WIDTH) << Level::genDepthBits
                                 | (yc * CHUNK_HEIGHT + y);
                        ...
                        for (int z = 0; z < CHUNK_WIDTH; z++) {
                            ...
                            const int falloffStart =
                                32; // chunks away from edge were we start doing
                                    // fall-off
...
void RandomLevelSource::buildSurfaces(
    ...
    doubleArray depthBuffer(
        16 * 16
    ); // 4J - used to be declared with class level scope but moved here for
       // thread safety

    depthBuffer = perlinNoise3->getRegion(
        depthBuffer,
        xOffs * 16,
        zOffs * 16,
        0,
        ...
        for (int z = 0; z < 16; z++) {
            ...
            for (int y = Level::genDepthMinusOne; y >= 0; y--) {
                ...
                if (y <= 1
                             + random->nextInt(
                                 2
                             )) // 4J - changed to make the bedrock not have
                                // bits you can get stuck in
...
LevelChunk* RandomLevelSource::getChunk(int xOffs, int zOffs) {
    ...
    level->getBiomeSource()
        ->getBiomeBlock(biomes, xOffs * 16, zOffs * 16, 16, 16, true);
    ...
    //    levelChunk->recalcHeightmap();		// 4J - removed & moved into its
    //    own method
...
doubleArray RandomLevelSource::getHeights(
    doubleArray buffer,
    int         x,
    int         y,
    int         z,
    ...
    if (FLOATING_ISLANDS) {
        fis = floatingIslandScale
                  ->getRegion(fis, x, y, z, xSize, 1, zSize, 1.0, 0, 1.0);
        fi = floatingIslandNoise
                 ->getRegion(fi, x, y, z, xSize, 1, zSize, 500.0, 0, 500.0);
    }
    ...
    g_lperlinNoise1_SPU
        .set(lperlinNoise1, ar, x, y, z, xSize, ySize, zSize, s, hs, s);
    g_lperlinNoise2_SPU
        .set(lperlinNoise2, br, x, y, z, xSize, ySize, zSize, s, hs, s);
        ...
        for (int z = 0; z < 16; z++) {
            ...
            if (level->shouldFreezeIgnoreNeighbors(x + xo, y - 1, z + zo)) {
                level->setTileNoUpdate(
                    ...
                    z + zo,
                    Tile::ice_Id
                ); // 4J - changed from setTile, otherwise we end up creating a
                   // *lot* of dynamic water tiles as these ice tiles are set
            }
...
RandomLevelSource::getMobsAt(MobCategory* mobCategory, int x, int y, int z) {
