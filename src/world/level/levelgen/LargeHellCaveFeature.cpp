...
void LargeHellCaveFeature::addRoom(
    int       xOffs,
    int       zOffs,
    byteArray blocks,
    double    xRoom,
...
void LargeHellCaveFeature::addTunnel(
    int       xOffs,
    int       zOffs,
    byteArray blocks,
    double    xCave,
            ...
            for (int zz = z0; zz < z1; zz++) {
                ...
                for (int yy = y1 - 1; yy >= y0; yy--) {
                    ...
                    if (yd > -0.7 && xd * xd + yd * yd + zd * zd < 1) {
                        int block = blocks[p];
                        if (block == Tile::hellRock_Id || block == Tile::dirt_Id
                            || block == Tile::grass_Id) {
                            blocks[p] = (byte)0;
                        }
                    }
...
void LargeHellCaveFeature::addFeature(
    Level*    level,
    int       x,
    int       z,
    int       xOffs,
