...
void CustomLevelSource::buildSurfaces(
    int        xOffs,
    int        zOffs,
    byteArray  blocks,
    BiomeArray biomes
            ...
            for (int y = Level::maxBuildHeight - 1; y >= 0; y--) {
                ...
                if (y
                    <= 1
                           + random->nextInt(
                               2
                           )) // 4J - changed to make the bedrock not have bits
...
CustomLevelSource::getMobsAt(MobCategory* mobCategory, int x, int y, int z) {
#ifdef _OVERRIDE_HEIGHTMAP
