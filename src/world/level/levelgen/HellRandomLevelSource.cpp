...
#include "world/level/levelgen/feature/HellPortalFeature.h"
#include "world/level/levelgen/feature/HellSpringFeature.h"
#include "world/level/levelgen/feature/LightGemFeature.h"
...
HellRandomLevelSource::HellRandomLevelSource(Level* level, std::int64_t seed) {
...
void HellRandomLevelSource::prepareHeights(
    int       xOffs,
    int       zOffs,
    byteArray blocks
) {
...
void HellRandomLevelSource::buildSurfaces(
    int       xOffs,
    int       zOffs,
    byteArray blocks
) {
    ...
    sandBuffer = perlinNoise2->getRegion(
        sandBuffer,
        xOffs * 16,
        zOffs * 16,
        0,
    ...
    gravelBuffer = perlinNoise2->getRegion(
        gravelBuffer,
        xOffs * 16,
        109,
        zOffs * 16,
                                ...
                                if (sand) {
                                    ...
                                    if (random->nextInt(16) == 0) {
                                        ...
                                        int genDepthMinusOne  = Level::
                                            genDepthMinusOne; // Take into local
                                                              // int for PS4 as
                                                              // min takes a
                                                              // reference to
                                                              // the const int
...
doubleArray HellRandomLevelSource::getHeights(
    doubleArray buffer,
    int         x,
    int         y,
    int         z,
...
bool HellRandomLevelSource::save(
    bool              force,
