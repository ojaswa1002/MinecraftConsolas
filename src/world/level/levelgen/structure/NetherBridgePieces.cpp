...
NetherBridgePieces::PieceWeight::PieceWeight(
...
NetherBridgePieces::PieceWeight* NetherBridgePieces::bridgePieceWeights
    [NetherBridgePieces::BRIDGE_PIECEWEIGHTS_COUNT] = {
        ...
        new PieceWeight(
            ...
            15,
            2
        ), // 4J Stu - Increased weight to ensure that we have these (was 5),
           // required for Blazes and therefore required for brewing
        new PieceWeight(
            EPieceClass_CastleEntrance,
            15,
            1
        ), // 4J Stu - Increased weight to ensure that we have these (was 5),
           // required for CastleStalkRoom, and therefore required for brewing
...
NetherBridgePieces::PieceWeight* NetherBridgePieces::castlePieceWeights
    [NetherBridgePieces::CASTLE_PIECEWEIGHTS_COUNT] = {
        new PieceWeight(
           ...
           // required for Nether Wart, and therefore required for brewing
...
NetherBridgePieces::findAndCreateBridgePieceFactory(
    ...
    if (pieceClass == EPieceClass_BridgeStraight) {
        ...
        structurePiece = RoomCrossing::createPiece(
            pieces,
            random,
            footX,
            footY,
        ...
        structurePiece = StairsRoom::createPiece(
            pieces,
            random,
            footX,
            footY,
...
NetherBridgePieces::NetherBridgePiece::generatePiece(
    ...
    {
        return BridgeEndFiller::createPiece(
            pieces,
            random,
            footX,
            footY,
...
StructurePiece* NetherBridgePieces::NetherBridgePiece::generateAndAddPiece(
    ...
    StructurePiece* newPiece = generatePiece(
        startPiece,
        availablePieces,
        pieces,
        random,
...
bool NetherBridgePieces::NetherBridgePiece::isOkBox(
    BoundingBox* box,
    StartPiece*  startPiece
) {
    bool bIsOk = false;
    ...
    if (box != NULL) {
        ...
        int xzSize =
            (startPiece->m_level->getLevelData()->getXZSize()
...
bool NetherBridgePieces::BridgeStraight::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool NetherBridgePieces::BridgeEndFiller::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool NetherBridgePieces::BridgeCrossing::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool NetherBridgePieces::RoomCrossing::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool NetherBridgePieces::StairsRoom::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool NetherBridgePieces::MonsterThrone::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool NetherBridgePieces::CastleStalkRoom::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
