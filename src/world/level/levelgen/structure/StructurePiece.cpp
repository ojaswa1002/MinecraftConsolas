...
void StructurePiece::addChildren(
...
TilePos* StructurePiece::getLocatorPosition() {
    return new TilePos(
        boundingBox->getXCenter(),
        boundingBox->getYCenter(),
...
int StructurePiece::getOrientationData(int tile, int data) {
    ...
    } else if (tile == Tile::pistonBase_Id || tile == Tile::pistonStickyBase_Id
               || tile == Tile::lever_Id || tile == Tile::dispenser_Id) {
        if (orientation == Direction::SOUTH) {
...
void StructurePiece::placeBlock(
    Level*       level,
    int          block,
    int          data,
    int          x,
...
void StructurePiece::generateAirBox(
    Level*       level,
    BoundingBox* chunkBB,
    int          x0,
    int          y0,
...
void StructurePiece::generateBox(
    Level*       level,
    BoundingBox* chunkBB,
    int          x0,
    int          y0,
        ...
        for (int x = x0; x <= x1; x++) {
            for (int z = z0; z <= z1; z++) {
                ...
                selector->next(
                    ...
                    y,
                    z,
                    y == y0 || y == y1 || x == x0 || x == x1 || z == z0
                        || z == z1
                );
                placeBlock(
                    level,
                    selector->getNextId(),
                    selector->getNextData(),
                    x,
                    y,
                    z,
                    chunkBB
...
void StructurePiece::generateMaybeBox(
    Level*       level,
    BoundingBox* chunkBB,
    Random*      random,
    float        probability,
...
void StructurePiece::maybeGenerateBlock(
    Level*       level,
    BoundingBox* chunkBB,
    Random*      random,
    float        probability,
...
void StructurePiece::generateUpperHalfSphere(
    Level*       level,
    BoundingBox* chunkBB,
    int          x0,
    int          y0,
...
void StructurePiece::generateAirColumnUp(
    Level*       level,
    int          x,
    int          startY,
    int          z,
...
void StructurePiece::fillColumnDown(
    Level*       level,
    int          tile,
    int          tileData,
    int          x,
...
bool StructurePiece::createChest(
    Level*               level,
    BoundingBox*         chunkBB,
    Random*              random,
    int                  x,
...
bool StructurePiece::createDispenser(
    Level*               level,
    BoundingBox*         chunkBB,
    Random*              random,
    int                  x,
...
void StructurePiece::createDoor(
    Level*       level,
    BoundingBox* chunkBB,
    Random*      random,
    int          x,
