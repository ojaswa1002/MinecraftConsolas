    ...
    for (int z = boundingBox->z0; z <= boundingBox->z1; z++) {
        for (int x = boundingBox->x0; x <= boundingBox->x1; x++) {
            if (chunkBB->isInside(x, 64, z)) {
                total +=
                    max(level->getTopSolidBlock(x, z),
...
bool ScatteredFeaturePieces::JunglePyramidPiece::postProcess(
    ...
    if (!placedTrap1) {
        placedTrap1 = createDispenser(
            ...
            1,
            Facing::NORTH,
            WeighedTreasureArray(dispenserItems, DISPENSER_ITEMS_COUNT),
            2
        );
    }
    ...
    placeBlock(
        ...
        SmoothStoneBrickTile::TYPE_DETAIL,
        8,
        -2,
        11,
        chunkBB
    ...
    placeBlock(
        level,
        Tile::pistonStickyBase_Id,
        getOrientationData(Tile::pistonStickyBase_Id, Facing::WEST),
        10,
        -2,
        8,
        chunkBB
    ...
    placeBlock(
        level,
        Tile::diode_off_Id,
        getOrientationData(Tile::diode_off_Id, Direction::NORTH),
        10,
        -2,
        10,
        chunkBB
