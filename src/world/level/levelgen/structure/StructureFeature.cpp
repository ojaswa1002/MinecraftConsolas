...
void StructureFeature::addFeature(
    Level*    level,
    int       x,
    int       z,
    int       xOffs,
...
bool StructureFeature::postProcess(
    Level*  level,
    Random* random,
    int     chunkX,
    int     chunkZ
...
bool StructureFeature::isInsideFeature(int cellX, int cellY, int cellZ) {
    ...
    for (auto it = cachedStructures.begin(); it != cachedStructures.end();
        ...
        if (pStructureStart->isValid()) {
            if (pStructureStart->getBoundingBox()
                    ->intersects(cellX, cellZ, cellX, cellZ)) {
                /*
                ...
                */
                ...
                for (auto it2 = pieces->begin(); it2 != pieces->end(); it2++) {
                    ...
                    if (piece->getBoundingBox()
                            ->isInside(cellX, cellY, cellZ)) {
                        return true;
                    }
                }
            }
...
TilePos* StructureFeature::getNearestGeneratedFeature(
    ...
    std::int64_t xx     = (cellX >> 4) * xScale;
    std::int64_t zz     = (cellZ >> 4) * zScale;
    ...
    if (selected != NULL) {
        ...
        if (guesstimatedFeaturePositions != NULL) {
            ...
            for (auto it = guesstimatedFeaturePositions->begin();
                 it != guesstimatedFeaturePositions->end();
                 ++it) {
                int    dx   = (*it).x - cellX;
                int    dy   = (*it).y - cellY;
                int    dz   = (*it).z - cellZ;
