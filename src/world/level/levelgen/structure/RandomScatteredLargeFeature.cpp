...
bool RandomScatteredLargeFeature::isFeatureChunk(
    int  x,
    int  z,
    bool bIsSuperflat
) {
    ...
    if (forcePlacement
        ...
        bool biomeOk =
            level->getBiomeSource()
                ->containsOnly(x * 16 + 8, z * 16 + 8, 0, allowedBiomes);
        if (biomeOk) {
...
StructureStart*
RandomScatteredLargeFeature::createStructureStart(int x, int z) {
...
RandomScatteredLargeFeature::ScatteredFeatureStart::ScatteredFeatureStart(
    ...
    if (level->getBiome(chunkX * 16 + 8, chunkZ * 16 + 8) == Biome::jungle) {
        ScatteredFeaturePieces::JunglePyramidPiece* startRoom =
            new ScatteredFeaturePieces::JunglePyramidPiece(
                random,
                chunkX * 16,
                chunkZ * 16
            );
        ...
        ScatteredFeaturePieces::DesertPyramidPiece* startRoom =
            new ScatteredFeaturePieces::DesertPyramidPiece(
                random,
                chunkX * 16,
                chunkZ * 16
            );
