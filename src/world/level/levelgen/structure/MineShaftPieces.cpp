...
StructurePiece* MineShaftPieces::createRandomShaftPiece(
    ...
    if (randomSelection >= 80) {
        ...
        if (stairsBox != NULL) {
            return new MineShaftPieces::MineShaftStairs(
                genDepth,
                random,
                stairsBox,
                direction
...
bool MineShaftPieces::MineShaftCorridor::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool MineShaftPieces::MineShaftCrossing::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
    ...
    if (isTwoFloored) {
        ...
        generateBox(
            ...
            chunkBB,
            boundingBox->x0 + 1,
            boundingBox->y0 + DEFAULT_SHAFT_HEIGHT,
            boundingBox->z0 + 1,
            boundingBox->x1 - 1,
            boundingBox->y0 + DEFAULT_SHAFT_HEIGHT,
            boundingBox->z1 - 1,
