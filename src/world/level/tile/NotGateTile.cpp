...
bool NotGateTile::isToggledTooFrequently(
    Level* level,
    int    x,
    int    y,
    int    z,
...
void NotGateTile::tick(Level* level, int x, int y, int z, Random* random) {
    ...
    if (recentToggles.find(level) != recentToggles.end()) {
        ...
        while (!toggles->empty()
               && level->getTime() - toggles->front().when
                      > RECENT_TOGGLE_TIMER) {
            toggles->pop_front();
        }
    }
        ...
        if (neighborSignal) {
            ...
            if (isToggledTooFrequently(level, x, y, z, true)) {
                ...
                for (int i = 0; i < 5; i++) {
                    ...
                    level
                        ->addParticle(eParticleType_smoke, xx, yy, zz, 0, 0, 0);
                }
            }
        }
    } else {
...
void NotGateTile::levelTimeChanged(
    Level*       level,
    std::int64_t delta,
    std::int64_t newTime
) {
