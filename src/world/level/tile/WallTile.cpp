...
AABB* WallTile::getAABB(Level* level, int x, int y, int z) {
    ...
    /*	4J-JEV:
        Stopping the width changing here, it's causing cows/mobs/passers-by to
       'jump' up when they are pressed against the wall and then the wall
...
bool WallTile::shouldRenderFace(
    LevelSource* level,
    int          x,
    int          y,
    int          z,
