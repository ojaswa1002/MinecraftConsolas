...
HitResult*
TrapDoorTile::clip(Level* level, int xt, int yt, int zt, Vec3* a, Vec3* b) {
...
int TrapDoorTile::getPlacedOnFaceDataValue(
    Level* level,
    int    x,
    int    y,
    int    z,
...
bool TrapDoorTile::attachesTo(int id) {
    ...
    return tile != NULL
        ...
        || tile == Tile::lightGem || (dynamic_cast<HalfSlabTile*>(tile) != NULL)
