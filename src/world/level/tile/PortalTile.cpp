...
void PortalTile::tick(Level* level, int x, int y, int z, Random* random) {
    ...
    if (level->dimension->isNaturalDimension()
        ...
        if (y0 > 0 && !level->isSolidBlockingTile(x, y0 + 1, z)) {
            ...
            int  result  = 0;
            bool spawned = MonsterPlacerItem::spawnMobAt(
                               level,
                               57,
                               x + .5,
                               y0 + 1.1,
...
bool PortalTile::trySpawnPortal(
    Level* level,
    int    x,
    int    y,
    int    z,
...
bool PortalTile::shouldRenderFace(
    LevelSource* level,
    int          x,
    int          y,
    int          z,
