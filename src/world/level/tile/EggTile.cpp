...
void EggTile::teleport(Level* level, int x, int y, int z) {
    ...
    for (int i = 0; i < 1000; i++) {
        ...
        if (level->getTile(xt, yt, zt) == 0) {
            ...
            //		{
            //			double d = level->random->nextDouble(); // j < count / 2
            //? 0 :
            //// 1;
            ...
            //			double _x = xt + (x - xt) * d +
            //(level->random->nextDouble() - 0.5) * 1 + 0.5f; 			double
            //_y = yt + (y - yt) * d + level->random->nextDouble() * 1 - 0.5f;
            // double _z = zt + (z - zt) * d + (level->random->nextDouble() -
            // 0.5) * 1 + 0.5f;
...
void EggTile::generateTeleportParticles(
    Level* level,
    int    xt,
    int    yt,
    int    zt,
