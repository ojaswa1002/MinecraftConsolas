...
bool VineTile::updateSurvival(Level* level, int x, int y, int z) {
    ...
    if (newFacings > 0) {
        for (int d = 0; d <= 3; d++) {
            int facing = 1 << d;
            if ((facings & facing) != 0) {
                if (!isAcceptableNeighbor(level->getTile(
                        x + Direction::STEP_X[d],
                        y,
...
void VineTile::tick(Level* level, int x, int y, int z, Random* random) {
    if (!level->isClientSide) {
        if (level->random->nextInt(4) == 0) {
            ...
            } else if (testFacing >= Facing::NORTH && testFacing <= Facing::EAST
                       && (currentFacings & (1 << testDirection)) == 0) {
                // 4J - Brought side spread check forward from 1.2.3
                if (noSideSpread) return;

                int edgeTile = level->getTile(
...
int VineTile::getPlacedOnFaceDataValue(
    Level* level,
    int    x,
    int    y,
    int    z,
