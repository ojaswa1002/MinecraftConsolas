...
void PistonBaseTile::registerIcons(IconRegister* iconRegister) {
    ...
    iconPlatform = iconRegister->registerIcon(
        isSticky ? PLATFORM_STICKY_TEX : PLATFORM_TEX
    );
...
void PistonBaseTile::setPlacedBy(
    Level*               level,
    int                  x,
    int                  y,
    int                  z,
...
void PistonBaseTile::neighborChanged(
    Level* level,
    int    x,
    int    y,
    int    z,
...
void PistonBaseTile::triggerEvent(
    Level* level,
    int    x,
    int    y,
    int    z,
...
int PistonBaseTile::getNewFacing(
    Level*                  level,
    int                     x,
    int                     y,
    int                     z,
...
bool PistonBaseTile::isPushable(
    int    block,
    Level* level,
    int    cx,
    int    cy,
...
bool PistonBaseTile::createPush(
    Level* level,
    int    sx,
    int    sy,
    int    sz,
    ...
    for (int i = 0; i < MAX_PUSH_DEPTH + 1; i++) {
        ...
        if (Tile::tiles[block]->getPistonPushReaction()
            ...
            Tile::tiles[block]->spawnResources(
                level,
                cx,
                cy,
                cz,
                level->getData(cx, cy, cz),
                0
            );
