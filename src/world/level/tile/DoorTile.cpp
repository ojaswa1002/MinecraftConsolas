...
Icon* DoorTile::getTexture(LevelSource* level, int x, int y, int z, int face) {
    ...
    return icons
        [texBase + (flip ? DOOR_TILE_TEXTURE_COUNT : 0) + (upper ? 1 : 0)];
}
...
HitResult*
DoorTile::clip(Level* level, int xt, int yt, int zt, Vec3* a, Vec3* b) {
...
bool DoorTile::mayPlace(Level* level, int x, int y, int z) {
    ...
    return (
        level->isTopSolidBlocking(x, y - 1, z) && Tile::mayPlace(level, x, y, z)
