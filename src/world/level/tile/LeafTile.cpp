...
const std::wstring LeafTile::TEXTURES[2][4] = {
    ...
    {L"leaves_opaque",
     L"leaves_spruce_opaque",            L"leaves_opaque",
...
void LeafTile::tick(Level* level, int x, int y, int z, Random* random) {
    ...
    if ((currentData & UPDATE_LEAF_BIT) != 0
        ...
        if (level
                ->hasChunksAt(x - r2, y - r2, z - r2, x + r2, y + r2, z + r2)) {
...
void LeafTile::playerDestroy(
    ...
    if (!level->isClientSide && player->getSelectedItem() != NULL
        ...
        popResource(
            ...
            z,
            std::shared_ptr<ItemInstance>(
                new ItemInstance(Tile::leaves_Id, 1, data & LEAF_TYPE_MASK)
            )
        );
    } else {
