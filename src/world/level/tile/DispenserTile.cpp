...
#include "world/item/MonsterPlacerItem.h"
...
void DispenserTile::neighborChanged(
    Level* level,
    int    x,
    int    y,
    int    z,
...
void DispenserTile::setPlacedBy(
    Level*               level,
    int                  x,
    int                  y,
    int                  z,
...
int DispenserTile::dispenseItem(
    std::shared_ptr<DispenserTileEntity> trap,
    Level*                               level,
    std::shared_ptr<ItemInstance>        item,
    Random*                              random,
    ...
    case Item::arrow_Id: {
            ...
            < Level::MAX_DISPENSABLE_PROJECTILES) // 4J - added limit
        {
            ...
            // not sending a message here, since we will probably get flooded
            // with them when people have automatic dispensers for spawn eggs
            return LEAVE_ITEM;
        }
    } break;
    case Item::egg_Id: {
        ...
        if (currentProjectiles
            < Level::MAX_DISPENSABLE_PROJECTILES) // 4J - added limit
        {
            std::shared_ptr<ThrownEgg> egg =
                std::shared_ptr<ThrownEgg>(new ThrownEgg(level, xp, yp, zp));
            ...
            < Level::MAX_DISPENSABLE_PROJECTILES) // 4J - added limit
        {
            if (PotionItem::isThrowable(item->getAuxValue())) {
                std::shared_ptr<ThrownPotion> potion =
                    std::shared_ptr<ThrownPotion>(
                        new ThrownPotion(level, xp, yp, zp, item->getAuxValue())
                    );
    ...
    case Item::minecart_furnace_Id: {
            ...
            < Level::MAX_CONSOLE_MINECARTS) // 4J - added limit
        {
            std::shared_ptr<Minecart> minecart =
                std::shared_ptr<Minecart>(new Minecart(
                    level,
                    xp,
                    yp,
                    zp,
    ...
    case Item::boat_Id: {
        ...
        if (bLaunchBoat
            && level->countInstanceOf(eTYPE_BOAT, true)
