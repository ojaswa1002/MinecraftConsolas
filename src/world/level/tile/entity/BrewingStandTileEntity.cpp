...
BrewingStandTileEntity::getItem(unsigned int slot) {
    if (slot >= 0 && slot < items.length) {
        return items[slot];
    }
    return nullptr;
...
std::shared_ptr<ItemInstance>
BrewingStandTileEntity::removeItem(unsigned int slot, int count) {
...
std::shared_ptr<ItemInstance>
BrewingStandTileEntity::removeItemNoUpdate(int slot) {
    if (slot >= 0 && slot < items.length) {
        std::shared_ptr<ItemInstance> item = items[slot];
        items[slot]                        = nullptr;
        return item;
...
void BrewingStandTileEntity::setItem(
    unsigned int                  slot,
