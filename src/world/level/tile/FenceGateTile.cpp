...
void FenceGateTile::setPlacedBy(
    Level*               level,
    int                  x,
    int                  y,
    int                  z,
        ...
        bool soundOnly /*=false*/) // 4J added soundOnly param
{
    if (soundOnly) {
        ...
        level->levelEvent(
            player,
            LevelEvent::SOUND_OPEN_DOOR,
            x,
            y,
            z,
            0
        ); // 4J - changed event to pass player rather than NULL as the source
           // of the event so we can filter the broadcast properly
        return false;
    }
...
void FenceGateTile::neighborChanged(
    Level* level,
    int    x,
    int    y,
    int    z,
...
bool FenceGateTile::shouldRenderFace(
    LevelSource* level,
    int          x,
    int          y,
    int          z,
