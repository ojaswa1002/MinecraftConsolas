...
protected:
    virtual Tile* sendTileData(
        unsigned char importantMask = 15
    ); // 4J - added importantMask to indicate which bits in the data are
       // important
protected:
    virtual void  init();
...
public:
    ...
    virtual void
    setShape(float x0, float y0, float z0, float x1, float y1, float z1);
    ...
    virtual bool
    isSolidRender(bool isServerLevel = false); // 4J - Added isServerLevel param
    ...
    virtual float getDestroyProgress(
...
public:
    ...
    virtual HitResult*
    clip(Level* level, int xt, int yt, int zt, Vec3* a, Vec3* b);
...
public:
    ...
    virtual bool
    TestUse(Level* level, int x, int y, int z, std::shared_ptr<Player> player);
    virtual bool
    use(Level*                  level,
        int                     x,
        int                     y,
    ...
    virtual void
    stepOn(Level* level, int x, int y, int z, std::shared_ptr<Entity> entity);
    ...
    virtual void
    attack(Level* level, int x, int y, int z, std::shared_ptr<Player> player);
    ...
    virtual int
    getColor(LevelSource* level, int x, int y, int z, int data); // 4J added
...
public:
    ...
    virtual void
    triggerEvent(Level* level, int x, int y, int z, int b0, int b1);
...
public:
    ...
    virtual float getShadeBrightness(
        LevelSource* level,
        int          x,
        int          y,
        int          z
    ...
    virtual void playerWillDestroy(
        Level*                  level,
        int                     x,
        int                     y,
        int                     z,
