...
MapItemSavedData::MapDecoration::MapDecoration(
    char img,
    char x,
    char y,
    char rot,
...
void MapItemSavedData::tickCarriedBy(
    std::shared_ptr<Player>       player,
    ...
    for (auto it = carriedBy.begin(); it != carriedBy.end();) {
                                 ...
                                 //! item->isFramed() ))
        {
            ...
            if (!playerLevel->isClientSide && hp->player->dimension == 0
                ...
                } else if (currentPortalDecoration != nonPlayerDecorations.end()
                           && !atLeastOnePlayerInTheEnd) {
...
#if 0
			for(auto it,playerLevel->entities.begin( = t,playerLevel->entities.begin(); it != playerLevel->entities.end(); ++it)
			{
                ...
                for (auto it3 = players->players.begin();
                    ...
                    if (decorationPlayer != NULL
...
#endif
                        {
                            ...
                            if (dimension < 0) {
                                int s = step / 10;
                                rot = (char)((s * s * 34187121 + s * 121) >> 15
                                             & 15);
                            }
            ...
            // 				{
            // 					decorations.push_back(new MapDecoration(img, x,
            // y, rot));
            // 				}
            // 			}
        }
