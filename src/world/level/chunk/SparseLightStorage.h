...
public:
    ...
    ); // Set all lighting values from a data array of length 16384 (128 x 16 x
       // 16 x 0.5).
    void getData(
        byteArray    retArray,
        unsigned int retOffset
    ); // Gets all lighting values into an array of length 16384.
    ...
    int getDataRegion(
        ...
        int       z1,
        int       offset
    ); // Updates the data at offset position dataInOut with a region of
       // lighting information - external ordering compatible with java
       // DataLayer
