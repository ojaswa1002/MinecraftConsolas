...
CompressedTileStorage::CompressedTileStorage(CompressedTileStorage* copyFrom) {
    ...
    if (allocatedSize > 0) {
        indicesAndData = (unsigned char*)XPhysicalAlloc(
            ...
            4096,
            PAGE_READWRITE
        ); //(unsigned char *)malloc(allocatedSize);
...
CompressedTileStorage::CompressedTileStorage(
    ...
    indicesAndData = (unsigned char*)
        XPhysicalAlloc(32768 + 4096, MAXULONG_PTR, 4096, PAGE_READWRITE);
...
CompressedTileStorage::CompressedTileStorage(bool isEmpty) {
    ...
    // amount
    indicesAndData = (unsigned char*)
        XPhysicalAlloc(1024, MAXULONG_PTR, 4096, PAGE_READWRITE);
#else
    indicesAndData = (unsigned char*)
        XPhysicalAlloc(4096, MAXULONG_PTR, 4096, PAGE_READWRITE);
#endif //__PS3__
    unsigned short* indices = (unsigned short*)indicesAndData;
    // unsigned char *data = indicesAndData + 1024;
...
bool CompressedTileStorage::isRenderChunkEmpty(
    int y
) // y == 0, 16, 32... 112 (representing a 16 byte range)
{
    int             block;
    ...
    for (int x = 0; x < 16; x += 4)
        for (int z = 0; z < 16; z += 4) {
            ...
            std::uint64_t* comp = (std::uint64_t*)&blockIndices[block];
...
inline void CompressedTileStorage::getBlockAndTile(
    int* block,
    int* tile,
    int  x,
    int  y,
...
void CompressedTileStorage::setData(byteArray dataIn, unsigned int inOffset) {
    ...
    for (int i = 0; i < 512; i++) {
        ...
        std::int32_t i32_1        = 1;
        for (int j = 0; j < 64;
             j++) // This loop of 64 is to go round the 4 x 4 tiles in the block
        {
...
#else
        std::uint64_t usedFlags[4] = {0, 0, 0, 0};
        ...
        if (indexTypeNew == INDEX_TYPE_0_OR_8_BIT) {
            if (_blockIndices[i] & INDEX_TYPE_0_BIT_FLAG) {
                ...
                for (int j = 0; j < 64; j++) // This loop of 64 is to go round
                                             // the 4 x 4 x 4 tiles in the block
                {
            ...
            int tiletypecount = 1 << bitspertile; // will be 2, 4 or 16 (from
                                                  // index values of 0, 1, 2)
            // int tiletypemask = tiletypecount - 1;		// will be 1, 3 or
            // 15 (from index values of 0, 1, 2)
            ...
            int indexmask_bits = 7 >> indexTypeNew; // will be 7, 3 or 1 (from
                                                    // index values of 0, 1, 2)
            int indexmask_bytes = 62 >> indexshift; // will be 7, 15 or 31 (from
                                                    // index values of 0, 1, 2)
...
void CompressedTileStorage::getData(
    byteArray    retArray,
    ...
    for (int i = 0; i < 512; i++) {
        ...
        if (indexType == INDEX_TYPE_0_OR_8_BIT) {
            ...
            int indexmask_bits = 7 >> indexType;    // will be 7, 3 or 1 (from
                                                    // index values of 0, 1, 2)
            int indexmask_bytes = 62 >> indexshift; // will be 7, 15 or 31 (from
                                                    // index values of 0, 1, 2)
...
int CompressedTileStorage::get(int x, int y, int z) {
    ...
    if (indexType == INDEX_TYPE_0_OR_8_BIT) {
        ...
        int tiletypecount =
            1
            << bitspertile; // will be 2, 4 or 16 (from index values of 0, 1, 2)
        int tiletypemask =
            tiletypecount
            - 1; // will be 1, 3 or 15 (from index values of 0, 1, 2)
        int indexshift =
    ...
    for (int pass = 0; pass < 2; pass++) {
        ...
        if (indexType == INDEX_TYPE_0_OR_8_BIT) {
            if (blockIndices[block] & INDEX_TYPE_0_BIT_FLAG) {
                ...
                if (val
                    == ((blockIndices[block] >> INDEX_TILE_SHIFT)
                        & INDEX_TILE_MASK)) {
                    LeaveCriticalSection(&cs_write);
                    return;
                }
...
int CompressedTileStorage::setDataRegion(
    byteArray           dataIn,
    int                 x0,
    int                 y0,
    int                 z0,
...
int CompressedTileStorage::getDataRegion(
    byteArray dataInOut,
    int       x0,
    int       y0,
    int       z0,
    ...
    for (int i = 0; i < 512; i++) {
        ...
        if (upgradeBlock == -1) {
            if (indexType == INDEX_TYPE_0_OR_8_BIT) {
                ...
                int indexshift = 3 - indexType; // will be 3, 2 or 1 (from index
                                                // values of 0, 1, 2)
                int indexmask_bits = 7 >> indexType; // will be 7, 3 or 1 (from
                                                     // index values of 0, 1, 2)
                int indexmask_bytes =
            ...
            case INDEX_TYPE_0_OR_8_BIT:
                ...
                memToAlloc += 64;
                break;
                // Note that INDEX_TYPE_8_BIT|INDEX_TYPE_0_BIT_FLAG not in here
                // as it doesn't need any further allocation
            }
        }
    }
        ...
        for (int i = 0; i < 512; i++) {
            ...
            if (!done) {
                ...
                if (indexTypeOld == INDEX_TYPE_0_OR_8_BIT) {
                    ...
                    int bitspertile = 1
                                   << indexTypeOld; // will be 1, 2 or 4 (from
                                                    // index values of 0, 1, 2)
                    int tiletypecount = 1 << bitspertile;  // will be 2, 4 or 16
                    int tiletypemask  = tiletypecount - 1; // will be 1, 3 or 15
                    int indexshift =
                    ...
                    for (int j = 0; j < 64; j++) {
                        ...
                        unpacked_data[j] = tile_types
                            [(packed_data[idx] >> bit) & tiletypemask];
                    }
                }
                ...
                if (indexTypeNew == INDEX_TYPE_0_OR_8_BIT) {
                    ...
                    int bitspertile = 1
                                   << indexTypeNew; // will be 1, 2 or 4 (from
                                                    // index values of 0, 1, 2)
                    int tiletypecount = 1
                                     << bitspertile; // will be 2, 4 or 16 (from
                                                     // index values of 0, 1, 2)
                    int tiletypemask =
                        tiletypecount - 1; // will be 1, 3 or 15 (from index
                                           // values of 0, 1, 2)
                    int tiledatasize =
...
int CompressedTileStorage::getAllocatedSize(
    int* count0,
    int* count1,
    int* count2,
    int* count4,
...
void CompressedTileStorage::read(DataInputStream* dis) {
    ...
    if (allocatedSize > 0) {
        ...
        indicesAndData = (unsigned char*)
            XPhysicalAlloc(allocatedSize, MAXULONG_PTR, 4096, PAGE_READWRITE);
