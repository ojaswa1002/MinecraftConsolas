...
// (1) The storage details for the class are now packed into a single
// std::int64_t, which contains both a pointer to the data that is required and
// a count of how many planes worth
...
// (2) The data pointer referenced in this std::int64_t points to an area of
// memory which is 128 + 128 * plane_count bytes long, where the first 128 bytes
// stoere the plane indices, and
...
private:
    ...
    std::int64_t
        dataAndCount; // Contains packed-together data pointer (lower
...
public:
    ...
    ); // Set all data values from a data array of length 16384 (128 x 16 x 16 x
       // 0.5).
    void getData(
        byteArray    retArray,
        unsigned int retOffset
    ); // Gets all data values into an array of length 16384.
    ...
    int setDataRegion(
        ...
        void*               param,
        int                 yparam
    ); // Sets a region of data values with the data at offset position in the
    ...
    void
    getPlaneIndicesAndData(unsigned char** planeIndices, unsigned char** data);
