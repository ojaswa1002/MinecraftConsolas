...
RegionFile::RegionFile(ConsoleSaveFile* saveFile, File* path) {
    ...
    */
    ...
    for (int i = 0; i < SECTOR_INTS; ++i) {
        ...
        {
            m_saveFile
                ->readFile(fileEntry, &lastModValue, 4, &numberOfBytesRead);
...
std::int64_t RegionFile::lastModified() { return _lastModified; }
...
DataInputStream*
RegionFile::getChunkDataInputStream(int x, int z) // TODO - was synchronized
{
    if (outOfBounds(x, z)) {
    ...
    Compression::getCompression()->SetDecompressionType(
        m_saveFile->getSavePlatform()
    ); // if this save is from another platform, set the correct decompression
       // type

    if (useRLE) {
        Compression::getCompression()
            ->DecompressLZXRLE(decomp, &readDecompLength, data, length);
    } else {
        Compression::getCompression()
            ->Decompress(decomp, &readDecompLength, data, length);
    }

    Compression::getCompression()->SetDecompressionType(
        SAVE_FILE_PLATFORM_LOCAL
    ); // and then set the decompression back to the local machine's standard
       // type
...
void RegionFile::write(int x, int z, byte* data, int length) // TODO - was
                                                             // synchronized
{
    ...
    Compression::getCompression()
        ->CompressLZXRLE(compData, &compLength, data, length);
    ...
    m_saveFile->LockSaveAccess();
    {
        ...
        if (sectorNumber != 0 && sectorsAllocated == sectorsNeeded) {
            ...
            if (runLength >= sectorsNeeded) {
                ...
                PIXBeginNamedEvent(
                    0,
                    "Expanding storage for %d sectors\n",
                    sectorsNeeded
                );
                /*
