...
bool OldChunkStorage::saveEntities(
    LevelChunk*  lc,
    Level*       level,
    CompoundTag* tag
) {
...
void OldChunkStorage::save(
    ...
    if (ticksInChunk != NULL) {
        std::int64_t levelTime = level->getTime();
...
void OldChunkStorage::save(LevelChunk* lc, Level* level, CompoundTag* tag) {
    ...
    tag->putShort(
        ...
        lc->terrainPopulated
    ); // 4J - changed from "TerrainPopulated" to "TerrainPopulatedFlags" as now
       // stores a bitfield, java stores a bool
...
void OldChunkStorage::loadEntities(
    LevelChunk*  lc,
    Level*       level,
    CompoundTag* tag
) {
    ...
    if (tag->get(L"TerrainPopulated")) {
        ...
        if (levelChunk->terrainPopulated >= 1)
            levelChunk->terrainPopulated =
                LevelChunk::sTerrainPopulatedAllNeighbours
                | LevelChunk::sTerrainPostPostProcessed; // Convert from old
                                                         // bool type to new
                                                         // bitfield
    } else {
        // New style short
