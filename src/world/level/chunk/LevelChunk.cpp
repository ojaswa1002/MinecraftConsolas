    ...
    for (int x = 0; x < 16; x++)
        ...
        //            level->depthBits;			// 4J - removed
            ...
            if (!level->dimension->hasCeiling) {
                ...
                do {
                    br -= Tile::lightBlock
                        [blockData[Index + offset + yy]]; // 4J - blocks->get()
                                                          // was blocks[p + yy]
                    if (br > 0) {
    ...
    for (int x = 0; x < 16; x++)
        for (int z = 0; z < 16; z++) {
            ...
            for (int y = 0; y < Level::COMPRESSED_CHUNK_SECTION_HEIGHT; y++) {
                ...
                int                    emit   = Tile::lightEmission
                    [blocks->get(x, y, z)]; // 4J - blocks->get() was blocks[p +
                                            // y]
                if (emit > 0) {
                    //					printf("(%d,%d,%d)",this->x * 16 + x, y,
                    // this->z * 16 + z);
        ...
        for (int x = 0; x < 16; x++)
            for (int z = 0; z < 16; z++) {
                ...
                if (bForce
                    ...
                    if (!bForce) // 4J - if doing a full forced thing over every
                                 // single column, we don't need to do these
                                 // offset checks too
                    {
                        if (xOffs - 1 >= minXZ)
...
void LevelChunk::recalcHeight(int x, int yStart, int z) {
    ...
    while (y > 0
           && Tile::lightBlock
                      [blocks->get(
                           x,
                           (y - 1) % Level::COMPRESSED_CHUNK_SECTION_HEIGHT,
                           z
                       )
                       & 0xff]
                  == 0) // 4J - blocks->get() was blocks[p + y - 1]
    {
        y--;
        blocks = (y - 1) >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT
                   ? upperBlocks
    ...
    //    level->lightColumnChanged(x, z, y, yOld);		// 4J - this call moved
    //    below & corrected - see comment further down
    heightmap[z << 4 | x] = (byte)y;
    ...
    if (!level->dimension->hasCeiling) {
        ...
        while (y > 0 && br > 0) {
            ...
            if (br < 0) br = 0;
            skyLight
                ->set(x, (y % Level::COMPRESSED_CHUNK_SECTION_HEIGHT), z, br);
...
bool LevelChunk::setTileAndData(int x, int y, int z, int _tile, int _data) {
    ...
    if (_tile != 0) {
        if (!level->isClientSide) {
            ...
            if (_tile == Tile::fire_Id) {
                if (!Tile::tiles[_tile]->mayPlace(level, xOffs, y, zOffs)) {
                    ...
                    //					blocks[x << level->depthBitsPlusFour | z
                    //<< level->depthBits | y] = 0;
                }
            }
        }
        ...
        // Vita mate
        //		if (_tile > 0 && dynamic_cast<EntityTile *>(Tile::tiles[_tile])
        //!= NULL)
...
int LevelChunk::getBrightness(LightLayer::variety layer, int x, int y, int z) {
    if (layer == LightLayer::Sky) {
        ...
        return blockLight
            ->get(x, y % Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z);
    } else return 0;
}

// 4J added
void LevelChunk::getNeighbourBrightnesses(
    int*                brightnesses,
    ...
    if (layer == LightLayer::Sky) {
        ...
        SparseLightStorage* blockLight =
            y >= Level::COMPRESSED_CHUNK_SECTION_HEIGHT ? upperBlockLight
                                                        : lowerBlockLight;
        blockLight
            ->set(x, y % Level::COMPRESSED_CHUNK_SECTION_HEIGHT, z, brightness);
    }
}
...
std::shared_ptr<TileEntity> LevelChunk::getTileEntity(int x, int y, int z) {
    ...
    if (it == tileEntities.end()) {
        LeaveCriticalSection(
            &m_csTileEntities
        ); // Note: don't assume iterator is valid for tileEntities after this
           // point
        ...
        // tileEntity = tileEntities[pos];		// 4J - TODO - this doesn't seem
        // right - assignment wrong way? Check
    ...
    if (loaded) {
        ...
        if (it != tileEntities.end()) {
            ...
            if (te != NULL) {
                if (level->isClientSide) {
                    app.DebugPrintf(
                        "Removing tile entity of type %d\n",
                        te->GetType()
                    );
                }
                te->setRemoved();
                      ...
                      // entities/tile-entities its saved in the entitiesTag.
    {
        ...
        if (!level->isClientSide) {
            ...
            for (std::unordered_map<
                     ...
                     TilePosKeyHash,
                     TilePosKeyEq>::iterator it = tileEntities.begin();
                 it != itEnd;
                 it++) {
...
int LevelChunk::getBlocksAndData(
    byteArray* data,
    int        x0,
    int        y0,
    int        z0,
    ...
    for (int x = x0; x < x1; x++)
        for (int z = z0; z < z1; z++)
        {
            int slot = (x << level->depthBitsPlusFour | z << level->depthBits |
    y0) >> 1; int len = (y1 - y0) / 2; System::arraycopy(blockLight->data, slot,
    data, p, len); p += len;
        }
    ...
    for (int x = x0; x < x1; x++)
        ...
        {
            int slot = (x << level->depthBitsPlusFour | z << level->depthBits |
    y0) >> 1; int len = (y1 - y0) / 2; System::arraycopy(skyLight->data, slot,
    data, p, len); p += len;
        }
        */
...
bool LevelChunk::testSetBlocksAndData(
    byteArray data,
    int       x0,
    int       y0,
    int       z0,
...
int LevelChunk::setBlocksAndData(
    byteArray data,
    int       x0,
    int       y0,
    int       z0,
    ...
    for (int x = x0; x < x1; x++)
        ...
        {
            int slot = x << level->depthBitsPlusFour | z << level->depthBits |
    y0; int len = y1 - y0; System::arraycopy(data, p, &blocks, slot, len); p +=
    len;
        }*/
    ...
    for (int x = x0; x < x1; x++)
        ...
        {
            int slot = (x << level->depthBitsPlusFour | z << level->depthBits |
    y0) >> 1; int len = (y1 - y0) / 2; System::arraycopy(data, p,
    &blockLight->data, slot, len); p += len;
        }
...
Random* LevelChunk::getRandom(std::int64_t l) {
    return new Random(
        (level->getSeed() + x * x * 4987142 + x * 5947611 + z * z * 4392871l
         + z * 389711)
        ^ l
    );
}
...
void LevelChunk::checkPostProcess(
    ChunkSource* source,
    ChunkSource* parent,
    int          x,
    int          z
...
void LevelChunk::updateBiomeFlags(int x, int z) {
    ...
    if ((columnFlags[slot] & (eColumnFlag_biomeOk << shift)) == 0) {
        ...
        int        zOffs = (this->z * 16) + z;
        BiomeArray biomes;
        level->getBiomeSource()
            ->getBiomeBlock(biomes, xOffs, zOffs, 1, 1, true);
        if (biomes[0]->hasRain())
...
int LevelChunk::getBlocksAllocatedSize(
    int* count0,
    int* count1,
    int* count2,
    int* count4,
    ...
    return lowerBlocks
        ->getAllocatedSize(count0, count1, count2, count4, count8);
}
...
byteArray LevelChunk::getReorderedBlocksAndData(
    int  x0,
    int  y0,
    int  z0,
    int  xs,
...
void LevelChunk::reorderBlocksAndDataToXZY(
    int        y0,
    int        xs,
    int        ys,
    int        zs,
