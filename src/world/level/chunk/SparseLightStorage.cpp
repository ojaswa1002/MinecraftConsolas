...
SparseLightStorage::SparseLightStorage(bool sky) {
    ...
    // XPhysicalFree.
#ifdef _XBOX
    unsigned char* planeIndices = (unsigned char*)
        XPhysicalAlloc(128 * 128, MAXULONG_PTR, 4096, PAGE_READWRITE);
#else
...
#pragma warning(disable : 4826)
    dataAndCount = 0x007F000000000000L
                 | (((std::int64_t)planeIndices) & 0x0000ffffffffffffL);
...
#pragma warning(disable : 4826)
    dataAndCount = 0x0000000000000000L
                 | (((std::int64_t)planeIndices) & 0x0000ffffffffffffL);
#pragma warning(default : 4826)
#ifdef LIGHT_COMPRESSION_STATS
    count = 0;
...
SparseLightStorage::SparseLightStorage(SparseLightStorage* copyFrom) {
    ...
    std::int64_t   sourceDataAndCount = copyFrom->dataAndCount;
...
#pragma warning(disable : 4826)
    dataAndCount = (sourceDataAndCount & 0xffff000000000000L)
                 | (((std::int64_t)destIndicesAndData) & 0x0000ffffffffffffL);
#pragma warning(default : 4826)
...
void SparseLightStorage::setData(byteArray dataIn, unsigned int inOffset) {
    ...
    for (int y = 0; y < 128; y++) {
        ...
        if (planeIndices[y] < 128) {
            ...
            for (int xz = 0; xz < 128;
                 xz++) // 128 ( 16 x 16 x 0.5 ) in loop as packing 2 values into
                       // each destination byte
            {
...
#pragma warning(disable : 4826)
    std::int64_t newDataAndCount =
        ((std::int64_t)planeIndices) & 0x0000ffffffffffffL;
#pragma warning(default : 4826)
    newDataAndCount |= ((std::int64_t)allocatedPlaneCount) << 48;
...
void SparseLightStorage::set(int x, int y, int z, int val) {
    ...
    int planeIndex = x * 16 + z; // Index within this xz plane
    int byteIndex =
        planeIndex / 2; // Byte index within the plane (2 tiles stored per byte)
    int shift = (planeIndex & 1) * 4; // Bit shift within the byte
    int mask  = 0xf0 >> shift;
...
// of data copied.
int SparseLightStorage::setDataRegion(
    byteArray dataIn,
    int       x0,
    int       y0,
    int       z0,
...
int SparseLightStorage::getDataRegion(
    byteArray dataInOut,
    int       x0,
    int       y0,
    int       z0,
...
void SparseLightStorage::addNewPlane(int y) {
    ...
    do {
        ...
        std::int64_t lastDataAndCount = dataAndCount;
...
#pragma warning(disable : 4826)
        std::int64_t newDataAndCount =
            ((std::int64_t)dataPointer) & 0x0000ffffffffffffL;
#pragma warning(default : 4826)
        newDataAndCount |= ((std::int64_t)linesUsed) << 48;
        ...
        // write took place
        std::int64_t lastDataAndCount2 = InterlockedCompareExchangeRelease64(
            &dataAndCount,
            newDataAndCount,
            lastDataAndCount
        );
...
void SparseLightStorage::getPlaneIndicesAndData(
    unsigned char** planeIndices,
    unsigned char** data
) {
    unsigned char* indicesAndData =
...
void SparseLightStorage::updateDataAndCount(std::int64_t newDataAndCount) {
...
#pragma warning(disable : 4826)
        std::int64_t newDataAndCount =
            ((std::int64_t)newIndicesAndData) & 0x0000ffffffffffffL;
#pragma warning(default : 4826)
        newDataAndCount |= ((std::int64_t)planesToAlloc) << 48;
...
#pragma warning(disable : 4826)
    std::int64_t newDataAndCount =
        ((std::int64_t)dataPointer) & 0x0000ffffffffffffL;
#pragma warning(default : 4826)
    newDataAndCount |= ((std::int64_t)count) << 48;
