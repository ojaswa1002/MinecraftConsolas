...
//			0 - the data for this block is represented at 1 bit per tile. Data
// pointed to is 2 bytes describing the 2 possible tiles stored in this block,
// followed by 32 bytes of data (total 34 bytes) 			1 - the data for
...
// data (total 68 bytes) 			2 - the data for this block is represented
// at 4 bit per tile. Data pointed to is 16 bytes describing the 16 possible
// tiles stored in this block, followed by 128 bytes of data (total 144 bytes)
// 3 - if bit 2 is 0, then this block is represented at 8 bits per tile. Data
// pointed to is 64 bytes, offset must be a multiple of 4 (since bit 2 can also
// be thought of as being 				the low bit of the offset (divided by 2
// as in the other cases), and is zero)
//			  - if bit 2 is 1, then this block is represented at 0 bits per
// tile. The upper 8 bits of the index store the tile value that is used by the
// entire block.
// So:
...
public:
    ...
    ); // Construct with data in passed in array of length 32768 (128 x 16 x 16)
    CompressedTileStorage(bool isEmpty);
    ~CompressedTileStorage();
    bool isSameAs(CompressedTileStorage* other);
    bool isRenderChunkEmpty(
        int y
    ); // Determine if 16x16x16 render-sized chunk is actually empty
private:
    inline static int getIndex(int block, int tile);
    inline static void
    getBlockAndTile(int* block, int* tile, int x, int y, int z);
...
public:
    ...
    typedef void (*tileUpdatedCallback)(
        int   x,
        int   y,
        int   z,
        void* param,
    ...
    int setDataRegion(
        ...
        void*               param,
        int                 yparam
    ); // Sets a region of tile values with the data at offset position in the
       // array dataIn - external ordering compatible with java DataLayer
    bool testSetDataRegion(
        byteArray dataIn,
        int       x0,
        int       y0,
        int       z0,
