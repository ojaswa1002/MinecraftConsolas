...
public:
    ...
    void
    getBlockData(byteArray data); // Sets data in passed in array of size 32768,
                                  // from the block data in this chunk
    int getBlocksAllocatedSize(
        int* count0,
        int* count1,
        int* count2,
        int* count4,
    ...
    unsigned char
        columnFlags[16 * 8]; // 4J - lighting update brought forward from 1.8.2,
                             // was a bool array but now mixed with other flags
                             // in our version, and stored in nybbles
    Level* level;
...
public:
    void setDataData(
        byteArray data
    ); // Set data to that passed in in the input array of size 32768
...
public:
    void getSkyLightData(
        byteArray data
    ); // Get a byte array of length 16384 ( 128 x 16 x 16 x 0.5 ), containing
       // sky light data. Ordering same as java version.
    void getBlockLightData(
        byteArray data
    ); // Get a byte array of length 16384 ( 128 x 16 x 16 x 0.5 ), containing
       // block light data. Ordering same as java version.
    void setSkyLightData(
        byteArray data
    ); // Set sky light data to data passed in input byte array of length 16384.
       // This data must be in original (java version) order
    void setBlockLightData(
        byteArray data
    ); // Set block light data to data passed in input byte array of length
       // 16384. This data must be in original (java version) order
...
public:
    ...
    virtual void getNeighbourBrightnesses(
        int*                brightnesses,
    ...
    virtual void getEntitiesOfClass(
        const type_info&                      ec,
        AABB*                                 bb,
        std::vector<std::shared_ptr<Entity>>& es
    );
    ...
    virtual int  getBlocksAndData(
     ...
     ); // 4J - added includeLighting parameter
    static void tileUpdatedCallback(
        int   x,
        int   y,
        int   z,
        void* param,
    ...
    virtual bool testSetBlocksAndData(
        byteArray data,
        int       x0,
        int       y0,
        int       z0,
    ...
    void
    checkPostProcess(ChunkSource* source, ChunkSource* parent, int x, int z);
...
public:
    ...
    byteArray
    getReorderedBlocksAndData(int x, int y, int z, int xs, int& ys, int zs);
    static void
    reorderBlocksAndDataToXZY(int y0, int xs, int ys, int zs, byteArray* data);
