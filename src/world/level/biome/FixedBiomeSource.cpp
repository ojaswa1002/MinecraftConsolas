...
void FixedBiomeSource::getTemperatureBlock(
...
floatArray
FixedBiomeSource::getTemperatureBlock(int x, int z, int w, int h) const {
...
void FixedBiomeSource::getDownfallBlock(
...
floatArray
FixedBiomeSource::getDownfallBlock(int x, int z, int w, int h) const {
...
void FixedBiomeSource::getBiomeBlock(
    BiomeArray& biomes,
    int         x,
    int         z,
    int         w,
...
// 4J - caller is responsible for deleting biomes array, plus any optional
// arrays output if pointers are passed in (_temperatures, _downfalls)
void FixedBiomeSource::getBiomeIndexBlock(
...
// 4J - caller is responsible for deleting biomes array, plus any optional
// arrays output if pointers are passed in (_temperatures, _downfalls)
void FixedBiomeSource::getRawBiomeBlock(
    BiomeArray& biomes,
    int         x,
    int         z,
    int         w,
...
// arrays output if pointers are passed in (_temperatures, _downfalls)
BiomeArray
FixedBiomeSource::getRawBiomeBlock(int x, int z, int w, int h) const {
    BiomeArray biomes;
    getRawBiomeBlock(biomes, x, z, w, h);
    return biomes;
}

TilePos* FixedBiomeSource::findBiome(
    int     x,
    int     z,
    int     r,
    Biome*  toFind,
...
bool FixedBiomeSource::containsOnly(
    int                 x,
    int                 z,
    int                 r,
    std::vector<Biome*> allowed
