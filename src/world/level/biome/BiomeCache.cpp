...
BiomeCache::Block::Block(int x, int z, BiomeCache* parent) {
    // 	temps = floatArray(ZONE_SIZE * ZONE_SIZE, false);		// MGH - added
    // "no clear" flag to arrayWithLength 	downfall = floatArray(ZONE_SIZE *
...
Biome* BiomeCache::Block::getBiome(int x, int z) {
    ...
    int biomeIndex = biomeIndices
        [(x & ZONE_SIZE_MASK) | ((z & ZONE_SIZE_MASK) << ZONE_SIZE_BITS)];
...
BiomeCache::Block* BiomeCache::getBlockAt(int x, int z) {
    ...
    x                 >>= ZONE_SIZE_BITS;
    z                 >>= ZONE_SIZE_BITS;
    std::int64_t slot   = (((std::int64_t)x) & 0xffffffffl)
                      | ((((std::int64_t)z) & 0xffffffffl) << 32l);
    ...
    if (utime > DECAY_TIME / 4 || utime < 0) {
        ...
        for (auto it = all.begin(); it != all.end();) {
            ...
            if (time > DECAY_TIME || time < 0) {
                it = all.erase(it);
                std::int64_t slot =
                    (((std::int64_t)block->x) & 0xffffffffl)
                    | ((((std::int64_t)block->z) & 0xffffffffl) << 32l);
                cached.erase(slot);
                delete block;
            } else {
                ++it;
