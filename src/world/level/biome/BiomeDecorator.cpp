...
void BiomeDecorator::decorate() {
    ...
    for (int i = 0; i < hugeMushrooms; i++) {
        ...
        int z = zo + random->nextInt(16) + 8;
        hugeMushroomFeature
            ->place(level, random, x, level->getHeightmap(x, z), z);
    }
    ...
    PIXBeginNamedEvent(
        0,
        "Decorate bush/waterlily/mushroom/reeds/pumpkins/cactuses"
    );
...
void BiomeDecorator::decorateDepthSpan(
    int      count,
    Feature* feature,
    int      y0,
    int      y1
...
void BiomeDecorator::decorateDepthAverage(
    int      count,
    Feature* feature,
    int      yMid,
    int      ySpan
