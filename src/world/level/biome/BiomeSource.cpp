...
#include "world/level/newbiome/layer/Layer.h"
...
void BiomeSource::getDownfallBlock(
    floatArray& downfalls,
    int         x,
    int         z,
    int         w,
...
// temperatures array is for output only. 4J - removal of separate temperature &
// downfall layers brought forward from 1.2.3
void BiomeSource::getTemperatureBlock(
    floatArray& temperatures,
    int         x,
    int         z,
    int         w,
...
// 4J added
void BiomeSource::getRawBiomeIndices(
    intArray& biomes,
    int       x,
    int       z,
    int       w,
...
void BiomeSource::getRawBiomeBlock(
    BiomeArray& biomes,
    int         x,
    int         z,
    int         w,
...
void BiomeSource::getBiomeBlock(
    BiomeArray& biomes,
    int         x,
    int         z,
    int         w,
    ...
    if (useCache && w == 16 && h == 16 && (x & 0xf) == 0 && (z & 0xf) == 0) {
        ...
        delete tmp.data; // MGH - added, the caching creates this array from the
                         // indices now.
                         // return biomes;
    }
...
// 4J - caller is responsible for deleting biomes array
void BiomeSource::getBiomeIndexBlock(
    byteArray& biomeIndices,
    int        x,
    int        z,
    int        w,
 ...
 */
TilePos*
BiomeSource::findBiome(int x, int z, int r, Biome* toFind, Random* random) {
    int x0 = ((x - r) >> 2);
    int z0 = ((z - r) >> 2);
    int x1 = ((x + r) >> 2);
    int z1 = ((z + r) >> 2);
...
#ifdef __PSVITA__
std::int64_t BiomeSource::findSeed(
    LevelType* generator,
    bool*      pServerRunning
) // MGH - added pRunning, so we can early out of this on Vita as it can take up
  // to 60 secs
#else
...
#endif
{
...
#endif
    {
...
#endif
        {
            ...
            BiomeArray   biomes =
                biomeSource
                    ->getBiomeBlock(-27 * 16, -27 * 16, 54 * 16, 54 * 16);
