...
public:
    BiomeSource(std::int64_t seed, LevelType* generator);
...
public:
#ifdef __PSVITA__
    static std::int64_t findSeed(
        LevelType* generator,
        bool*      pServerRunning
    ); // MGH - added pRunning, so we can early out of this on Vita as it can
       // take up to 60 secs							// 4J added
#else
    static std::int64_t findSeed(LevelType* generator); // 4J added
#endif
    ~BiomeSource();
...
public:
    ...
    virtual void
    getDownfallBlock(floatArray& downfalls, int x, int z, int w, int h) const;
    ...
    virtual void       getTemperatureBlock(
    ...
    virtual void
    getRawBiomeBlock(BiomeArray& biomes, int x, int z, int w, int h) const;
    virtual void getRawBiomeIndices(
        intArray& biomes,
        int       x,
        int       z,
        int       w,
    ...
    virtual void      getBiomeIndexBlock(
     ...
     */
    virtual TilePos*
    findBiome(int x, int z, int r, Biome* toFind, Random* random);
     ...
     */
    virtual TilePos*
    findBiome(int x, int z, int r, std::vector<Biome*> allowed, Random* random);
