...
void InventoryMenu::_init(std::shared_ptr<Inventory> inventory, bool active) {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        addSlot(new ArmorSlot(
            i,
            inventory,
            inventory->getContainerSize() - 1 - i,
            8,
            8 + i * 18
        ));
    }
                                   ...
                                   // using it, so removed to simplify things
{
    ...
    resultSlots->setItem(
        0,
        Recipes::getInstance()->getItemFor(craftSlots, owner->level)
    );
    MemSect(0);
}
...
InventoryMenu::quickMoveStack(std::shared_ptr<Player> player, int slotIndex) {
