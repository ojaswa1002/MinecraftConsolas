...
void RepairMenu::createResult() {
    ...
    if (input == NULL) {
        ...
        if (itemName.length() > 0
            ...
            if (DEBUG_COST) {
                app.DebugPrintf(
                    "Naming cost; price is now %d (went up by %d)",
                    price,
                    namingCost
                );
            }
            ...
            if (input->hasCustomHoverName()) {
                ...
                if (DEBUG_COST) {
                    app.DebugPrintf(
                        "Already-named tax; tax is now %d (went up by %d)",
                        tax,
                        (namingCost / 2)
                    );
                }
        ...
        for (auto it = enchantments->begin(); it != enchantments->end(); ++it) {
            ...
            if (DEBUG_COST) {
                app.DebugPrintf(
                    "Enchantment tax; tax is now %d (went up by %d)",
                    tax,
                    (count + level * fee)
                );
            }
...
RepairMenu::quickMoveStack(std::shared_ptr<Player> player, int slotIndex) {
