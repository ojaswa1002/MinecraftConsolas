...
void EnchantmentMenu::slotsChanged() // 4J used to take a
                                     // std::shared_ptr<Container> container but
                                     // wasn't using it, so removed to simplify
                                     // things
{
...
bool EnchantmentMenu::clickMenuButton(std::shared_ptr<Player> player, int i) {
    ...
    if (costs[i] > 0 && item != NULL
        && (player->experienceLevel >= costs[i]
...
EnchantmentMenu::quickMoveStack(std::shared_ptr<Player> player, int slotIndex) {
