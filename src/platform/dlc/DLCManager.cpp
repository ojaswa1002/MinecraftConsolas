...
DWORD DLCManager::getPackIndexContainingSkin(
...
bool DLCManager::readDLCDataFile(
...
bool DLCManager::readDLCDataFile(
    ...
    if (bSuccess == FALSE) {
        ...
        app.DebugPrintf(
            "Failed to read %s from DLC content package\n",
            path.c_str()
        );
...
bool DLCManager::processDLCDataFile(
    DWORD&   dwFilesProcessed,
    PBYTE    pbData,
    DWORD    dwLength,
    DLCPack* pack
    ...
    for (unsigned int i = 0; i < uiFileCount; i++) {
        ...
        if (dlcTexturePack != NULL) {
            ...
            bool  validPack                 = processDLCDataFile(
                texturePackFilesProcessed,
                pbTemp,
                pFile->uiFileSize,
                dlcTexturePack
            );
            pack->SetDataPointer(
                NULL
            ); // If it's a child pack, it doesn't own the data
...
DWORD DLCManager::retrievePackIDFromDLCDataFile(
    const std::string& path,
    DLCPack*           pack
) {
    DWORD        packId = 0;
