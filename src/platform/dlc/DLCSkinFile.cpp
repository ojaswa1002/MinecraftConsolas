...
void DLCSkinFile::addParameter(
    DLCManager::EDLCParameterType type,
    const std::wstring&           value
) {
    switch (type) {
    case DLCManager::e_DLCParamType_DisplayName: {
        // 4J Stu - In skin pack 2, the name for Zap is mis-spelt with two p's
        // as Zapp dlcskin00000109.png
    ...
    case DLCManager::e_DLCParamType_ThemeName:
        m_themeName = value;
        break;
    case DLCManager::e_DLCParamType_Free: // If this parameter exists, then mark
                                          // this as free
        m_bIsFree = true;
        break;
    case DLCManager::e_DLCParamType_Credit: // If this parameter exists, then
                                            // mark this as free
                                            // add it to the DLC credits list

        // we'll need to justify this text since we don't have a lot of room for
        // lines of credits
        {
    ...
    case DLCManager::e_DLCParamType_Box: {
        ...
        swscanf(
            value.c_str(),
            L"%10ls%f%f%f%f%f%f%f%f",
            wchBodyPart,
#else
        swscanf_s(
            value.c_str(),
            L"%9ls%f%f%f%f%f%f%f%f",
            wchBodyPart,
            10,
#endif
            &pSkinBox->fX,
...
std::wstring
DLCSkinFile::getParameterAsString(DLCManager::EDLCParameterType type) {
    switch (type) {
    case DLCManager::e_DLCParamType_DisplayName:
