...
DLCAudioFile::getParameterType(const std::wstring& paramName) {
...
void DLCAudioFile::addParameter(
    EAudioType          type,
    EAudioParameterType ptype,
    const std::wstring& value
) {
    switch (ptype) {
    case e_AudioParamType_Credit: // If this parameter exists, then mark this as
                                  // free
    ...
    for (unsigned int i = 0; i < uiFileCount; i++) {
        ...
        for (unsigned int j = 0; j < uiParameterCount; j++) {
            ...
            if (it != parameterMapping.end()) {
                addParameter(
                    type,
                    (EAudioParameterType)pParams->dwType,
