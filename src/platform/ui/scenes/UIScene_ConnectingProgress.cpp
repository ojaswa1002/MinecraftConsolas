...
void UIScene_ConnectingProgress::handleLoseFocus() {
    ...
    app.DebugPrintf(
        "UIScene_ConnectingProgress time taken = %d millisecs\n",
        millisecsTaken
    );
    app.DebugPrintf(
        "---------------------------------------------------------\n"
...
void UIScene_ConnectingProgress::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
    ...
    case eControl_Confirm:
        if (m_showingButton) {
            if (m_iPad != ProfileManager.GetPrimaryPad()
                ...
                // player should be removed and the viewports updated again This
                // is actually done in the tick as we can't pull down the scene
                // we are currently in from here
