...
: UIScene(iPad, parentLayer) {
    ...
    if (m_bNotInGame && app.GetMashupPackWorlds(m_iPad) != 0xFFFFFFFF) {
        ...
        m_checkboxMashupWorlds.init(
            app.GetString(IDS_UNHIDE_MASHUP_WORLDS),
    ...
    m_sliderAutosave
        .init(autosaveLabels[ucValue], eControl_Autosave, 0, 8, ucValue);
    ...
    m_sliderDifficulty
        .init(difficultyLabels[ucValue], eControl_Difficulty, 0, 3, ucValue);

    std::wstring wsText = app.GetString(
        m_iDifficultySettingA
...
void UIScene_SettingsOptionsMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_SettingsOptionsMenu::handleSliderMove(
    F64 sliderId,
    F64 currentValue
) {
