...
static UIScene_HowToPlay::SHowToPlayPageDef gs_aPageDefs[eHowToPlay_NumPages] =
    {
        ...
        {IDS_HOW_TO_PLAY_CHEST,
         UIScene_HowToPlay::eHowToPlay_LabelSCInventory,
         2                                                      }, // eHowToPlay_Chest
        ...
        {IDS_HOW_TO_PLAY_CRAFTING,
         UIScene_HowToPlay::eHowToPlay_LabelCItem,
         3                                                      }, // eHowToPlay_InventoryCrafting
        {IDS_HOW_TO_PLAY_CRAFT_TABLE,
         UIScene_HowToPlay::eHowToPlay_LabelCTItem,
         3                                                      }, // eHowToPlay_CraftTable
        {IDS_HOW_TO_PLAY_FURNACE,
         UIScene_HowToPlay::eHowToPlay_LabelFFuel,
         4                                                      }, // eHowToPlay_Furnace
        {IDS_HOW_TO_PLAY_DISPENSER,
         UIScene_HowToPlay::eHowToPlay_LabelDText,
         2                                                      }, // eHowToPlay_Dispenser
        {IDS_HOW_TO_PLAY_BREWING,
         UIScene_HowToPlay::eHowToPlay_LabelBBrew,
...
UIScene_HowToPlay::UIScene_HowToPlay(
    int      iPad,
    void*    initData,
    UILayer* parentLayer
)
: UIScene(iPad, parentLayer) {
    ...
    TelemetryManager->RecordMenuShown(
        m_iPad,
        eUIScene_HowToPlay,
        (ETelemetry_HowToPlay_SubMenuId)eStartPage
    );
...
void UIScene_HowToPlay::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
#ifdef __PS3__
            // If it's the blu ray, or the first Japanese digital game, there's
            // no What's New until the first patch, which will take this line
            // out
            if (StorageManager.GetBootTypeDisc()
                || (app.GetProductSKU() == e_sku_SCEJ)) {
...
// replaceAll(replacedText,L"{*TITLE_UPDATE_NAME*}",app.GetString(IDS_TITLE_UPDATE_NAME));
    ...
    for (int index = finalText.find(L"\r\n", lastIndex, 2);
        ...
        paragraphs.push_back(
            finalText.substr(lastIndex, index - lastIndex) + L" "
        );
        lastIndex = index + 2;
    }
    paragraphs.push_back(
