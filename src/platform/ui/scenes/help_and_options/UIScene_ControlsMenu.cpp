...
void UIScene_ControlsMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_ControlsMenu::PositionText(
    int           iPad,
    int           iTextID,
    ...
    if (uiVal & _360_JOY_BUTTON_BACK) {
        ...
        PositionTextDirect(
            iPad,
            iTextID,
            (InputManager.UsingRemoteVita() ? e_PadTouch : e_PadBack),
            true
        );
#else
...
void UIScene_ControlsMenu::PositionTextDirect(
    int  iPad,
    int  iTextID,
