...
#include "platform/ui/controls/UIControl_PlayerSkinPreview.h"
...
UIScene_SkinSelectMenu::UIScene_SkinSelectMenu(
    int      iPad,
    void*    initData,
    UILayer* parentLayer
)
    ...
    if (app.StartInstallDLCProcess(m_iPad)) {
        ...
        if (app.m_dlcManager.getPackCount(DLCManager::e_DLCType_Skin) > 0) {
            ...
            if (m_currentPack != NULL) {
                m_packIndex = app.m_dlcManager.getPackIndex(
                                  m_currentPack,
                                  bFound,
...
void UIScene_SkinSelectMenu::updateTooltips() {
    ui.SetTooltips(
        m_iPad,
        m_bNoSkinsToShow ? -1 : IDS_TOOLTIPS_SELECT_SKIN,
        IDS_TOOLTIPS_CANCEL,
        -1,
        -1,
        -1,
...
void UIScene_SkinSelectMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
  ...
  // you can't see the store
...
#endif
                            {
                                ...
                                ui.RequestMessageBox(
                                    ...
                                    2,
                                    iPad,
                                    &UIScene_SkinSelectMenu::UnlockSkinReturned,
                                    this,
                                    app.GetStringTable(),
                                    NULL,
                                    0,
...
void UIScene_SkinSelectMenu::customDraw(IggyCustomDrawCallbackRegion* region) {
    ...
    if (characterId == -1) {
        ...
        if (region->stencil_func_ref != 0)
            RenderManager.StateSetStencil(
                GL_EQUAL,
                region->stencil_func_ref,
                region->stencil_func_mask
            );
#elif __PSVITA__
...
void UIScene_SkinSelectMenu::handleSkinIndexChanged() {
    ...
    if (m_currentPack != NULL) {
        ...
        bLicensed = m_currentPack->hasPurchasedFile(
            DLCManager::e_DLCType_Skin,
            m_selectedSkinPath
        );
    ...
    m_characters[eCharacter_Current].SetTexture(
        m_selectedSkinPath,
        backupTexture
    );
    ...
    for (BYTE i = 0; i < sidePreviewControlsR; ++i) {
        if (showNext) {
            ...
            m_characters[eCharacter_Next1 + i].SetTexture(
                otherSkinPath,
                backupTexture
            );
    ...
    for (BYTE i = 0; i < sidePreviewControlsL; ++i) {
        if (showPrevious) {
                ...
                case SKIN_SELECT_PACK_FAVORITES:
                    if (uiCurrentFavoriteC > 0) {
                        ...
                        swprintf(
                            ...
                            256,
                            L"dlcskin%08d.png",
                            app.GetPlayerFavoriteSkin(m_iPad, previousIndex)
                        );
            ...
            m_characters[eCharacter_Previous1 + i].SetTexture(
                otherSkinPath,
                backupTexture
            );
...
void UIScene_SkinSelectMenu::handlePackIndexChanged() {
    if (m_packIndex >= SKIN_SELECT_MAX_DEFAULTS) {
        m_currentPack = app.m_dlcManager.getPack(
            m_packIndex - SKIN_SELECT_MAX_DEFAULTS,
            DLCManager::e_DLCType_Skin
        );
    } else {
        m_currentPack = NULL;
            ...
            if (app.GetPlayerFavoriteSkinsCount(m_iPad) > 0) {
                ...
                swprintf(
                    ...
                    app.GetPlayerFavoriteSkin(
                        m_iPad,
                        app.GetPlayerFavoriteSkinsPos(m_iPad)
                    )
                );
...
void UIScene_SkinSelectMenu::updatePackDisplay() {
    ...
    if (nextPackIndex >= SKIN_SELECT_MAX_DEFAULTS) {
        DLCPack* thisPack = app.m_dlcManager.getPack(
            nextPackIndex - SKIN_SELECT_MAX_DEFAULTS,
            DLCManager::e_DLCType_Skin
        );
...
int UIScene_SkinSelectMenu::getNextPackIndex(DWORD sourceIndex) {
    ...
    ++nextPack;
    if (nextPack > app.m_dlcManager.getPackCount(DLCManager::e_DLCType_Skin) - 1
                       + SKIN_SELECT_MAX_DEFAULTS) {
...
void UIScene_SkinSelectMenu::setCharacterSelected(bool selected) {
    ...
    IggyResult out   = IggyPlayerCallMethodRS(
        ...
        m_funcSetPlayerCharacterSelected,
        1,
        value
    );
}
...
#ifdef __PSVITA__
void UIScene_SkinSelectMenu::handleTouchInput(
    unsigned int iPad,
    S32          x,
    S32          y,
    int          iId,
...
void UIScene_SkinSelectMenu::HandleDLCInstalled() {
    ...
    if (app.StartInstallDLCProcess(m_iPad) == false) {
        ...
        app.DebugPrintf(
            4,
            "UIScene_SkinSelectMenu::HandleDLCInstalled - not doing a mount, "
            "so re-enable input\n"
        );
        m_bIgnoreInput = false;
    } else {
