...
: UIScene(iPad, parentLayer) {
    ...
    m_buttons[BUTTON_ALL_OPTIONS].init(
        app.GetString(IDS_OPTIONS),
        BUTTON_ALL_OPTIONS
    );
    m_buttons[BUTTON_ALL_AUDIO].init(
        app.GetString(IDS_AUDIO),
        BUTTON_ALL_AUDIO
    );
    m_buttons[BUTTON_ALL_CONTROL].init(
        app.GetString(IDS_CONTROL),
        BUTTON_ALL_CONTROL
    );
    m_buttons[BUTTON_ALL_GRAPHICS].init(
        app.GetString(IDS_GRAPHICS),
        BUTTON_ALL_GRAPHICS
    );
    m_buttons[BUTTON_ALL_UI].init(
...
void UIScene_SettingsMenu::handlePress(F64 controlId, F64 childId) {
    ...
    case BUTTON_ALL_RESETTODEFAULTS: {
        ...
        ui.RequestMessageBox(
            ...
            2,
            m_iPad,
            &UIScene_SettingsMenu::ResetDefaultsDialogReturned,
            this,
            app.GetStringTable(),
            NULL,
            0,
