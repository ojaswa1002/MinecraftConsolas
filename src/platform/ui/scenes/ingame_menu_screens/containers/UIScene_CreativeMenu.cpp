...
UIScene_CreativeMenu::UIScene_CreativeMenu(
    int      iPad,
    void*    _initData,
    UILayer* parentLayer
)
...
void UIScene_CreativeMenu::handleTouchInput(
    unsigned int iPad,
    S32          x,
    S32          y,
    int          iId,
    ...
    if (bRepeat && iId == ETouchInput_TouchSlider
        ...
        // clamp
        if (fPosition > 1) fPosition = 1.0f;
        else if (fPosition < 0) fPosition = 0.0f;
...
void UIScene_CreativeMenu::handleTouchBoxRebuild() {
    addTimer(
        GAME_CREATIVE_TOUCHUPDATE_TIMER_ID,
...
void UIScene_CreativeMenu::handleOtherClicked(
    int           iPad,
    ...
    case eSectionInventoryCreativeTab_7: {
        ECreativeInventoryTabs tab =
            (ECreativeInventoryTabs)((int)eCreativeInventoryTab_BuildingBlocks
                                     + (int)eSection
...
void UIScene_CreativeMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_CreativeMenu::GetPositionOfSection(
    ESceneSection eSection,
    UIVec2D*      pPosition
) {
    switch (eSection) {
...
void UIScene_CreativeMenu::GetItemScreenData(
    ESceneSection eSection,
    int           iItemIndex,
    UIVec2D*      pPosition,
    UIVec2D*      pSize
...
void UIScene_CreativeMenu::setSectionSelectedSlot(
    ESceneSection eSection,
    int           x,
    int           y
) {
...
void UIScene_CreativeMenu::updateScrollCurrentPage(
    int currentPage,
    int pageCount
) {
    IggyDataValue result;
