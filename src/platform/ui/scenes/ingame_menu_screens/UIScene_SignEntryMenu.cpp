...
void UIScene_SignEntryMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_SignEntryMenu::handlePress(F64 controlId, F64 childId) {
    ...
    case eControl_Line4: {
        ...
        m_bIgnoreInput = true;
#ifdef _XBOX_ONE
        // 4J-PB - Xbox One uses the Windows virtual keyboard, and doesn't have
        // the Xbox 360 Latin keyboard type, so we can't restrict the input set
        // to alphanumeric. The closest we get is the emailSmtpAddress type.
