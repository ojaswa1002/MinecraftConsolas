...
#include "platform/ui/all_platforms/IUIScene_PauseMenu.h"
...
#ifdef _DURANGO
#include "durango/source files/leaderboards/DurangoStatsDebugger.h"
#endif

#ifdef __PSVITA__
#include "psvita/source files/network/SonyCommerce_Vita.h"
#endif
...
: UIScene(iPad, parentLayer) {
    ...
    m_buttons[BUTTON_PAUSE_RESUMEGAME].init(
    ...
    m_buttons[BUTTON_PAUSE_HELPANDOPTIONS].init(
        ...
        BUTTON_PAUSE_HELPANDOPTIONS
    );
    m_buttons[BUTTON_PAUSE_LEADERBOARDS].init(
        app.GetString(IDS_LEADERBOARDS),
        BUTTON_PAUSE_LEADERBOARDS
    );
#ifdef _DURANGO
    m_buttons[BUTTON_PAUSE_XBOXHELP].init(
        app.GetString(IDS_XBOX_HELP_APP),
        BUTTON_PAUSE_XBOXHELP
    );
#else
    m_buttons[BUTTON_PAUSE_ACHIEVEMENTS].init(
...
#endif
    m_buttons[BUTTON_PAUSE_EXITGAME].init(
...
#ifdef _XBOX_ONE
    bool bDisplayBanTip = !g_NetworkManager.IsLocalGame() && !bIsisPrimaryHost
                       && !ProfileManager.IsGuest(m_iPad);
#endif
...
void        UIScene_PauseMenu::handlePreReload() {
    ...
    if (ProfileManager.GetPrimaryPad() == m_iPad) {
        ...
        m_buttons[BUTTON_PAUSE_SAVEGAME].setAllPossibleLabels(
            2,
            saveButtonLabels
        );
    }
...
void UIScene_PauseMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
    ...
    case VK_PAD_X:
        ...
        if (bIsisPrimaryHost) {
            ...
            // changes, we need to update the pause menu and tooltips Fix for
            // #12531 - TCR 001: BAS Game Stability: When a player selects to
            // change a storage device, and repeatedly backs out of the SD
    ...
    case ACTION_MENU_Y: {
      ...
      // get them to sign in to online
        ...
        if (bUserisClientSide) {
            // 4J Stu - Added check in 1.8.2 bug fix (TU6) to stop repeat key
            // presses
    ...
    case ACTION_MENU_RIGHT_SCROLL:
        if (bDisplayBanTip) {
            ...
            ui.RequestMessageBox(
                ...
                IDS_ACTION_BAN_LEVEL_DESCRIPTION,
                uiIDA,
                2,
                iPad,
...
#if !(                                                                         \
    defined(_XBOX) || defined(_WIN64)                                          \
) // 4J Stu - Temp to get the win build running, but so we check this for other
  // platforms
        ...
        if (ProfileManager.IsFullVersion()) {
            ...
            if (m_iPad == ProfileManager.GetPrimaryPad()) {
                ...
                if (g_NetworkManager.IsHost()
                    ...
                    ui.RequestMessageBox(
                        IDS_EXIT_GAME,
                        IDS_CONFIRM_EXIT_GAME_CONFIRM_DISCONNECT,
                        uiIDA,
                        2,
                        m_iPad,
...
void UIScene_PauseMenu::PerformActionSaveGame() {
    ...
    if (!Minecraft::GetInstance()->skins->isUsingDefaultSkin()) {
        ...
        if (!m_pDLCPack->hasPurchasedFile(DLCManager::e_DLCType_Texture, L"")) {
            ...
            if (app.DLCInstallProcessCompleted()
                && !SonyCommerce_Vita::
                       getDLCUpgradePending()) // MGH - devtrack #5861 On vita
                                               // it can take a bit after the
                                               // install has finished to
                                               // register the purchase, so make
                                               // sure we don't end up asking to
                                               // purchase again
#endif
            {
                ui.RequestMessageBox(
...
#endif
    {
        ...
        if (!m_savesDisabled) {
            ...
            uiIDA[1] = IDS_CONFIRM_OK;
            ui.RequestMessageBox(
                IDS_TITLE_DISABLE_AUTOSAVE,
                IDS_CONFIRM_DISABLE_AUTOSAVE,
                uiIDA,
                2,
                m_iPad,
                &IUIScene_PauseMenu::DisableAutosaveDialogReturned,
                this,
                app.GetStringTable(),
                NULL,
                0,
            ...
            if (bSaveExists) {
                ...
                uiIDA[1] = IDS_CONFIRM_OK;
                ui.RequestMessageBox(
                    IDS_TITLE_ENABLE_AUTOSAVE,
                    IDS_CONFIRM_ENABLE_AUTOSAVE,
                    uiIDA,
                    2,
                    m_iPad,
...
int UIScene_PauseMenu::SaveGame_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
...
int UIScene_PauseMenu::MustSignInReturnedPSN(
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        ...
        case eAction_ViewInvitesPSN:
            SQRNetworkManager_PS3::AttemptPSNSignIn(
                &UIScene_PauseMenu::ViewInvites_SignInReturned,
                pClass
            );
            break;
        case eAction_SaveGamePSN:
            SQRNetworkManager_PS3::AttemptPSNSignIn(
                &UIScene_PauseMenu::SaveGame_SignInReturned,
                pClass
            );
            break;
        case eAction_BuyTexturePackPSN:
            SQRNetworkManager_PS3::AttemptPSNSignIn(
                &UIScene_PauseMenu::BuyTexturePack_SignInReturned,
                pClass
            );
            break;
        }
...
int UIScene_PauseMenu::ViewLeaderboards_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
...
int UIScene_PauseMenu::BuyTexturePack_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
...
int UIScene_PauseMenu::ViewInvites_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
...
bool UIScene_PauseMenu::CheckForPatch() {
    ...
    if (bPatchAvailable) {
        std::int32_t ret = sceErrorDialogInitialize();
        if (ret == SCE_OK) {
