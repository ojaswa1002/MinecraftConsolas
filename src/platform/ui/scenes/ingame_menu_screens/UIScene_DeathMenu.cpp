...
void UIScene_DeathMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_DeathMenu::handlePress(F64 controlId, F64 childId) {
    ...
    case eControl_ExitGame: {
        Minecraft* pMinecraft = Minecraft::GetInstance();
        // 4J-PB - fix for #8333 - BLOCKER: If player decides to exit game, then
        // cancels the exit player becomes stuck at game over screen
        // m_bIgnoreInput = true;
