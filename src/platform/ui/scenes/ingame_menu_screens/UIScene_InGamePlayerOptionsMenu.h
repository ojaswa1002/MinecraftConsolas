...
private:
    ...
    UI_MAP_ELEMENT(
        m_checkboxes[eControl_UseContainers],
        "CheckboxUseContainers"
    )
    UI_MAP_ELEMENT(
        m_checkboxes[eControl_AttackPlayers],
        "CheckboxAttackPlayers"
    )
    UI_MAP_ELEMENT(
        m_checkboxes[eControl_AttackAnimals],
...
public:
    ...
    static int KickPlayerReturned(
        void*                      pParam,
        int                        iPad,
        C4JStorage::EMessageResult result
    );
    static void
    OnPlayerChanged(void* callbackParam, INetworkPlayer* pPlayer, bool leaving);
