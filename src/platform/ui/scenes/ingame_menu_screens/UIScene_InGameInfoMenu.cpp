...
: UIScene(iPad, parentLayer) {
    ...
    m_buttonGameOptions.init(
        app.GetString(IDS_HOST_OPTIONS),
    ...
    for (DWORD i = 0; i < playerCount; ++i) {
        ...
        if (player != NULL) {
            ...
            m_playerList.addItem(
                playerName,
                app.GetPlayerColour(m_players[i]),
                voiceStatus
            );
        }
    }
    ...
    g_NetworkManager.RegisterPlayerChangedCallback(
        m_iPad,
        &UIScene_InGameInfoMenu::OnPlayerChanged,
        this
    );
        ...
        if (player != NULL) {
            ...
            if (icon != m_playersColourState[i]) {
                ...
                m_playerList.setPlayerIcon(
                    i,
                    (int)app.GetPlayerColour(m_players[i])
                );
            }
...
void UIScene_InGameInfoMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
        ...
        if (pressed && iPad == ProfileManager.GetPrimaryPad()) {
            ...
            if (!ProfileManager.IsSignedInLive(iPad)) {
                ...
                ui.RequestMessageBox(
                    ...
                    &UIScene_InGameInfoMenu::MustSignInReturnedPSN,
                    this,
                    app.GetStringTable()
                );
            } else
...
void UIScene_InGameInfoMenu::handlePress(F64 controlId, F64 childId) {
    ...
    case eControl_GamePlayers:
        ...
        INetworkPlayer* selectedPlayer =
            g_NetworkManager.GetPlayerBySmallId(m_players[currentSelection]);
     ...
     && defined(_DEBUG_MENUS_ENABLED))
                ...
                *smallId      = m_players[currentSelection];
                ...
                ui.RequestMessageBox(
                    IDS_UNLOCK_KICK_PLAYER_TITLE,
                    IDS_UNLOCK_KICK_PLAYER,
                    uiIDA,
                    2,
                    m_iPad,
                    &UIScene_InGameInfoMenu::KickPlayerReturned,
                    smallId,
                    app.GetStringTable(),
                    NULL,
                    0,
...
void UIScene_InGameInfoMenu::OnPlayerChanged(
    void*           callbackParam,
...
int         UIScene_InGameInfoMenu::MustSignInReturnedPSN(
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        ...
        SQRNetworkManager_PS3::AttemptPSNSignIn(
            &UIScene_InGameInfoMenu::ViewInvites_SignInReturned,
            pClass
        );
#else // __PSVITA__
...
int UIScene_InGameInfoMenu::ViewInvites_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
