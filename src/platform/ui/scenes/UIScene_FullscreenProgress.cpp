...
: UIScene(iPad, parentLayer) {
    ...
    swprintf(
        startTags,
        64,
        L"<font color=\"#%08x\"><p align=center>",
    ...
    thread->SetProcessor(
        CPU_CORE_UI_SCENE
    ); // TODO 4J Stu - Make sure this is a good thread/core to use
...
void UIScene_FullscreenProgress::updateTooltips() {
    ui.SetTooltips(
        ...
        m_threadCompleted ? -1 : m_cancelText,
        -1,
        -1
    );
}
        ...
        if (exitcode != S_OK) {
            ...
            if ((m_CompletionData->bRequiresUserAction == TRUE)
                ...
                case e_ProgressCompletion_AutosaveNavigateBack:
                    ...
                    {
                        // 4J Stu - Fix for #65437 - Customer Encountered: Code:
                        // Settings: Autosave option doesn't work when the Host
                        // goes into idle state during gameplay. Autosave
                        // obviously cannot occur if an ignore autosave menu is
                        // displayed, so even if we navigate back to a scene and
                        // not empty then we still want to reset this flag which
                        // was set true by the navigate to the fullscreen
                        // progress
                ...
                case e_ProgressCompletion_NavigateBackToScene:
                    ...
                    // interfered with any other scene at all, so just navigate
                    // back
                    navigateBack();
                    break;
...
void UIScene_FullscreenProgress::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_FullscreenProgress::handlePress(F64 controlId, F64 childId) {
    if (m_threadCompleted && (int)controlId == eControl_Confirm) {
        ...
        case e_ProgressCompletion_NavigateBackToScene:
            ...
            ui.UpdatePlayerBasePositions();
            // 4J Stu - If used correctly this scene will not have interfered
            // with any other scene at all, so just navigate back
            navigateBack();
            break;
        case e_ProgressCompletion_CloseUIScenes:
            app.DebugPrintf("e_ProgressCompletion_CloseUIScenes\n");
