...
: UIScene(iPad, parentLayer) {
    ...
    m_buttonResetTutorial.init(
        L"Reset profile tutorial progress",
    ...
    for (unsigned int i = 0; i < Enchantment::validEnchantments.size(); ++i) {
        ...
        for (unsigned int level = ench->getMinLevel();
            ...
            m_buttonListEnchantments.addItem(
                app.GetString(ench->getDescriptionId()) + _toString<int>(level)
            );
        }
    }
...
void UIScene_DebugOverlay::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
