...
UIScene_DebugSetCamera::UIScene_DebugSetCamera(
    int      iPad,
    void*    initData,
    UILayer* parentLayer
)
: UIScene(iPad, parentLayer) {
    ...
    m_checkboxLockPlayer
        .init(L"Lock Player", eControl_LockPlayer, app.GetFreezePlayers());
...
void UIScene_DebugSetCamera::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_DebugSetCamera::handlePress(F64 controlId, F64 childId) {
    ...
    case eControl_Elevation:
        ...
        InputManager.RequestKeyboard(
            ...
            (DWORD)0,
            25,
            &UIScene_DebugSetCamera::KeyboardCompleteCallback,
            this,
...
void UIScene_DebugSetCamera::handleCheckboxToggled(
    F64  controlId,
    bool selected
) {
    switch ((int)controlId) {
...
int UIScene_DebugSetCamera::KeyboardCompleteCallback(
    LPVOID lpParam,
    bool   bRes
) {
    UIScene_DebugSetCamera* pClass = (UIScene_DebugSetCamera*)lpParam;
    std::uint16_t           pchText[2048]; //[128];
    ZeroMemory(pchText, 2048 /*128*/ * sizeof(std::uint16_t));
