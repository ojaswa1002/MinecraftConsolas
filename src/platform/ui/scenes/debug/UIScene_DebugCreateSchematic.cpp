...
UIScene_DebugCreateSchematic::UIScene_DebugCreateSchematic(
    int      iPad,
    void*    initData,
    UILayer* parentLayer
)
: UIScene(iPad, parentLayer) {
    ...
    m_checkboxUseCompression
        .init(L"Use Compression", eControl_UseCompression, false);
...
void UIScene_DebugCreateSchematic::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
void UIScene_DebugCreateSchematic::handlePress(F64 controlId, F64 childId) {
    ...
    case eControl_Create: {
        ...
        if (m_data->startX > 0 && m_data->startX % 2 != 0) m_data->startX -= 1;
        else if (m_data->startX < 0 && m_data->startX % 2 != 0)
            m_data->startX -= 1;
        if (m_data->startY < 0) m_data->startY = 0;
        else if (m_data->startY > 0 && m_data->startY % 2 != 0)
            m_data->startY -= 1;
        if (m_data->startZ > 0 && m_data->startZ % 2 != 0) m_data->startZ -= 1;
        else if (m_data->startZ < 0 && m_data->startZ % 2 != 0)
            m_data->startZ -= 1;

        // We want the end to be odd to have a total size that is even
        if (m_data->endX > 0 && m_data->endX % 2 == 0) m_data->endX += 1;
        else if (m_data->endX < 0 && m_data->endX % 2 == 0) m_data->endX += 1;
        if (m_data->endY > Level::maxBuildHeight)
            m_data->endY = Level::maxBuildHeight;
        else if (m_data->endY > 0 && m_data->endY % 2 == 0) m_data->endY += 1;
        else if (m_data->endY < 0 && m_data->endY % 2 == 0) m_data->endY += 1;
        if (m_data->endZ > 0 && m_data->endZ % 2 == 0) m_data->endZ += 1;
...
void UIScene_DebugCreateSchematic::handleCheckboxToggled(
    ...
    case eControl_UseCompression:
        if (selected) m_data->compressionType = APPROPRIATE_COMPRESSION_TYPE;
        else m_data->compressionType = Compression::eCompressionType_RLE;
        break;
    }
}

int UIScene_DebugCreateSchematic::KeyboardCompleteCallback(
    LPVOID lpParam,
    bool   bRes
) {
