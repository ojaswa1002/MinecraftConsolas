...
void UIScene_QuadrantSignin::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
            ...
            if (pressed) {
                ...
                if (ProfileManager.IsSignedIn(iPad)) {
...
#ifdef _DURANGO
                    // On Durango, if we don't navigate forward here, then when
                    // we are on the main menu, it (re)gains focus & that causes
                    // our users to get cleared
                    ...
                    ProfileManager.RequestSignInUI(
                        ...
                        true,
                        true,
                        &UIScene_QuadrantSignin::SignInReturned,
                        this,
                        iPad
                    );
                }
...
int UIScene_QuadrantSignin::SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
...
#ifdef _DURANGO
void UIScene_QuadrantSignin::checkAllPrivilegesCallback(
    LPVOID lpParam,
    bool   hasPrivileges,
    int    iPad
) {
        ...
        if (ProfileManager.IsSignedIn(i) && InputManager.IsPadConnected(i)) {
            ...
            if (!m_iconRequested[i]) {
                ...
                if (ProfileManager.GetProfileAvatar(
                        i,
                        &UIScene_QuadrantSignin::AvatarReturned,
                        this
                    )) {
...
void UIScene_QuadrantSignin::setControllerState(
    int               iPad,
...
int UIScene_QuadrantSignin::AvatarReturned(
    LPVOID lpParam,
    PBYTE  pbThumbnail,
