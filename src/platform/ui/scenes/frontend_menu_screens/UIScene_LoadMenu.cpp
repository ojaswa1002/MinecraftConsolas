...
int UIScene_LoadMenu::LoadSaveDataThumbnailReturned(
    ...
    if (pbThumbnail && dwThumbnailBytes) {
        pClass->registerSubstitutionTexture(
            pClass->m_thumbnailName,
            pbThumbnail,
            dwThumbnailBytes
        );
...
: IUIScene_StartGame(iPad, parentLayer) {
    ...
    if (getSceneResolution() == eSceneResolution_1080) {
        ...
        m_checkboxOnline.init(
            app.GetString(IDS_ONLINE_GAME),
            eControl_OnlineGame,
            bOnlineGame
        );
    }
    ...
    || defined(__PSVITA__)
        ...
        L10nResult lres = UTF8stoUTF16s(
            (std::uint8_t*)params->saveDetails->UTF8SaveFilename,
            &srclen,
            u16Message,
            &dstlen
        );
        ...
        memcpy(
            u16Message,
            params->saveDetails->UTF16SaveFilename,
...
#endif
        {
            ...
            if (bNeedToGetTPD == true) {
                ...
                for (unsigned int i = 0;
                     i < app.GetDLCInfoTexturesOffersCount();
                     ++i) {
                    bTexturePackAlreadyListed = false;
#if defined(__PS3__) || defined(__ORBIS__)
                    char* pchName = app.GetDLCInfoTextures(i);
        ...
        if (bHostOptionsRead) {
            m_MoreOptionsParams.bPVP =
                app.GetGameHostOption(uiHostOptions, eGameHostOption_PvP) > 0
                    ? TRUE
                    : FALSE;
            m_MoreOptionsParams.bTrust       = app.GetGameHostOption(
                                             uiHostOptions,
                                             eGameHostOption_TrustPlayers
                                         ) > 0
                                                 ? TRUE
                                                 : FALSE;
            m_MoreOptionsParams.bFireSpreads = app.GetGameHostOption(
                                                   uiHostOptions,
            ...
            m_bHasBeenInCreative = app.GetGameHostOption(
                                       uiHostOptions,
...
#ifdef __ORBIS__
int UIScene_LoadMenu::ContinueOffline(
    void*                      pParam,
    int                        iPad,
...
void UIScene_LoadMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
#ifdef _DURANGO
void UIScene_LoadMenu::checkPrivilegeCallback(
    LPVOID lpParam,
    bool   hasPrivilege,
    int    iPad
) {
        ...
        if (m_pDLCPack
            ...
            else */
            {
                ...
                ui.RequestMessageBox(
                    ...
                    1,
                    m_iPad,
                    &TrialTexturePackWarningReturned,
                    this,
                    app.GetStringTable(),
                    NULL,
                    0,
    ...
    case GAME_CREATE_ONLINE_TIMER_ID: {
        ...
        if (bMultiplayerAllowed != m_bMultiplayerAllowed) {
            if (bMultiplayerAllowed) {
                ...
                if (bGameSetting_Online) {
                    m_MoreOptionsParams.bInviteOnly =
                        (app.GetGameSettings(m_iPad, eGameSetting_InviteOnly)
                         != 0)
                            ? TRUE
                            : FALSE;
                    m_MoreOptionsParams.bAllowFriendsOfFriends =
        ...
        if (m_bGameModeSurvival != true || m_bHasBeenInCreative) {
            ...
            if (m_bGameModeSurvival) {
                ui.RequestMessageBox(
                    ...
                    IDS_CONFIRM_START_SAVEDINCREATIVE,
                    uiIDA,
                    2,
                    m_iPad,
            ...
            ui.RequestMessageBox(
                IDS_TITLE_START_GAME,
                IDS_CONFIRM_START_HOST_PRIVILEGES,
                uiIDA,
                2,
                m_iPad,
                &UIScene_LoadMenu::ConfirmLoadReturned,
        ...
        if (isOnlineGame && !isSignedInLive) {
            ...
            if (npAvailability == SCE_NP_ERROR_AGE_RESTRICTION) {
                ...
                ui.RequestMessageBox(
                    ...
                    2,
                    iPadNotSignedInLive,
                    &UIScene_LoadMenu::MustSignInReturnedPSN,
                    pClass,
                    app.GetStringTable(),
                    NULL,
                    0,
...
void UIScene_LoadMenu::StartGameFromSave(
    ...
    if (CGameNetworkManager::usingAdhocMode()) {
        if (SQRNetworkManager_AdHoc_Vita::
                GetAdhocStatus()) // && pClass->m_MoreOptionsParams.bOnlineGame)
            isClientSide = true;
    }
#endif // __PSVITA__
    ...
    app.SetGameHostOption(
        ...
        pClass->m_bGameModeSurvival ? GameType::SURVIVAL->getId()
...
void UIScene_LoadMenu::LoadLevelGen(LevelGenerationOptions* levelGen) {
    ...
    // netherwart etc.
    app.SetResetNether(
        (m_MoreOptionsParams.bResetNether == TRUE) ? true : false
    );
    // clear out the app's terrain features list
    app.ClearTerrainFeaturePosition();
