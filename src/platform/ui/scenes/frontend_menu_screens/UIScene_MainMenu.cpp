...
: UIScene(iPad, parentLayer) {
    ...
    m_buttons[(int)eControl_PlayGame].init(
    ...
    m_buttons[(int)eControl_Leaderboards].init(
        app.GetString(IDS_LEADERBOARDS),
        eControl_Leaderboards
    );
    m_buttons[(int)eControl_Achievements].init(
...
#ifndef _DURANGO
    m_buttons[(int)eControl_Exit].init(
        app.GetString(IDS_EXIT_GAME),
        eControl_Exit
    );
#else
    m_buttons[(int)eControl_XboxHelp].init(
...
void UIScene_MainMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
    ...
    case ACTION_MENU_X:
        if (pressed && ProfileManager.IsFullVersion()) {
            ...
            ui.RequestMessageBox(
                IDS_SELECT_NETWORK_MODE_TITLE,
                IDS_SELECT_NETWORK_MODE_TEXT,
                uiIDA,
                2,
                XUSER_INDEX_ANY,
                &UIScene_MainMenu::SelectNetworkModeReturned,
                this
            );
        }
        break;
...
void UIScene_MainMenu::handlePress(F64 controlId, F64 childId) {
...
#ifdef __ORBIS__
    {
        ...
        ProfileManager.RefreshChatAndContentRestrictions(
            RefreshChatAndContentRestrictionsReturned_PlayGame,
            this
        );
    }
#else
...
#ifdef __ORBIS__
        ProfileManager.RefreshChatAndContentRestrictions(
            RefreshChatAndContentRestrictionsReturned_Leaderboards,
            this
        );
#else
    ...
    case eControl_Exit:
        if (ProfileManager.IsFullVersion()) {
            ...
            ui.RequestMessageBox(
                ...
                IDS_WARNING_ARCADE_TEXT,
                uiIDA,
                2,
                XUSER_INDEX_ANY,
                &UIScene_MainMenu::ExitGameReturned,
                this
            );
        } else {
#ifdef _XBOX_ONE
    ...
    if (signInReturnedFunc != NULL) {
        if (ProfileManager.IsSignedIn(primaryPad)) {
            ...
            ui.RequestMessageBox(
                IDS_MUST_SIGN_IN_TITLE,
                IDS_MUST_SIGN_IN_TEXT,
                uiIDA,
                2,
                primaryPad,
                &UIScene_MainMenu::MustSignInReturned,
                this,
                app.GetStringTable()
            );
        }
...
void UIScene_MainMenu::customDrawSplash(IggyCustomDrawCallbackRegion* region) {
    ...
    float sss =
        1.8f
        - Mth::abs(
            Mth::sin(System::currentTimeMillis() % 1000 / 1000.0f * PI * 2)
            * 0.1f
        );
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        ...
        case eAction_RunLeaderboards:
            ProfileManager.RequestSignInUI(
                ...
                false,
                true,
                &UIScene_MainMenu::Leaderboards_SignInReturned,
                pClass,
                iPad
            );
            break;
        case eAction_RunAchievements:
            ProfileManager.RequestSignInUI(
                ...
                false,
                true,
                &UIScene_MainMenu::Achievements_SignInReturned,
                pClass,
                iPad
            );
            break;
        ...
        case eAction_RunXboxHelp:
            ProfileManager.RequestSignInUI(
                ...
                false,
                true,
                &UIScene_MainMenu::XboxHelp_SignInReturned,
                pClass,
                iPad
            );
            break;
        ...
        for (int i = 0; i < XUSER_MAX_COUNT; i++) {
            ...
            if (ProfileManager.IsSignedIn(i)) {
                ProfileManager
                    .SetCurrentGameActivity(i, CONTEXT_PRESENCE_MENUS, false);
            }
        }
    }
...
int UIScene_MainMenu::MustSignInReturnedPSN(
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        ...
        if (pClass->m_eAction == eAction_RunGamePSN) {
            ...
            else CreateLoad_SignInReturned(pClass, true, 0);
        } else {
            pClass->m_bIgnorePress = false;
        }
    }
...
int UIScene_MainMenu::HelpAndOptions_SignInReturned(
    ...
    if (bContinue) {
        ...
        // status changes.
        ProfileManager
            .SetCurrentGameActivity(iPad, CONTEXT_PRESENCE_MENUS, false);
...
#ifdef _XBOX_ONE
int UIScene_MainMenu::ChooseUser_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
...
int UIScene_MainMenu::CreateLoad_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
            ...
            if (ProfileManager.IsFullVersion()) {
                ...
                if (bSignedInLive) {
                    ...
                    ProfileManager.DisplayOfflineProfile(
                        &CScene_Main::CreateLoad_OfflineProfileReturned,
                        pClass,
                        ProfileManager.GetPrimaryPad()
                    );
#else
...
#ifdef _DURANGO
int UIScene_MainMenu::XboxHelp_SignInReturned(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
    ...
    if (ProfileManager.IsGuest(iPad)) {
        ...
        if (ProfileManager.IsFullVersion()) {
            ...
            if (app.GetGameSettings(
                ...
                app.DebugPrintf(
                    "Adhoc mode signed in : %s\n",
                    bSignedInLive ? "true" : "false"
                );
            } else {
                CGameNetworkManager::setAdhocMode(false);
                app.DebugPrintf(
                    "PSN mode signed in : %s\n",
                    bSignedInLive ? "true" : "false"
                );
            }
...
void UIScene_MainMenu::tick() {
    ...
    if (m_bLaunchFullVersionPurchase) {
        ...
        if ((iCommerceState == CConsoleMinecraftApp::eCommerce_State_Online)
            || (iCommerceState
                == CConsoleMinecraftApp::eCommerce_State_Error)) {
            m_bLaunchFullVersionPurchase = false;
            m_bIgnorePress               = false;
            updateTooltips();
