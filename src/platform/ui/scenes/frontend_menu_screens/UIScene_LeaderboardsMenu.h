...
private:
    ...
    struct Leaderboard {
        DWORD
        m_totalEntryCount; // Either total number of entries in leaderboard,
                           // or total number of results for a friends query
        std::vector<LeaderboardEntry> m_entries;
        DWORD                         m_numColumns;
    };

    Leaderboard
        m_leaderboard; // All leaderboard data for the currently selected filter
    ...
    static int
    ExitLeaderboards(void* pParam, int iPad, C4JStorage::EMessageResult result);
...
private:
    ...
    virtual bool OnStatsReadComplete(
