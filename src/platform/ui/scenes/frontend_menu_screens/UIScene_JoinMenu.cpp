...
#endif
                {
                    ...
                    if (m_selectedSession->data.players[i].isSignedIntoPSN()
                        ...
                        sprintf(
                            suffix,
                            " (%d)",
                            m_selectedSession->data.players[i].getQuadrant() + 1
                        );
...
void UIScene_JoinMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
    ...
    case ACTION_MENU_Y:
        if (m_selectedSession != NULL
            ...
            PlayerUID uid =
                m_selectedSession->searchResult
                    .m_playerXuids[m_buttonListPlayers.getCurrentSelection()];
            if (uid != INVALID_XUID)
...
#ifdef _DURANGO
void UIScene_JoinMenu::checkPrivilegeCallback(
    LPVOID lpParam,
    bool   hasPrivilege,
    int    iPad
) {
