...
#ifdef __PSVITA__
#include "psvita/source files/network/SQRNetworkManager_AdHoc_Vita.h"
#endif
...
: IUIScene_StartGame(iPad, parentLayer) {
    ...
    m_buttonCreateWorld.init(
        app.GetString(IDS_CREATE_NEW_WORLD),
        eControl_NewWorld
    );
...
void UIScene_CreateWorldMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
#ifdef _DURANGO
void UIScene_CreateWorldMenu::checkPrivilegeCallback(
    LPVOID lpParam,
    bool   hasPrivilege,
    int    iPad
) {
...
void UIScene_CreateWorldMenu::handleTimerComplete(int id) {
    ...
    case CHECKFORAVAILABLETEXTUREPACKS_TIMER_ID: {
        ...
        // for each iConfig, check if the data is available, and add it to the
        // List, then remove it from the viConfig
...
int UIScene_CreateWorldMenu::KeyboardCompleteSeedCallback(
    LPVOID lpParam,
    bool   bRes
) {
    UIScene_CreateWorldMenu* pClass = (UIScene_CreateWorldMenu*)lpParam;
    ...
    if (m_bGameModeSurvival != true
        ...
        if (m_bGameModeSurvival != true) {
            ui.RequestMessageBox(
                ...
                2,
                m_iPad,
                &UIScene_CreateWorldMenu::ConfirmCreateReturned,
                this,
                app.GetStringTable(),
                NULL,
                0,
...
// sign-in or not
void UIScene_CreateWorldMenu::CreateGame(
    UIScene_CreateWorldMenu* pClass,
...
int UIScene_CreateWorldMenu::MustSignInReturnedPSN(
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        SQRNetworkManager_Orbis::AttemptPSNSignIn(
            &UIScene_CreateWorldMenu::StartGame_SignInReturned,
            pClass,
            false,
            iPad
        );
