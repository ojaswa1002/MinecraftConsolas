...
: UIScene(iPad, parentLayer) {
    ...
    m_checkboxes[eLaunchCheckbox_PVP].init(
        app.GetString(IDS_PLAYER_VS_PLAYER),
        eLaunchCheckbox_PVP,
        m_params->bPVP
    );
    m_checkboxes[eLaunchCheckbox_TrustSystem].init(
        app.GetString(IDS_TRUST_PLAYERS),
        eLaunchCheckbox_TrustSystem,
        m_params->bTrust
    );
    m_checkboxes[eLaunchCheckbox_FireSpreads].init(
        app.GetString(IDS_FIRE_SPREADS),
        eLaunchCheckbox_FireSpreads,
        m_params->bFireSpreads
    );
    m_checkboxes[eLaunchCheckbox_TNT].init(
        app.GetString(IDS_TNT_EXPLODES),
        eLaunchCheckbox_TNT,
        m_params->bTNT
    );
    ...
    m_checkboxes[eLaunchCheckbox_ResetNether].init(
        app.GetString(IDS_RESET_NETHER),
    ...
    if (m_loadedResolution == eSceneResolution_1080) {
        ...
        m_checkboxes[eLaunchCheckbox_DisableSaving].init(
            app.GetString(IDS_DISABLE_SAVING),
...
void UIScene_LaunchMoreOptionsMenu::handleCheckboxToggled(
    F64  controlId,
    bool selected
) {
    // CD - Added for audio
...
void UIScene_LaunchMoreOptionsMenu::handleFocusChange(
    F64 controlId,
    F64 childId
) {
    int stringId = 0;
...
void UIScene_LaunchMoreOptionsMenu::handleSliderMove(
    F64 sliderId,
    F64 currentValue
) {
