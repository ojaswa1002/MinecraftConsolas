    ...
    case PLAYER_ONLINE_TIMER_ID:
        ...
        if (ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad())
            ...
            C4JStorage::EMessageResult result = ui.RequestMessageBox(
                ...
                UIScene_DLCOffersMenu::ExitDLCOffersMenu,
                this,
                app.GetStringTable()
            );
        }
...
void UIScene_DLCOffersMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
AB"); break;
    ...
    */
    ...
    case ACTION_MENU_OTHER_STICK_UP:
        // don't pass down PageUp or PageDown because this will cause conflicts
        // between the buttonlist and scrollable html text component
    ...
    case eControl_OffersList: {
       ...
       // is the item purchasable?
        ...
        StorageManager.InstallOffer(
            1,
            StorageManager.GetOffer(iIndex).wszProductID,
            NULL,
            NULL
        );
#else
...
//
// 		auto itEnd = m_vIconRetrieval.end();
// 		for (auto it = m_vIconRetrieval.begin(); it != itEnd; it++)
// 		{
//
// 		}
//
        ...
        for (int i = 0; i < uiDLCCount; i++) {
            ...
            if (pDLC->eDLCType == (eDLCContentType)m_iProductInfoIndex) {
                ...
                if (pContentData != NULL) {
                    ...
                    m_buttonListOffers
                        .addItem(wstrTemp, false, OrderA[i].uiContentIndex);
                }
#else
                m_buttonListOffers.addItem(
                    wstrTemp,
                    xOffer.fUserHasPurchased,
                    OrderA[i].uiContentIndex
                );
#endif
