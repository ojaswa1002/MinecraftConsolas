...
#include "win/extraX64client.h"
#include "win/strings.h"
#include "world/SoundTypes.h"
#include "world/level/LevelSettings.h"
#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
#include "platform/network/sony/SonyHttp.h"
#include "platform/network/sony/SonyRemoteStorage.h"
#endif
...
: UIScene(iPad, parentLayer) {
    ...
    app.AddDLCRequest(
        e_Marketplace_Content
    ); // content is skin packs, texture packs and mash-up packs
#endif
...
void UIScene_LoadOrJoinMenu::tick() {
    ...
    if (hasFocus(m_iPad)) {
        ...
        if (m_bUpdateSaveSize) {
            if ((m_iDefaultButtonsC > 0)
                && (m_iSaveListIndex >= m_iDefaultButtonsC)) {
                m_spaceIndicatorSaves.selectSave(
                    m_iSaveListIndex - m_iDefaultButtonsC
                );
            } else {
            ...
            if (m_pSaveDetails != NULL) {
                ...
                for (unsigned int i = 0; i < m_pSaveDetails->iSaveC; ++i) {
                    ...
                    m_buttonListSaves.addItem(
                        m_pSaveDetails->SaveInfoA[i].UTF16SaveTitle,
                        L""
                    );

                    m_saveDetails[i].saveId = i;
                    memcpy(
                        m_saveDetails[i].UTF16SaveName,
                        m_pSaveDetails->SaveInfoA[i].UTF16SaveTitle,
                        128
                    );
                    memcpy(
                        m_saveDetails[i].UTF16SaveFilename,
        ...
        if (!m_bExitScene && m_bSavesDisplayed && !m_bRetrievingSaveThumbnails
            && !m_bAllLoaded) {
    ...
    case SCE_COMMON_DIALOG_STATUS_FINISHED: {
        ...
        // 4J-JEV: Fix for PS4 #5148 - [ONLINE] If the user attempts to join a
        // game when they do not have Playstation Plus, the title will lose all
        // functionality.
        m_bIgnoreInput = false;
    } break;
    default:
    ...
    for (auto it = app.getLevelGenerators()->begin();
        ...
        if (uiTexturePackID != 0) {
            ...
            if (dwImageBytes > 0 && pbImageData) {
                ...
                m_buttonListSaves.setTextureName(
                    m_buttonListSaves.getItemCount() - 1,
                    imageName
                );
            }
        }
...
void UIScene_LoadOrJoinMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
...
#endif
                        ui.RequestMessageBox(
                            IDS_TOOLTIPS_SAVEOPTIONS,
                            IDS_TEXT_SAVEOPTIONS,
                            uiIDA,
                            numOptions,
                            iPad,
                            &UIScene_LoadOrJoinMenu::SaveOptionsDialogReturned,
                            this,
                            app.GetStringTable(),
                            NULL,
                            0,
    ...
    case ACTION_MENU_RIGHT: {
        // if we are on the saves menu, check there are games in the games list
        // to move to
        if (DoesSavesListHaveFocus()) {
...
int UIScene_LoadOrJoinMenu::KeyboardCompleteWorldNameCallback(
    LPVOID lpParam,
    bool   bRes
) {
    ...
    case eControl_SavesList: {
        ...
        if ((int)childId == JOIN_LOAD_CREATE_BUTTON_INDEX) {
            ...
            PSAVE_INFO pSaveInfo =
                &m_pSaveDetails->SaveInfoA[((int)childId) - m_iDefaultButtonsC];
                             ...
                             // blocks useset for corrupt files
            {
                ...
                ui.RequestMessageBox(
                    ...
                    IDS_CORRUPT_OR_DAMAGED_SAVE_TEXT,
                    uiIDA,
                    2,
                    ProfileManager.GetPrimaryPad(),
                    &UIScene_LoadOrJoinMenu::DeleteSaveDialogReturned,
...
#ifdef _XBOX_ONE
                    // On XB1, saves might need syncing, in which case inform
                    // the user so they can decide whether they want to wait for
                    // this to happen
                    if (m_pSaveDetails->SaveInfoA[params->iSaveGameInfoIndex]
                            .needsSync) {
                        ...
                        ui.RequestMessageBox(
                            ...
                            IDS_CONFIRM_SYNC_REQUIRED,
                            uiIDA,
                            2,
                            ProfileManager.GetPrimaryPad(),
                            &NeedSyncMessageReturned,
                            this,
                            app.GetStringTable(),
                            NULL,
                            0,
        ...
        if (m_initData->selectedSession->data.texturePackParentId != 0) {
            ...
            if (bHasTexturePackInstalled == false) {
                ...
                ui.RequestMessageBox(
                    ...
                    2,
                    m_iPad,
                    &UIScene_LoadOrJoinMenu::TexturePackDialogReturned,
                    this,
                    app.GetStringTable(),
                    NULL,
                    0,
...
void UIScene_LoadOrJoinMenu::LoadSaveFromCloud() {
    ...
    mbstowcs(
        wFileName,
        app.getRemoteStorage()->getLocalFilename(),
...
int UIScene_LoadOrJoinMenu::SaveOptionsDialogReturned(
    ...
    case C4JStorage::EMessage_ResultFourthOption: // upload to cloud
    {
        ...
        ui.RequestMessageBox(
            ...
            IDS_SAVE_TRANSFER_TEXT,
            uiIDA,
            2,
            iPad,
            &UIScene_LoadOrJoinMenu::SaveTransferDialogReturned,
            pClass,
            app.GetStringTable(),
            NULL,
            0,
    ...
    case C4JStorage::EMessage_ResultFourthOption: // copy save
    {
        ...
        ui.RequestMessageBox(
            ...
            2,
            iPad,
            &UIScene_LoadOrJoinMenu::CopySaveDialogReturned,
            pClass,
            app.GetStringTable(),
            NULL,
            0,
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        ...
        if (ProfileManager.IsSignedIn(iPad)) {
            if (ProfileManager.IsSignedInLive(iPad)) {
                ...
                StorageManager
                    .InstallOffer(1, (WCHAR*)ProductId.c_str(), NULL, NULL);
            } else {
...
int                            UIScene_LoadOrJoinMenu::MustSignInReturnedPSN(
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        ...
        SQRNetworkManager_PS3::AttemptPSNSignIn(
            &UIScene_LoadOrJoinMenu::PSN_SignInReturned,
            pClass
        );
#elif defined __PSVITA__
...
int UIScene_LoadOrJoinMenu::CrossSaveDeleteOnErrorReturned(
    LPVOID lpParam,
    bool   bRes
) {
    UIScene_LoadOrJoinMenu* pClass = (UIScene_LoadOrJoinMenu*)lpParam;
    ...
    while (pClass->m_eSaveTransferState != eSaveTransfer_Idle) {
        ...
        case eSaveTransfer_GettingRemoteSaveInfo:
            ...
            if (app.getRemoteStorage()->waitingForSaveInfo() == false) {
                if (app.getRemoteStorage()->saveIsAvailable()) {
                    ...
                    ui.RequestMessageBox(
                        IDS_TOOLTIPS_SAVETRANSFER_DOWNLOAD,
                        IDS_SAVE_TRANSFER_NOT_AVAILABLE_TEXT,
                        uiIDA,
                        1,
                        ProfileManager.GetPrimaryPad(),
                        RemoteSaveNotFoundCallback,
                        pClass,
                        app.GetStringTable(),
                        NULL,
                        0,
        ...
        case eSaveTransfer_CreateDummyFile: {
            ...
            StorageManager.GetDefaultSaveImage(
                &pbDataSaveImage,
                &dwDataSizeSaveImage
            ); // Get the default save thumbnail (as set by SetDefaultImages)
               // for use on saving games t
            StorageManager.GetDefaultSaveThumbnail(
                &pbThumbnailData,
                &dwThumbnailDataSize
            ); // Get the default save image (as set by SetDefaultImages) for
               // use on saving games that

            BYTE bTextMetadata[88];
            ZeroMemory(bTextMetadata, 88);
            int iTextMetadataBytes = app.CreateImageTextData(
                bTextMetadata,
                app.getRemoteStorage()->getSaveSeed(),
                true,
                app.getRemoteStorage()->getSaveHostOptions(),
        ...
        case eSaveTransfer_GetSavesInfo: {
            // we can't cancel here, we need the saves info so we can delete the
            // file
            if (pClass->m_saveTransferDownloadCancelled) {
                WCHAR wcTemp[256];
                swprintf(
        ...
        case eSaveTransfer_CreatingNewSave: {
            ...
            StorageManager.ResetSaveData();
            {
                ...
                StorageManager.GetDefaultSaveImage(
                    &pbDataSaveImage,
                    &dwDataSizeSaveImage
                ); // Get the default save thumbnail (as set by
                   // SetDefaultImages) for use on saving games t
                StorageManager.GetDefaultSaveThumbnail(
                    &pbThumbnailData,
                    &dwThumbnailDataSize
                ); // Get the default save image (as set by SetDefaultImages)
        ...
        case eSaveTransfer_Converting: {
            ...
            app.getRemoteStorage()
                ->waitForStorageManagerIdle(); // we need to wait for the save
                                               // system to be idle here, as
                                               // Flush doesn't check for it.
            pSave->Flush(false, false);
        } break;
        case eSaveTransfer_Saving: {
            ...
            // ConsoleSaveSplit instance, which needs to be around until all the
            // sub files have completed saving.
            ...
            if (bSaveFileCreated) {
                if (pClass->m_saveTransferDownloadCancelled) {
                    ...
                    swprintf(
                        ...
                        256,
                        app.GetString(IDS_CANCEL)
                    ); // MGH - should change this string to "cancelling
                       // download"
                    m_wstrStageText = wcTemp;
                    pMinecraft->progressRenderer->progressStage(
                        m_wstrStageText
                ...
                // if the save file has already been created we have to delete
                // it again if there's been an error
                PSAVE_DETAILS pSaveDetails  = StorageManager.ReturnSavesInfo();
                int           saveInfoIndex = -1;
                for (int i = 0; i < pSaveDetails->iSaveC; i++) {
                    if (strcmp(
        ...
        case eSaveTransfer_ErrorMesssage: {
            ...
            if (bSaveFileCreated) // save file has been created, then deleted.
                pClass->m_iState = e_SavesRepopulateAfterDelete;
            else pClass->m_iState = e_SavesRepopulate;
            pClass->updateTooltips();
        } break;
...
void UIScene_LoadOrJoinMenu::SaveTransferReturned(
    LPVOID                    lpParam,
    SonyRemoteStorage::Status s,
    int                       error_code
) {
...
void UIScene_LoadOrJoinMenu::CancelSaveTransferCallback(LPVOID lpParam) {
    ...
    ui.SetTooltips(
        ...
        -1,
        -1
    ); // MGH -  added - remove the "cancel" tooltip, so the player knows it's
       // underway (really needs a "cancelling" message)
}
    ...
    while (pClass->m_eSaveUploadState != eSaveUpload_Idle) {
        ...
        case eSaveUpload_UploadingFileData: {
            ...
            if (pClass->m_saveTransferUploadCancelled
                && bAbortCalled == false) {
                // we only really want to be able to cancel during the download
                // of data, if it's taking a long time
                app.getRemoteStorage()->abort();
                bAbortCalled = true;
            }
        } break;
...
void UIScene_LoadOrJoinMenu::SaveUploadReturned(
    LPVOID                    lpParam,
    SonyRemoteStorage::Status s,
    int                       error_code
) {
    ...
    if (pClass->m_saveTransferUploadCancelled) {
        UINT uiIDA[1] = {IDS_CONFIRM_OK};
        ui.RequestMessageBox(
            IDS_CANCEL_UPLOAD_TITLE,
            IDS_CANCEL_UPLOAD_TEXT,
            uiIDA,
            1,
            ProfileManager.GetPrimaryPad(),
            CrossSaveUploadFinishedCallback,
    ...
    while ((pStateContainer->m_eSaveTransferState
        ...
        case C4JStorage::eSaveTransfer_FileDataRetrieved:
            ...
            case eSaveTransferFile_Metadata: {
                ...
                // 4J Stu - Don't set this any more. We added it so that we
                // could share the ban list data for this save However if the
                // player downloads the same save multiple times, it will
                // overwrite the previous version with that filname, and they
                // could have made changes to it.
                ...
                if (thumbnailSize > 0) {
                    ...
                    StorageManager
                        .SetSaveImages(ba.data, ba.length, NULL, 0, NULL, 0);
        ...
        case C4JStorage::eSaveTransfer_Saving:
            ...
            while (StorageManager.GetSaveState()
                ...
                // only place ticking the StorageManager. You WILL get crashes.
                // StorageManager.Tick();
            }
#endif
...
void UIScene_LoadOrJoinMenu::RequestFileSize(
...
#endif
    {
        ...
        } while (pClass->m_eSaveTransferState == C4JStorage::eSaveTransfer_Busy
