...
const int UIScene_LeaderboardsMenu::TitleIcons
    [UIScene_LeaderboardsMenu::NUM_LEADERBOARDS][7] = {
        {UIControl_LeaderboardList::e_ICON_TYPE_WALKED,
         UIControl_LeaderboardList::e_ICON_TYPE_FALLEN,
         Item::minecart_Id,
         Item::boat_Id,
         NULL},
        {Tile::dirt_Id,
    ...
    LEADERBOARD_DESCRIPTORS[UIScene_LeaderboardsMenu::NUM_LEADERBOARDS][4] = {
        {
         ...
         IDS_LEADERBOARD_TRAVELLING_PEACEFUL
            ...
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                4,                            true,
         IDS_LEADERBOARD_TRAVELLING_EASY
            ),                                                                       // Travelling Easy
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                4,                                                                     true,
         IDS_LEADERBOARD_TRAVELLING_NORMAL
            ...
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                4,                                   true,
         IDS_LEADERBOARD_TRAVELLING_HARD
            ),                                                                              // Travelling Hard
        },
        {
         UIScene_LeaderboardsMenu::LeaderboardDescriptor(
         ...
         IDS_LEADERBOARD_MINING_BLOCKS_NORMAL
            ...
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                7,                                                     false,
         IDS_LEADERBOARD_MINING_BLOCKS_HARD
            ),                                                            // Mining Hard
        },
        {
         UIScene_LeaderboardsMenu::LeaderboardDescriptor(
         ...
         IDS_LEADERBOARD_FARMING_PEACEFUL
            ...
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                6,                                   false,
         IDS_LEADERBOARD_FARMING_EASY
            ),                                                     // Farming Easy
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                6,                            false,
         IDS_LEADERBOARD_FARMING_NORMAL
            ),                                                                                              // Farming Normal
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                6, false,
         IDS_LEADERBOARD_FARMING_HARD
            ), // Farming Hard
        },
        {
         UIScene_LeaderboardsMenu::LeaderboardDescriptor(0,            false, -1),                               //
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                7, false,
         IDS_LEADERBOARD_KILLS_EASY
            ),   // Kills Easy
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                7, false,
         IDS_LEADERBOARD_KILLS_NORMAL
            ),                                                                                                                // Kills Normal
            UIScene_LeaderboardsMenu::LeaderboardDescriptor(
                7,                                   false,
         IDS_LEADERBOARD_KILLS_HARD
            ), // Kills Hard
        },
};
...
void UIScene_LeaderboardsMenu::updateTooltips() {
    ...
    ui.SetTooltips(
        ...
        iTooltipFriendRequest,
        IDS_TOOLTIPS_BACK,
        IDS_TOOLTIPS_CHANGE_FILTER,
        iTooltipGamerCardOrProfile
    );
}

void UIScene_LeaderboardsMenu::updateComponents() {
    m_parentLayer
        ->showComponent(m_iPad, eUIComponent_Panorama, !app.GetGameStarted());
...
void UIScene_LeaderboardsMenu::handleInput(
    int   iPad,
    int   key,
    bool  repeat,
    bool  pressed,
    ...
    /*app.DebugPrintf(
            "     m_newSel = %i       [bottomId] = %i       [topId] = %i, [size]
    ...
    case ACTION_MENU_RIGHT_SCROLL: {
        // Do nothing if a stats read is currently in progress, otherwise the
        // system complains about to many read requests
            ...
            if (selection >= GetEntryStartIndex()
                && selection
                       < (GetEntryStartIndex()
                          + m_leaderboard.m_entries.size())) {
                PlayerUID uid =
                    m_leaderboard.m_entries[selection - GetEntryStartIndex()]
                        .m_xuid;
                if (uid != INVALID_XUID) {
    ...
    case ACTION_MENU_A: {
#ifdef _DURANGO
        // Send friend request if the filter mode is not friend, and they're not
        // a friend or a pending friend
#ifdef _XBOX
        if (m_currentFilter != LeaderboardManager::eFM_Friends)
#endif
        {
...
bool UIScene_LeaderboardsMenu::OnStatsReadComplete(
    LeaderboardManager::eStatsReturn retIn,
    int                              numResults,
...
bool UIScene_LeaderboardsMenu::RetrieveStats() {
    if (app.DebugSettingsOn()
        ...
        for (unsigned int entryIndex = 0; entryIndex < NUM_ENTRIES;
            ...
            swprintf(
                m_leaderboard.m_entries[entryIndex].m_wcRank,
                12,
                L"12345678"
            ); //(int)m_leaderboard.m_entries[entryIndex].m_rank);

            swprintf(
                m_leaderboard.m_entries[entryIndex].m_gamerTag,
                17,
                L"WWWWWWWWWWWWWWWW"
            );
            ...
            for (unsigned int i = 0; i < m_leaderboard.m_numColumns; i++) {
                if (!isDistanceLeaderboard) {
                    m_leaderboard.m_entries[entryIndex].m_columns[i] =
                        USHRT_MAX;
                    swprintf(
                        m_leaderboard.m_entries[entryIndex].m_wcColumns[i],
                        12,
                        L"%u",
                        m_leaderboard.m_entries[entryIndex].m_columns[i]
                    );
...
void UIScene_LeaderboardsMenu::CopyLeaderboardEntry(
    LeaderboardManager::ReadScore* statsRow,
    ...
    for (unsigned int i = 0; i < statsRow->m_statsSize; i++) {
        ...
        if (!isDistanceLeaderboard) {
            ...
            if (iDigitC < 4) {
                // m
                swprintf(
                    leaderboardEntry->m_wcColumns[i],
                    12,
                    L"%um",
    ...
    case PLAYER_ONLINE_TIMER_ID:
        ...
        if (ProfileManager.IsSignedInLive(ProfileManager.GetPrimaryPad())
            ...
            C4JStorage::EMessageResult result = ui.RequestMessageBox(
                ...
                UIScene_LeaderboardsMenu::ExitLeaderboards,
                this,
                app.GetStringTable()
            );
        }
