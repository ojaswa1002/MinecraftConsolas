...
UIGroup::UIGroup(EUIGroup group, int iPad) {
    ...
    m_tooltips = (UIComponent_Tooltips*)m_layers[(int)eUILayer_Tooltips]
                     ->addComponent(0, eUIComponent_Tooltips);
...
void UIGroup::ReloadAll() {
    ...
    for (; highestRenderable >= 0; --highestRenderable) {
        if (highestRenderable < eUILayer_COUNT)
            m_layers[highestRenderable]->ReloadAll(
...
void UIGroup::HandleDLCMountingComplete() {
    ...
    for (unsigned int i = 0; i < eUILayer_COUNT; ++i) {
        app.DebugPrintf(
            "UIGroup::HandleDLCMountingComplete - m_layers[%d]\n",
            i
        );
...
void UIGroup::PrintTotalMemoryUsage(
    std::int64_t& totalStatic,
    std::int64_t& totalDynamic
) {
    std::int64_t groupStatic  = 0;
    std::int64_t groupDynamic = 0;
    ...
    app.DebugPrintf(
        app.USER_SR,
        "-- Group static: %d, Group dynamic: %d\n",
        groupStatic,
        groupDynamic
    );
