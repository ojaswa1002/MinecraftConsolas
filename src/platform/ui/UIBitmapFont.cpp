...
void UIAbstractBitmapFont::registerFont() {
    ...
    IggyFontInstallBitmapUTF8(
        m_bitmapFontProvider,
        m_fontname.c_str(),
        -1,
        IGGY_FONTFLAG_none
    );
...
S32 RADLINK UIAbstractBitmapFont::GetCodepointGlyph_Callback(
    void* user_context,
    U32   codepoint
) {
...
rrbool RADLINK
UIAbstractBitmapFont::IsGlyphEmpty_Callback(void* user_context, S32 glyph) {
...
IggyGlyphMetrics*
UIBitmapFont::GetGlyphMetrics(S32 glyph, IggyGlyphMetrics* metrics) {
    ...
    if (m_cFontData->glyphIsWhitespace(glyph)) metrics->x1 = 0.0f;
    else metrics->x1 = glyphAdvance;
...
// the bitmap into
rrbool UIBitmapFont::GetGlyphBitmap(
    S32                  glyph,
    F32                  pixel_scale,
    IggyBitmapCharacter* bitmap
) {
    ...
    {
        DebugData dData =
            {m_cFontData->getFontName(), (long)pixel_scale, (long)glyphScale};
    ...
    // (%f)\n", 				glyph, GetUnicode(glyph), pixel_scale,
    // targetPixelScale,
    // glyphScale);
    ...
    // bitmap->pixel_scale_correct = (float) m_glyphHeight;	// Scales the glyph
    // to desired size. bitmap->pixel_scale_correct = pixel_scale;			//
    // Always the same size (not desired size). bitmap->pixel_scale_correct =
    // pixel_scale * 0.5;		// Doubles original size.
    // bitmap->pixel_scale_correct = pixel_scale * 2;		// Halves original
    // size.
...
void RADLINK UIBitmapFont::FreeGlyphBitmap(
    S32                  glyph,
    F32                  pixel_scale,
    IggyBitmapCharacter* bitmap
) {
