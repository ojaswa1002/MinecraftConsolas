...
unsigned short SFontData::Codepoints[FONTSIZE] = {
    ...
    /*	Ω		α		β		γ		δ		ε		ζ		η		θ ι κ
       λ		μ		ν		ξ		ο		π		ρ		ς		σ τ υ
       φ	*/
    0x03A9,
    0x03B1,
    0x03B2,
...
: m_unicodeMap(sFontData.m_uiGlyphCount + 2) {
    ...
    for (unsigned short glyph = 0; glyph < sFontData.m_uiGlyphCount; glyph++) {
        ...
        moveCursor(
            topLeft,
            column * sFontData.m_uiGlyphWidth,
            row * sFontData.m_uiGlyphHeight
        );

        assert(
            ((column + 1) * sFontData.m_uiGlyphWidth) < sFontData.m_uiGlyphMapX
        );
        assert(
            ((row + 1) * sFontData.m_uiGlyphHeight) < sFontData.m_uiGlyphMapY
        );
