...
// #define POINTER_PANEL_OVER_REACH	(42.0f)		// Amount beyond edge of panel
// which pointer can go over to drop items. - comes from the pointer size in the
// scene
...
protected:
...
#ifdef __ORBIS__
    bool m_bFirstTouchStored[XUSER_MAX_COUNT]; // monitor the first position of
    ...
    virtual void
    GetPositionOfSection(ESceneSection eSection, UIVec2D* pPosition) = 0;
    ...
    virtual void InitDataAssociations(
        int                    iPad,
        AbstractContainerMenu* menu,
        int                    startIndex = 0
    ) = 0;
    ...
    virtual bool
    handleValidKeyPress(int iUserIndex, int buttonNum, BOOL quickKeyHeld);
    virtual void
    handleOutsideClicked(int iPad, int buttonNum, BOOL quickKeyHeld);
    ...
    virtual void
    setSectionSelectedSlot(ESceneSection eSection, int x, int y) = 0;
    virtual void setFocusToPointer(int iPad)                     = 0;
    virtual void SetPointerText(
        const std::wstring&        description,
        std::vector<std::wstring>& unformattedStrings,
        bool                       newSlot
    ) = 0;
    virtual std::shared_ptr<ItemInstance>
                 getSlotItem(ESceneSection eSection, int iSlot) = 0;
