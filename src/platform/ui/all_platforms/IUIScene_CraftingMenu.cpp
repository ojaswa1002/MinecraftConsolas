        ...
        if (m_pPlayer && m_pPlayer->inventory) {
            ...
            if (app.DebugSettingsOn()
                ...
                if (pMinecraft->localgameModes[iPad] != NULL) {
                    ...
                    if (tutorial != NULL) {
                        tutorial->createItemSelected(
                            pTempItemInst,
                            pRecipeIngredientsRequired[iRecipe].bCanMake[iPad]
                        );
                    }
                }
                ...
                if (pRecipeIngredientsRequired[iRecipe].bCanMake[iPad]) {
                    ...
                    // TODO 4J Stu - handleCraftItem should do a lot more than
                    // what it does, loads of the "can we craft" code should
                    // also probably be shifted to the GameMode
                    pMinecraft->localgameModes[iPad]->handleCraftItem(
                        iRecipe,
                        m_pPlayer
                    );
                    ...
                    for (int i = 0;
                        ...
                        for (int j = 0;
                             j
                             < pRecipeIngredientsRequired[iRecipe].iIngValA[i];
                             j++) {
                            ...
                            // 4J Stu - Fix for #13097 - Bug: Milk Buckets are
                            // removed when crafting Cake
                            if (ingItemInst != NULL) {
                    ...
                    // 4J Stu - Fix for #13119 - We should add the item after we
                    ...
                    // We've used some ingredients from our inventory, so update
                    // the recipes we can make
                    CheckRecipesAvailable();
                    // don't reset the vertical slots - we want to stay where we
                    // are
                    UpdateVerticalSlots();
                    UpdateHighlight();
                } else {
            ...
            for (int j = 0; j < pRecipeIngredientsRequired[i].iIngC; j++) {
                            ...
                            if (m_pPlayer->inventory->items[k]->GetCount()
                                ...
                                for (unsigned int l = 0;
                                    ...
                                    if (m_pPlayer->inventory->items[l]
                                        != NULL) {
                                        if ((m_pPlayer->inventory->items[l]->id
                                             == pRecipeIngredientsRequired[i]
                                                    .iIngIDA[j])
                                            && ((pRecipeIngredientsRequired[i]
                                                     .iIngAuxValA[j]
                                ...
                                if (iTotalCount >= pRecipeIngredientsRequired[i]
                                                       .iIngValA[j]) {
                                    bFoundA[j] = true;
                                }
                            }
                ...
                if (bFoundA[j] == false) {
                    ...
                    while (iMissing != 0) {
                        ...
                        if (((pRecipeIngredientsRequired[i].uiGridA[iGridIndex]
                              & 0x00FFFFFF)
                             == pRecipeIngredientsRequired[i].iIngIDA[j])
                            && ((pRecipeIngredientsRequired[i].iIngAuxValA[j]
                                 == Recipes::ANY_AUX_VALUE)
...
void IUIScene_CraftingMenu::UpdateTooltips() {
    ...
    case DISPLAY_INVENTORY:
        ui.SetTooltips(
            getPad(),
            bDisplayCreate ? IDS_TOOLTIPS_CREATE : -1,
            IDS_TOOLTIPS_EXIT,
