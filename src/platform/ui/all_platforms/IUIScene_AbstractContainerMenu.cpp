...
void IUIScene_AbstractContainerMenu::Initialize(
    ...
    bool                   autoDeleteMenu,
    int                    startIndex,
    ESceneSection          firstSection,
    ESceneSection          maxSection,
    bool                   bNavigateBack
) {
    assert(menu != NULL);
...
int IUIScene_AbstractContainerMenu::GetSectionDimensions(
...
void IUIScene_AbstractContainerMenu::SetToolTip(
...
void IUIScene_AbstractContainerMenu::onMouseTick() {
    ...
    if (eSectionUnderPointer == eSectionNone) {
        ...
        if (!bStickInput) {
            ...
            int iNumRows;
            int iNumColumns;
            int iNumItems = GetSectionDimensions(
                eSectionUnderPointer,
                &(iNumColumns),
                &(iNumRows)
            );
            ...
            if ((m_eCurrTapState != eTapNone
                ...
                iNumItems = GetSectionDimensions(
                    eSectionUnderPointer,
                    &(iNumColumns),
                    &(iNumRows)
                );
...
bool IUIScene_AbstractContainerMenu::handleKeyDown(
    int  iPad,
    int  iAction,
    bool bRepeat
) {
    ...
    case ACTION_MENU_Y:
        if (!bRepeat) {
            ...
            // Fix for #58583 - TU6: Content: UI: The Quick Move button prompt
            // disappears even though it still works No quick move tooltip is
            // shown if something is carried, so disable the action as well
            // if(!bIsItemCarried)
            {
                validKeyPress = TRUE;
        ...
        break;
        // 4J Stu - Also enable start to exit the scene. This key is also not
        // constrained by the tutorials.
    ...
    case ACTION_MENU_B: {
        ...
        // packet loss. We need to make sure that we call closeContainer()
        // anytime this menu is closed, even if it is forced to close by some
        // other reason (like the player dying) Therefore I have moved this call
        // to the OnDestroy() method to make sure that it always happens.
        ...
        // to let it know that we have closed the ui incase we need to do things
        // when that happens
...
bool IUIScene_AbstractContainerMenu::handleValidKeyPress(
    int  iUserIndex,
    int  buttonNum,
    BOOL quickKeyHeld
) {
...
void IUIScene_AbstractContainerMenu::handleOutsideClicked(
    int  iPad,
    int  buttonNum,
    BOOL quickKeyHeld
) {
    ...
    slotClicked(
        AbstractContainerMenu::CLICKED_OUTSIDE,
        buttonNum,
        quickKeyHeld ? true : false
    );
}

void IUIScene_AbstractContainerMenu::handleOtherClicked(
    int           iPad,
    ESceneSection eSection,
    int           buttonNum,
    bool          quickKey
...
void IUIScene_AbstractContainerMenu::slotClicked(
    int  slotId,
    int  buttonNum,
    bool quickKey
) {
...
bool IUIScene_AbstractContainerMenu::IsSameItemAs(
    std::shared_ptr<ItemInstance> itemA,
