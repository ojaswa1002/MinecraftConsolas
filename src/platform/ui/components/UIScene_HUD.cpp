    ...
    if (getMovie() && app.GetGameStarted()) {
        ...
        if (pMinecraft->localplayers[m_iPad]->dimension == 1) {
            if (EnderDragonRenderer::bossInstance == NULL) {
                ...
                SetDragonHealth(
                    (float)boss->getSynchedHealth() / boss->getMaxHealth()
                );
            }
        } else {
            ShowDragonHealth(false);
...
void UIScene_HUD::HideSelectedLabel() {
    ...
    IggyResult    out = IggyPlayerCallMethodRS(
        ...
        &result,
        IggyPlayerRootPath(getMovie()),
        m_funcHideSelectedLabel,
        0,
        NULL
    );
}

void UIScene_HUD::render(
    S32                      width,
    S32                      height,
        ...
        for (unsigned int i = 0; i < CHAT_LINES_COUNT; ++i) {
            ...
            if (opacity > 0) {
                ...
                m_labelChatText[i].setLabel(
                    pGui->getMessagesCount(m_iPad) ? pGui->getMessage(m_iPad, i)
                                                   : L""
                );
...
void UIScene_HUD::handleGameTick() {
    if (getMovie() && app.GetGameStarted()) {
        ...
        bool bDisplayGui = app.GetGameStarted() && !ui.GetMenuDisplayed(m_iPad)
