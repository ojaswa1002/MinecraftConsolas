...
UIComponent_Tooltips::UIComponent_Tooltips(
    int      iPad,
    void*    initData,
    UILayer* parentLayer
)
...
void UIComponent_Tooltips::render(
    S32                      width,
    S32                      height,
...
void UIComponent_Tooltips::SetTooltips(
    int  iA,
    int  iB,
    int  iX,
    int  iY,
...
void UIComponent_Tooltips::_SetTooltip(
    unsigned int        iToolTipId,
...
#ifdef __PSVITA__
void UIComponent_Tooltips::handleTouchInput(
    unsigned int iPad,
    S32          x,
    S32          y,
    int          iId,
    ...
    if (bReleased) {
        ...
        case ETouchInput_Touch_X:
            app.DebugPrintf(
                "ToolTip Map Touch to _PSV_JOY_BUTTON_SQUARE\n",
                iId
            );
        ...
        case ETouchInput_Touch_RightTrigger:
            app.DebugPrintf(
                "ToolTip Map Touch to _PSV_JOY_BUTTON_SELECT\n",
                iId
            );
...
void UIComponent_Tooltips::handleReload() {
    ...
    for (unsigned int i = 0; i < eToolTipNumButtons; ++i) {
        _SetTooltip(
            i,
            app.GetString(m_tooltipValues[i].iString),
            m_tooltipValues[i].show,
            true
        );
    }
    _Relayout();
...
void UIComponent_Tooltips::handleInput(
    ...
    if (m_overrideSFX[iPad][key]) {
        ...
        case ACTION_MENU_OK:
        case ACTION_MENU_CANCEL:
            // 4J-PB - We get both A and OK, and B and Cancel, so only play a
            // sound on one of them.
            sendInputToMovie(key, repeat, pressed, released);
            break;
        case ACTION_MENU_A:
        case ACTION_MENU_X:
