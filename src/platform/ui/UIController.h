...
#include "platform/ui/all_platforms/IUIController.h"
...
private:
    ...
    std::unordered_map<size_t, UIScene*>
        m_registeredCallbackScenes; // A collection of scenes and unique id's
                                    ...
                                    // can safely handle when they get destroyed
...
private:
    void loadSkins();
    IggyLibrary
    loadSkin(const std::wstring& skinPath, const std::wstring& skinName);
...
protected:
    static rrbool RADLINK ExternalFunctionCallback(
...
public:
    ...
    setupCustomDraw(UIScene* scene, IggyCustomDrawCallbackRegion* region) = 0;
    virtual CustomDrawData*
                 calculateCustomDraw(IggyCustomDrawCallbackRegion* region) = 0;
...
public:
    virtual void beginIggyCustomDraw4J(
    ...
    void
    setupCustomDrawMatrices(UIScene* scene, CustomDrawData* customDrawRegion);
    void setupCustomDrawGameStateAndMatrices(
        UIScene*        scene,
        CustomDrawData* customDrawRegion
    );
...
protected:
    static void RADLINK CustomDrawCallback(
        void*                         user_callback_data,
        Iggy*                         player,
        IggyCustomDrawCallbackRegion* Region
    );
    ...
    virtual void          destroySubstitutionTexture(
...
public:
    ...
    void unregisterSubstitutionTexture(
        const std::wstring& textureName,
        bool                deleteData
    );
...
public:
    ...
    // TOOLTIPS
    virtual void
    SetTooltipText(unsigned int iPad, unsigned int tooltip, int iTextID);
    virtual void SetEnableTooltips(unsigned int iPad, BOOL bVal);
    virtual void
    ShowTooltip(unsigned int iPad, unsigned int tooltip, bool show);
    ...
    virtual void
    EnableTooltip(unsigned int iPad, unsigned int tooltip, bool enable);
    ...
    virtual void
    ShowSavingMessage(unsigned int iPad, C4JStorage::ESavingMessage eVal);
    ...
    virtual C4JStorage::EMessageResult RequestMessageBox(
        ...
        LPVOID          lpParam                                    = NULL,
        C4JStringTable* pStringTable                               = NULL,
        WCHAR*          pwchFormatString                           = NULL,
