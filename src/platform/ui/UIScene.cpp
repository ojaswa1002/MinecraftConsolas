...
#include "platform/ui/controls/UIControl_Base.h"
...
void                    UIScene::loadMovie() {
    ...
    std::int64_t beforeLoad = ui.iggyAllocCount;
    swf = IggyPlayerCreateFromMemory(baFile.data, baFile.length, NULL);
    std::int64_t afterLoad = ui.iggyAllocCount;
    IggyPlayerInitializeAndTickRS(swf);
    std::int64_t afterTick = ui.iggyAllocCount;
...
void UIScene::PrintTotalMemoryUsage(
    ...
    rrbool            res;
    int               iteration    = 0;
    std::int64_t      sceneStatic  = 0;
    std::int64_t      sceneDynamic = 0;
    while (
        res =
            IggyDebugGetMemoryUseInfo(swf, NULL, "", 0, iteration, &memoryInfo)
    ) {
            ...
            if ((m_cachedSlotDraw.size() + 1) == m_expectedCachedSlotCount) {
                ...
                if (!useCommandBuffers || m_needsCacheRendered) {
                    ...
                    for (auto it = m_cachedSlotDraw.begin();
                         it != m_cachedSlotDraw.end();
                         ++it) {
                        CachedSlotDrawData* drawData = *it;
                        ui.setupCustomDrawMatrices(
                            this,
...
void UIScene::_customDrawSlotControl(
    CustomDrawData*               region,
    int                           iPad,
    std::shared_ptr<ItemInstance> item,
    float                         fAlpha,
    bool                          isFoil,
...
//	{
//		app.DebugPrintf("A scene is trying to navigate forwards, but it's parent
// layer is NULL!\n"); #ifndef _CONTENT_PACKAGE
//		__debugbreak();
// #endif
//	}
...
void UIScene::sendInputToMovie(
    ...
    IggyMakeEventKey(
        ...
        (IggyKeycode)iggyKeyCode,
        IGGY_KEYLOC_Standard
    );
...
void UIScene::externalCallback(IggyExternalFunctionCallUTF16* call) {
    if (wcscmp((wchar_t*)call->function_name.string, L"handlePress") == 0) {
        ...
        handlePress(call->arguments[0].number, call->arguments[1].number);
                   ...
                   (wchar_t*)call->function_name.string,
                   L"handleFocusChange"
               )
               == 0) {
        if (call->num_arguments != 2) {
        ...
        if (call->arguments[0].type != IGGY_DATATYPE_number
            || call->arguments[1].type != IGGY_DATATYPE_number) {
            app.DebugPrintf(
                "Arguments for handleFocusChange were not of the correct type\n"
            );
#ifndef _CONTENT_PACKAGE
            __debugbreak();
#endif
    ...
    } else if (wcscmp((wchar_t*)call->function_name.string, L"handleInitFocus")
               == 0) {
        if (call->num_arguments != 2) {
            app.DebugPrintf(
    ...
    } else if (wcscmp((wchar_t*)call->function_name.string, L"handleSliderMove")
               == 0) {
        if (call->num_arguments != 2) {
            app.DebugPrintf(
        ...
        if (call->arguments[0].type != IGGY_DATATYPE_number
            || call->arguments[1].type != IGGY_DATATYPE_number) {
            app.DebugPrintf(
                "Arguments for handleSliderMove were not of the correct type\n"
            );
#ifndef _CONTENT_PACKAGE
            __debugbreak();
#endif
...
void UIScene::UpdateSceneControls() {
    auto itEnd = GetControls()->end();
    for (auto it = GetControls()->begin(); it != itEnd; it++) {
