...
bool UIControl_Base::setupControl(
    UIScene*           scene,
...
void UIControl_Base::setLabel(
    const std::wstring& label,
    bool                instant,
    bool                force
) {
    if (force
        || ((!m_label.empty() || !label.empty())
            && m_label.compare(label) != 0))
...
const wchar_t* UIControl_Base::getLabel() {
    ...
    if (result.type == IGGY_DATATYPE_string_UTF16) {
        m_label = std::wstring(
            (wchar_t*)result.string16.string,
...
void UIControl_Base::setAllPossibleLabels(
    int     labelCount,
    wchar_t labels[][256]
) {
    IggyDataValue    result;
