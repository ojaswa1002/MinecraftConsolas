...
void UIControl_PlayerSkinPreview::SetTexture(
...
void UIControl_PlayerSkinPreview::
    SetFacing(ESkinPreviewFacing facing, bool bAnimate /*= false*/) {
    switch (facing) {
    ...
    if (renderer != NULL) {
        ...
        if (m_pvAdditionalModelParts && m_pvAdditionalModelParts->size() != 0) {
            for (auto it = m_pvAdditionalModelParts->begin();
                 it != m_pvAdditionalModelParts->end();
                 ++it) {
...
// actual entity
void UIControl_PlayerSkinPreview::render(
    EntityRenderer* renderer,
    double          x,
    double          y,
    double          z,
