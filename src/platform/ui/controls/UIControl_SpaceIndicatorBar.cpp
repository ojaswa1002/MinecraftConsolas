...
bool UIControl_SpaceIndicatorBar::setupControl(
    UIScene*           scene,
...
void UIControl_SpaceIndicatorBar::init(
...
void UIControl_SpaceIndicatorBar::addSave(std::int64_t size) {
    float startPercent = (float)((m_currentTotal - m_min)) / (m_max - m_min);

    m_sizeAndOffsets.push_back(
        std::pair<std::int64_t, float>(size, startPercent)
    );
...
void UIControl_SpaceIndicatorBar::selectSave(int index) {
    if (index >= 0 && index < m_sizeAndOffsets.size()) {
        std::pair<std::int64_t, float> values = m_sizeAndOffsets[index];
...
void UIControl_SpaceIndicatorBar::setSaveSize(std::int64_t size) {
    m_currentSave = size;
...
void UIControl_SpaceIndicatorBar::setTotalSize(std::int64_t size) {
