...
#include "client/renderer/entity/EnderDragonRenderer.h"
...
static void RADLINK TraceCallback(
    void*       user_callback_data,
    Iggy*       player,
...
static void* RADLINK                     AllocateFunction(
...
static void RADLINK
DeallocateFunction(void* alloc_callback_user_data, void* ptr) {
    UIController* controller = (UIController*)alloc_callback_user_data;
    EnterCriticalSection(&controller->m_Allocatorlock);
...
void UIController::postInit() {
    ...
    IggySetAS3ExternalFunctionCallbackUTF16(
        &UIController::ExternalFunctionCallback,
        this
    );
    IggySetTextureSubstitutionCallbacks(
        &UIController::TextureSubstitutionCreateCallback,
        &UIController::TextureSubstitutionDestroyCallback,
        this
    );
...
void UIController::SetupFont() {
    ...
    case XC_LANGUAGE_TCHINESE:
        m_mcTTFFont =
            new UITTFFont("Common/Media/font/CHT/DFTT_R5.TTC", 0x203B); // CHT
        break;
    ...
    case XC_LANGUAGE_KOREAN:
        m_mcTTFFont =
            new UITTFFont("Common/Media/font/KOR/BOKMSD.ttf", 0x2022); // KOR
        break;
#endif
    default:
        bBitmapFont = true;
            ...
            for (unsigned int key = 0; key <= ACTION_MAX_MENU; ++key) {
                ...
                if (pressed || released) {
                    ...
                    if (firstUnfocussedUnhandledPad < 0
                        && !m_groups[(int)eUIGroup_Fullscreen]->HasFocus(
                            iPad
                        )) {
...
void UIController::handleKeyPress(unsigned int iPad, unsigned int key) {
    ...
    if (repeat || pressed || released) {
        ...
        m_groups[(int)eUIGroup_Fullscreen]
            ->handleInput(iPad, key, repeat, pressed, released, handled);
        if (!handled) {
...
void UIController::renderScenes() {
    ...
    if (m_iggyPerfmonEnabled) {
        ...
        IggyPerfmonTickAndDraw(
            ...
            getScreenWidth(),
            getScreenHeight()
        ); // perfmon draw area in window coords
    }
#endif
}
...
void UIController::setupCustomDrawMatrices(
    ...
    if (!RenderManager.IsHiDef() && !RenderManager.IsWidescreen()) {
        ...
        right =
            left
            + ((sceneWidth * customDrawRegion->mat[0])) * customDrawRegion->x1;
    } else
#endif
    {
        left = m_tileOriginX
             + (sceneWidth + customDrawRegion->mat[(0 * 4) + 3] * sceneWidth)
                   / 2;
        right = left
              + ((sceneWidth * customDrawRegion->mat[0]) / 2)
                    * customDrawRegion->x1;
    }
    ...
    bottom = top
           + (sceneHeight * -customDrawRegion->mat[(1 * 4) + 1]) / 2
                 * customDrawRegion->y1;
    ...
    glTranslatef(
        (m_fScreenWidth + customDrawRegion->mat[(0 * 4) + 3] * m_fScreenWidth)
            / 2,
        (m_fScreenHeight - customDrawRegion->mat[(1 * 4) + 3] * m_fScreenHeight)
            / 2,
        0
    );
    // Iggy scales are based on a double-size target
    glScalef(
        (m_fScreenWidth * customDrawRegion->mat[0]) / 2,
        (m_fScreenHeight * -customDrawRegion->mat[(1 * 4) + 1]) / 2,
        1.0f
    );
}
...
GDrawTexture* RADLINK UIController::TextureSubstitutionCreateCallback(
    ...
    if (it != uiController->m_substitutionTextures.end()) {
        app.DebugPrintf(
            "Found substitution texture %ls, with %d bytes\n",
        ...
        if (image.getData() != NULL) {
            ...
            *destroy_callback_data = (void*)id;
            ...
            app.DebugPrintf(
                "Found substitution texture %ls (%d) - %dx%d\n",
                (wchar_t*)texture_name,
                id,
                image.getWidth(),
                image.getHeight()
        ...
        app.DebugPrintf(
            "Could not find substitution texture %ls\n",
            (wchar_t*)texture_name
        );
        return NULL;
    }
...
void UIController::registerSubstitutionTexture(
...
// NAVIGATION
bool UIController::NavigateToScene(
    int      iPad,
    EUIScene scene,
    void*    initData,
    EUILayer layer,
    ...
    case eUIScene_FullscreenProgress: {
        // 4J Stu - The fullscreen progress scene should not interfere with any
        // other scene stack, so should be placed in it's own group/layer
    ...
    case eUIScene_EndPoem: {
        // The end poem scene shouldn't interfere with other scenes, but will be
        // underneath the autosave progress
        group = eUIGroup_Fullscreen;
...
bool UIController::NavigateBack(
    int      iPad,
    bool     forceUsePad,
    EUIScene eScene,
    EUILayer eLayer
    ...
    if (app.GetGameStarted()) {
        bool navComplete = m_groups[(int)eUIGroup_Fullscreen]
                               ->NavigateBack(iPad, eScene, eLayer);
        ...
        navComplete = m_groups[(int)eUIGroup_Fullscreen]
                          ->NavigateBack(iPad, eScene, eLayer);
        if (!m_groups[(int)eUIGroup_Fullscreen]->GetMenuDisplayed())
...
size_t UIController::RegisterForCallbackId(UIScene* scene) {
    ...
    newId &= 0xFFFFFF; // Chop off the top byte, we don't need any more accuracy
                       // than that
    newId |=
        (scene->getSceneType()
...
void UIController::CloseUIScenes(int iPad, bool forceIPad) {
    ...
    SetMenuDisplayed(
        (group == eUIGroup_Fullscreen ? XUSER_INDEX_ANY : iPad),
...
bool UIController::IsIgnoreAutosaveMenuDisplayed(int iPad) {
    ...
    return m_groups[(int)eUIGroup_Fullscreen]->IsIgnoreAutosaveMenuDisplayed()
        || (group != eUIGroup_Fullscreen
...
void UIController::SetTooltipText(
    unsigned int iPad,
...
void UIController::SetTooltips(
    ...
    if (m_groups[(int)group]->getTooltips())
        m_groups[(int)group]
            ->getTooltips()
            ->SetTooltips(iA, iB, iX, iY, iLT, iRT, iLB, iRB, iLS, forceUpdate);
}

void UIController::EnableTooltip(
    unsigned int iPad,
    unsigned int tooltip,
    bool         enable
) {
...
void UIController::AnimateKeyPress(
    int  iPad,
    int  iAction,
    bool bRepeat,
    bool bPressed,
    ...
    if (m_groups[(int)group]->getTooltips())
        m_groups[(int)group]
            ->getTooltips()
            ->handleInput(iPad, iAction, bRepeat, bPressed, bReleased, handled);
}
...
void UIController::SetSelectedItem(
    unsigned int        iPad,
...
void UIController::HandleDLCLicenseChange() {
    for (unsigned int i = 0; i < eUIGroup_COUNT; ++i) {
        app.DebugPrintf(
            "UIController::HandleDLCLicenseChange - m_groups[%d]\n",
            i
        );
...
void UIController::UpdateAutosaveCountdownTimer(unsigned int uiSeconds) {
    ...
    swprintf(
        wcAutosaveCountdown,
        100,
        app.GetString(IDS_AUTOSAVE_COUNTDOWN),
        uiSeconds
    );
    if (m_groups[(int)eUIGroup_Fullscreen]->getPressStartToPlay())
        m_groups[(int)eUIGroup_Fullscreen]
...
void UIController::ShowSavingMessage(
    unsigned int               iPad,
...
C4JStorage::EMessageResult UIController::RequestUGCMessageBox(
    ...
    return ui.RequestMessageBox(
        title,
        IDS_CHAT_RESTRICTION_UGC,
        uiIDA,
        1,
        iPad,
        Func,
...
#elif defined(__PSVITA__)
    ProfileManager.ShowSystemMessage(
        SCE_MSG_DIALOG_SYSMSG_TYPE_TRC_PSN_AGE_RESTRICTION,
        iPad
    );
    return C4JStorage::EMessage_ResultAccept;
#else
...
void UIController::TouchBoxAdd(
    ...
    puiElement->x2 = (S32)((
        (float)pControl->getXPos() + (float)iControlWidth
        + (float)iMainPanelOffsetX
    ));
    puiElement->y2 = (S32)((
        (float)pControl->getYPos() + (float)iControlHeight
...
void UIController::TouchBoxRebuild(UIScene* pUIScene) {
    ...
    // rebuild boxes
    auto itEnd = pUIScene->GetControls()->end();
    for (auto it = pUIScene->GetControls()->begin(); it != itEnd; it++) {
...
void UIController::TouchBoxesClear(UIScene* pUIScene) {
    ...
    auto itEnd = m_TouchBoxes[eUIGroup][eUILayer][eUIscene].end();
    for (auto it = m_TouchBoxes[eUIGroup][eUILayer][eUIscene].begin();
         it != itEnd;
         it++) {
...
void UIController::HandleTouchInput(
    ...
    if (bPressed && !bRepeat && !bReleased) // PRESSED HANDLING
    {
        ...
        case UIControl::eButton:
            ...
            // override bPressed to false. we only want the button to trigger on
            // touch release!
            bPressed = false;
            break;
        ...
        case UIControl::eCheckBox:
            ...
            // override bPressed. we only want the checkbox to trigger on touch
            // release!
            bPressed = false;
            break;
        case UIControl::eButtonList:
            ...
            //  tell list where we tapped it so it can set focus to the correct
            //  button
        ...
        case UIControl::eTexturePackList:
            ...
            // override bPressed. we only want the TexturePack List to trigger
            // on touch release!
            bPressed = false;
            break;
        case UIControl::eTextInput:
            ...
            // override bPressed to false. we only want the textinput to trigger
            // on touch release!
            bPressed = false;
            break;
        ...
        case UIControl::eTouchControl:
            // pass on touch input to relevant parent scene so we can handle it
            // there!
    ...
    } else if (bRepeat) // REPEAT HANDLING
    {
        ...
        case UIControl::eSlider:
            ...
            UIControl_Slider* pSlider =
                (UIControl_Slider*)m_ActiveUIElement->pControl;
            float fNewSliderPos = ((float)x - (float)m_ActiveUIElement->x1)
    ...
    if (bReleased) // RELEASED HANDLING
    {
        ...
        case UIControl::eButtonList:
            ...
            break;
        case UIControl::eTexturePackList:
            // trigger texturepack list on release (ONLY if the finger is still
            // on it!)
            if (m_HighlightedUIElement
                && m_ActiveUIElement->pControl
                       == m_HighlightedUIElement->pControl) {
...
void UIController::SendTouchInput(
    ...
    // Send the key to the fullscreen group first
    m_groups[(int)eUIGroup_Fullscreen]
        ->handleInput(iPad, key, bRepeat, bPressed, bReleased, handled);
    if (!handled) {
