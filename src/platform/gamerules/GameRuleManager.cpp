...
void GameRuleManager::loadGameRules(DLCPack* pack) {
    ...
    for (int i = 0; i < gameRulesCount; ++i) {
        DLCGameRulesHeader* dlcHeader =
            (DLCGameRulesHeader*)
                pack->getFile(DLCManager::e_DLCType_GameRulesHeader, i);
        DWORD dSize;
    ...
    for (int i = 0; i < gameRulesCount; ++i) {
        DLCGameRulesFile* dlcFile =
            (DLCGameRulesFile*)
                pack->getFile(DLCManager::e_DLCType_GameRules, i);
...
void GameRuleManager::loadGameRules(
...
) //(DLCGameRulesFile *dlcFile, StringTable *strings)
{
    ...
    if (compressionType == Compression::eCompressionType_None) {
        ...
        default:
            ...
            break;
            /* 4J-JEV:
                Each platform has only 1 method of compression, 'compression.h'
               file deals with it.

                    case Compression::eCompressionType_LZXRLE:
                        app.DebugPrintf("De-compressing game rules with:
               LZX+RLE\n"); Compression::getCompression()->DecompressLZXRLE(
    ...
    for (int type = (int)ConsoleGameRules::eGameRuleType_Root;
         type < (int)ConsoleGameRules::eGameRuleType_Count;
         ++type) {
        for (UINT i = 0; i < numStrings; ++i) {
...
void GameRuleManager::readAttributes(
    DataInputStream*           dis,
    std::vector<std::wstring>* tagsAndAtts,
...
void GameRuleManager::loadDefaultGameRules() {
    ...
    if (app.getArchiveFileSize(fpTutorial) >= 0) {
        ...
        if (app.m_dlcManager
                .readDLCDataFile(dwFilesProcessed, fpTutorial, pack, true)) {
...
bool GameRuleManager::loadGameRulesPack(File* path) {
    ...
    if (path->exists()) {
        ...
        DWORD    dwFilesProcessed = 0;
        if (app.m_dlcManager
                .readDLCDataFile(dwFilesProcessed, path->getPath(), pack)) {
            app.m_dlcManager.addPack(pack);
            success = true;
        } else {
            delete pack;
...
LEVEL_GEN_ID
GameRuleManager::addLevelGenerationOptions(LevelGenerationOptions* lgo) {
