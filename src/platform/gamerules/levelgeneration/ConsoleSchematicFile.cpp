...
void ConsoleSchematicFile::save(DataOutputStream* dos) {
    if (dos != NULL) {
        ...
        Compression::getCompression()
            ->CompressLZXRLE(ba.data, &ba.length, m_data.data, m_data.length);
...
void ConsoleSchematicFile::load(DataInputStream* dis) {
    if (dis != NULL) {
        ...
        if (compressionType == Compression::eCompressionType_None) {
            ...
            default:
                app.DebugPrintf(
                    "Unrecognized compression type for Schematic file (%d)\n",
                    (int)compressionType
                );
        ...
        if (entityTags != NULL) {
            for (int i = 0; i < entityTags->size(); i++) {
                ...
                m_entities.push_back(
                    std::pair<Vec3*, CompoundTag*>(
                        Vec3::newPermanent(x, y, z),
                        (CompoundTag*)eTag->copy()
                    )
                );
            }
        }
...
std::int64_t ConsoleSchematicFile::applyBlocksAndData(
    LevelChunk*        chunk,
    AABB*              chunkBox,
    AABB*              destinationBox,
...
std::int64_t ConsoleSchematicFile::applyLighting(
    LevelChunk*        chunk,
    AABB*              chunkBox,
    AABB*              destinationBox,
    ESchematicRotation rot
        ...
        for (int y = yStart; y < yEnd; y++)
            for (int zz = zStart; zz < zEnd; zz++) {
                ...
                if (Tile::lightEmission[chunk->getTile(x, y, z)]) {
                    ...
                    chunk->level
                        ->checkLight(LightLayer::Block, xx, y, zz, true, true);
                }
            }
...
void ConsoleSchematicFile::generateSchematicFile(
    ...
    else if (yStart > 0 && yStart % 2 != 0) yStart -= 1;
    if (zStart > 0 && zStart % 2 != 0) zStart -= 1;
    else if (zStart < 0 && zStart % 2 != 0) zStart -= 1;

    // We want the end to be odd to have a total size that is even
    if (xEnd > 0 && xEnd % 2 == 0) xEnd += 1;
    else if (xEnd < 0 && xEnd % 2 == 0) xEnd += 1;
    if (yEnd > Level::maxBuildHeight) yEnd = Level::maxBuildHeight;
    else if (yEnd > 0 && yEnd % 2 == 0) yEnd += 1;
    else if (yEnd < 0 && yEnd % 2 == 0) yEnd += 1;
    if (zEnd > 0 && zEnd % 2 == 0) zEnd += 1;
    else if (zEnd < 0 && zEnd % 2 == 0) zEnd += 1;
...
void ConsoleSchematicFile::getBlocksAndData(
    LevelChunk* chunk,
    byteArray*  data,
    int         x0,
    int         y0,
...
ConsoleSchematicFile::getTileEntitiesInRegion(
    LevelChunk* chunk,
    int         x0,
    int         y0,
    int         z0,
