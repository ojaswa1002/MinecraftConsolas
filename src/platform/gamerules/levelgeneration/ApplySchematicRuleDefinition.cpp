...
void ApplySchematicRuleDefinition::writeAttributes(
...
void ApplySchematicRuleDefinition::processSchematic(
    AABB*       chunkBox,
    LevelChunk* chunk
) {
    ...
    if (chunkBox->intersects(m_locationBox)) {
        ...
        m_schematic
            ->applyTileEntities(chunk, chunkBox, m_locationBox, m_rotation);
        PIXEndNamedEvent();
...
void ApplySchematicRuleDefinition::processSchematicLighting(
    AABB*       chunkBox,
    LevelChunk* chunk
) {
    if (m_completed) return;
    ...
    if (chunkBox->intersects(m_locationBox)) {
        ...
        m_totalBlocksChangedLighting +=
            m_schematic
                ->applyLighting(chunk, chunkBox, m_locationBox, m_rotation);
        PIXEndNamedEvent();
...
bool ApplySchematicRuleDefinition::checkIntersects(
    int x0,
    int y0,
    int z0,
    int x1,
