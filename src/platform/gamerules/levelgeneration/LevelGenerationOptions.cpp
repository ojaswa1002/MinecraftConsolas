...
void LevelGenerationOptions::writeAttributes(
...
GameRuleDefinition*
LevelGenerationOptions::addChild(ConsoleGameRules::EGameRuleType ruleType) {
...
void LevelGenerationOptions::addAttribute(
    ...
    if (attributeName.compare(L"seed") == 0) {
        ...
        m_spawnPos->x = value;
        app.DebugPrintf(
            "LevelGenerationOptions: Adding parameter spawnX=%d\n",
            value
        );
        ...
        m_spawnPos->y = value;
        app.DebugPrintf(
            "LevelGenerationOptions: Adding parameter spawnY=%d\n",
            value
        );
        ...
        m_spawnPos->z = value;
        app.DebugPrintf(
            "LevelGenerationOptions: Adding parameter spawnZ=%d\n",
            value
        );
...
void LevelGenerationOptions::processSchematics(LevelChunk* chunk) {
    PIXBeginNamedEvent(
        0,
        "Processing schematics for chunk (%d,%d)",
        chunk->x,
        chunk->z
    );
    AABB* chunkBox = AABB::newTemp(
        chunk->x * 16,
        0,
        chunk->z * 16,
        chunk->x * 16 + 16,
    ...
    for (auto it = m_structureRules.begin(); it != m_structureRules.end();
        ...
        if (structureStart->getBoundingBox()
                ->intersects(cx, cz, cx + 15, cz + 15)) {
...
void LevelGenerationOptions::processSchematicsLighting(LevelChunk* chunk) {
    PIXBeginNamedEvent(
        0,
        "Processing schematics (lighting) for chunk (%d,%d)",
        chunk->x,
        chunk->z
    );
    AABB* chunkBox = AABB::newTemp(
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"We have already loaded schematic file %ls\n",
            filename.c_str()
        );
#endif
...
LevelGenerationOptions::getSchematicFile(const std::wstring& filename) {
    ConsoleSchematicFile* schematic = NULL;
    // If we have already loaded this, just return
    auto it = m_schematics.find(filename);
    if (it != m_schematics.end()) {
...
void LevelGenerationOptions::getBiomeOverride(
    int   biomeId,
    BYTE& tile,
    BYTE& topTile
) {
...
LevelGenerationOptions::getUnfinishedSchematicFiles() {
    ...
    for (auto it = usedFiles.begin(); it != usedFiles.end(); it++)
        out->insert(
            std::pair<std::wstring, ConsoleSchematicFile*>(
                *it,
                getSchematicFile(*it)
            )
        );
...
void LevelGenerationOptions::reset_finish() {
    ...
    // if (m_stringTable)			{ delete m_stringTable; m_stringTable =
    // NULL; }
...
std::int64_t LevelGenerationOptions::getLevelSeed() { return m_seed; }
