...
#include "platform/gamerules/levelgeneration/structureactions/XboxStructureActionPlaceSpawner.h"
...
GameRuleDefinition*
ConsoleGenerateStructure::addChild(ConsoleGameRules::EGameRuleType ruleType) {
...
void ConsoleGenerateStructure::writeAttributes(
...
void ConsoleGenerateStructure::addAttribute(
    ...
    if (attributeName.compare(L"x") == 0) {
        ...
        m_x       = value;
        app.DebugPrintf(
            "ConsoleGenerateStructure: Adding parameter x=%d\n",
            m_x
        );
        ...
        m_y       = value;
        app.DebugPrintf(
            "ConsoleGenerateStructure: Adding parameter y=%d\n",
            m_y
        );
        ...
        m_z       = value;
        app.DebugPrintf(
            "ConsoleGenerateStructure: Adding parameter z=%d\n",
            m_z
        );
...
bool ConsoleGenerateStructure::postProcess(
    Level*       level,
    Random*      random,
    BoundingBox* chunkBB
) {
...
bool ConsoleGenerateStructure::checkIntersects(
    int x0,
    int y0,
    int z0,
    int x1,
