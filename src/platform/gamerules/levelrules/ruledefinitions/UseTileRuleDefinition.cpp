...
void UseTileRuleDefinition::writeAttributes(
...
void UseTileRuleDefinition::addAttribute(
...
bool UseTileRuleDefinition::onUseTile(
    GameRule* rule,
    int       tileId,
    int       x,
    int       y,
    ...
    if (m_tileId == tileId) {
        if (!m_useCoords
            || (m_coordinates.x == x && m_coordinates.y == y
                && m_coordinates.z == z)) {
            if (!getComplete(rule)) {
                statusChanged = true;
