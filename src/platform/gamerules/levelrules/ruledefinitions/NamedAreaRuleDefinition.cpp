...
void NamedAreaRuleDefinition::writeAttributes(
...
void NamedAreaRuleDefinition::addAttribute(
    ...
    if (attributeName.compare(L"name") == 0) {
        ...
        wprintf(
            L"NamedAreaRuleDefinition: Adding parameter name=%ls\n",
            m_name.c_str()
        );
#endif
        ...
        app.DebugPrintf(
            "NamedAreaRuleDefinition: Adding parameter x0=%f\n",
            m_area->x0
        );
        ...
        app.DebugPrintf(
            "NamedAreaRuleDefinition: Adding parameter y1=%f\n",
            m_area->y1
        );
    } else if (attributeName.compare(L"z1") == 0) {
        m_area->z1 = _fromString<int>(attributeValue);
        app.DebugPrintf(
            "NamedAreaRuleDefinition: Adding parameter z1=%f\n",
            m_area->z1
        );
    } else {
