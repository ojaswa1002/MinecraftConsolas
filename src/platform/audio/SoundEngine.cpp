...
#include "platform/dlc/DLCAudioFile.h"
...
#ifdef __ORBIS__
#include <audioout.h>
// #define __DISABLE_MILES__			// MGH disabled for now as it crashes if
// we call sceNpMatching2Initialize
#endif
...
void SoundEngine::playStreaming(
...
char* SoundEngine::ConvertSoundPathToName(
    const std::wstring& name,
    bool                bConvertSpaces
) {
    return NULL;
...
void SoundEngine::init(Options* pOptions) {
    ...
    m_hDriver = AIL_open_digital_driver(
        48000,
        16,
        iNumberOfChannels,
    ...
    if (m_hDriver == 0) {
        app.DebugPrintf(
            "Couldn't open digital sound driver. (%s)\n",
            AIL_last_error()
        );
        AIL_shutdown();
#ifdef __ORBIS__
    ...
    if (app.GetBootedFromDiscPatch()) {
        ...
        sprintf(
            szBankName,
            "%s/%s",
            app.GetBDUsrDirPath(szTempSoundFilename),
            m_szSoundPath
        );
        app.DebugPrintf(
...
void SoundEngine::SetStreamingSounds(
    int iOverworldMin,
    int iOverWorldMax,
    int iNetherMin,
    int iNetherMax,
...
void SoundEngine::updateMiles() {
    ...
    updateSystemMusicPlaying(
        outPortState.output == SCE_AUDIO_OUT_STATE_OUTPUT_UNKNOWN
    );
#endif
    ...
    if (m_validListenerCount == 1) {
        for (int i = 0; i < MAX_LOCAL_PLAYERS; i++) {
            ...
            if (m_ListenerA[i].bValid) {
                ...
                ); // Flipped sign of z as Miles is expecting left handed coord
                   // system
                    ...
                    if (m_validListenerCount > 1) {
                        ...
                        for (int i = 0; i < MAX_LOCAL_PLAYERS; i++) {
                            if (m_ListenerA[i].bValid) {
                                ...
                                x = fabs(
                                    m_ListenerA[i].vPosition.x - game_data->x
                                );
                                y = fabs(
                                    m_ListenerA[i].vPosition.y - game_data->y
                                );
                                z = fabs(
                                    m_ListenerA[i].vPosition.z - game_data->z
                                );
                                fDist = x + y + z;
                        ...
                        fDist = sqrtf(
                            (fClosestX * fClosestX) + (fClosestY * fClosestY)
                            + (fClosestZ * fClosestZ)
                        );
                        ...
                        AIL_set_sample_3D_position(
                            ...
                            game_data->y,
                            -game_data->z
                        ); // Flipped sign of z as Miles is expecting left
                           // handed coord system
                    }
                }
                break;
...
int SoundEngine::GetRandomishTrack(int iStart, int iEnd) {
    ...
    for (int i = 0; i <= ((iEnd - iStart) / 2); i++) {
        ...
        if (m_bHeardTrackA[iVal] == false) {
            // not heard this
            app.DebugPrintf(
                "(%d) Not heard track %d yet, so playing it now\n",
                i,
                iVal
            );
...
int SoundEngine::getMusicID(int iDomain) {
    ...
    if (pMinecraft == NULL) {
        ...
        return GetRandomishTrack(
            m_iStream_Overworld_Min,
    ...
    if (pMinecraft->skins->isUsingDefaultSkin()) {
        ...
        case LevelData::DIMENSION_END:
            ...
            // whether the dragon is alive or not, but we've not added the dead
            // dragon music yet
            return m_iStream_End_Min;
        case LevelData::DIMENSION_NETHER:
            return GetRandomishTrack(
                m_iStream_Nether_Min,
...
void SoundEngine::playMusicUpdate() {
        ...
        if (m_musicID != -1) {
            ...
            if (app.GetBootedFromDiscPatch()) {
                sprintf(
                    m_szStreamName,
                    "%s/%s",
                    app.GetBDUsrDirPath(m_szMusicPath),
                    m_szMusicPath
                );
                app.DebugPrintf(
                    "SoundEngine::playMusicUpdate - (booted from disc patch) "
                    "music path - %s",
                    m_szStreamName
                );
            } else {
            ...
            if (Minecraft::GetInstance()->skins->getSelected()->hasAudio()) {
                ...
                DLCAudioFile*   dlcAudioFile =
                    (DLCAudioFile*)
                        pack->getFile(DLCManager::e_DLCType_Audio, 0);

                app.DebugPrintf("Mashup pack \n");

                // build the name

                // if the music ID is beyond the end of the texture pack music
                // files, then it's a CD
                if (m_musicID < m_iStream_CD_1) {
                    ...
                    strcat(
                        (char*)m_szStreamName,
                        m_szStreamFileA
                            [m_musicID - m_iStream_CD_1 + eStream_CD_1]
                    );
                    strcat((char*)m_szStreamName, ".binka");
                }
            } else {
                // 4J-PB - if this is a PS3 disc patch, we have to check if the
                // music file is in the patch data
#ifdef __PS3__
                if (app.GetBootedFromDiscPatch()
    ...
    case eMusicStreamState_Opening:
        // If the open stream thread is complete, then we are ready to proceed
        // to actually playing
        if (!m_openStreamThread->isRunning()) {
            ...
            // 4J-PB - causes the falloff to be calculated on the PPU instead of
            // the SPU, and seems to resolve our distorted sound issue
            AIL_register_falloff_function_callback(
                hSample,
                &custom_falloff_function
            );
            ...
            if (m_StreamingAudioInfo.bIs3D) {
                AIL_set_sample_3D_distances(
                    ...
                    1,
                    0
                ); // Larger distance scaler for music discs
                if (m_validListenerCount > 1) {
                    float fClosest         = 10000.0f;
                    int   iClosestListener = 0;
                    float fClosestX = 0.0f, fClosestY = 0.0f, fClosestZ = 0.0f,
                          fDist;
                    // need to calculate the distance from the sound to the
                    // nearest listener - use Manhattan Distance as the decision
                    for (int i = 0; i < MAX_LOCAL_PLAYERS; i++) {
                        if (m_ListenerA[i].bValid) {
                            float x, y, z;
                    ...
                    AIL_set_sample_3D_position(
                        ...
                        m_StreamingAudioInfo.y,
                        -m_StreamingAudioInfo.z
                    ); // Flipped sign of z as Miles is expecting left handed
                       // coord system
                }
            } else {
    ...
    case eMusicStreamState_Playing:
        if (GetIsPlayingStreamingGameMusic()) {
            ...
            // we don't have any streaming audio in the world that moves, so
            // this isn't required unless we have more than one listener, and
            // are setting the listening position to the origin and setting a
            // fake position for the sound down  the z axis
            if (m_StreamingAudioInfo.bIs3D) {
                if (m_validListenerCount > 1) {
                    float fClosest         = 10000.0f;
                    int   iClosestListener = 0;
