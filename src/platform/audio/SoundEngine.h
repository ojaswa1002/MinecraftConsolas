...
public:
...
#endif
    virtual void
    play(int iSound, float x, float y, float z, float volume, float pitch);
    ...
    virtual void tick(
        std::shared_ptr<Mob>* players,
        float                 a
    ); // 4J - updated to take array of local players rather than single one
    ...
    virtual char* ConvertSoundPathToName(
        const std::wstring& name,
