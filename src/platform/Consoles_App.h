...
#ifdef _XBOX
#include "xbox/source files/xui/menu screens/XUI_Helper.h"
#include "xbox/source files/xui/menu screens/help & options/credits/XUI_HelpCredits.h"
#endif
#include <cstddef>
#include <memory>
...
public:
    ...
    std::vector<std::wstring> m_vCreditText; // hold the credit text lines so we
    ...
    static int DisplaySavingMessage(
        LPVOID                           pParam,
    ...
    void SetGameStarted(bool bVal) {
        if (bVal) DebugPrintf("SetGameStarted - true\n");
        else DebugPrintf("SetGameStarted - false\n");
        m_bGameStarted = bVal;
        m_bIsAppPaused = !bVal;
    }
    int  GetLocalPlayerCount(void);
    bool LoadInventoryMenu(
        int                          iPad,
        std::shared_ptr<LocalPlayer> player,
        bool                         bNavigateBack = false
    );
    bool LoadCreativeMenu(
        int                          iPad,
        std::shared_ptr<LocalPlayer> player,
        bool                         bNavigateBack = false
    );
    bool LoadEnchantingMenu(
        int                        iPad,
    ...
    void
    SetXuiServerAction(int iPad, eXuiServerAction action, LPVOID param = NULL) {
    ...
    static int OldProfileVersionCallback(
        LPVOID               pParam,
    ...
    int SetDefaultOptions(
                                             C4JStorage::PROFILESETTINGS* pSettings,
                                             const int                    iPad,
                                             bool                         bWriteProfile = true
                                         );
#ifdef __ORBIS__
    static int OptionsDataCallback(
        LPVOID                       pParam,
        int                          iPad,
    ...
    void
    SetOptionsCallbackStatus(int iPad, C4JStorage::eOptionsCallback eStatus);
#else
    static int DefaultOptionsCallback(
        LPVOID                        pParam,
    ...
    int
    SetDefaultOptions(C_4JProfile::PROFILESETTINGS* pSettings, const int iPad);
#endif
    ...
    void
    ValidateFavoriteSkins(int iPad); // check the DLC is available for the skins
...
public:
    ...
    void         CheckGameSettingsChanged(
                bool bOverride5MinuteTimer = false,
    ...
    unsigned int
         GetGameSettingsDebugMask(int iPad = -1, bool bOverridePlayer = false);
    ...
    static void
    SignInChangeCallback(LPVOID pParam, bool bVal, unsigned int uiSignInData);
    ...
    static void NotificationsCallback(
        LPVOID       pParam,
    ...
    static void UpsellReturnedCallback(
        LPVOID          pParam,
        eUpsellType     type,
    ...
    static int
    NowDisplayFullVersionPurchase(void* pParam, bool bContinue, int iPad);
...
public:
    // BAN LIST
    void AddLevelToBannedLevelList(
        int       iPad,
        PlayerUID xuid,
        char*     pszLevelName,
        bool      bWriteToTMS
    ...
    void RemoveLevelFromBannedLevelList(
        int       iPad,
        PlayerUID xuid,
        char*     pszLevelName
    );
...
private:
    ...
    static int UnlockFullExitReturned(
        void*                      pParam,
        int                        iPad,
        C4JStorage::EMessageResult result
    );
    static int UnlockFullSaveReturned(
        void*                      pParam,
        int                        iPad,
        C4JStorage::EMessageResult result
    );
    ...
    static int TrialOverReturned(
        void*                      pParam,
        int                        iPad,
        C4JStorage::EMessageResult result
    );
    static int ExitAndJoinFromInvite(
        void*                      pParam,
        int                        iPad,
        C4JStorage::EMessageResult result
    );
...
public:
    ...
    bool
    GetTerrainFeaturePosition(_eTerrainFeatureType eType, int* pX, int* pZ);
    ...
    bool GetDLCFullOfferIDForSkinID(
    ...
    // TMS
    void
    ReadDLCFileFromTMS(int iPad, eTMSAction action, bool bCallback = false);
    void
    ReadXuidsFileFromTMS(int iPad, eTMSAction action, bool bCallback = false);
    ...
    virtual void ReadBannedList(
        int        iPad,
        eTMSAction action    = (eTMSAction)0,
...
private:
    ...
    // TMS xuids.xml file 	bool m_bRead_TMS_DLCINFO_XML; // track whether we
    // have already read the TMS DLC.xml file
...
public:
    ...
    unsigned int
    GetGameHostOption(unsigned int uiHostSettings, eGameHostOption eVal);
...
public:
    ...
    unsigned int
         AddDLCRequest(eDLCMarketplaceType eContentType, bool bPromote = false);
    bool RetrieveNextDLCContent();
    bool CheckTMSDLCCanStop();
    static int
          DLCOffersReturned(void* pParam, int iOfferC, DWORD dwType, int iPad);
    ...
    unsigned int
        AddTMSPPFileTypeRequest(eDLCContentType eType, bool bPromote = false);
...
#ifdef _XBOX_ONE
    bool
    GetDLCFullOfferIDForPackID(const int iPackID, std::wstring& wsProductId);
...
public:
    ...
    void SetAdditionalSkinBoxes(
        DWORD     dwSkinID,
        SKIN_BOX* SkinBoxA,
        DWORD     dwSkinBoxC
    );
    std::vector<ModelPart*>*
    SetAdditionalSkinBoxes(DWORD dwSkinID, std::vector<SKIN_BOX*>* pvSkinBoxA);
    std::vector<ModelPart*>* GetAdditionalModelParts(DWORD dwSkinID);
    std::vector<SKIN_BOX*>*  GetAdditionalSkinBoxes(DWORD dwSkinID);
    void
    SetAnimOverrideBitmask(DWORD dwSkinID, unsigned int uiAnimOverrideBitmask);
    ...
    virtual int
    LoadLocalTMSFile(WCHAR* wchTMSFile, eFileExtensionType eExt) = 0;
...
public:
    ...
    std::wstring
    getFilePath(DWORD packId, std::wstring filename, bool bAddDataFolder);
