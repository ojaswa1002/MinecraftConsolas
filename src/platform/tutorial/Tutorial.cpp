...
#include "platform/tutorial/hints/DiggerItemHint.h"
#include "platform/tutorial/hints/LookAtEntityHint.h"
#include "platform/tutorial/hints/LookAtTileHint.h"
#include "platform/tutorial/hints/TutorialHint.h"
...
#include "platform/tutorial/tasks/TutorialTask.h"
...
Tutorial::Tutorial(int iPad, bool isFullTutorial /*= false*/) : m_iPad(iPad) {
    ...
    if (!isHintCompleted(e_Tutorial_Hint_Gold_Ore))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_Gold_Ore, this, goldOreItems, 1)
        );
    ...
    if (!isHintCompleted(e_Tutorial_Hint_Iron_Ore))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_Iron_Ore, this, ironOreItems, 1)
        );
    ...
    if (!isHintCompleted(e_Tutorial_Hint_Coal_Ore))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_Coal_Ore, this, coalOreItems, 1)
        );
    ...
    if (!isHintCompleted(e_Tutorial_Hint_Mushroom))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_Mushroom, this, mushroomItems, 2)
        );
    ...
    if (!isHintCompleted(e_Tutorial_Hint_Obsidian))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_Obsidian, this, obsidianItems, 1)
        );
    ...
    if (!isHintCompleted(e_Tutorial_Hint_Farmland))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_Farmland, this, farmlandItems, 1)
        );
    ...
    int pressurePlateItems[] = {
        Tile::pressurePlate_stone_Id,
    ...
    if (!isHintCompleted(e_Tutorial_Hint_Trapdoor))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_Trapdoor, this, trapdoorItems, 1)
        );
    ...
    int redstoneLampItems[] = {
        Tile::redstoneLight_Id,
    ...
    if (!isHintCompleted(e_Tutorial_Hint_WoolCarpet))
        addHint(
            e_Tutorial_State_Gameplay,
            new LookAtTileHint(e_Tutorial_Hint_WoolCarpet, this, carpetItems, 1)
        );
    ...
    if (isFullTutorial || !isStateCompleted(e_Tutorial_State_Inventory_Menu)) {
        ...
        inventoryOverviewTask->AddTask(new ChoiceTask(
            ...
            IDS_TUTORIAL_PROMPT_INV_OVERVIEW,
            true,
            ACTION_MENU_A,
            ACTION_MENU_B,
    ...
    if (isFullTutorial
        ...
        craftingOverviewTask->AddTask(new ChoiceTask(
            ...
            IDS_TUTORIAL_PROMPT_CRAFT_OVERVIEW,
            true,
            ACTION_MENU_A,
            ACTION_MENU_B,
            e_Tutorial_Completion_Complete_State,
    ...
    if (isFullTutorial || !isStateCompleted(e_Tutorial_State_Brewing_Menu)) {
        ...
        brewingOverviewTask->AddTask(new ChoiceTask(
            ...
            IDS_TUTORIAL_PROMPT_BREWING_MENU_OVERVIEW,
            true,
            ACTION_MENU_A,
            ACTION_MENU_B,
            e_Tutorial_Completion_Complete_State,
    ...
    if (isFullTutorial || !isStateCompleted(e_Tutorial_State_Enchanting_Menu)) {
        ...
        enchantingOverviewTask->AddTask(new ChoiceTask(
            ...
            IDS_TUTORIAL_PROMPT_ENCHANTING_MENU_OVERVIEW,
            true,
            ACTION_MENU_A,
            ACTION_MENU_B,
            e_Tutorial_Completion_Complete_State,
    ...
    if (isFullTutorial || !isStateCompleted(e_Tutorial_State_Riding_Minecart)) {
        addTask(
            ...
            new ChoiceTask(
                ...
                IDS_TUTORIAL_PROMPT_MINECART_OVERVIEW,
                true,
                ACTION_MENU_A,
                ACTION_MENU_B,
...
#endif
    ZeroMemory(
        pGameSettings->ucTutorialCompletion,
...
bool Tutorial::getCompleted(int completableId) {
    ...
    if (completableIndex >= 0
     ...
     || defined                                      __PSVITA__)
        ...
        int arrayIndex = completableIndex >> 3;
        int bitIndex   = 7 - (completableIndex % 8);
        return (pGameSettings->ucTutorialCompletion[arrayIndex] & 1 << bitIndex)
            == (1 << bitIndex);
    } else {
        return false;
    }
...
void Tutorial::setHintCompleted(TutorialHint* hint) {
    ...
    if (hintId != e_Tutorial_Hint_Always_On) {
        ...
        hints[m_CurrentState].erase(find(
            hints[m_CurrentState].begin(),
            hints[m_CurrentState].end(),
            hint
        ));
        delete hint;
    }
...
void Tutorial::tick() {
        ...
        while (it < constraintsToRemove[state].end()) {
            ...
            if ((*it).second > m_iTutorialConstraintDelayRemoveTicks) {
                ...
                constraints[state].erase(find(
                    constraints[state].begin(),
                    constraints[state].end(),
                    c
                ));
    ...
    if (!m_allTutorialsComplete
        ...
        for (auto it = constraints[m_CurrentState].begin();
             it < constraints[m_CurrentState].end();
             ++it) {
            TutorialConstraint* constraint = *it;
                ...
                if ((!task->ShowMinimumTime()
                    ...
                    if (activeTasks[m_CurrentState].size() > 0) {
                        ...
                        case e_Tutorial_Completion_Complete_State_Gameplay_Constraints: {
                            ...
                            // gameplay state so that they are in effect for a
                            // bit longer
                            auto itCon =
                            ...
                            while (
                                ...
                                constraintsToRemove[e_Tutorial_State_Gameplay]
                                    .push_back(
                                        std::pair<
                                            TutorialConstraint*,
                                            unsigned char>(
                                ...
                                itCon =
                                    constraintsToRemove[m_CurrentState].erase(
                                        itCon
                                    );
                            }
                        }
    ...
    if (message != NULL
        ...
        if (!message->m_titleString.empty() || message->m_titleId > 0) {
            ...
            ui.SetTutorialDescription(m_iPad, &popupInfo);
                   ...
                   m_lastMessage != NULL && m_lastMessage->m_messageId != -1
...
bool Tutorial::setMessage(
    const std::wstring& messageString,
    int                 icon,
    int                 auxValue
) {
    PopupMessageDetails* message = new PopupMessageDetails();
...
void Tutorial::completeUsingItem(std::shared_ptr<ItemInstance> item) {
    ...
    if (m_CurrentState != e_Tutorial_State_Gameplay) {
        for (auto it = activeTasks[e_Tutorial_State_Gameplay].begin();
             it < activeTasks[e_Tutorial_State_Gameplay].end();
             ++it) {
            TutorialTask* task = *it;
            task->completeUsingItem(item);
        }
...
void Tutorial::startDestroyBlock(
    std::shared_ptr<ItemInstance> item,
    Tile*                         tile
) {
    int hintNeeded = -1;
...
void Tutorial::createItemSelected(
    std::shared_ptr<ItemInstance> item,
    bool                          canMake
) {
    int hintNeeded = -1;
...
bool Tutorial::canMoveToPosition(
    double xo,
    double yo,
    double zo,
    double xt,
...
void Tutorial::
    RemoveConstraint(TutorialConstraint* c, bool delayedRemove /*= false*/) {
    ...
    if (c->getQueuedForRemoval()) {
        ...
        for (auto it = constraintsToRemove[m_CurrentState].begin();
             it < constraintsToRemove[m_CurrentState].end();
             ++it) {
            if (it->first == c) {
        ...
        it = find(
            constraints[e_Tutorial_State_Gameplay].begin(),
            constraints[e_Tutorial_State_Gameplay].end(),
            c
        );
        if (it != constraints[e_Tutorial_State_Gameplay].end())
            constraints[e_Tutorial_State_Gameplay].erase(find(
                constraints[e_Tutorial_State_Gameplay].begin(),
                constraints[e_Tutorial_State_Gameplay].end(),
                c
            ));
...
#ifdef _XBOX
void Tutorial::
    changeTutorialState(eTutorial_State newState, CXuiScene* scene /*= NULL*/)
#else
void Tutorial::
    changeTutorialState(eTutorial_State newState, UIScene* scene /*= NULL*/)
#endif
{
    ...
    if (hintsOn) {
        ...
        if (currentTask[m_CurrentState] != NULL
            ...
            activeTasks[m_CurrentState].erase(find(
                activeTasks[m_CurrentState].begin(),
