...
#include "platform/Consoles_App.h"
...
#include "util/console/ThreadName.h"
...
std::int64_t CGameNetworkManager::messageQueue[512];
std::int64_t CGameNetworkManager::byteQueue[512];
...
void CGameNetworkManager::DoWork() {
    ...
    if (app.GetNotifications()->size() != 0) {
        ...
        case XN_LIVE_INVITE_ACCEPTED:
            s_pPlatformNetworkManager->Notify(
                pNotification->dwNotification,
...
bool CGameNetworkManager::StartNetworkGame(
    Minecraft* minecraft,
    LPVOID     lpParameter
) {
#ifdef _DURANGO
    ...
    if (lpParameter != NULL) {
        ...
        if (param->levelGen != NULL) {
            if (app.getLevelGenerationOptions() == NULL) {
                app.DebugPrintf(
                    "Game rule was not loaded, and seed is required. Exiting.\n"
                );
                return false;
            } else {
                param->seed = seed =
    ...
    if (g_NetworkManager.IsHost()) {
        ...
        C4JThread* thread = new C4JThread(
            &CGameNetworkManager::ServerThreadProc,
            lpParameter,
            "Server",
            256 * 1024
        );
    ...
    while (!IsReadyToPlayOrIdle()) {
        ...
        pMinecraft->progressRenderer->progressStage(
            g_NetworkManager.CorrectErrorIDS(IDS_PROGRESS_SAVING_TO_DISC)
        ); // "Finalizing..." vaguest message I could find
    ...
    if (connection->isStarted() && !connection->isClosed()) {
        ...
        for (int idx = 0; idx < XUSER_MAX_COUNT; ++idx) {
            ...
            connection->send(
                std::shared_ptr<PreLoginPacket>(new PreLoginPacket(
            ...
            if (ProfileManager.IsSignedIn(idx) && !connection->isClosed()) {
                ...
                connection->close();
                auto it = find(
                    createdConnections.begin(),
                    createdConnections.end(),
                    connection
                );
...
void CGameNetworkManager::RegisterPlayerChangedCallback(
    ...
    s_pPlatformNetworkManager
        ->RegisterPlayerChangedCallback(iPad, callback, callbackParam);
}
...
void CGameNetworkManager::UnRegisterPlayerChangedCallback(
    ...
    s_pPlatformNetworkManager
        ->UnRegisterPlayerChangedCallback(iPad, callback, callbackParam);
}
...
void CGameNetworkManager::HostGame(
    int           localUsersMask,
    bool          bOnlineGame,
    bool          bIsPrivate,
    ...
    s_pPlatformNetworkManager->HostGame(
        localUsersMask,
        bOnlineGame,
        bIsPrivate,
        publicSlots,
...
std::vector<FriendSessionInfo*>* CGameNetworkManager::GetSessionList(
    ...
    return s_pPlatformNetworkManager
        ->GetSessionList(iPad, localPlayers, partyOnly);
}

bool CGameNetworkManager::GetGameSessionInfo(
    int                iPad,
    SessionID          sessionId,
    FriendSessionInfo* foundSession
) {
    return s_pPlatformNetworkManager
        ->GetGameSessionInfo(iPad, sessionId, foundSession);
}
...
bool CGameNetworkManager::JoinGameFromInviteInfo(
    ...
    return s_pPlatformNetworkManager
        ->JoinGameFromInviteInfo(userIndex, userMask, pInviteInfo);
}

CGameNetworkManager::eJoinGameResult CGameNetworkManager::JoinGame(
    FriendSessionInfo* searchResult,
    int                localUsersMask
) {
...
#if defined(__PS3__) || defined(__PSVITA__)
            ProfileManager
                .GetChatAndContentRestrictions(iPad, false, &noUGC, NULL, NULL);
#elif defined(__ORBIS__)
            ProfileManager
                .GetChatAndContentRestrictions(iPad, false, NULL, &noUGC, NULL);
#endif
...
int CGameNetworkManager::ServerThreadProc(void* lpParameter) {
    ...
    MinecraftServer::main(
        seed,
        lpParameter
    ); // saveData, app.GetGameHostOption(eGameHostOption_All));
...
int CGameNetworkManager::PSNSignInReturned_0(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
...
int CGameNetworkManager::PSNSignInReturned_1(
    void* pParam,
    bool  bContinue,
    int   iPad
) {
        ...
        for (unsigned int index = 0; index < XUSER_MAX_COUNT; ++index) {
            if (ProfileManager.IsSignedIn(index)
                ...
                for (auto it = players->players.begin();
                     it < players->players.end();
                     ++it) {
                    std::shared_ptr<ServerPlayer> servPlayer = *it;
                    if (servPlayer->getXuid() == localPlayerXuid) {
                        servPlayer->connection->connection->getSocket()
                            ->setPlayer(g_NetworkManager
                                            .GetLocalPlayerByUserIndex(index));
                    }
                }
...
void CGameNetworkManager::SystemFlagSet(
    INetworkPlayer* pNetworkPlayer,
    int             index
) {
...
bool CGameNetworkManager::SystemFlagGet(
    INetworkPlayer* pNetworkPlayer,
    int             index
) {
...
void CGameNetworkManager::CreateSocket(
    INetworkPlayer* pNetworkPlayer,
    bool            localPlayer
) {
    Minecraft* pMinecraft = Minecraft::GetInstance();
    ...
    if (localPlayer && mpPlayer != NULL && mpPlayer->connection != NULL) {
        ...
        if (localPlayer && g_NetworkManager.IsInGameplay()) {
            ...
            app.DebugPrintf(
                "Creating new client connection for idx: %d\n",
                idx
            );
...
void CGameNetworkManager::PlayerJoining(INetworkPlayer* pNetworkPlayer) {
    if (g_NetworkManager.IsInGameplay()) // 4J-JEV: Wait to do this at
                                         // StartNetworkGame if not in-game yet.
    {
...
void CGameNetworkManager::PlayerLeaving(INetworkPlayer* pNetworkPlayer) {
    if (pNetworkPlayer->IsLocal()) {
        ProfileManager.SetCurrentGameActivity(
            pNetworkPlayer->GetUserIndex(),
            CONTEXT_PRESENCE_IDLE,
            false
        );

        TelemetryManager->RecordPlayerSessionExit(
            pNetworkPlayer->GetUserIndex(),
            app.GetDisconnectReason()
        );
    }
#ifdef _XBOX
    else {
...
void CGameNetworkManager::GameInviteReceived(
    int                userIndex,
    const INVITE_INFO* pInviteInfo
) {
#ifdef __ORBIS__
    ...
    if (joiningUsers > 1 && !RenderManager.IsHiDef()
        ...
        ui.RequestMessageBox(
            ...
            IDS_CONNECTION_FAILED_NO_SD_SPLITSCREEN,
            uiIDA,
            1,
            XUSER_INDEX_ANY,
            NULL,
...
void CGameNetworkManager::
    AddLocalPlayerFailed(int idx, bool serverFull /* = false*/) {
    Minecraft::GetInstance()->connectionDisconnected(
        idx,
        serverFull ? DisconnectPacket::eDisconnect_ServerFull
                   : DisconnectPacket::eDisconnect_ConnectionCreationFailed
    );
}
