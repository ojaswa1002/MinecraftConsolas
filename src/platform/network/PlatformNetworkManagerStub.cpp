...
void CPlatformNetworkManagerStub::NotifyPlayerJoined(IQNetPlayer* pQNetPlayer) {
    ...
    if (pQNetPlayer->IsLocal()) {
        ...
        if (m_pIQNet->IsHost() && !m_bHostChanged) {
            ...
            for (auto it = m_machineQNetPrimaryPlayers.begin();
                 it < m_machineQNetPrimaryPlayers.end();
                 ++it) {
    ...
    app.DebugPrintf(
        "Player 0x%p \"%ls\" joined; %s; voice %i; camera %i.\n",
        pQNetPlayer,
    ...
    for (int idx = 0; idx < XUSER_MAX_COUNT; ++idx) {
        if (playerChangedCallback[idx] != NULL)
            playerChangedCallback[idx](
                playerChangedCallbackParam[idx],
                networkPlayer,
                false
            );
    }
...
bool CPlatformNetworkManagerStub::Initialise(
    CGameNetworkManager* pGameNetworkManager,
    int                  flagIndexSize
) {
...
bool CPlatformNetworkManagerStub::
    SessionHasSpace(unsigned int spaceRequired /*= 1*/) {
    return true;
}
...
bool CPlatformNetworkManagerStub::_LeaveGame(
    bool bMigrateHost,
    bool bLeaveRoom
) {
    return true;
...
int CPlatformNetworkManagerStub::JoinGame(
...
void CPlatformNetworkManagerStub::UpdateAndSetGameSessionData(
    ...
    //
    // 			// We can call this from NotifyPlayerLeaving but at that point
    // the player is still considered in the session 			if(
    // pNetworkPlayer != pNetworkPlayerLeaving )
    // 			{
    // 				m_hostGameSessionData.players[i] = ((NetworkPlayerXbox
    // *)pNetworkPlayer)->GetUID();
    //
    // 				char *temp;
...
void CPlatformNetworkManagerStub::SystemFlagAddPlayer(
    ...
    for (unsigned int i = 0; i < m_playerFlags.size(); i++) {
        if (pNetworkPlayer->IsSameSystem(m_playerFlags[i]->m_pNetworkPlayer)) {
            memcpy(
                newPlayerFlags->flags,
                m_playerFlags[i]->flags,
...
// shares that system
void CPlatformNetworkManagerStub::SystemFlagSet(
    INetworkPlayer* pNetworkPlayer,
    int             index
) {
...
// here
bool CPlatformNetworkManagerStub::SystemFlagGet(
    INetworkPlayer* pNetworkPlayer,
    int             index
) {
    ...
    for (unsigned int i = 0; i < m_playerFlags.size(); i++) {
        if (m_playerFlags[i]->m_pNetworkPlayer == pNetworkPlayer) {
            return (
                (m_playerFlags[i]->flags[index / 8] & (128 >> (index % 8))) != 0
            );
        }
    }
    return false;
...
bool CPlatformNetworkManagerStub::GetGameSessionInfo(
    int                iPad,
    SessionID          sessionId,
    FriendSessionInfo* foundSessionInfo
) {
    return false;
}
...
INetworkPlayer*
CPlatformNetworkManagerStub::addNetworkPlayer(IQNetPlayer* pQNetPlayer) {
...
INetworkPlayer*
CPlatformNetworkManagerStub::getNetworkPlayer(IQNetPlayer* pQNetPlayer) {
...
INetworkPlayer*
CPlatformNetworkManagerStub::GetLocalPlayerByUserIndex(int userIndex) {
...
INetworkPlayer*
CPlatformNetworkManagerStub::GetPlayerBySmallId(unsigned char smallId) {
...
bool CPlatformNetworkManagerStub::JoinGameFromInviteInfo(
    ...
    return (
        m_pIQNet->JoinGameFromInviteInfo(userIndex, userMask, pInviteInfo)
        == S_OK
    );
}
