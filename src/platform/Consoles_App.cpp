...
#include "platform/dlc/DLCManager.h"
#include "platform/dlc/DLCPack.h"
#include "platform/dlc/DLCSkinFile.h"
#include "platform/gamerules/GameRuleManager.h"
...
#include "platform/ui/scenes/UIScene_FullscreenProgress.h"
...
#include "win/gameconfig/Minecraft.spa.h"
#include "win/sentient/SentientTelemetryCommon.h"
#include "win/social/SocialManager.h"
...
#include "Console_Debug_enum.h"
#include "Minecraft_Macros.h"
#ifndef _XBOX
#include "platform/ui/all_platforms/ArchiveFile.h"
#endif
...
#include "xbox/source files/xui/XUI_TextEntry.h"
#include "xbox/source files/xui/XUI_XZP_Icons.h"
#include "xbox/source files/xui/menu screens/pause/XUI_PauseMenu.h"
#else
#include "platform/ui/scenes/ingame_menu_screens/UIScene_PauseMenu.h"
#endif
#ifdef __PS3__
#include <sys/tty.h>
#endif
...
CMinecraftApp::CMinecraftApp() {
    if (GAME_SETTINGS_PROFILE_DATA_BYTES != sizeof(GAME_SETTINGS)) {
        ...
        DebugPrintf(
            ...
            GAME_SETTINGS_PROFILE_DATA_BYTES
        );
...
void CMinecraftApp::SetAction(int iPad, eXuiAction action, LPVOID param) {
    if (m_eXuiAction[iPad] == eAppAction_ExitWorldCapturedThumbnail
        ...
        app.DebugPrintf(
            "Changing App action for pad %d from %d to %d\n",
            iPad,
            m_eXuiAction[iPad],
            action
        );
...
bool CMinecraftApp::LoadInventoryMenu(
    int                          iPad,
...
bool CMinecraftApp::LoadCreativeMenu(
    int                          iPad,
    std::shared_ptr<LocalPlayer> player,
    bool                         bNavigateBack
) {
...
bool CMinecraftApp::LoadCrafting2x2Menu(
    int                          iPad,
    std::shared_ptr<LocalPlayer> player
) {
    bool success = true;
...
bool CMinecraftApp::LoadCrafting3x3Menu(
    int                          iPad,
    std::shared_ptr<LocalPlayer> player,
    int                          x,
    int                          y,
...
bool CMinecraftApp::LoadEnchantingMenu(
    int                        iPad,
...
bool CMinecraftApp::LoadContainerMenu(
    int                        iPad,
...
bool CMinecraftApp::LoadTrapMenu(
    int                                  iPad,
    std::shared_ptr<Container>           inventory,
...
bool CMinecraftApp::LoadSignEntryMenu(
    int                             iPad,
...
bool CMinecraftApp::LoadRepairingMenu(
    int                        iPad,
    std::shared_ptr<Inventory> inventory,
    Level*                     level,
    int                        x,
...
bool CMinecraftApp::LoadTradingMenu(
    int                        iPad,
    std::shared_ptr<Inventory> inventory,
...
#if (defined __PS3__ || defined __ORBIS__ || defined _DURANGO                  \
     || defined                                      __PSVITA__)
int CMinecraftApp::SetDefaultOptions(
...
#ifdef __ORBIS__
int CMinecraftApp::OptionsDataCallback(
    LPVOID                       pParam,
    int                          iPad,
...
int CMinecraftApp::OldProfileVersionCallback(
    LPVOID               pParam,
    ...
    case PROFILE_VERSION_2:
        // need to fill in values for the new profile data. No need to save the
        // profile - that'll happen if they get changed, or if the auto save for
        // the profile kicks in
        {
            ...
            // because of the profile bug that seemed to stop the awards of some
            // Changing this to check the system achievements at sign-in and
            // award any that the game says we have and the system says we
            // haven't
    ...
    case PROFILE_VERSION_4: {
        ...
        // Set the online flag to on, so it's not saved if a game starts offline
        // when the user didn't change it to be offline (xbox disconnected from
        // LIVE)
        pGameSettings->uiBitmaskValues |=
            GAMESETTING_ONLINE; // eGameSetting_GameSetting_Online - on
        // TU12
...
void CMinecraftApp::ActionGameSettings(int iPad, eGameSetting eVal) {
    ...
    case eGameSetting_ViewBob:
        // 4J-PB - not handled here any more - it's read from the gamesettings
        // per player
    ...
    case eGameSetting_ControlScheme:
        InputManager.SetJoypadMapVal(
            iPad,
            (GameSettingsA[iPad]->usBitmaskValues & 0x30) >> 4
        );
        break;
    ...
    case eGameSetting_ControlSouthPaw:
        ...
        if (GameSettingsA[iPad]->usBitmaskValues & 0x80) {
            ...
            InputManager
                .SetJoypadStickTriggerMap(iPad, TRIGGER_MAP_0, TRIGGER_MAP_1);
            InputManager
                .SetJoypadStickTriggerMap(iPad, TRIGGER_MAP_1, TRIGGER_MAP_0);
        } else {
            // Right handed.
            ...
            InputManager
                .SetJoypadStickTriggerMap(iPad, TRIGGER_MAP_0, TRIGGER_MAP_0);
            InputManager
                .SetJoypadStickTriggerMap(iPad, TRIGGER_MAP_1, TRIGGER_MAP_1);
        }
        break;
    ...
    case eGameSetting_GamertagsVisible: {
        ...
        if (bInGame && g_NetworkManager.IsHost()
            ...
            app.SetGameHostOption(
                ...
                ((GameSettingsA[iPad]->usBitmaskValues & 0x0008) != 0) ? 1 : 0
            );
            app.SetXuiServerAction(
                iPad,
...
void CMinecraftApp::SetPlayerSkin(int iPad, DWORD dwSkinId) {
    ...
    TelemetryManager->RecordSkinChanged(
        iPad,
...
DWORD CMinecraftApp::GetPlayerSkinId(int iPad) {
    ...
    if (GET_IS_DLC_SKIN_FROM_BITMASK(dwSkin)) {
        ...
        if (Pack) {
            ...
            bool bLicensed = Pack->hasPurchasedFile(
...
void CMinecraftApp::SetPlayerFavoriteSkin(
    int          iPad,
    int          iIndex,
...
void CMinecraftApp::SetGameSettings(
    int           iPad,
    eGameSetting  eVal,
    unsigned char ucVal
) {
    ...
    case eGameSetting_Online:
        ...
        break;
    case eGameSetting_InviteOnly:
        if ((GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_INVITEONLY)
            != (ucVal & 0x01) << 2) {
            if (ucVal == 1) {
    ...
    case eGameSetting_BedrockFog:
        if ((GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_BEDROCKFOG)
            != (ucVal & 0x01) << 6) {
            if (ucVal == 1) {
        ...
        break;
    case eGameSetting_DisplayHUD:
        if ((GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_DISPLAYHUD)
            != (ucVal & 0x01) << 7) {
            if (ucVal == 1) {
        ...
        break;
    case eGameSetting_DisplayHand:
        if ((GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_DISPLAYHAND)
            != (ucVal & 0x01) << 8) {
            if (ucVal == 1) {
    ...
    case eGameSetting_CustomSkinAnim:
        if ((GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_CUSTOMSKINANIM)
            != (ucVal & 0x01) << 9) {
            if (ucVal == 1) {
                GameSettingsA[iPad]->uiBitmaskValues |=
    ...
    // TU9
    case eGameSetting_DeathMessages:
        if ((GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_DEATHMESSAGES)
            != (ucVal & 0x01) << 10) {
            if (ucVal == 1) {
                GameSettingsA[iPad]->uiBitmaskValues |=
    ...
    case eGameSetting_AnimatedCharacter:
        ...
        break;
    case eGameSetting_PS3_EULA_Read:
        if ((GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_PS3EULAREAD)
            != (ucVal & 0x01) << 16) {
            if (ucVal == 1) {
...
unsigned char CMinecraftApp::GetGameSettings(int iPad, eGameSetting eVal) {
    ...
    case eGameSetting_Online:
        ...
        break;
    case eGameSetting_InviteOnly:
        return (GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_INVITEONLY)
            >> 2;
        break;
    case eGameSetting_FriendsOfFriends:
    ...
    case eGameSetting_DisplayUpdateMessage:
        ...
        break;
    case eGameSetting_BedrockFog:
        return (GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_BEDROCKFOG)
            >> 6;
        break;
    case eGameSetting_DisplayHUD:
        return (GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_DISPLAYHUD)
            >> 7;
        break;
    case eGameSetting_DisplayHand:
        return (GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_DISPLAYHAND)
            >> 8;
        break;
    case eGameSetting_CustomSkinAnim:
        return (GameSettingsA[iPad]->uiBitmaskValues
    ...
    case eGameSetting_UISize: {
        unsigned char ucVal =
            (GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_UISIZE) >> 11;
        return ucVal;
    } break;
    case eGameSetting_UISizeSplitscreen: {
    ...
    case eGameSetting_PS3_EULA_Read:
        return (GameSettingsA[iPad]->uiBitmaskValues & GAMESETTING_PS3EULAREAD)
            >> 16;
...
void CMinecraftApp::CheckGameSettingsChanged(
    bool bOverride5MinuteTimer,
    int  iPad
) {
...
int CMinecraftApp::DisplaySavingMessage(
    void*                      pParam,
    ...
    for (int i = 0; i < XUSER_MAX_COUNT; i++) {
        ...
        if (eAction != eAppAction_Idle) {
            ...
            case eAppAction_SocialPost:
                if (ProfileManager.IsFullVersion()) {
                    ...
                    if (CSocialManager::Instance()->IsTitleAllowedToPostImages()
                        ...
                        // m_bwasHidingGui = pMinecraft->options->hideGui; // 4J
                        // Stu - Removed 1.8.2 bug fix (TU6) as don't need this
                ...
                if (!GetChangingSessionType()) {
                    ...
                    if (ProfileManager.IsFullVersion()) {
                        ...
                        ui.RequestMessageBox(
                            IDS_UNLOCK_TITLE,
                            IDS_UNLOCK_TOSAVE_TEXT,
                            uiIDA,
                            2,
                            i,
                            &CMinecraftApp::UnlockFullSaveReturned,
                            this,
                            app.GetStringTable()
                        );
                    }
            ...
            case eAppAction_AutosaveSaveGame: {
                ...
                // stop the dialog asking if we want to overwrite it coming up
                // on an autosave
                bool bSaveExists;
            ...
            case eAppAction_SaveGameCapturedThumbnail:
                ...
                else*/
                {
                    ...
                    // pulls their network cable out while saving the title will
                    // hang.
...
#else
                // turn off the gamertags in splitscreen for the primary player,
                // since they are about to be made fullscreen
                ui.HideAllGameUIElements();
     ...
     || defined                                      __PSVITA__)
                    ...
                    // 				{
                    // 					// we need to remove the qnetplayer, or
                    // this player won't be able to get back into the game until
                    // qnet times out and removes them
            ...
            case eAppAction_ExitPlayerPreLogin: {
                int iPlayerC = g_NetworkManager.GetPlayerCount();
                // Since the player is exiting, let's flush any profile writes
                // for them, and hope we're not breaking TCR 136...
#if (defined __PS3__ || defined __ORBIS__ || defined _DURANGO                  \
     || defined                                      __PSVITA__)
                StorageManager.ForceQueuedProfileWrites(i);
#else
...
#ifdef _XBOX
                // tell the xui scenes a splitscreen player left - has to come
                // after removeLocalPlayerIdx which calls
                // updatePlayerViewportAssignments
                XUIMessage                             xuiMsg;
                CustomMessage_Splitscreenplayer_Struct myMsgData;
                CustomMessage_Splitscreenplayer(&xuiMsg, &myMsgData, false);
                ...
                // Are we offline or online, and how many players are there
                if (iPlayerC
                    > 2) // one player is about to leave here - they'll be set
                         // to idle in the qnet manager player leave
                {
                    for (int iPlayer = 0; iPlayer < XUSER_MAX_COUNT;
                         iPlayer++) {
                        if ((iPlayer != i)
            ...
            case eAppAction_OptionsSaveNoSpace: {
                ...
                app.SetOptionsSaveDataDialogRunning(
                    true
                ); // m_bOptionsSaveDataDialogRunning = true;
            ...
            case eAppAction_ExitWorld:
                ...
                // reset the flag stopping new dlc message being shown if you've
     ...
     || defined                                      __PSVITA__)
                ...
                for (int j = 0; j < XUSER_MAX_COUNT; j++) {
                    if (ProfileManager.IsSignedIn(j)) {
                        app.DebugPrintf(
                            "Stats save for an offline game for the player at "
                                                                 "index %d\n",
                            0
                        );
                        Minecraft::GetInstance()->forceStatsSave(j);
                ...
                // 4J-PB - cancel any possible string verifications queued with
                // LIVE
                // InputManager.CancelAllVerifyInProgress();

                if (ProfileManager.IsFullVersion()) {
                    // In a split screen, only the primary player actually quits
                    // the game, others just remove their players
                    ...
                    // ask the player if they would like to upgrade, or they'll
                    // lose the level
                    UINT uiIDA[2];
                    uiIDA[0] = IDS_CONFIRM_OK;
                    uiIDA[1] = IDS_CONFIRM_CANCEL;
                    ui.RequestMessageBox(
                        ...
                        2,
                        i,
                        &CMinecraftApp::UnlockFullExitReturned,
                        this,
                        app.GetStringTable()
                    );
                }
            ...
            case eAppAction_ExitWorldCapturedThumbnail: {
                ...
                SetChangingSessionType(
                    true
                ); // Added to stop handling ethernet disconnects
                ...
                for (unsigned int idx = 0; idx < XUSER_MAX_COUNT; ++idx) {
                    ...
                    // 4J Stu - Fix for #13257 - CRASH: Gameplay: Title crashed
                    // after exiting the tutorial It doesn't matter if they were
                    // in the tutorial already
                ...
                // server, so make sure the player knows why 4J Stu - Changed -
                // Don't use the FullScreenProgressScreen for action, use a
                // dialog instead
            ...
            case eAppAction_Respawn: {
                ...
                if (app.GetLocalPlayerCount() > 1) {
                    ...
                    // loading or flushing of the cache, just a simple respawn
                    ...
                    // &CScene_Death::RespawnThreadProc; loadingParams->lpParam
                    // = (LPVOID)i;
                    ...
                    // 4J Stu - We don't need this on a thread in multiplayer as
                    // respawning is asynchronous.
                    pMinecraft->localplayers[i]->respawn();
                    ...
                    // completionData->bShowLogo=TRUE;
                    // completionData->type =
                    // e_ProgressCompletion_CloseUIScenes; completionData->iPad
                    // = i; loadingParams->completionData = completionData;
            ...
            case eAppAction_PrimaryPlayerSignedOut: {
                ...
                // If the player signs out before the game started the server
                // can be killed a bit earlier to stop the loading or saving of
                // a new game continuing running while the UI/Guide is up
                ...
                // need to clear the player stats - can't assume it'll be done
                // in setlevel - we may not be in the game
            ...
            case eAppAction_EthernetDisconnected: {
                ...
                // 4J Stu - Fix for #12530 -TCR 001 BAS Game Stability: Title
                // will crash if the player disconnects while starting a new
                // world and then opts to play the tutorial once they have been
                ...
                if (!g_NetworkManager.IsLeavingGame()) {
                    ...
                    // 4J-PB - not the same as a signout. We should only leave
                    // the game if this machine is not the host. We shouldn't
                    // get rid of the save device either.
                    if (g_NetworkManager.IsHost()) {
                        ...
                        if (!g_NetworkManager.IsLocalGame()
                            ...
                            //	 1. We're early enough in the create/load game
                            // that we can do a really minimal shutdown or
                            //   2. We're far enough in (game has started but
                            //   the actual game started flag hasn't been set)
                            //   that we should just wait until we're in the
                            //   game and switch to offline mode
                            ...
                            if (!gameStarted) {
                                ...
                                // Fix for #12530 - TCR 001 BAS Game Stability:
                                // Title will crash if the player disconnects
                                // while starting a new world and then opts to
                                // play the tutorial once they have been
                                // returned to the Main Menu. 4J Stu - Leave the
                                // session
                        ...
                        // need to clear the player stats - can't assume it'll
                        // be done in setlevel - we may not be in the game
                        StatsCounter* pStats =
                            Minecraft::GetInstance()->stats[i];
                        pStats->clear();
                        UINT uiIDA[1];
            ...
            case eAppAction_PrimaryPlayerSignedOutReturned: {
                ...
                if (g_NetworkManager.IsLeavingGame()) {
                    // 4J Stu - If we are already leaving the game, then we just
                    // need to signal that the player signed out to stop saves
            ...
            case eAppAction_DashboardTrialJoinFromInvite: {
                ...
                ui.RequestMessageBox(
                    IDS_UNLOCK_TITLE,
                    IDS_UNLOCK_ACCEPT_INVITE,
                    uiIDA,
                    2,
                    i,
                    &CMinecraftApp::UnlockFullInviteReturned,
                    this,
                    app.GetStringTable()
                );
            } break;
            case eAppAction_ExitAndJoinFromInvite: {
                ...
                if (ProfileManager.IsFullVersion()
                    ...
                    ui.RequestMessageBox(
                        IDS_EXIT_GAME,
                        IDS_CONFIRM_LEAVE_VIA_INVITE,
                        uiIDA,
                        3,
                        i,
                        &CMinecraftApp::ExitAndJoinFromInviteSaveDialogReturned,
                        this,
                        app.GetStringTable(),
                        0,
                        0,
...
#endif
                {
                    if (!ProfileManager.IsFullVersion()) {
                        ...
                        ui.RequestMessageBox(
                            ...
                            2,
                            i,
                            &CMinecraftApp::ExitAndJoinFromInvite,
                            this,
                            app.GetStringTable(),
                            0,
                            0,
            ...
            case eAppAction_ExitAndJoinFromInviteConfirmed: {
                ...
                // 4J-PB - may have been using a texture pack with audio , so
                // clean up anything texture pack related here

                // unload any texture pack audio
                // if there is audio in use, clear out the audio, and unmount
                // the pack
                ...
                if (pTexPack->hasAudio()) {
                    ...
                    // audio from the default texture pack now reset the
                    // streaming sounds back to the normal ones
#ifndef _XBOX
            ...
            case eAppAction_JoinFromInvite: {
                ...
                // CPlatformNetworkManager::ExitAndJoinFromInviteThreadProc, as
                // g_NetworkManager.JoinGameFromInviteInfo ultimately can call
                // NavigateToScene,
                ...
                app.DebugPrintf(
                    "Changing Primary Pad on an invite accept - pad was %d, "
                    "and is now %d\n",
                    ProfileManager.GetPrimaryPad(),
...
#ifdef _XBOX_ONE
                // 4J Stu - If a player is signed in (i.e. locked) but not in
                // the mask, unlock them
                for (unsigned int index = 0; index < XUSER_MAX_COUNT; ++index) {
            ...
            case eAppAction_ChangeSessionType: {
                // If we are not in gameplay yet, then wait until the server is
                // setup before changing the session type
                if (g_NetworkManager.IsInGameplay()) {
                    // This kicks off a thread that waits for the server to end,
                    // then closes the current session, starts a new one and
                    // joins the local players into it
                    ...
                    if (!GetChangingSessionType()
                        ...
                        // #49084 - TU5: Code: Gameplay: The title crashes every
                        // time client navigates to 'Play game' menu and
                        // loads/creates new game after a "Connection to Xbox
                        // LIVE was lost" message has appeared.
     ...
     || defined                                      __PSVITA__)
            ...
            case eAppAction_ProfileReadError:
                ...
                // that for handling eAppAction_PrimaryPlayerSignedOut, although
                // I don't think we should have got as far as needing to halt
                // the server, or running the game, before returning to the menu
                if (!app.GetGameStarted()) MinecraftServer::HaltServer(true);
            ...
            case eAppAction_BanLevel: {
                // It's possible that this state can get set after the game has
                // been exited (e.g. by network disconnection) so we can't ban
                ...
                if (g_NetworkManager.IsInGameplay()
                    ...
                    // write the level to the banned level list, and exit the
                    // world
                    AddLevelToBannedLevelList(
                        i,
                        ((NetworkPlayerXbox*)pHost)->GetUID(),
                        GetUniqueMapName(),
                        true
                    );
...
#endif
                    // primary player would exit the world, secondary would exit
                    // the player
            ...
            case eAppAction_LevelInBanLevelList: {
                ...
                // If not the primary player, but the primary player has banned
                // this level and decided not to unban then we may have left the
                // game by now
                if (player) {
                    swprintf(
                        wchFormat,
                    ...
                    C4JStorage::EMessageResult result = ui.RequestMessageBox(
                        IDS_BANNED_LEVEL_TITLE,
                        IDS_PLAYER_BANNED_LEVEL,
                        uiIDA,
                        2,
                        i,
                        &CMinecraftApp::BannedLevelDialogReturned,
                        this,
                        app.GetStringTable(),
                        wchFormat
                    );
            ...
            case eAppAction_ReloadTexturePack: {
                ...
                // 4J-PB  - If the texture pack has audio, we need to switch to
                // this
            ...
            case eAppAction_TexturePackRequired: {
                ...
                ui.RequestMessageBox(
                    ...
                    IDS_DLC_TEXTUREPACK_NOT_PRESENT,
                    uiIDA,
                    2,
                    ProfileManager.GetPrimaryPad(),
        ...
        if (eTMS != eTMSAction_Idle) {
            ...
            case eTMSAction_TMSPP_BannedListFile:
                ...
                if (app.TMSPP_ReadBannedList(
                    ...
                    == false) {
                    // we don't have a banned list in TMSPP, so we should check
                    // TMS
                    app.ReadBannedList(
                        i,
                        eTMSAction_TMS_RetrieveFiles_Complete,
            ...
            case eTMSAction_TMSPP_RetrieveFiles_Complete:
                ...
                eTMSAction_TMS_RetrieveFiles_RunPlayGame: #ifdef _XBOX
                                ...
                                // pass in the next app action on the call or
                callback completing
                ...
                callback completing
                            ...
                            case eTMSAction_TMS_BannedListFile:
                                ...
                                // pass in the next app action on the call or
                callback completing app.ReadBannedList(i,
            ...
            case eTMSAction_TMS_RetrieveFiles_Complete:
                ...
                // 					// ensure we've applied this player's
                // settings
...
int CMinecraftApp::UnlockFullInviteReturned(
    void*                      pParam,
    int                        iPad,
...
int CMinecraftApp::UnlockFullSaveReturned(
    void*                      pParam,
    int                        iPad,
    C4JStorage::EMessageResult result
) {
...
int CMinecraftApp::UnlockFullExitReturned(
    void*                      pParam,
    int                        iPad,
    C4JStorage::EMessageResult result
) {
...
int CMinecraftApp::TrialOverReturned(
    void*                      pParam,
    int                        iPad,
    C4JStorage::EMessageResult result
) {
...
void CMinecraftApp::SignInChangeCallback(
    LPVOID       pParam,
        ...
        if (((uiSignInData & (1 << iPrimaryPlayer)) == 0)
            ...
            if (g_NetworkManager.IsInSession()) {
                ...
                if (hasGuestIdChanged) {
                    ...
                    uiIDA[0] = IDS_CONFIRM_OK;
                    ui.RequestMessageBox(
                        IDS_GUEST_ORDER_CHANGED_TITLE,
                        IDS_GUEST_ORDER_CHANGED_TEXT,
                        uiIDA,
                        1,
                        ProfileManager.GetPrimaryPad(),
                        NULL,
...
void CMinecraftApp::NotificationsCallback(
    LPVOID       pParam,
    ...
    case XN_LIVE_CONTENT_INSTALLED:
        // Need to inform xuis that we've possibly had DLC installed
        {
            ...
            //  Clear the DLC installed flag to cause a GetDLC to run if it's
            //  called
    ...
    case XN_SYS_STORAGEDEVICESCHANGED: {
        ...
        // selected, and that pack's content device is no longer valid... then
        // pull the plug on audio streaming, as if we leave this until later
        // xact gets locked up attempting to destroy the streamed wave bank.
        ...
        if (pTexPack->hasAudio()) {
            ...
            if (XContentGetDeviceState(deviceID, NULL) != ERROR_SUCCESS) {
                ...
                // playStreaming does what it is meant to do, and also so that
                // we don't try and unmount this again, or play any sounds from
                // it in the future
                pTexPack->setHasAudio(false);
                // need to stop the streaming audio - by playing streaming audio
                // from the default texture pack now
                Minecraft::GetInstance()
...
int CMinecraftApp::MustSignInFullVersionPurchaseReturned(
    ...
    if (result == C4JStorage::EMessage_ResultAccept) {
        ...
        SQRNetworkManager_PS3::AttemptPSNSignIn(
            &CMinecraftApp::NowDisplayFullVersionPurchase,
            &app,
            true
        );
#elif defined __PSVITA__
...
#endif
void CMinecraftApp::UpsellReturnedCallback(
    LPVOID          pParam,
    eUpsellType     type,
    ...
    case eUpsellResponse_NotAllowedOnline: // On earning a trophy in the trial
                                           // version, where the user is
                                           // underage and can't go online to
                                           // buy the game, but they selected to
                                           // buy the game on the trophy upsell
        uiIDA[0] = IDS_CONFIRM_OK;
        ui.RequestMessageBox(
            IDS_ONLINE_SERVICE_TITLE,
            IDS_CONTENT_RESTRICTION,
...
void CMinecraftApp::SetDebugSequence(const char* pchSeq) {
    InputManager
        .SetDebugSequence(pchSeq, &CMinecraftApp::DebugInputCallback, this);
}
...
int CMinecraftApp::MarketplaceCountsCallback(
    LPVOID                       pParam,
    C4JStorage::DLC_TMS_DETAILS* pTMSDetails,
    int                          iPad
) {
    app.DebugPrintf(
        "Marketplace Counts= New - %d Total - %d\n",
        pTMSDetails->dwNewOffers,
        pTMSDetails->dwTotalOffers
    );
...
bool CMinecraftApp::StartInstallDLCProcess(int iPad) {
    app.DebugPrintf(
        "--- CMinecraftApp::StartInstallDLCProcess: pad=%i.\n",
        iPad
    );
    ...
    if ((app.DLCInstallProcessCompleted() == false)
        ...
        StorageManager
            .GetInstalledDLC(iPad, &CMinecraftApp::DLCInstalledCallback, this);
        return true;
    } else {
        app.DebugPrintf(
...
int CMinecraftApp::DLCInstalledCallback(
    LPVOID pParam,
    int    iInstalledC,
    int    iPad
) {
...
void CMinecraftApp::MountNextDLC(int iPad) {
    ...
    if (m_iTotalDLCInstalled < m_iTotalDLC) {
        ...
        if (StorageManager.MountInstalledDLC(
            ...
            // corrupt DLC
            app.DebugPrintf(
                "Failed to mount DLC %d for pad %d\n",
                m_iTotalDLCInstalled,
                iPad
            );
            ++m_iTotalDLCInstalled;
        ...
        */
        ...
        if (!Minecraft::GetInstance()->skins->isUsingDefaultSkin()) {
            ...
            if (pParentPack
                    ->hasPurchasedFile(DLCManager::e_DLCType_Texture, L"")) {
...
int CMinecraftApp::DLCMountedCallback(
    LPVOID pParam,
    int    iPad,
    DWORD  dwErr,
    DWORD  dwLicenceMask
    ...
    if (dwErr != ERROR_SUCCESS) {
        ...
        if (pack == NULL) {
            app.DebugPrintf(
                "Pack \"%ls\" is not installed, so adding it\n",
                CONTENT_DATA_DISPLAY_NAME(ContentData)
            );
...
#elif defined _XBOX_ONE
            pack = new DLCPack(
                ContentData.wszDisplayName,
                ContentData.wszProductID,
                dwLicenceMask
            );
#else
...
// 		 // if the file is not already in the memory textures, then read it from
// TMS 		 if(!bRes)
// 		 {
// 			 BYTE *pBuffer=NULL;
// 			 DWORD dwSize=0;
// 			 // 4J-PB - out for now for DaveK so he doesn't get the birthday
// cape #ifdef _CONTENT_PACKAGE 			 C4JStorage::ETMSStatus eTMSStatus;
...
void CMinecraftApp::HandleDLC(DLCPack* pack) {
    ...
    if (INVALID_HANDLE_VALUE == hFind) {
        ...
        do {
            ...
            if ((GetFileAttributes(szFullFilename) & FILE_ATTRIBUTE_DIRECTORY)
                ...
                if (texturePCKPath.exists()) {
                    ...
                    m_dlcManager
                        .readDLCDataFile(dwFilesProcessed, wsTemp, pack);
                } else {
                    m_dlcManager.readDLCDataFile(
                        dwFilesProcessed,
                        szFullFilename,
                        pack
                    );
...
#else
                m_dlcManager
                    .readDLCDataFile(dwFilesProcessed, szFullFilename, pack);
...
void CMinecraftApp::AddMemoryTextureFile(
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"Incrementing the memory texture file count for %ls\n",
            wName.c_str()
        );
#endif
        pData = (*it).second;
...
#ifndef _CONTENT_PACKAGE
        wprintf(
            L"Decrementing the memory texture file count for %ls\n",
            wName.c_str()
        );
#endif
...
#ifndef _CONTENT_PACKAGE
            wprintf(
                L"Erasing the memory texture file data for %ls\n",
                wName.c_str()
            );
#endif
            delete[] pData;
...
void CMinecraftApp::GetMemFileDetails(
    const std::wstring& wName,
    PBYTE*              ppbData,
    DWORD*              pdwBytes
) {
...
void CMinecraftApp::ProcessInvite(
    DWORD              dwUserIndex,
...
int CMinecraftApp::ExitAndJoinFromInvite(
    void*                      pParam,
    int                        iPad,
    C4JStorage::EMessageResult result
) {
        ...
        if (result == C4JStorage::EMessage_ResultDecline) // Save
        {
            ...
            if (!Minecraft::GetInstance()->skins->isUsingDefaultSkin()) {
                ...
                if (!pDLCPack->hasPurchasedFile(
                    ...
                    ULONGLONG       ullOfferID_Full;
                    app.GetDLCFullOfferIDForPackID(
                        pDLCTexPack->getDLCParentPackId(),
                        &ullOfferID_Full
                    );
                    ...
                    ui.RequestMessageBox(
                        ...
                        IDS_WARNING_DLC_TRIALTEXTUREPACK_TEXT,
                        uiIDA,
                        2,
                        iPad,
...
std::wstring CMinecraftApp::FormatHTMLString(
    ...
    swprintf(
        replacements,
        64,
        L"<font color=\"#%08x\" shadowcolor=\"#%08x\">",
        GetHTMLColour(eHTMLColor_0),
        shadowColour
    );
...
std::wstring
CMinecraftApp::GetActionReplacement(int iPad, unsigned char ucAction) {
    unsigned int input = InputManager.GetGameJoypadMaps(
        InputManager.GetJoypadMapVal(iPad),
        ucAction
    );
...
HRESULT CMinecraftApp::RegisterMojangData(
    WCHAR*    pXuidName,
    PlayerUID xuid,
    WCHAR*    pSkin,
    WCHAR*    pCape
...
HRESULT CMinecraftApp::RegisterDLCData(
    WCHAR*        pType,
    WCHAR*        pBannerName,
    int           iGender,
    std::uint64_t ullOfferID_Full,
    std::uint64_t ullOfferID_Trial,
    WCHAR*        pFirstSkin,
...
#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
bool CMinecraftApp::GetDLCFullOfferIDForSkinID(
    const std::wstring& FirstSkin,
    ULONGLONG*          pullVal
) {
...
bool CMinecraftApp::GetDLCFullOfferIDForPackID(
    const int     iPackID,
    std::wstring& ProductId
) {
    auto it = DLCTextures_PackID.find(iPackID);
...
int CMinecraftApp::RemoteSaveThreadProc(void* lpParameter) {
    ...
    // line 38 + 8 bytes	C++
    // ...
    //  	default.exe!CMinecraftApp::RemoteSaveThreadProc(void * lpParameter)
    //  line 6694	C++
...
#ifdef _XBOX_ONE
void CMinecraftApp::AddLevelToBannedLevelList(
    int             iPad,
...
bool CMinecraftApp::IsInBannedLevelList(
    int       iPad,
    PlayerUID xuid,
    char*     pszLevelName
) {
...
void CMinecraftApp::RemoveLevelFromBannedLevelList(
    int       iPad,
    PlayerUID xuid,
    char*     pszLevelName
) {
...
void CMinecraftApp::SetGameHostOption(
...
unsigned int CMinecraftApp::GetGameHostOption(
    unsigned int    uiHostSettings,
    eGameHostOption eVal
) {
    ...
    case eGameHostOption_Tutorial:
        // special case - tutorial is offline, but we want the gamertag option,
        // and set Easy mode, structures on, fire on, tnt on, pvp on, trust
        // players on
        return (
            (uiHostSettings & GAME_HOST_OPTION_BITMASK_GAMERTAGS)
            | GAME_HOST_OPTION_BITMASK_TRUSTPLAYERS
            | GAME_HOST_OPTION_BITMASK_FIRESPREADS
            | GAME_HOST_OPTION_BITMASK_TNT | GAME_HOST_OPTION_BITMASK_PVP
...
bool CMinecraftApp::CanRecordStatsAndAchievements() {
    ...
    return !(
        app.GetGameHostOption(eGameHostOption_HasBeenInCreative)
...
void CMinecraftApp::GetImageTextData(
    PBYTE          pbImageData,
    DWORD          dwImageBytes,
        ...
        if (uiChunkType == PNG_TAG_tEXt) // tEXt
        {
            ...
            while (pszKeyword < ucPtr + uiCount + uiChunkLen) {
                ...
                if (strcmp(szKeyword, "4J_SEED") == 0) {
                    ...
                    unsigned int uiValueC = 0;
                    unsigned char
                        pszHostOptions[8]; // Hex representation of unsigned int
                    ...
                    unsigned int uiValueC = 0;
                    unsigned char
                        pszTexturePack[8]; // Hex representation of unsigned int
...
unsigned int CMinecraftApp::CreateImageTextData(
    PBYTE        bTextMetadata,
    std::int64_t seed,
    bool         hasSeed,
    unsigned int uiHostOptions,
    ...
    _itoa_s(
        uiHostOptions,
        (char*)&bTextMetadata[iTextMetadataBytes + 15],
        9,
        16
    );
...
void CMinecraftApp::AddTerrainFeaturePosition(
...
bool CMinecraftApp::GetTerrainFeaturePosition(
    _eTerrainFeatureType eType,
    int*                 pX,
    int*                 pZ
) {
    for (auto it = m_vTerrainFeatures.begin(); it < m_vTerrainFeatures.end();
...
void CMinecraftApp::UpdatePlayerInfo(
    BYTE         networkSmallId,
...
std::wstring CMinecraftApp::getEntityName(eINSTANCEOF type) {
    ...
    case eTYPE_LAVASLIME:
        ...
        // 4J-PB - fix for #107167 - Customer Encountered: TU12: Content: UI:
        // There is no information what killed Player after being slain by Iron
        // Golem.
...
unsigned int
CMinecraftApp::AddDLCRequest(eDLCMarketplaceType eType, bool bPromote) {
    // lock access
    ...
    for (auto it = m_DLCDownloadQueue.begin(); it != m_DLCDownloadQueue.end();
        ...
        if (pCurrent->dwType == m_dwContentTypeA[eType]) {
            ...
            if (pCurrent->eState == e_DLC_ContentState_Retrieving
                ...
                // promote
                if (bPromote) {
                    m_DLCDownloadQueue.erase(
                        m_DLCDownloadQueue.begin() + iPosition
                    );
                    m_DLCDownloadQueue.insert(
                        m_DLCDownloadQueue.begin(),
                        pCurrent
                    );
                }
                LeaveCriticalSection(&csDLCDownloadQueue);
...
unsigned int
CMinecraftApp::AddTMSPPFileTypeRequest(eDLCContentType eType, bool bPromote) {
#if !defined(__PS3__) && !defined(__ORBIS__) && !defined(__PSVITA__)
    // lock access
                ...
                // if(iIndex!=-1)
                {
                    ...
                    if (!bPresent) // retrieve it from TMSPP
                    {
                        ...
                        for (auto it = m_TMSPPDownloadQueue.begin();
                            ...
                            if (wcscmp(pDLC->wchBanner, pCurrent->wchFilename)
                                == 0) {
                                bAlreadyInQueue = true;
                                break;
                            }
    ...
    for (auto it = m_DLCDownloadQueue.begin(); it != m_DLCDownloadQueue.end();
        ...
        if (pCurrent->eState == e_DLC_ContentState_Idle) {
            ...
            C4JStorage::EDLCStatus status = StorageManager.GetDLCOffers(
                ProfileManager.GetPrimaryPad(),
                &CMinecraftApp::DLCOffersReturned,
                this,
                pCurrent->dwType
            );
...
#if !defined(__PS3__) && !defined(__ORBIS__) && !defined(__PSVITA__)
#ifdef _XBOX_ONE
int CMinecraftApp::TMSPPFileReturned(
    LPVOID pParam,
    int    iPad,
    int    iUserData,
    LPVOID lpvData,
...
int CMinecraftApp::TMSPPFileReturned(
    ...
    for (auto it = pClass->m_TMSPPDownloadQueue.begin();
         it != pClass->m_TMSPPDownloadQueue.end();
         ++it) {
        TMSPPRequest* pCurrent = *it;
#if defined(_XBOX) || defined(_WINDOWS64)
...
#endif
        {
            ...
            if (pFileData != NULL) {
                ...
                case e_DLC_TexturePackData: {
                    // 4J-PB - we need to allocate memory for the file data and
                    // copy into it, since the current data is a reference into
                    // the blob download memory
                ...
                case e_DLC_TexturePackData: {
                    app.DebugPrintf(
                        "--- Got texturepack data %ls\n",
                        pCurrent->wchFilename
                    );
                    // get the config value for the texture pack
                    int iConfig = app.GetTPConfigVal(pCurrent->wchFilename);
                    app.AddMemoryTPDFile(
                        iConfig,
                        pFileData->pbData,
                        pFileData->dwSize
                    );
...
#ifdef _XBOX_ONE
                app.DebugPrintf(
                    "TMSImageReturned failed (%ls)...\n",
                    wchFilename
                );
#else
                app.DebugPrintf(
                    "TMSImageReturned failed (%s)...\n",
                    szFilename
                );
#endif
            }
    ...
    for (auto it = m_TMSPPDownloadQueue.begin();
        ...
        if (pCurrent->eState == e_TMS_ContentState_Queued) {
            ...
            C4JStorage::ETMSStatus status = StorageManager.TMSPP_ReadFile(
                ProfileManager.GetPrimaryPad(),
                pCurrent->eStorageFacility,
                pCurrent->eFileTypeVal,
                szFilename,
                pCurrent->CallbackFunc,
                this
            );
            switch (status) {
            ...
            app.DebugPrintf(
                "RetrieveNextTMSPPContent - type = %d, %ls\n",
                pCurrent->eType,
                pCurrent->wchFilename
            );
            // eTitleStorageState
...
int CMinecraftApp::DLCOffersReturned(
    void* pParam,
    int   iOfferC,
    DWORD dwType,
    int   iPad
    ...
    for (auto it = pClass->m_DLCDownloadQueue.begin();
         it != pClass->m_DLCDownloadQueue.end();
         ++it) {
        DLCRequest* pCurrent = *it;
...
void CMinecraftApp::SetAdditionalSkinBoxes(
    DWORD     dwSkinID,
    SKIN_BOX* SkinBoxA,
    DWORD     dwSkinBoxC
) {
...
std::vector<ModelPart*>* CMinecraftApp::SetAdditionalSkinBoxes(
    DWORD                   dwSkinID,
    std::vector<SKIN_BOX*>* pvSkinBoxA
) {
    EntityRenderer* renderer =
        EntityRenderDispatcher::instance->getRenderer(eTYPE_PLAYER);
    Model*                   pModel      = renderer->getModel();
...
CMinecraftApp::GetAdditionalModelParts(DWORD dwSkinID) {
    EnterCriticalSection(&csAdditionalModelParts);
...
void CMinecraftApp::SetAnimOverrideBitmask(
    DWORD        dwSkinID,
...
DWORD CMinecraftApp::getSkinIdFromPath(const std::wstring& skin) {
    ...
    if (skin.size() >= 14) {
        ...
        // hex
        if (dlcSkin) ss << std::dec << skinValue.c_str();
        else ss << std::hex << skinValue.c_str();
        ss >> skinId;
...
std::wstring CMinecraftApp::getFilePath(
    DWORD        packId,
...
std::wstring CMinecraftApp::getRootPath(
    DWORD packId,
    bool  allowOverride,
    bool  bAddDataFolder
) {
