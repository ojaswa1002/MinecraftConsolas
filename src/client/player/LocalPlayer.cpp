...
void LocalPlayer::aiStep() {
    ...
    if (abilities.flying) {
        ...
        if (ullButtonsPressed & (1LL << MINECRAFT_ACTION_SNEAK_TOGGLE))
            yd -= (0.15 + 0.42); // 4J - for flying mode,
    ...
    if (!m_bIsIdle && InputManager.GetIdleSeconds(m_iPad) > PLAYER_IDLE_TIME) {
        ProfileManager
            .SetCurrentGameActivity(m_iPad, CONTEXT_PRESENCE_IDLE, false);
        m_bIsIdle = true;
    } else if (m_bIsIdle
...
void LocalPlayer::hurtTo(int newHealth, ETelemetryChallenges damageSource) {
    ...
    if (this->health <= 0) {
        ...
        TelemetryManager->RecordPlayerDiedOrFailed(
            GetXboxPad(),
            0,
            y,
            0,
            ...
            if (justPickedupWool) {
                ...
                for (unsigned int i = 0; i < 16; i++) {
                    if (pStats->getTotalValue(
                            GenericStats::itemsCollected(Tile::cloth_Id, i)
                        )
                        > 0)
                        woolCount++;
                }
                ...
                if (woolCount >= 16)
                    awardStat(
                        GenericStats::rainbowCollection(),
...
void LocalPlayer::setExperienceValues(
...
void LocalPlayer::handleMouseDown(int button, bool down) {
    ...
    if (down && minecraft->hitResult != NULL
        ...
        minecraft->gameMode
            ->continueDestroyBlock(x, y, z, minecraft->hitResult->f);
    ...
    if (minecraft->hitResult == NULL) {
        ...
        if (button == 0) {
            ...
            if (!((y == 0) || ((y == 127) && level->dimension->hasCeiling))
                || level->dimension->id == 1) {
                minecraft->gameMode
                    ->startDestroyBlock(x, y, z, minecraft->hitResult->f);
            }
        } else {
...
void LocalPlayer::handleCollectItem(std::shared_ptr<ItemInstance> item) {
    if (item != NULL) {
        ...
        gameMode->getTutorial()
            ->onTake(item, itemCountAnyAux, itemCountThisAux);
    }
