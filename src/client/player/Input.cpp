...
void Input::tick(LocalPlayer* player) {
    ...
    if (pMinecraft->localgameModes[iPad]->isInputAllowed(MINECRAFT_ACTION_LEFT)
    ...
    if (!player->abilities.flying) {
        if ((player->ullButtonsPressed & (1LL << MINECRAFT_ACTION_SNEAK_TOGGLE))
            && pMinecraft->localgameModes[iPad]->isInputAllowed(
    ...
    if (pMinecraft->localgameModes[iPad]->isInputAllowed(
        ...
        tx = InputManager.GetJoypadStick_RX(iPad)
           * (((float)
                   app.GetGameSettings(iPad, eGameSetting_Sensitivity_InGame))
