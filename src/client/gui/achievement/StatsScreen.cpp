...
void StatsScreen::postInit() {
    ...
    buttons.push_back(
        blockButton = new Button(
            BUTTON_BLOCKITEMSTATS_ID,
            width / 2 - 46,
            height - 52,
            100,
            20,
            language->getElement(L"stat.blocksButton")
        )
    );
    buttons.push_back(
        itemButton = new Button(
            ...
            100,
            20,
            language->getElement(L"stat.itemsButton")
        )
    );
...
void StatsScreen::GeneralStatisticsList::selectItem(
    int  item,
    bool doubleClick
) {}
...
void StatsScreen::StatisticsList::renderHeader(int x, int y, Tesselator* t) {
    ...
    if (sortColumn != -1) {
        ...
        parent
            ->blitSlotIcon(x + offset, y + SLOT_BG_Y, image, SLOT_BG_SIZE * 0);
    }
}

void StatsScreen::StatisticsList::clickedHeader(
    int headerMouseX,
    int headerMouseY
) {
    headerPressed = -1;
...
void StatsScreen::StatisticsList::renderStat(
    ItemStat* stat,
    int       x,
    int       y,
    bool      shaded
...
void StatsScreen::StatisticsList::renderMousehoverTooltip(
    ItemStat* stat,
    int       x,
    int       y
) {
...
void StatsScreen::ItemStatisticsList::renderHeader(
    int         x,
    int         y,
    Tesselator* t
) {
...
void StatsScreen::ItemStatisticsList::renderItem(
    int         i,
    int         x,
    int         y,
    int         h,
...
std::wstring
StatsScreen::ItemStatisticsList::getHeaderDescriptionId(int column) {
...
void StatsScreen::BlockStatisticsList::renderHeader(
    int         x,
    int         y,
    Tesselator* t
) {
...
void StatsScreen::BlockStatisticsList::renderItem(
    int         i,
    int         x,
    int         y,
    int         h,
...
std::wstring
StatsScreen::BlockStatisticsList::getHeaderDescriptionId(int column) {
    if (column == COLUMN_CRAFTED) {
        return L"stat.crafted";
    } else if (column == COLUMN_USED) {
        return L"stat.used";
