...
void CreateWorldScreen::init() {
    ...
    nameEdit = new EditBox(
        ...
        200,
        20,
        language->getElement(L"testWorld")
    ); // 4J - test - should be L"selectWorld.newWorld"
...
void CreateWorldScreen::updateResultFolder() {
    ...
    for (int i = 0; i < SharedConstants::ILLEGAL_FILE_CHARACTERS_LENGTH; i++) {
        ...
        while (
            (pos =
                 resultFolder.find(SharedConstants::ILLEGAL_FILE_CHARACTERS[i]))
            != std::wstring::npos
        ) {
...
void CreateWorldScreen::render(int xm, int ym, float a) {
    ...
    drawString(
        font,
        language->getElement(L"selectWorld.enterName"),
        width / 2 - 100,
        47,
        0xa0a0a0
    );
    ...
    drawString(
        font,
        language->getElement(L"selectWorld.enterSeed"),
        width / 2 - 100,
        104,
        0xa0a0a0
    );
    drawString(
        font,
        language->getElement(L"selectWorld.seedInfo"),
        width / 2 - 100,
        140,
        0xa0a0a0
    );
