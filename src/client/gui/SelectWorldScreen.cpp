...
void SelectWorldScreen::postInit() {
    ...
    buttons.push_back(
        selectButton = new Button(
            ...
            150,
            20,
            language->getElement(L"selectWorld.select")
        )
    );
    buttons.push_back(
        deleteButton = new Button(
            ...
            70,
            20,
            language->getElement(L"selectWorld.rename")
        )
    );
    buttons.push_back(
...
void SelectWorldScreen::WorldSelectionList::selectItem(
    int  item,
    bool doubleClick
) {
    parent->selectedWorld = item;
    bool active =
        (this->parent->selectedWorld >= 0
...
void SelectWorldScreen::WorldSelectionList::renderItem(
    ...
    std::int64_t size = levelSummary->getSizeOnDisk();
