...
void Gui::render(float a, bool mouseFree, int xMouse, int yMouse) {
    ...
    ScreenSizeCalculator
        ssc(minecraft->options, minecraft->width, minecraft->height, guiScale);
    ...
    case C4JRender::VIEWPORT_TYPE_SPLIT_BOTTOM:
        iSafezoneXHalf =
            ...
            / 10; // 5% (need to treat the whole screen is 2x this screen)
        iSafezoneYHalf =
            splitYOffset + screenHeight / 10; // 5%  (need to treat the whole
    ...
    ); // 4J - added - this did actually get set in renderVignette but that code
       // is currently commented out

    if (Minecraft::useFancyGraphics()) {
        renderVignette(
            minecraft->player->getBrightness(a),
            screenWidth,
            screenHeight
        );
    }
    ...
    if (!minecraft->gameMode->isCutScene()) {
        ...
        RenderManager.StateSetBlendFactor(
            0xffffff | (((unsigned int)fVal) << 24)
        );
        ...
        if (bDisplayGui) {
            ...
            if (bTwoPlayerSplitscreen) {
                ...
                blit(
                    iWidthOffset + (screenWidth - quickSelectWidth) / 2 - 1
                ...
                blit(
                    iWidthOffset + screenWidth / 2 - quickSelectWidth / 2,
                    iHeightOffset + screenHeight - iSafezoneYHalf
                        - iTooltipsYOffset,
                    0,
                    0,
                ...
                blit(
                    iWidthOffset + screenWidth / 2 - quickSelectWidth / 2 - 1
                        + inventory->selected * 20,
                    iHeightOffset + screenHeight - iSafezoneYHalf
                        - iTooltipsYOffset - 1,
                    0,
            ...
            if (bTwoPlayerSplitscreen) {
                blit(
                    iWidthOffset + screenWidth / 2 - 7,
                    (iHeightOffset + screenHeight) / 2 - 7,
                    0,
                    0,
                    16,
                    16
        ...
        if (bDisplayGui) {
            ...
            if (minecraft->gameMode->canHurtPlayer()) {
                ...
                for (int i = 0; i < FoodConstants::MAX_FOOD / 2; i++) {
                    ...
                    if (minecraft->player->getFoodData()->getSaturationLevel()
                        <= 0) {
                ...
                if (minecraft->player->isUnderLiquid(Material::water)) {
                    int count = (int)ceil(
                        (minecraft->player->getAirSupply() - 2) * 10.0f
                        / Player::TOTAL_AIR_SUPPLY
                    );
                    int extra = (int)ceil(
                                    (minecraft->player->getAirSupply()) * 10.0f
                                    / Player::TOTAL_AIR_SUPPLY
                                )
                              - count;
        ...
        ////////////////////////////
        if (bDisplayGui) {
            //		glDisable(GL_BLEND);		4J - removed - we want to be
            // able to fade our gui
        ...
        if (app.GetGameSettings(iPad, eGameSetting_AnimatedCharacter)) {
            ...
            if (bDisplayGui
                ...
                EntityRenderDispatcher::instance
                    ->render(minecraft->player, 0, 0, 0, 0, 1);
                ...
                minecraft->player->setSharedFlag(
                    Entity::FLAG_ONFIRE,
                    ofireflag
                );
                glPopMatrix();
    ...
    if (minecraft->player->getSleepTimer() > 0) {
        ...
        fill(
            0,
            0,
            screenWidth / fScaleFactorWidth,
            screenHeight / fScaleFactorHeight,
            color
        );
    ...
    if (minecraft->options->renderDebug) {
        ...
        font->drawShadow(
            ClientConstants::VERSION_STRING + L" (" + minecraft->fpsString
                + L")",
            iSafezoneXHalf + 2,
            20,
            0xffffff
        ...
        font->drawShadow(
            L"Seed: "
                + _toString<std::int64_t>(
                    minecraft->level->getLevelData()->getSeed()
                ),
            iSafezoneXHalf + 2,
            32 + 00,
            0xffffff
        ...
        font->drawShadow(
            minecraft->gatherStats1(),
            iSafezoneXHalf + 2,
            32 + 10,
            0xffffff
        );
        font->drawShadow(
            minecraft->gatherStats2(),
            iSafezoneXHalf + 2,
            32 + 20,
            0xffffff
        );
        font->drawShadow(
            minecraft->gatherStats3(),
            iSafezoneXHalf + 2,
            32 + 30,
            0xffffff
        );
        font->drawShadow(
            minecraft->gatherStats4(),
            iSafezoneXHalf + 2,
            32 + 40,
            0xffffff
        );
        ...
        // 10, 0xffffff);
        {
            ...
            String msg = "Used memory: " + (used * 100 / max) + "% (" + (used /
            1024 / 1024) + "MB) of " + (max / 1024 / 1024) + "MB";
            drawString(font, msg, screenWidth - font.width(msg) - 2, 2,
            0xe0e0e0); msg = "Allocated memory: " + (total * 100 / max) + "% ("
            + (total / 1024 / 1024) + "MB)"; drawString(font, msg, screenWidth -
        ...
        drawString(
            font,
            L"x: " + _toString<double>(minecraft->player->x) + L"/ Head: "
                + _toString<double>(xBlockPos) + L"/ Chunk: "
        ...
        drawString(
            font,
            L"y: " + _toString<double>(minecraft->player->y) + L"/ Head: "
                + _toString<double>(yBlockPos),
            iSafezoneXHalf + 2,
            iYPos + 8 * 1,
            0xe0e0e0
        );
        drawString(
            font,
            L"z: " + _toString<double>(minecraft->player->z) + L"/ Head: "
                + _toString<double>(zBlockPos) + L"/ Chunk: "
        ...
        if (minecraft->level != NULL
            ...
            drawString(
                font,
                L"b: " + biome->m_name + L" (" + _toString<int>(biome->id)
                    + L")",
                iSafezoneXHalf + 2,
                iYPos,
                0xe0e0e0
        ...
        for (int i = 0; i < XUSER_MAX_COUNT; i++) {
            if (minecraft->localplayers[i]
                ...
                while (guiMessages[i].size() > 50) {
                 ...
                 && app.GetGameSettings(iPad, eGameSetting_DeathMessages)
                        == 0)) {
...
void Gui::renderGraph(
    ...
    for (int i = 0; i < dataLength; i++) {
        ...
        if (dataA != NULL) {
            ...
            std::int64_t aVal = dataA[i] / dataAScale;

            t->vertex(
                (float)(xScale * i + 0.5f),
                (float)(height - aVal + 0.5f),
                (float)(0)
            );
            t->vertex(
                (float)(xScale * i + 0.5f),
        ...
        if (dataB != NULL) {
            ...
            std::int64_t bVal = dataB[i] / dataBScale;

            t->vertex(
                (float)(xScale * i + (xScale - 1) + 0.5f),
                (float)(height - bVal + 0.5f),
                (float)(0)
            );
            t->vertex(
                (float)(xScale * i + (xScale - 1) + 0.5f),
...
void Gui::renderStackedGraph(
    ...
    t->begin(GL_LINES);
    std::int64_t thisVal = 0;
    std::int64_t topVal  = 0;
    for (int i = 0; i < dataLength; i++) {
        thisVal = 0;
        topVal  = 0;
        int col = ((i - dataPos) & (dataLength - 1)) * 255 / dataLength;
        ...
        for (unsigned int horiz = 1; horiz < 7; ++horiz) {
            ...
            t->vertex(
                (float)(0 + 0.5f),
                (float)(height - (horiz * 100) + 0.5f),
                (float)(0)
            );
            t->vertex(
                (float)(dataLength + 0.5f),
                (float)(height - (horiz * 100) + 0.5f),
                (float)(0)
            );
        }
