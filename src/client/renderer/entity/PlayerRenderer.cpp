...
#include "client/renderer/tileentity/SkullTileRenderer.h"
...
int PlayerRenderer::prepareArmor(
    std::shared_ptr<Mob> _player,
    int                  layer,
    float                a
) {
    ...
    if (itemInstance != NULL) {
        ...
        if (dynamic_cast<ArmorItem*>(item)) {
            ...
            bindTexture(
                L"armor/" + MATERIAL_NAMES[armorItem->modelIndex] + L"_"
                + _toString<int>(layer == 2 ? 2 : 1) + L".png"
            );
...
void PlayerRenderer::prepareSecondPassArmor(
    ...
    if (itemInstance != NULL) {
        ...
        if (dynamic_cast<ArmorItem*>(item)) {
            ...
            bindTexture(
                L"armor/" + MATERIAL_NAMES[armorItem->modelIndex] + L"_"
                + _toString<int>(layer == 2 ? 2 : 1) + L"_b.png"
            );
...
void PlayerRenderer::renderName(
    ...
    if (Minecraft::renderNames() && mob != entityRenderDispatcher->cameraEntity
        ...
        )) // 4J-JEV: Todo, move to LivingEntityRenderer.
    {
        float  size = 1.60f;
...
void PlayerRenderer::additionalRendering(std::shared_ptr<Mob> _mob, float a) {
    ...
    if (headGear != NULL) {
        ...
        if ((uiAnimOverrideBitmask
             & (1 << HumanoidModel::eAnim_DontRenderArmour))
            == 0) {
            glPushMatrix();
            ...
            if (headGear->getItem()->id < 256) {
                ...
                this->entityRenderDispatcher->itemInHandRenderer
                    ->renderItem(mob, headGear, 0);
    ...
    if (bindTexture(mob->customTextureUrl2, L"") && !mob->isInvisible()) {
        ...
        flap += sin((mob->walkDistO + (mob->walkDist - mob->walkDistO) * a) * 6)
              * 32 * pow;
        if (mob->isSneaking()) {
            flap += 25;
        }
    ...
    if (item != NULL) {
        ...
        if (item->id < 256
            && TileRenderer::canRender(
                Tile::tiles[item->id]->getRenderShape()
            )) {
            float s = 8 / 16.0f;
        ...
        if (item->getItem()->hasMultipleSpriteLayers()) {
            for (int layer = 0; layer <= 1; layer++) {
                ...
                glColor4f(red, g, b, 1);
                this->entityRenderDispatcher->itemInHandRenderer
                    ->renderItem(mob, item, layer, false);
            }
        } else {
            this->entityRenderDispatcher->itemInHandRenderer
                ->renderItem(mob, item, 0);
        }
...
void PlayerRenderer::setupPosition(
    std::shared_ptr<Mob> _mob,
    double               x,
    double               y,
    double               z
...
void PlayerRenderer::renderShadow(
