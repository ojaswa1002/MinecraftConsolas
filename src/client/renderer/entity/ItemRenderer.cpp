...
void ItemRenderer::render(
    std::shared_ptr<Entity> _itemEntity,
    double                  x,
    double                  y,
    double                  z,
    float                   rot,
...
void ItemRenderer::renderItemBillboard(
...
void ItemRenderer::renderGuiItem(
    Font*                         font,
    Textures*                     textures,
...
void ItemRenderer::renderGuiItem(
    ...
    if (item->getIconType() == Icon::TYPE_TERRAIN
        ...
        glScalef(
            ...
            16.0f * fScaleY,
            1.0f
        );                              // Scale to 0 to 16*scale range
        glTranslatef(0.5f, 0.5f, 0.0f); // Translate to 0 to 1 range
        glScalef(
            0.55f,
            0.55f,
            -1.0f
        ); // Scale to occupy full -0.5 to 0.5 bounding region (just touching
           // top & bottom) 0.55 comes from 1/(1+sqrt(2)/sqrt(3)) which is
           // determined by the angles that the cube is rotated in an
        ...
        glRotatef(
            ...
            0.0f,
            0.0f
        ); // Rotate round x axis (centre at origin)
        glRotatef(
            45.0f,
            0.0f,
            1.0f,
            0.0f
        ); // Rotate round y axis (centre at origin)
#endif
...
void ItemRenderer::renderAndDecorateItem(
    ...
    if (isFoil || item->isFoil()) {
        ...
        glBlendFunc(
            GL_DST_COLOR,
            GL_ONE
        ); // 4J - changed blend equation from GL_DST_COLOR, GL_DST_COLOR so we
           // can fade this out
...
void ItemRenderer::renderGuiItemDecorations(
    ...
    if (item->isDamaged()) {
        int p = (int)Math::round(
            13.0
            - (double)item->getDamageValue() * 13.0
                  / (double)item->getMaxDamage()
        );
        int cc = (int)Math::round(
            255.0
            - (double)item->getDamageValue() * 255.0
                  / (double)item->getMaxDamage()
        );
        glDisable(GL_LIGHTING);
        glDisable(GL_DEPTH_TEST);
        ...
        fillRect(
            ...
            x + 3,
            y + 13,
            m_iPotionStrengthBarWidth[item->GetPotionStrength()],
            2,
            0x00e1eb
        );
