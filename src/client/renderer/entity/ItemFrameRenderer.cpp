...
void ItemFrameRenderer::render(
    std::shared_ptr<Entity> _itemframe,
    double                  x,
    double                  y,
    double                  z,
    float                   rot,
...
void ItemFrameRenderer::drawItem(std::shared_ptr<ItemFrame> entity) {
    ...
    glTranslatef(
        (-7.25f / 16.0f) * Direction::STEP_X[entity->dir],
        -0.18f,
    ...
    if (itemEntity->getItem()->getItem() == Item::map) {
        ...
        if (data != NULL) {
            entityRenderDispatcher->itemInHandRenderer->minimap->render(
                nullptr,
                entityRenderDispatcher->textures,
                data,
                entity->entityId
            );
        }
        ...
        if (itemEntity->getItem()->getItem() == Item::compass) {
            ...
            ct->updateFromPosition(
                ...
                entity->x,
                entity->z,
                Mth::wrapDegrees((float)(180 + entity->dir * 90)),
                false,
                true
            );
            ct->rot  = compassRot;
        ...
        EntityRenderDispatcher::instance
            ->render(itemEntity, 0, 0, 0, 0, 0, true);
