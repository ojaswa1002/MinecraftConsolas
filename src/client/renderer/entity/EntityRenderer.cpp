...
bool EntityRenderer::bindTexture(
...
void EntityRenderer::renderFlame(
...
void EntityRenderer::renderTileShadow(
    Tile*  tt,
    double x,
    double y,
    double z,
...
void EntityRenderer::renderFlat(
    float x0,
    float y0,
    float z0,
    float x1,
...
void EntityRenderer::postRender(
    ...
    if (!entityRenderDispatcher
             ->isGuiRender) // 4J - added, don't render shadow in gui as it uses
                            // its own blending, and we have globally enabled
                            // blending for interface opacity
    {
        if (bRenderPlayerShadow
            && entityRenderDispatcher->options->fancyGraphics
            && shadowRadius > 0 && !entity->isInvisible()) {
            double dist = entityRenderDispatcher
                              ->distanceToSqr(entity->x, entity->y, entity->z);
