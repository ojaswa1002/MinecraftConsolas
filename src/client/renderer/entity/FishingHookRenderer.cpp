...
void FishingHookRenderer::render(
    std::shared_ptr<Entity> _hook,
    double                  x,
    double                  y,
    double                  z,
    float                   rot,
    ...
    if (hook->owner != NULL) {
        ...
        vv->xRot(
            -(hook->owner->xRotO + (hook->owner->xRot - hook->owner->xRotO) * a)
            * PI / 180
        );
        vv->yRot(
            -(hook->owner->yRotO + (hook->owner->yRot - hook->owner->yRotO) * a)
            * PI / 180
        );
        vv->yRot(swing2 * 0.5f);
        vv->xRot(-swing2 * 0.7f);
