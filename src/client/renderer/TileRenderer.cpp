...
#include "world/level/tile/BrewingStandTile.h"
#include "world/level/tile/Bush.h"
#include "world/level/tile/CauldronTile.h"
#include "world/level/tile/CocoaTile.h"
#include "world/level/tile/DeadBushTile.h"
#include "world/level/tile/DiodeTile.h"
#include "world/level/tile/DirectionalTile.h"
...
#include "world/level/tile/FlowerPotTile.h"
...
#include "world/level/tile/TheEndPortalFrameTile.h"
#include "world/level/tile/ThinFenceTile.h"
#include "world/level/tile/Tile.h"
#include "world/level/tile/TreeTile.h"
#include "world/level/tile/TripWireSourceTile.h"
#include "world/level/tile/TripWireTile.h"
#include "world/level/tile/VineTile.h"
#include "world/level/tile/WallTile.h"
#include "world/level/tile/piston/PistonBaseTile.h"
#include "world/level/tile/piston/PistonExtensionTile.h"
    ...
    if (cache) {
        ...
        if ((id & 0xffff8000) == 0) // Check 0 <= id <= 32767
        {
            ...
            if (cache[id] & cache_isTranslucentAt_valid)
                return (
                    (cache[id] & cache_isTranslucentAt_flag)
...
float TileRenderer::getShadeBrightness(
    Tile*        tt,
    LevelSource* level,
    int          x,
    int          y,
    ...
    if (cache) {
        ...
        if ((id & 0xffff8000) == 0) // Check 0 <= id <= 32767
        {
            if (cache[id] & cache_isSolidBlockingTile_valid)
                return (
                    (cache[id] & cache_isSolidBlockingTile_flag) ? 0.2f : 1.0f
                );
...
int TileRenderer::getLightColor(
    Tile*        tt,
    LevelSource* level,
    int          x,
    int          y,
    ...
    if (cache) {
        ...
        if ((id & 0xffff8000) == 0) // Check 0 <= id <= 32767
        {
            ...
            if ((xx >= 0) && (xx <= 15) && (zz >= 0) && (zz <= 15) && (y >= 0)
                ...
                unsigned char ucTileId = tileIds
                    [offset
...
void TileRenderer::setFixedShape(
    float x0,
    float y0,
    float z0,
    float x1,
...
) // 4J added forceData, forceEntity param
{
    ...
    case Tile::SHAPE_BLOCK: {
        ...
        // checked noCulling and shouldRenderFace directly where faceFlags is
        // used now AP - I moved this check from
        // tesselateBlockInWorldWithAmbienceOcclusionTexLighting to be even
        // earlier to speed up early rejection. The flags are then passed down
        // to avoid creating them again. These changes in combination have more
        // than halved the time it takes to reject a block on Vita
        int faceFlags = 0;
        if (noCulling) {
            faceFlags = 0x3f;
        } else {
            // these block types can take advantage of a faster version of
            // shouldRenderFace there are others but this is an easy check which
            // covers the majority Note: This now covers rock, grass, dirt,
            // stoneBrice, wood, sapling, unbreakable, sand, gravel, goldOre,
            // ironOre, coalOre, treeTrunk
    ...
    case Tile::SHAPE_PORTAL_FRAME:
        retVal =
            tesselateAirPortalFrameInWorld((TheEndPortalFrameTile*)tt, x, y, z);
        break;
...
bool TileRenderer::tesselateAirPortalFrameInWorld(
    TheEndPortalFrameTile* tt,
    int                    x,
    int                    y,
    int                    z
) {
...
bool TileRenderer::tesselateBedInWorld(Tile* tt, int x, int y, int z) {
    ...
    // render wooden underside
    {
        ...
        if (SharedConstants::TEXTURE_LIGHTING) {
            ...
            t->color(
                r10 * centerBrightness,
                g10 * centerBrightness,
...
bool TileRenderer::tesselateBrewingStandInWorld(
    BrewingStandTile* tt,
    int               x,
    int               y,
    int               z
...
bool TileRenderer::tesselateCauldronInWorld(
    CauldronTile* tt,
    int           x,
    int           y,
    int           z
    ...
    const float cWidth =
        (2.0f / 16.0f)
        - (1.0f
           / 128.0f); // 4J - Moved by 1/128th (smallest movement possible with
                      // our vertex storage) to remove gap at edge of cauldron
    ...
    if (waterLevel > 0) {
        ...
        renderFaceUp(
            tt,
            x,
            y - 1.0f + (6.0f + waterLevel * 3.0f) / 16.0f,
            z,
            liquidTex
        );
    }
...
bool TileRenderer::tesselateFlowerPotInWorld(
    FlowerPotTile* tt,
    int            x,
    int            y,
    int            z
...
bool TileRenderer::tesselateAnvilInWorld(
    AnvilTile* tt,
    int        x,
    int        y,
    int        z,
...
float TileRenderer::tesselateAnvilPiece(
    AnvilTile* tt,
    int        x,
    int        y,
    int        z,
...
void TileRenderer::tesselateDiodeInWorld(
    DiodeTile* tt,
    int        x,
    int        y,
    int        z,
...
void TileRenderer::renderPistonArmUpDown(
    float x0,
    float x1,
    float y0,
    float y1,
...
void TileRenderer::renderPistonArmNorthSouth(
    float x0,
    float x1,
    float y0,
    float y1,
...
void TileRenderer::renderPistonArmEastWest(
    float x0,
    float x1,
    float y0,
    float y1,
...
) // 4J added forceData param
{
    ...
    case Facing::DOWN:
        ...
        t->tex2(
            getLightColor(tt, level, x, y, z)
        ); // 4J added - renderPistonArmDown doesn't set its own tex2 so just
           // inherited from previous tesselateBlockInWorld
        renderPistonArmUpDown(
            x + leftEdge,
            x + rightEdge,
            y + thickness,
            y + thickness + armLength,
            z + rightEdge,
            z + rightEdge,
            br * 0.8f,
            armLengthPixels
    ...
    case Facing::WEST:
        ...
        t->tex2(getLightColor(tt, level, x, y, z));
        renderPistonArmEastWest(
            x + thickness,
            x + thickness + armLength,
            y + leftEdge,
            y + leftEdge,
            z + rightEdge,
            z + leftEdge,
...
bool TileRenderer::tesselateTripwireSourceInWorld(
    Tile* tt,
    int   x,
    int   y,
    int   z
...
bool TileRenderer::tesselateDustInWorld(Tile* tt, int x, int y, int z) {
    ...
    bool w =
        RedStoneDustTile::shouldConnectTo(level, x - 1, y, z, Direction::WEST)
        || (!level->isSolidBlockingTile(x - 1, y, z)
            && RedStoneDustTile::shouldConnectTo(
                level,
                x - 1,
                y - 1,
                z,
    ...
    bool e =
        RedStoneDustTile::shouldConnectTo(level, x + 1, y, z, Direction::EAST)
        || (!level->isSolidBlockingTile(x + 1, y, z)
            && RedStoneDustTile::shouldConnectTo(
                level,
                x + 1,
    ...
    bool n =
        RedStoneDustTile::shouldConnectTo(level, x, y, z - 1, Direction::NORTH)
    ...
    bool s =
        RedStoneDustTile::shouldConnectTo(level, x, y, z + 1, Direction::SOUTH)
        || (!level->isSolidBlockingTile(x, y, z + 1)
            && RedStoneDustTile::shouldConnectTo(
                level,
                x,
    ...
    if (pic == 0) {
        ...
        t->vertexUV(
            ...
            (float)(y + dustOffset),
            (float)(z1),
            crossTexture->getU(u1, true),
            crossTexture->getV(v1)
        );
        t->vertexUV(
            (float)(x1),
            (float)(y + dustOffset),
            (float)(z0),
            crossTexture->getU(u1, true),
            crossTexture->getV(v0)
        );
        t->vertexUV(
            (float)(x0),
            (float)(y + dustOffset),
            (float)(z0),
            crossTexture->getU(u0, true),
            crossTexture->getV(v0)
        );
        t->vertexUV(
            (float)(x0),
        ...
        if (blsmooth > 0) {
            ...
            if (smoothShapeLighting
                && false) // minecraft->options->ambientOcclusion >=
                          ...
                          // we work out exactly what its peformance
                          // implications are/what it fixes
            {
                float _ll1 = (llx0Z + llxYZ + ll00Z + ll0YZ) / 4.0f;
...
int TileRenderer::blend(
    ...
    int top =
        (int)((double)((a >> 16) & 0xff) * fa + (double)((b >> 16) & 0xff) * fb
              + (double)((c >> 16) & 0xff) * fc
              + (double)((d >> 16) & 0xff) * fd)
        & 0xff;
...
void TileRenderer::renderFaceDown(
    ...
    if (downFlip == FLIP_CCW) {
        ...
        v00 = tex->getV(
            SharedConstants::WORLD_RESOLUTION - tileShapeX1 * 16.0f,
            true
        );
        u11 = tex->getU(tileShapeZ1 * 16.0f, true);
        v11 = tex->getV(
            SharedConstants::WORLD_RESOLUTION - tileShapeX0 * 16.0f,
            true
        );
        ...
        // reshape
        u00 = tex->getU(
            SharedConstants::WORLD_RESOLUTION - tileShapeZ1 * 16.0f,
            true
        );
        v00 = tex->getV(tileShapeX0 * 16.0f, true);
        u11 = tex->getU(
            SharedConstants::WORLD_RESOLUTION - tileShapeZ0 * 16.0f,
            true
        );
...
void TileRenderer::renderNorth(
    ...
    double v00 = tex->getV(
        SharedConstants::WORLD_RESOLUTION - tileShapeY1 * 16.0f,
        true
    );
    double v11 = tex->getV(
        SharedConstants::WORLD_RESOLUTION - tileShapeY0 * 16.0f,
        true
    );
    if (xFlipTexture) {
        double tmp = u00;
...
void TileRenderer::renderTile(
    ...
    if (shape == Tile::SHAPE_BLOCK || shape == Tile::SHAPE_TREE
        ...
        glRotatef(90, 0, 1, 0);
        glTranslatef(-0.5f, -0.5f, -0.5f);
        EntityTileRenderer::instance
            ->render(tile, data, brightness, fAlpha, setColor, useCompiled);
