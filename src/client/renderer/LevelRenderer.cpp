...
#include "client/particle/CritParticle2.h"
#include "client/particle/DragonBreathParticle.h"
#include "client/particle/DripParticle.h"
#include "client/particle/EchantmentTableParticle.h"
#include "client/particle/EnderParticle.h"
#include "client/particle/ExplodeParticle.h"
#include "client/particle/FlameParticle.h"
#include "client/particle/FootstepParticle.h"
#include "client/particle/HeartParticle.h"
#include "client/particle/HugeExplosionParticle.h"
#include "client/particle/HugeExplosionSeedParticle.h"
#include "client/particle/LavaParticle.h"
#include "client/particle/NetherPortalParticle.h"
...
#include "client/particle/SnowShovelParticle.h"
#include "client/particle/SpellParticle.h"
#include "client/particle/SplashParticle.h"
#include "client/particle/SuspendedParticle.h"
#include "client/particle/SuspendedTownParticle.h"
...
#include "world/item/RecordingItem.h"
#include "world/level/BlockDestructionProgress.h"
#include "world/level/Level.h"
#include "world/level/chunk/ChunkSource.h"
#include "world/level/chunk/LevelChunk.h"
#include "world/level/dimension/Dimension.h"
#include "world/level/material/Material.h"
#include "world/level/newbiome/layer/IntCache.h"
#include "world/level/tile/EggTile.h"
...
const int overworldSize = LEVEL_MAX_WIDTH + LevelRenderer::PLAYER_VIEW_DISTANCE
...
const int LevelRenderer::MAX_LEVEL_RENDER_SIZE[3] =
    {overworldSize, netherSize, endSize};
...
LevelRenderer::LevelRenderer(Minecraft* mc, Textures* textures) {
    ...
    totalChunks = offscreenChunks = occludedChunks = renderedChunks =
        emptyChunks                                = 0;
    for (int i = 0; i < 4; i++) {
        //		sortedChunks[i] = NULL;	// 4J - removed - not sorting our chunks
        // anymore
    ...
    chunkLists = MemoryTracker::genLists(
        getGlobalChunkCount() * 2
    ); // *2 here is because there is one renderlist per chunk here for each of
       // the opaque & transparent layers
    ...
    glDepthMask(
        false
    ); // 4J - added to get depth mask disabled within the command buffer
    float yy;
    int   s = 64;
    int   d = (256 / s) + 2;
    yy      = (float)(16);
    ...
    // HALO ring for the texture pack
    {
        ...
        for (unsigned int i = 0; i <= ARC_SEGMENTS; ++i) {
            ...
            t->vertexUV(
                ...
                (HALO_RING_RADIUS * sin(i * ARC_RADIANS)),
                0 - width,
                u,
                0
            );
...
void LevelRenderer::setLevel(int playerIndex, MultiPlayerLevel* level) {
    ...
    if (level != NULL) {
        ...
        if (chunks[playerIndex].data != NULL) {
            ...
            // not sorting our chunks anymore sortedChunks[playerIndex] = NULL;
            //// 4J - removed - not sorting our chunks anymore
        }
...
void LevelRenderer::allChanged(int playerIndex) {
    ...
    if (chunks[playerIndex].data != NULL) {
        ...
        delete chunks[playerIndex].data;
        //		delete sortedChunks[playerIndex];	// 4J - removed - not
        ...
        for (int y = 0; y < yChunks; y++) {
            for (int z = 0; z < zChunks; z++) {
                ...
                chunks[playerIndex][(z * yChunks + y) * xChunks + x].chunk->id =
                    count++;
                //				sortedChunks[playerIndex]->at((z * yChunks + y)
                //* xChunks + x) = chunks[playerIndex]->at((z * yChunks + y) *
                // xChunks + x);	// 4J - removed - not sorting our chunks
                // anymore
...
void LevelRenderer::renderEntities(Vec3* cam, Culler* culler, float a) {
    ...
    for (auto it = entities.begin(); it != itEndEnts; it++) {
        ...
        if ((entity->shouldRender(cam)
            ...
            if (!level[playerIndex]->hasChunkAt(
                    Mth::floor(entity->x),
                    0,
                    Mth::floor(entity->z)
                )) {
...
int LevelRenderer::render(
    ...
    if (xd * xd + yd * yd + zd * zd > 4 * 4) {
        ...
        //		sort(sortedChunks[playerIndex]->begin(),sortedChunks[playerIndex]->end(),
        // DistanceChunkSorter(player));	// 4J - removed - not sorting our
        // chunks anymore
    }
    Lighting::turnOff();
...
int LevelRenderer::renderChunks(int from, int to, int layer, double alpha) {
    ...
    auto itEnd = _renderChunks.end();
    for (auto it = _renderChunks.begin(); it != itEnd; it++) {
        Chunk* chunk = *it; //_renderChunks[i];

        int list = -1;
        for (int l = 0; l < lists; l++) {
            if (renderLists[l]
                    .isAt(chunk->xRender, chunk->yRender, chunk->zRender)) {
                list = l;
            }
        }
        if (list < 0) {
            list = lists++;
            renderLists[list].init(
                chunk->xRender,
                chunk->yRender,
                chunk->zRender,
                xOff,
...
void LevelRenderer::renderSky(float alpha) {
    if (mc->level->dimension->id == 1) {
        ...
        glDepthMask(false);
        textures->bind(
            textures->loadTexture(TN_MISC_TUNNEL)
        ); // 4J was L"/1_2_2/misc/tunnel.png"
        Tesselator* t = Tesselator::getInstance();
    ...
    float* c = level[playerIndex]->dimension->getSunriseColor(
        level[playerIndex]->getTimeOfDay(alpha),
        alpha
    );
    if (c != NULL) {
        glDisable(GL_TEXTURE_2D);
    ...
    glPushMatrix();
    {
        ...
        ); // 4J was L"/1_2_2/terrain/moon_phases.png"
...
void LevelRenderer::renderClouds(float alpha) {
    ...
    if (app.GetGameSettings(ProfileManager.GetPrimaryPad(), eGameSetting_Clouds)
        == 0) {
        return;
    }
    ...
    ); // 4J was L"/environment/clouds.png"
    glEnable(GL_BLEND);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
    ...
    double xo   = mc->cameraTargetPlayer->xo
...
bool LevelRenderer::updateDirtyChunks() {
    ...
    */
    PIXAddNamedCounter(
        ((float)memAlloc) / (1024.0f * 1024.0f),
...
#endif
        if (index == 1)
            dirtyChunkPresent = true; // 1 is a special value passed to let this
                                      // thread know that a chunk which isn't on
                                      // this stack has been set to dirty
        else if (index > 1) {
            int i2 = index - 2;
    ...
    if (!nearestClipChunks.empty()) {
        ...
        for (; index >= 0; --index) {
            ...
            if ((veryNearCount > 0))
                bAtomic =
                    true; // MGH -  if veryNearCount, then we're trying to
                          // rebuild atomically, so do it all on the main thread

            if (bAtomic || (index == 0)) {
                // PIXBeginNamedEvent(0,"Rebuilding near chunk %d %d
                // %d",chunk->x, chunk->y, chunk->z); 		static std::int64_t
                // totalTime =
                // 0; 		static std::int64_t countTime = 0;
                ...
                if (index != 0)
                    s_rebuildCompleteEvents->Set(
                        index - 1
                    ); // MGH - this rebuild happening on the main thread
                       // instead, mark the thread it should have been running
                       // on as complete
    ...
    // in an atomic unit
    if (veryNearCount > 1) {
        destroyedTileManager->updatedChunkAt(
            chunk->level,
            chunk->x,
            chunk->y,
            chunk->z,
...
void LevelRenderer::renderDestroyAnimation(
    Tesselator*             t,
    std::shared_ptr<Player> player,
    float                   a
) {
    ...
    if (!destroyingBlocks.empty()) {
        ...
        while (it != destroyingBlocks.end()) {
                           ...
                           // the list is shared in split screen
            {
                ...
                ); // 4J renamed to differentiate from tesselateInWorld
            }
            ++it;
        }
        ...
        for (int y = _y0; y <= _y1; y++) {
            for (int z = _z0; z <= _z1; z++) {
                ...
                if (index > -1) {
                    ...
                    PIXSetMarker(
                        0,
                        "Setting chunk %d %d %d dirty",
                        x * 16,
                        y * 16,
                        z * 16
                    );
#else
                    PIXSetMarkerDeprecated(
                        0,
                        "Setting chunk %d %d %d dirty",
                        x * 16,
                        y * 16,
                ...
                //				setGlobalChunkFlag(x * 16, y * 16, z * 16,
                // level, CHUNK_FLAG_DIRTY);
            }
        }
    }
    //	LeaveCriticalSection(&m_csDirtyChunks);
...
void LevelRenderer::playStreamingMusic(
...
void LevelRenderer::addParticle(
    ePARTICLE_TYPE eParticleType,
    double         x,
    double         y,
    double         z,
    double         xa,
...
std::shared_ptr<Particle> LevelRenderer::addParticleInternal(
    ...
    case eParticleType_reddust:
        particle = std::shared_ptr<Particle>(
            new RedDustParticle(lev, x, y, z, (float)xa, (float)ya, (float)za)
        );
        break;
    case eParticleType_snowballpoof:
        particle = std::shared_ptr<Particle>(
            new BreakingItemParticle(lev, x, y, z, Item::snowBall, textures)
        );
        break;
    ...
    case eParticleType_slime:
        particle = std::shared_ptr<Particle>(
            new BreakingItemParticle(lev, x, y, z, Item::slimeBall, textures)
        );
        break;
...
void LevelRenderer::levelEvent(
    ...
    case LevelEvent::SOUND_CLICK_FAIL:
        ...
        level[playerIndex]
            ->playLocalSound(x, y, z, eSoundType_RANDOM_CLICK, 1.0f, 1.2f);
        break;
    case LevelEvent::SOUND_CLICK:
        level[playerIndex]
            ->playLocalSound(x, y, z, eSoundType_RANDOM_CLICK, 1.0f, 1.0f);
        break;
    case LevelEvent::SOUND_LAUNCH:
        level[playerIndex]
            ->playLocalSound(x, y, z, eSoundType_RANDOM_BOW, 1.0f, 1.2f);
        break;
    ...
    case LevelEvent::PARTICLES_POTION_SPLASH: {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            std::shared_ptr<Particle> spellParticle = addParticleInternal(
                particleName,
                xp + xs * 0.1,
                yp + 0.3,
                zp + zs * 0.1,
    ...
    case LevelEvent::ENDERDRAGON_FIREBALL_SPLASH: {
        ...
        for (int i = 0; i < 200; i++) {
            ...
            std::shared_ptr<Particle> acidParticle = addParticleInternal(
                particleName,
                xp + xs * 0.1,
                yp + 0.3,
                zp + zs * 0.1,
    ...
    case LevelEvent::PARTICLES_DESTROY_BLOCK: {
        ...
        mc->particleEngine->destroy(
            x,
            y,
            z,
            data & Tile::TILE_NUM_MASK,
    ...
    case LevelEvent::PARTICLES_MOBTILE_SPAWN: {
        for (int i = 0; i < 20; i++) {
            double xP =
                x + 0.5 + (level[playerIndex]->random->nextFloat() - 0.5) * 2;
            double yP =
                y + 0.5 + (level[playerIndex]->random->nextFloat() - 0.5) * 2;
            double zP =
                z + 0.5 + (level[playerIndex]->random->nextFloat() - 0.5) * 2;

            level[playerIndex]
                ->addParticle(eParticleType_smoke, xP, yP, zP, 0, 0, 0);
            level[playerIndex]
                ->addParticle(eParticleType_flame, xP, yP, zP, 0, 0, 0);
        }
        break;
    }
        ...
        if (rci != NULL) {
            ...
            if (!mc->soundEngine->GetIsPlayingStreamingGameMusic()) {
                level[playerIndex]->playStreamingMusic(
                    ...
                    y,
                    z
                ); // 4J - used to pass NULL, but using empty string here now
                   // instead
            }
        }
    ...
    case LevelEvent::SOUND_ZOMBIE_CONVERTED:
        ...
        break;
        // 4J Added TU9 to fix #77475 - TU9: Content: Art: Dragon egg teleport
        // particle effect isn't present.
    case LevelEvent::END_EGG_TELEPORT:
        // 4J Added to show the paricles when the End egg teleports after being
        // attacked
...
void LevelRenderer::destroyTileProgress(
    int id,
    int x,
    int y,
    int z,
...
int LevelRenderer::getGlobalIndexForChunk(
    int x,
    int y,
    int z,
    int dimensionId
...
int LevelRenderer::getGlobalChunkCountForOverworld() {
    return (
        MAX_LEVEL_RENDER_SIZE[0] * MAX_LEVEL_RENDER_SIZE[0] * CHUNK_Y_COUNT
    );
}

unsigned char
LevelRenderer::getGlobalChunkFlags(int x, int y, int z, Level* level) {
...
void LevelRenderer::setGlobalChunkFlags(
    int           x,
    int           y,
    int           z,
    Level*        level,
...
void LevelRenderer::setGlobalChunkFlag(
    int           index,
    unsigned char flag,
    unsigned char shift
) {
...
void LevelRenderer::clearGlobalChunkFlag(
    int           x,
    int           y,
    int           z,
    Level*        level,
...
bool LevelRenderer::getGlobalChunkFlag(
    int           x,
    int           y,
    int           z,
    Level*        level,
...
unsigned char
LevelRenderer::incGlobalChunkRefCount(int x, int y, int z, Level* level) {
    int index = getGlobalIndexForChunk(x, y, z, level);
    if (index != -1) {
...
unsigned char
LevelRenderer::decGlobalChunkRefCount(int x, int y, int z, Level* level) {
    int index = getGlobalIndexForChunk(x, y, z, level);
    if (index != -1) {
        unsigned char flags = globalChunkFlags[index];
        unsigned char refCount =
...
void LevelRenderer::fullyFlagRenderableTileEntitiesToBeRemoved() {
    EnterCriticalSection(&m_csRenderableTileEntities);
    auto itChunkEnd = renderableTileEntities.end();
    for (auto it = renderableTileEntities.begin(); it != itChunkEnd; it++) {
        auto itTEEnd = it->second.end();
        for (auto it2 = it->second.begin(); it2 != itTEEnd; it2++) {
...
void LevelRenderer::DestroyedTileManager::destroyingTileAt(
    Level* level,
    int    x,
    int    y,
    int    z
...
void LevelRenderer::DestroyedTileManager::updatedChunkAt(
    Level* level,
    int    x,
    int    y,
    int    z,
...
void LevelRenderer::DestroyedTileManager::addAABBs(
    Level*    level,
    AABB*     box,
    AABBList* boxes
) {
...
void LevelRenderer::staticCtor() {
    ...
    for (unsigned int i = 0; i < MAX_CHUNK_REBUILD_THREADS; ++i) {
        ...
        if ((i % 3) == 0)
            ...
            ); // On Orbis, this core is also used for Matching 2, and that
               // priority of that seems to be always at default no matter what
               // we set it to. Prioritise this below Matching 2.
#endif
        } else if ((i % 3) == 2)
