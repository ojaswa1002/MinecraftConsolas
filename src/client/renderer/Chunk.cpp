...
#ifdef __PS3__
#include "ps3/chunkrebuild_spu/ChunkRebuildData.h"
#include "ps3/chunkrebuild_spu/TileRenderer_SPU.h"
#include "ps3/compressedtile_spu/CompressedTileStorage_SPU.h"
...
void Chunk::rebuild() {
    ...
    level->getChunkAt(x, z)->getBlockData(
        tileArray
    ); // 4J - TODO - now our data has been re-arranged, we could just extra the
       // vertical slice of this chunk rather than the whole thing
    ...
    if (bb) {
        bb->set(
            bounds.boundingBox[0],
            bounds.boundingBox[1],
            bounds.boundingBox[2],
            bounds.boundingBox[3],
            bounds.boundingBox[4],
            bounds.boundingBox[5]
        );
    }
...
bool Chunk::isEmpty() {
    ...
    return levelRenderer->getGlobalChunkFlag(
        ...
        z,
        level,
        LevelRenderer::CHUNK_FLAG_EMPTYBOTH
    );
}
...
void Chunk::setDirty() {
    ...
    __debugbreak();
    levelRenderer
        ->setGlobalChunkFlag(x, y, z, level, LevelRenderer::CHUNK_FLAG_DIRTY);
}

void Chunk::clearDirty() {
    levelRenderer
        ->clearGlobalChunkFlag(x, y, z, level, LevelRenderer::CHUNK_FLAG_DIRTY);
