...
#include "client/renderer/texture/PreStitchedTextureMap.h"
...
Textures::Textures(TexturePackRepository* skins, Options* options) {
    ...
    */
    ...
    terrain = new PreStitchedTextureMap(
        Icon::TYPE_TERRAIN,
        L"terrain",
        L"textures/blocks/",
        missingNo,
    ...
    items = new PreStitchedTextureMap(
        Icon::TYPE_ITEM,
        L"items",
        L"textures/items/",
        missingNo,
...
intArray Textures::loadTexturePixels(
    TEXTURE_NAME        texId,
...
#endif
                        int col = Texture::crispBlend(
                            Texture::crispBlend(c0, c1),
...
// texture bytes are already in order, and so doesn't do any of the extra
// copying round that the original java version does
void Textures::replaceTextureDirect(
    shortArray rawPixels,
    int        w,
    int        h,
    int        id
) {
...
HttpTexture* Textures::addHttpTexture(
    const std::wstring&   url,
...
MemTexture* Textures::addMemTexture(
...
void Textures::tick(
    bool updateTextures,
    bool tickDynamics
) // 4J added updateTextures parameter & tickDynamics
{
    MemSect(22);
    if (tickDynamics) {
        ...
        // RenderManager.TextureDynamicUpdateEnd();	// 4J added - see comment
        // above
    }
...
) // 4J was InputStream *in
{
    ...
    if (!skins->isUsingDefaultSkin()
        ...
        img   = skins->getSelected()->getImageResource(
            ...
            isTu,
            drive
        ); // new BufferedImage(name,false,isTu,drive);
    } else {
        ...
        if (IsOriginalImage(texId, name) || isTu) {
            ...
            img = skins->getDefault()->getImageResource(
                ...
                isTu,
                drive
            ); // new BufferedImage(L"/1_2_2" + name,false,isTu,drive);
        }
    }
