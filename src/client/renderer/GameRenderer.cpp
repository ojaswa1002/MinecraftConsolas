...
#include "util/java/FloatBuffer.h"
...
GameRenderer::GameRenderer(Minecraft* mc) {
    ...
    for (int i = 0; i < NUM_LIGHT_TEXTURES; i++)
        lightPixels[i] = intArray(
            (int*)RenderManager.allocIOMem(16 * 16 * sizeof(int)),
            16 * 16
        );
#else
    for (int i = 0; i < NUM_LIGHT_TEXTURES; i++)
...
void GameRenderer::tick(bool first) // 4J - add bFirst
{
    ...
    float brr = mc->level->getBrightness(
        Mth::floor(mc->cameraTargetPlayer->x),
...
void GameRenderer::bobView(float a) {
    ...
    glTranslatef(
        (float)Mth::sin(b * PI) * bob * 0.5f,
        -(float)abs(Mth::cos(b * PI) * bob),
        0
    );
...
void GameRenderer::getFovAndAspect(
    float& fov,
    float& aspect,
    float  a,
    bool   applyEffects
...
void GameRenderer::setupCamera(float a, int eye) {
    ...
    bool bNoBobbingAnim = (mc->player->getAnimOverrideBitmask()
                           & (1 << HumanoidModel::eAnim_NoBobbing))
                       != 0;
...
void GameRenderer::updateLightTexture(float a) {
    ...
    for (int j = 0; j < XUSER_MAX_COUNT; j++) {
        ...
        mc->textures->replaceTextureDirect(
            lightPixels[j],
            16,
            16,
...
float GameRenderer::getNightVisionScale(
    std::shared_ptr<Player> player,
    float                   a
) {
...
void GameRenderer::renderLevel(float a, std::int64_t until) {
    ...
    //	if (mc->cameraTargetPlayer == NULL)	// 4J - removed condition as we want
    // to update this is mc->player changes for different local players
        ...
        if (true) // (mc->options->fancyGraphics)
        {
            ...
            if (visibleWaterChunks > 0) {
                ...
                levelRenderer->render(
                    ...
                    a,
                    updateChunks
                ); // 4J - chanaged, used to be renderSameAsLast but we don't
                ...
                if (Tile::tiles[t]->material == Material::lava) {
                    mc->particleEngine->add(
                        std::shared_ptr<SmokeParticle>(new SmokeParticle(
                            level,
                            x + xa,
                            y + 0.1f - Tile::tiles[t]->getShapeY0(),
                            z + za,
                            0,
                            0,
                            0
                    ...
                    mc->particleEngine->add(
                        std::shared_ptr<WaterDropParticle>(
...
void GameRenderer::setupFog(int i, float alpha) {
    ...
    if (player->hasEffect(MobEffect::blindness)) {
        ...
        if (duration < 20) {
            distance =
                5.0f
                + (renderDistance - 5.0f) * (1.0f - (float)duration / 20.0f);
        }
