        ...
        for (int yp = 0; yp < 16; yp++)
            for (int xp = 0; xp < 16; xp++) {
                ...
                t->color(
                    ...
                    0.8f * br,
                    1.0f
                ); // MGH - added the color here, as the glColour below wasn't
                   // making it through to render
...
void ItemInHandRenderer::renderItem(
    ...
    if (item->getIconType() == Icon::TYPE_TERRAIN && tile != NULL
        ...
        tileRenderer->renderTile(
            tile,
            item->getAuxValue(),
            SharedConstants::TEXTURE_LIGHTING ? 1.0f : mob->getBrightness(1)
        ...
        if (item != NULL && item->isFoil() && layer == 0) {
            ...
            mc->textures->bind(
                mc->textures->loadTexture(TN__BLUR__MISC_GLINT)
            ); // 4J was L"%blur%/misc/glint.png"
            glEnable(GL_BLEND);
            ...
            glColor4f(
                ...
                0.8f * br,
                1
            ); // MGH - for some reason this colour isn't making it through to
               // the render, so I've added to the tesselator for the glint geom
               // above
            ...
            glPushMatrix();
            glScalef(ss, ss, ss);
            sx = System::currentTimeMillis() % (3000 + 1873) / (3000 + 1873.0f)
               * 8;
            glTranslatef(-sx, 0, 0);
            glRotatef(10, 0, 0, 1);
            renderItem3D(t, 0, 0, 1, 1, 256, 256, 1 / 16.0f, true);
...
void ItemInHandRenderer::renderItem3D(
    Tesselator* t,
    float       u0,
    float       v0,
    float       u1,
...
void ItemInHandRenderer::render(float a) {
    ...
    if (SharedConstants::TEXTURE_LIGHTING) {
        br      = 1;
        int col = mc->level->getLightColor(
            Mth::floor(player->x),
            Mth::floor(player->y),
            Mth::floor(player->z),
            0
    ...
    if (item != NULL && item->id == Item::map->id) {
        ...
        glTranslatef(
            0.0f,
            0.0f * d - (1 - h) * 1.2f - tilt * 0.5f + 0.04f,
            -0.9f * d
        );
        ...
        if (data != NULL)
            minimap
                ->render(mc->player, mc->textures, data, mc->player->entityId);
        PIXEndNamedEvent();
