...
//
// where: cccc		 is a 15-bit (5 bits per x/y/z) origin position / offset for
// the whole quad. Each
//					 component is unsigned, and offset by 16 so has a range 0 to
// 31 actually representing -16 to 15
...
//        v required by the quad ud,vd		 are 8-bit unsigned fixed pont UV
//        deltas, which can be added to umin/vmin to get umax, vmax
//					 and therefore define the 4 corners of an axis aligned UV
// mapping
//        i          is a code per vertex that indicates which of umin/umax
//        should be used for u, and which
//					 of vmin/vmax should be used for v for this vertex. The
// coding is: 						0 - u = umin, v = vmin
// 1 - u = umin, v = vmax 						2 - u = umax, v = vmin
// 3 - u = umax, v = vmax 						4 - not axis aligned, use uv
// stored in the vertex data 4 on from this one 		  ll		 is an 8-bit
// (4 bit per u/v) index into the current lighting texture
//
...
void Tesselator::packCompactQuad() {
    ...
    if (axisAligned) {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            data[i * 2 + 1] |= m_t2[i] << 16;
            data[8 + i * 2] =
                0; // This includes x/y coordinate of each vert as (0,0) so they
                   // will be interpreted as a zero area quad
...
void Tesselator::tileParticleQuad(
    float x1,
    float y1,
    float z1,
    float u1,
...
void Tesselator::vertex(float x, float y, float z) {
    ...
    if (useCompactFormat360) {
        ...
        if (vertices % 4 == 0
            && ((p >= size - 8 * 2)
                || ((p / 2)
                    >= 65532))) // Max 65535 verts in D3D, so 65532 is the last
                                // point at the end of a quad to catch it
#else
