...
public:
    ...
    void
    setFixedShape(float x0, float y0, float z0, float x1, float y1, float z1);
...
private:
    bool tesselateAirPortalFrameInWorld(
    ...
    bool
    tesselateBrewingStandInWorld(BrewingStandTile* tt, int x, int y, int z);
...
private:
    bool tesselatePistonBaseInWorld(
        Tile* tt,
        int   x,
        int   y,
        int   z,
    ...
    void renderPistonArmNorthSouth(
        float x0,
        float x1,
        float y0,
        float y1,
...
public:
    ...
    Icon*
    getTexture(Tile* tile, LevelSource* level, int x, int y, int z, int face);
