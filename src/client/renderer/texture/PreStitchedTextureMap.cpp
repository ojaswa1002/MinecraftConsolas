...
#include "client/renderer/texture/custom/ClockTexture.h"
#include "client/renderer/texture/custom/CompassTexture.h"
...
PreStitchedTextureMap::PreStitchedTextureMap(
    int                 type,
...
void PreStitchedTextureMap::stitch() {
    ...
    for (auto it = texturesToAnimate.begin(); it != texturesToAnimate.end();
         ++it) {
...
void PreStitchedTextureMap::loadUVs() {
    ...
    if (iconType != Icon::TYPE_TERRAIN) {
        ...
        texturesByName[L"grass_top"]->setFlags(
            Icon::IS_GRASS_TOP
        ); // 4J added for faster determination of texture type in tesselation
        texturesByName.insert(
            stringIconMap::value_type(
                L"stone",
                new SimpleIcon(
        ...
        texturesToAnimate.push_back(
            std::pair<std::wstring, std::wstring>(L"water_flow", L"water_flow")
        );
        texturesByName.insert(
            stringIconMap::value_type(
                L"sandstone_bottom",
