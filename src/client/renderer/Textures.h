...
private:
    ...
    // IntBuffer *pixels;	// 4J - removed so we don't have a permanent buffer
    // kicking round using up 1MB
...
public:
    intArray
    loadTexturePixels(TEXTURE_NAME texId, const std::wstring& resourceName);
...
public:
    ...
    void replaceTextureDirect(
        intArray rawPixels,
        int      w,
        int      h,
        int      id
    ); // 4J added as optimisation
    void replaceTextureDirect(
    ...
    HttpTexture*
    addHttpTexture(const std::wstring& url, HttpTextureProcessor* processor);
    ...
    MemTexture*
    addMemTexture(const std::wstring& url, MemTextureProcessor* processor);
    // MemTexture *	getMemTexture(const std::wstring& url, MemTextureProcessor
    // *processor);
    void removeMemTexture(const std::wstring& url);

    void tick(
        bool updateTextures,
        bool tickDynamics = true
...
public:
    ...
    ); // Moved this to public for Font.cpp access
