...
#include "world/level/tile/entity/TheEndPortalTileEntity.h"
#include "world/level/tile/entity/TileEntity.h"

#include "TileEntityRenderDispatcher.h"

void TheEndPortalRenderer::render(
    std::shared_ptr<TileEntity> _table,
    double                      x,
    double                      y,
    double                      z,
    float                       a,
    ...
    for (int i = 0; i < 16; i++) {
        ...
        float ss1 = (float)(dd + Camera::yPlayerOffs);
        glTranslatef(
            Camera::xPlayerOffs * dist / ss1,
            Camera::zPlayerOffs * dist / ss1,
            -yy
        );
        ...
        t->useProjectedTexture(
            true
        ); // 4J added - turns on both the generation of texture coordinates in
           // the vertex shader & perspective divide of the texture coord in the
           // pixel shader
        t->begin();
...
FloatBuffer*
TheEndPortalRenderer::getBuffer(float a, float b, float c, float d) {
    lb->clear();
