...
#include "world/level/tile/piston/PistonPieceEntity.h"

PistonPieceRenderer::PistonPieceRenderer() { tileRenderer = NULL; }

void PistonPieceRenderer::render(
    std::shared_ptr<TileEntity> _entity,
    double                      x,
    double                      y,
    double                      z,
    float                       a,
    ...
    if (tile != NULL
        && entity->getProgress(a)
               <= 1) // 4J - changed condition from < to <= as our chunk update
                     // is async to main thread and so we can have to render
                     // these with progress of 1
    {
        ...
        glColor4f(
            ...
            1,
            1
        ); // 4J added - this wouldn't be needed in real opengl as the block
           // render has vertex colours and so this isn't use, but our pretend
           // gl always modulates with this
        ...
        t->offset(
            (float)x - entity->x + entity->getXOff(a),
        ...
        if (tile == Tile::pistonExtension && entity->getProgress(a) < 0.5f) {
            ...
            tileRenderer->tesselateInWorldNoCulling(
                tile,
                entity->x,
                entity->y,
                entity->z,
