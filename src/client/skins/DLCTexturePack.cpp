...
#include "platform/dlc/DLCColourTableFile.h"
#include "platform/dlc/DLCFile.h"
...
#include "platform/dlc/DLCTextureFile.h"
#include "platform/dlc/DLCUIDataFile.h"
...
void DLCTexturePack::loadIcon() {
    if (m_dlcInfoPack
            ->doesPackContainFile(DLCManager::e_DLCType_Texture, L"icon.png")) {
...
std::wstring DLCTexturePack::getAnimationString(
...
BufferedImage* DLCTexturePack::getImageResource(
    ...
    if (m_dlcDataPack)
        ...
        return fallback->getImageResource(
            File,
            filenameHasExtension,
            bTitleUpdateTexture,
            drive
...
void DLCTexturePack::loadColourTable() {
    ...
    if (m_dlcDataPack != NULL
        && m_dlcDataPack->doesPackContainFile(
            DLCManager::e_DLCType_ColourTable,
            L"colours.col"
        )) {
    ...
    if (m_dlcDataPack != NULL
        ...
        swprintf(
            szResourceLocator,
            LOCATOR_SIZE,
            L"memory://%08X,%04X#HTMLColours.col",
            pbData,
            dwSize
        );
        BYTE* data;
        ...
        if (XuiResourceLoadAll(szResourceLocator, &data, &dataLength) == S_OK) {
            ...
            swprintf(
                szResourceLocator,
                LOCATOR_SIZE,
                L"memory://%08X,%04X#xuiscene_colourtable.xur",
                pbData,
                dwSize
            );
            HXUIOBJ hScene;
    ...
    if (mountIndex > -1) {
...
#endif
        {
            ...
            if (app.getLevelGenerationOptions())
                app.getLevelGenerationOptions()->setLoadedData();
            app.DebugPrintf(
                "Failed to mount texture pack DLC %d for pad %d\n",
                mountIndex,
                ProfileManager.GetPrimaryPad()
            );
        } else {
            m_bLoadingData = true;
            app.DebugPrintf(
                "Attempted to mount DLC data for texture pack %d\n",
                mountIndex
            );
        }
    } else {
...
std::wstring DLCTexturePack::getFilePath(
    DWORD        packId,
...
int DLCTexturePack::packMounted(
    LPVOID pParam,
    int    iPad,
    DWORD  dwErr,
    DWORD  dwLicenceMask
                ...
                */
                ...
                if (pack->getDLCItemsCount(DLCManager::e_DLCType_Audio) > 0) {
                    ...
                    Minecraft::GetInstance()->soundEngine->SetStreamingSounds(
                        iOverworldStart,
                        iOverworldStart + iOverworldC,
                        iNetherStart,
                        iNetherStart + iNetherC,
                        iEndStart,
                        iEndStart + iEndC,
                        iEndStart + iEndC
                    ); // push the CD start to after
                }
#endif
            }
...
void DLCTexturePack::loadUI() {
    ...
    if (m_dlcDataPack != NULL
        ...
        swprintf(
            szResourceLocator,
            LOCATOR_SIZE,
            L"memory://%08X,%04X#skin_Minecraft.xur",
            pbData,
            dwSize
        );
