...
void TexturePackRepository::addDebugPacks() {
    ...
    if (packedTestFile.exists()) {
        ...
        if (pack == NULL) {
            wprintf(
                L"Pack \"%ls\" is not installed, so adding it\n",
                L"DLCTestPack"
            );
            ...
            if (app.m_dlcManager.readDLCDataFile(
                    dwFilesProcessed,
                    "GAME:\\DummyTexturePack\\TexturePack.pck",
                    pack
                )) {
...
void TexturePackRepository::downloadWebSkin(
...
TexturePack*
TexturePackRepository::addTexturePackFromDLC(DLCPack* dlcPack, DWORD id) {
    ...
    if (dlcPack != NULL) {
        ...
        if (dlcPack->hasPurchasedFile(DLCManager::e_DLCType_TexturePack, L"")) {
            wprintf(
                L"Added new FULL DLCTexturePack: %ls - id=%d\n",
                dlcPack->getName().c_str(),
                dwParentID
            );
        } else {
            wprintf(
                L"Added new TRIAL DLCTexturePack: %ls - id=%d\n",
                dlcPack->getName().c_str(),
                dwParentID
            );
        }
