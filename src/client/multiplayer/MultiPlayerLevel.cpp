...
MultiPlayerLevel::ResetInfo::ResetInfo(
    int x,
    int y,
    int z,
    int tile,
...
: Level(
      std::shared_ptr<MockedLevelStorage>(new MockedLevelStorage()),
      L"MpServer",
...
void MultiPlayerLevel::tick() {
    ...
    for (auto connection = connectionsTemp.begin();
         connection < connectionsTemp.end();
         ++connection) {
        (*connection)->tick();
    }
    PIXEndNamedEvent();
...
bool MultiPlayerLevel::setTileAndDataNoUpdate(
    int x,
    int y,
    int z,
    int tile,
...
bool MultiPlayerLevel::doSetTileAndData(
    int x,
    int y,
    int z,
    int tile,
    ...
    for (int xx = -1; xx <= 1; xx++)
        for (int yy = -1; yy <= 1; yy++)
            for (int zz = -1; zz <= 1; zz++) {
                ...
                if ((cz + zz) < -LEVEL_MAX_WIDTH) continue;
                chunksToAnimate.insert(
                    (((cx + xx) & 0xff) << 16) | (((cy + yy) & 0xff) << 8)
                    | (((cz + zz) & 0xff))
                );
            }
}
...
void MultiPlayerLevel::playLocalSound(
    double x,
    double y,
    double z,
    int    iSound,
    ...
    if (minDistSq < fClipSoundDist * fClipSoundDist) {
        minecraft->soundEngine
            ->play(iSound, (float)x, (float)y, (float)z, volume, pitch);
    }
}
...
// levelchunks
void MultiPlayerLevel::removeUnusedTileEntitiesInRegion(
    int x0,
    int y0,
    int z0,
    int x1,
