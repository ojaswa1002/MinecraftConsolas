...
void MultiPlayerGameMode::creativeDestroyBlock(
    Minecraft*           minecraft,
...
void MultiPlayerGameMode::startDestroyBlock(int x, int y, int z, int face) {
    ...
    if (localPlayerMode->isCreative()) {
        ...
        if (t > 0 && destroyProgress == 0)
            Tile::tiles[t]
                ->attack(minecraft->level, x, y, z, minecraft->player);
        if (t > 0
...
bool MultiPlayerGameMode::useItemOn(
        ...
        if (bTestUseOnly) {
            ...
            case Tile::bed_Id: // special case for a bed
                if (Tile::tiles[t]->TestUse(level, x, y, z, player)) {
                    return true;
                } else if (t == Tile::bed_Id) // 4J-JEV: You can still use items
                                              // on record players (ie. set fire
                                              // to them).
                {
            ...
            if (Tile::tiles[t]
                    ->use(level, x, y, z, player, face, clickX, clickY, clickZ))
                didSomething = true;
        }
    }
    ...
    if (!bTestUseOnly) {
        connection->send(
            std::shared_ptr<UseItemPacket>(new UseItemPacket(
                x,
                y,
                z,
                face,
...
bool MultiPlayerGameMode::useItem(
    std::shared_ptr<Player>       player,
    Level*                        level,
    std::shared_ptr<ItemInstance> item,
    bool                          bTestUseOnly
...
MultiPlayerGameMode::createPlayer(Level* level) {
...
void MultiPlayerGameMode::attack(
    std::shared_ptr<Player> player,
    std::shared_ptr<Entity> entity
) {
    ensureHasSentCarriedItem();
    connection->send(
        std::shared_ptr<InteractPacket>(new InteractPacket(
            player->entityId,
            entity->entityId,
...
bool MultiPlayerGameMode::interact(
    std::shared_ptr<Player> player,
    std::shared_ptr<Entity> entity
) {
    ensureHasSentCarriedItem();
...
std::shared_ptr<ItemInstance> MultiPlayerGameMode::handleInventoryMouseClick(
    ...
    connection->send(
        std::shared_ptr<ContainerClickPacket>(new ContainerClickPacket(
            containerId,
            slotNum,
            buttonNum,
            quickKeyHeld,
...
void MultiPlayerGameMode::handleInventoryButtonClick(
    int containerId,
    int buttonId
) {
    connection->send(
...
void MultiPlayerGameMode::handleCreativeModeItemAdd(
    ...
    if (localPlayerMode->isCreative()) {
        connection->send(
            std::shared_ptr<SetCreativeModeSlotPacket>(
...
void MultiPlayerGameMode::releaseUsingItem(std::shared_ptr<Player> player) {
    ...
    connection->send(
        std::shared_ptr<PlayerActionPacket>(new PlayerActionPacket(
            PlayerActionPacket::RELEASE_USE_ITEM,
            0,
            0,
            0,
            255
