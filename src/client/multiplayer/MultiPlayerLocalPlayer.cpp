...
MultiplayerLocalPlayer::MultiplayerLocalPlayer(
    Minecraft*        minecraft,
    Level*            level,
    User*             user,
...
void MultiplayerLocalPlayer::tick() {
    ...
    // tempY, tempZ, x, y, z) )
    if (minecraft->localgameModes[m_iPad]
            ->getTutorial()
            ->canMoveToPosition(tempX, tempY, tempZ, x, y, z)) {
        sendPosition();
    } else {
    ...
    if (idle != lastIdle) {
        if (idle)
            connection->send(
                std::shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(
                    shared_from_this(),
                    PlayerCommandPacket::START_IDLEANIM
                ))
            );
        else
            connection->send(
                std::shared_ptr<PlayerCommandPacket>(new PlayerCommandPacket(
                    shared_from_this(),
                    PlayerCommandPacket::STOP_IDLEANIM
                ))
            );
...
std::shared_ptr<ItemEntity> MultiplayerLocalPlayer::drop() {
    connection->send(
        std::shared_ptr<PlayerActionPacket>(
...
void MultiplayerLocalPlayer::respawn() {
    connection->send(
        std::shared_ptr<ClientCommandPacket>(
...
void MultiplayerLocalPlayer::hurtTo(
    int                  newHealth,
...
void MultiplayerLocalPlayer::setAndBroadcastCustomSkin(DWORD skinId) {
    ...
    wprintf(
        L"Skin for local player %ls has changed to %ls (%d)\n",
        name.c_str(),
        customTextureUrl.c_str(),
...
#ifndef _CONTENT_PACKAGE
    wprintf(
        L"Cape for local player %ls has changed to %ls\n",
        name.c_str(),
    ...
    if (getCustomCape() != oldCapeIndex)
        connection->send(
            std::shared_ptr<TextureChangePacket>(new TextureChangePacket(
                shared_from_this(),
                TextureChangePacket::e_TextureChange_Cape,
