...
#include "client/gui/ErrorScreen.h"
#include "client/gui/Font.h"
#include "client/gui/Gui.h"
#include "client/gui/InBedChatScreen.h"
#include "client/gui/Screen.h"
#include "client/gui/ScreenSizeCalculator.h"
#include "client/gui/achievement/AchievementPopup.h"
...
#include "platform/ConsoleGameMode.h"
...
#include "platform/trial/TrialMode.h"
...
#include "world/level/storage/McRegionLevelStorageSource.h"
...
#include "platform/ui/all_platforms/IUIScene_CreativeMenu.h"
...
Minecraft::OS Minecraft::getPlatform() { return xbox; }
            ...
            if (localplayers[i] != NULL) {
                ...
                if (app.GetGameStarted()) {
                    if ((localplayers[i]->m_iScreenSection
                        ...
                        && (localplayers[i]->m_iScreenSection <= C4JRender::
...
bool Minecraft::addLocalPlayer(int idx) {
    ...
    if (success) {
        ...
        for (int i = 0; i < XUSER_MAX_COUNT; i++) {
            ...
            {
                XuiBroadcastMessage(
                    CXuiSceneBase::GetPlayerBaseScene(i),
                    &xuiMsg
                );
            }
        }
...
void Minecraft::addPendingLocalConnection(
    int               idx,
...
std::shared_ptr<MultiplayerLocalPlayer> Minecraft::createExtraLocalPlayer(
    ...
    if (clientConnection == m_pendingLocalConnections[idx]) {
        ...
        if (levelpassedin == NULL)
            level->addEntity(
                localplayers[idx]
            ); // Don't add if we're passing the level in, we only do this from
               // the client connection & we'll be handling adding it ourselves
        ...
        //		// 4J-PB - can't call this when this function is called from the
        // qnet thread (GetGameStarted will be false)
        // if(app.GetGameStarted())
        //		{
        ...
        // while (running)
        {
            ...
            for (int i = 0; i < XUSER_MAX_COUNT; i++) {
                ...
                if (m_pPsPlusUpsell != NULL && m_pPsPlusUpsell->hasResponse()
                    ...
                    if (InputManager
                            .ButtonPressed(i, MINECRAFT_ACTION_INVENTORY))
                        localplayers[i]->ullButtonsPressed |=
                    ...
                    if (InputManager
                            .ButtonPressed(i, MINECRAFT_ACTION_CRAFTING))
                        localplayers[i]->ullButtonsPressed |=
                            1LL << MINECRAFT_ACTION_CRAFTING;
                    if (InputManager
                            .ButtonPressed(i, MINECRAFT_ACTION_PAUSEMENU)) {
                        localplayers[i]->ullButtonsPressed |=
                    ...
                    if (localplayers[i]->abilities.flying) {
                        if (InputManager
                                .ButtonDown(i, MINECRAFT_ACTION_SNEAK_TOGGLE))
                            localplayers[i]->ullButtonsPressed |=
                    ...
                    if (InputManager
                            .ButtonPressed(i, MINECRAFT_ACTION_GAME_INFO))
                        localplayers[i]->ullButtonsPressed |=
                    ...
                    if (app.DebugSettingsOn() && app.GetUseDPadForDebug()) {
                        ...
                        if (InputManager
                                .ButtonPressed(i, MINECRAFT_ACTION_DPAD_RIGHT))
                            localplayers[i]->ullButtonsPressed |=
                                1LL << MINECRAFT_ACTION_CHANGE_SKIN;
                        if (InputManager
                                .ButtonPressed(i, MINECRAFT_ACTION_DPAD_UP))
                            localplayers[i]->ullButtonsPressed |=
                                1LL << MINECRAFT_ACTION_FLY_TOGGLE;
                        if (InputManager
                                .ButtonPressed(i, MINECRAFT_ACTION_DPAD_DOWN))
                            localplayers[i]->ullButtonsPressed |=
                                1LL << MINECRAFT_ACTION_RENDER_DEBUG;
                        if (InputManager
                                .ButtonPressed(i, MINECRAFT_ACTION_DPAD_LEFT))
                            localplayers[i]->ullButtonsPressed |=
...
#ifndef __PSVITA__
                        if (InputManager
                                .ButtonDown(i, MINECRAFT_ACTION_DPAD_LEFT)) {
                            localplayers[i]->ullDpad_this |=
                                1LL << MINECRAFT_ACTION_DPAD_LEFT;
                            dirCount++;
                        }
                        if (InputManager
                                .ButtonDown(i, MINECRAFT_ACTION_DPAD_RIGHT)) {
                            localplayers[i]->ullDpad_this |=
                                1LL << MINECRAFT_ACTION_DPAD_RIGHT;
                            dirCount++;
                        }
                        if (InputManager
                                .ButtonDown(i, MINECRAFT_ACTION_DPAD_UP)) {
                            localplayers[i]->ullDpad_this |=
                                1LL << MINECRAFT_ACTION_DPAD_UP;
                            dirCount++;
                        }
                        if (InputManager
                                .ButtonDown(i, MINECRAFT_ACTION_DPAD_DOWN)) {
                            localplayers[i]->ullDpad_this |=
                    ...
                    if (InputManager
                            .ButtonPressed(i, MINECRAFT_ACTION_LEFT_SCROLL)
                        || InputManager
                               .ButtonPressed(i, MINECRAFT_ACTION_RIGHT_SCROLL))
...
void Minecraft::renderFpsMeter(std::int64_t tickTime) {
    ...
    Minecraft::tickTimes
        [(Minecraft::frameTimePos) & (Minecraft::frameTimes_length - 1)] =
            tickTime;
    Minecraft::frameTimes
        [(Minecraft::frameTimePos++) & (Minecraft::frameTimes_length - 1)] =
            now - lastTimer;
    lastTimer = now;
    ...
    t->vertex(
        (float)(Minecraft::frameTimes_length),
        (float)(height),
        (float)(0)
    );
    t->vertex(
    ...
    for (int i = 0; i < Minecraft::frameTimes_length; i++) {
        int col =
            ((i - Minecraft::frameTimePos) & (Minecraft::frameTimes_length - 1))
...
void Minecraft::resize(int width, int height) {
    ...
    if (screen != NULL) {
        ...
        int                  screenWidth  = ssc.getWidth();
        int                  screenHeight = ssc.getHeight();
        //        screen->init(this, screenWidth, screenHeight);	// 4J - TODO
        //        - put back in
    }
}
...
void Minecraft::tick(bool bFirst, bool bUpdateTextures) {
    ...
    if (screen == NULL && player != NULL) {
        if (player->getHealth() <= 0 && !ui.GetMenuDisplayed(iPad)) {
            setScreen(NULL);
                   ...
                   && level->isClientSide) {
            //            setScreen(new InBedChatScreen());		// 4J - TODO put
            //            back in
        }
    } else if (screen != NULL
    ...
    if (screen == NULL && !ui.GetMenuDisplayed(iPad)) {
        ...
        *piAction = -1;
            ...
            *piUse = IDS_TOOLTIPS_WAKEUP;
            ...
            if (hitResult != NULL) {
                ...
                case HitResult::TILE: {
                     ...
                     *	for noteblocks, enderportals and flowerpots in case of
                     * non-standard items. (ie. ignite behaviour)
                     */
                    ...
                    case Tile::cauldron_Id:
                        // special case for a cauldron of water and an empty
                        // bottle
                        if (itemInstance) {
                    ...
                    case Tile::cake_Id:
                        if (player->abilities.instabuild) // if in creative
                                                          // mode, we will mine
                        {
                            *piAction = IDS_TOOLTIPS_MINE;
                        } else {
                            if (player->getFoodData()
                                    ->needsFood()) // 4J-JEV: Changed from
                                                   // healthto hunger.
                            {
                ...
                case HitResult::ENTITY:
                    ...
                    case eTYPE_MUSHROOMCOW:
                        ...
                        if (player->inventory->IsHeldItem()) {
                            ...
                            case Item::bowl_Id:
                            case Item::
                                bucket_empty_Id: // You can milk a mooshroom
                                                 // with either a bowl (mushroom
                                                 // soup) or a bucket (milk)!
                                *piUse = IDS_TOOLTIPS_MILK;
                                break;
                            case Item::shears_Id: {
                                if (player->isAllowedToAttackAnimals())
                            ...
                            // 4J-PB - Fix for #13081 - No tooltip is displayed
                            // for hitting a cow when you have nothing in your
                            // hand nothing in your hand
                            if (player->isAllowedToAttackAnimals())
                                *piAction = IDS_TOOLTIPS_HIT;
                        }
                        break;
                    ...
                    case eTYPE_BOAT:
                        *piAction = IDS_TOOLTIPS_MINE;
                        ...
                        if (dynamic_pointer_cast<Boat>(player->riding)
                            != NULL) {
                    ...
                    case eTYPE_MINECART:
                        *piAction = IDS_TOOLTIPS_MINE;
                        ...
                        if (dynamic_pointer_cast<Minecart>(player->riding)
                            != NULL) {
                            *piUse = IDS_TOOLTIPS_EXIT;
                        } else {
                            switch (dynamic_pointer_cast<Minecart>(
                                        hitResult->entity
                            )
                                        ->type) {
                    ...
                    case eTYPE_SHEEP:
                        ...
                        if (player->inventory->IsHeldItem()) {
                            ...
                            case Item::dye_powder_Id: {
                                ...
                                // convert to tile-based color value (0 is white
                                // instead of black)
                                int newColor =
                                    ClothTile::getTileDataForItemAuxValue(
                                        heldItem->getAuxValue()
                                    );

                                // can only use a dye on sheep that haven't been
                                // sheared
                                if (!(sheep->isSheared()
                            ...
                            case Item::shears_Id: {
                                ...
                                // can only shear a sheep that hasn't been
                                // sheared
                                if (!sheep->isSheared()) {
                                    *piUse = IDS_TOOLTIPS_SHEAR;
                                }
                    ...
                    case eTYPE_PIG:
                        ...
                        if (dynamic_pointer_cast<Pig>(player->riding) != NULL) {
                            *piUse = IDS_TOOLTIPS_EXIT;
                            ...
                            if (dynamic_pointer_cast<Pig>(hitResult->entity)
                                    ->hasSaddle()) {
                                *piUse = IDS_TOOLTIPS_RIDE;
                            } else if (!dynamic_pointer_cast<Pig>(
                                            hitResult->entity
                                       )
                                            ->isBaby()) {
                                if (player->inventory->IsHeldItem()) {
                        ...
                        break;
                    case eTYPE_WOLF:
                        // can be tamed, fed, and made to sit/stand, or enter
                        // love mode
                        {
                            int                           iID      = -1;
                            std::shared_ptr<ItemInstance> heldItem = nullptr;
                            std::shared_ptr<Wolf>         wolf =
                                dynamic_pointer_cast<Wolf>(hitResult->entity);
                            ...
                            case Item::bone_Id:
                                ...
                                break;
                            case Item::enderPearl_Id:
                                // Use is throw, so don't change the tips for
                                // the wolf
                                break;
                            case Item::dye_powder_Id:
                                if (wolf->isTame()) {
                                    if (ClothTile::getTileDataForItemAuxValue(
                                            heldItem->getAuxValue()
                                        )
                                        != wolf->getCollarColor()) {
                    ...
                    case eTYPE_OZELOT: {
                        ...
                        std::shared_ptr<Ozelot>       ocelot =
                            dynamic_pointer_cast<Ozelot>(hitResult->entity);
                        ...
                        if (ocelot->isTame()) {
                            // 4J-PB - if you have a raw fish in your hand, you
                            // will feed the ocelot rather than have it
                            // sit/follow
                    ...
                    case eTYPE_PLAYER: {
                        // Fix for #58576 - TU6: Content: Gameplay: Hit button
                        // prompt is available when attacking a host who has
                        // "Invisible" option turned on
                        std::shared_ptr<Player> TargetPlayer =
                            dynamic_pointer_cast<Player>(hitResult->entity);

                        if (!TargetPlayer
                                 ->hasInvisiblePrivilege()) // This means they
                                                            // are invisible,
                                                            // not just that
                                                            // they have the
                                                            // privilege
                        {
                            if (app.GetGameHostOption(eGameHostOption_PvP)
                    ...
                    case eTYPE_ITEM_FRAME: {
                        std::shared_ptr<ItemFrame> itemFrame =
                            dynamic_pointer_cast<ItemFrame>(hitResult->entity);
                    ...
                    case eTYPE_VILLAGER: {
                        std::shared_ptr<Villager> villager =
                            dynamic_pointer_cast<Villager>(hitResult->entity);
                        if (!villager->isBaby()) {
                    ...
                    case eTYPE_ZOMBIE: {
                        std::shared_ptr<Zombie> zomb =
                            dynamic_pointer_cast<Zombie>(hitResult->entity);
                        std::shared_ptr<ItemInstance> heldItem = nullptr;
                        ...
                        if (zomb->isVillager()
                            ...
                            && heldItem != NULL
                            && heldItem->getItem()->id == Item::apple_gold_Id) {
        ...
        int wheel = 0;
        if (InputManager.GetValue(iPad, MINECRAFT_ACTION_LEFT_SCROLL, true) > 0
            && gameMode->isInputAllowed(MINECRAFT_ACTION_LEFT_SCROLL)) {
            wheel = 1;
        } else if (InputManager
                           .GetValue(iPad, MINECRAFT_ACTION_RIGHT_SCROLL, true)
                       > 0
        ...
        */
        if (player->isUsingItem()) {
            ...
            if (player->abilities.instabuild) {
                ...
                bool didClick = player->creativeModeHandleMouseClick(
                    1,
                    InputManager.ButtonDown(iPad, MINECRAFT_ACTION_USE)
                );
        ...
        if ((player->ullButtonsPressed & (1LL << MINECRAFT_ACTION_INVENTORY))
            ...
            std::shared_ptr<LocalPlayer> player =
                dynamic_pointer_cast<LocalPlayer>(
        ...
        if ((player->ullButtonsPressed & (1LL << MINECRAFT_ACTION_CRAFTING))
            ...
            else if ((hitResult != NULL) && (hitResult->type == HitResult::TILE)
                     && (level
                             ->getTile(hitResult->x, hitResult->y, hitResult->z)
                         == Tile::workBench_Id)) {
                // ui.PlayUISFX(eSFX_Press);
        ...
        if (
            (player->ullButtonsPressed & (1LL << MINECRAFT_ACTION_PAUSEMENU))
#ifdef _DURANGO
        ...
        std::uint64_t ullButtonsPressed = player->ullButtonsPressed;
        ...
        if (pTouchData->reportNum == 1) {
            ...
            if ((pTouchData->report[0].x > QuickSelectRect[iHudSize].left)
                ...
                && (pTouchData->report[0].y
                    < QuickSelectRect[iHudSize].bottom)) {
                    ...
                    if (localplayers[idx] != NULL) {
                        if (localplayers[idx]->level == levels[i]) {
                            ...
                            std::shared_ptr<Mob> cameraEntity =
                                cameraTargetPlayer;
                            double xOff = cameraEntity->xOld
                                        + (cameraEntity->x - cameraEntity->xOld)
                                              * timer->a;
                            double yOff = cameraEntity->yOld
                                        + (cameraEntity->y - cameraEntity->yOld)
                                              * timer->a;
                            double zOff = cameraEntity->zOld
...
// this one void Minecraft::setLevel(Level *level, const std::wstring& message,
...
void Minecraft::prepareLevel(int title) {
    ...
    for (int x = -r; x <= r; x += 16) {
        for (int z = -r; z <= r; z += 16) {
            if (progressRenderer != NULL)
                this->progressRenderer->progressStagePercentage(
                    (pp++) * 100 / max
                );
...
void Minecraft::startAndConnectTo(
    const std::wstring& name,
    ...
    */
    ...
    {
        if (userName != L""
            && sid != L"") // 4J - username & side were compared with NULL
...
void Minecraft::main() {
    ...
    {
        name = L"Player"
             + _toString<std::int64_t>(System::currentTimeMillis() % 1000);
        sessionId = L"-";
...
int Minecraft::maxSupportedTextureSize() {
    ...
    // 0, GL11.GL_TEXTURE_WIDTH); 	if (width != 0) { 		return texSize;
    //	}
    // }
    // return -1;
}
...
#ifdef _DURANGO
void Minecraft::inGameSignInCheckAllPrivilegesCallback(
    LPVOID lpParam,
    bool   hasPrivileges,
    int    iPad
) {
    ...
    if (!hasPrivileges) {
        ...
        if (!g_NetworkManager.SessionHasSpace()) {
            ...
            uiIDA[0] = IDS_OK;
            ui.RequestMessageBox(
                IDS_MULTIPLAYER_FULL_TITLE,
                IDS_MULTIPLAYER_FULL_TEXT,
                uiIDA,
                1
            );
            ProfileManager.RemoveGamepadFromGame(iPad);
...
bool Minecraft::addPendingClientTextureRequest(
    ...
    auto it = find(
        m_pendingTextureRequests.begin(),
        m_pendingTextureRequests.end(),
        textureName
    );
...
int Minecraft::MustSignInReturnedPSN(
    void*                      pParam,
    int                        iPad,
