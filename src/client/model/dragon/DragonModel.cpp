...
void DragonModel::prepareMobModel(
...
void DragonModel::render(
    ...
    for (int i = 0; i < 5; i++) {
        ...
        neck->xRot = rr
                   + (float)(dragon->getHeadPartYOffset(i, start, p)) * PI
                         / 180.0f * rotScale
                         * 5.0f; // 4J replaced "p[1] - start[1]" with call to
                                 // getHeadPartYOffset
    ...
    head->xRot = (float)(dragon->getHeadPartYOffset(6, start, p)) * PI / 180.0f
               * rotScale * 5.0f; // 4J Added
