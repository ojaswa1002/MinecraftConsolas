...
void AddEntityPacket::_init(
    ...
    this->type = type;
    this->data = data;
    if (data > -1) // 4J - changed "no data" value to be -1, we can have a valid
                   // entity id of 0
    {
        double xd = e->xd;
        double yd = e->yd;
...
AddEntityPacket::AddEntityPacket(
    ...
    _init(e, type, -1, xp, yp, zp, yRotp, xRotp); // 4J - changed "no data"
                                                  // value to be -1, we can have
                                                  // a valid entity id of 0
}
...
void AddEntityPacket::read(
    DataInputStream* dis
) // throws IOException  TODO 4J JEV add throws statement
{
    id   = dis->readShort();
    type = dis->readByte();
#ifdef _LARGE_WORLDS
