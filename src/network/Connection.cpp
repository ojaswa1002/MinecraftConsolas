...
#include "util/java/InputOutputStream/BufferedOutputStream.h"
...
Connection::~Connection() {
    ...
    running = false;
    if (dis)
        dis->close(); // The input stream needs closed before the readThread, or
                      // the readThread may get stuck whilst blocking waiting on
                      // a read
