...
#include "psvita/psvitaextras/zlib.h"
#elif defined __PS3__
#include "ps3/EdgeZLib.h"
#endif //__PS3__
...
HRESULT Compression::CompressLZXRLE(
    void*         pDestination,
...
HRESULT Compression::CompressRLE(
    void*         pDestination,
    unsigned int* pDestSize,
    void*         pSource,
    unsigned int  SrcSize
...
HRESULT Compression::DecompressRLE(
    void*         pDestination,
    unsigned int* pDestSize,
    void*         pSource,
    unsigned int  SrcSize
...
HRESULT Compression::Decompress(
    void*         pDestination,
    unsigned int* pDestSize,
    void*         pSource,
    unsigned int  SrcSize
...
VOID Compression::VitaVirtualDecompress(
    ...
    void*         pSource,
    unsigned int  SrcSize
) // (LPVOID buf, SIZE_T dwSize, LPVOID dst)
{
...
HRESULT Compression::DecompressWithType(
    void*         pDestination,
    unsigned int* pDestSize,
    void*         pSource,
    unsigned int  SrcSize
    ...
    case eCompressionType_RLE: // 4J-JEV, RLE is just that; don't want to break
                               // here though.
    ...
    case eCompressionType_PS3ZLIB:
     ...
     || defined                                         _WIN64)
        // Note that we're missing the normal zlib header and footer so we'll
        // use inflate to decompress the payload and skip all the CRC checking,
        // etc
        if (pDestination != NULL) {
            ...
            inflateEnd(&strm); // MGH - added, to clean up zlib, was causing a
