...
#include "util/java/InputOutputStream/OutputStream.h"
...
private:
    ...
    public:
        ...
        virtual void
        write(byteArray b, unsigned int offset, unsigned int length);
        virtual void close();
        virtual void flush() {}
    };
...
public:
    ...
    Socket(
        bool response = false
    ); // 4J - Create a local socket, for end 0 or 1 of a connection
    Socket(
        INetworkPlayer* player,
        bool            response  = false,
        bool            hostLocal = false
    ); // 4J - Create a socket for an INetworkPlayer
    SocketAddress* getRemoteSocketAddress();
    void
    pushDataToQueue(const BYTE* pbData, DWORD dwDataSize, bool fromHost = true);
