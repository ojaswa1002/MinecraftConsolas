...
Random::Random(std::int64_t seed) { setSeed(seed); }

void Random::setSeed(std::int64_t s) {
...
double Random::nextDouble() {
    return (((std::int64_t)next(26) << 27) + next(27)) / (double)(1LL << 53);
}
...
int Random::nextInt(int n) {
    ...
    if ((n & -n) == n) // i.e., n is a power of 2
        return (int)(((std::int64_t)next(31) * n)
                     >> 31); // 4J Stu - Made std::int64_t instead of long
...
std::int64_t Random::nextLong() {
    return ((std::int64_t)next(32) << 32) + next(32);
}
