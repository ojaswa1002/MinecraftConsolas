...
File::File(const std::wstring& pathname) //: parent( NULL )
{
    ...
    // 	printf("File::File - %s\n",buf);
    // #endif
    if (pathname.empty()) m_abstractPathName = std::wstring(L"");
    else m_abstractPathName = pathname;
...
std::vector<File*>* File::listFiles() const {
    ...
    else sprintf(filePath, "%s/%s", getUsrDirPath(), lpFileName);
    int         fd;
...
#ifdef _UNICODE
    return exists()
        && (GetFileAttributes(getPath().c_str()) & FILE_ATTRIBUTE_DIRECTORY)
               == FILE_ATTRIBUTE_DIRECTORY;
#else
    return exists()
        && (GetFileAttributes(wstringtofilename(getPath()))
            & FILE_ATTRIBUTE_DIRECTORY)
               == FILE_ATTRIBUTE_DIRECTORY;
#endif
}
