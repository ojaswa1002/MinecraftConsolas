...
FileEntry*
FileHeader::AddFile(const std::wstring& name, unsigned int length /* = 0 */) {
...
void FileHeader::ReadHeader(
    ...
    // case 8: // 4J Stu - SAVE_FILE_VERSION_NUMBER 2,3,4,5,6,7,8 are the same,
    // but: 							: Bumped it to 3 in TU5 to force older
    // builds (ie 0062) to generate
    // a new world when trying to load new saves 							:
    // Bumped it to 4 in TU9 to delete versions of The End that were generated
    // in builds prior to TU9 							: Bumped it to 5 in TU9
    // to update the map data that was only using 1 bit to determine dimension
    // : Bumped it to 6 for PS3 v1 to update map data mappings to use larger
    // PlayerUID 							: Bumped it to 7 for Durango v1 to
    // update map data mappings to use string based PlayerUID
    // : Bumped it to 8 for Durango v1 when to save the chunks in a different
    // compressed format
    ...
    // Legacy save versions, with updated code to convert the FileEntrySaveData
    // to the latest version 4J Stu - At time of writing, the tutorial save is
    // V1 so need to keep this for compatibility
    case SAVE_FILE_VERSION_PRE_LAUNCH: {
        // Read the header
        // We can then make headerPosition a FileEntrySaveData pointer and just
        // increment by one up to the number
        unsigned int i = 0;
        while (i < headerSize) {
            FileEntry* entry = new FileEntry();
            // assert(numberOfBytesRead == sizeof(FileEntrySaveData));
...
void FileHeader::AdjustStartOffsets(
    FileEntry* file,
...
FileHeader::getFilesWithPrefix(const std::wstring& prefix) {
    std::vector<FileEntry*>* files = NULL;

    for (unsigned int i = 0; i < fileTable.size(); ++i) {
        if (wcsncmp(fileTable[i]->data.filename, prefix.c_str(), prefix.size())
            == 0) {
            if (files == NULL) {
...
std::vector<FileEntry*>*
FileHeader::getDatFilesWithOnlineID(const PlayerUID& pUID) {
...
#endif
        {
            if (wcsncmp(&filenameOnly[onlineIDStart], onlineIDW, onlineIDSize)
                == 0) {
                if (files == NULL) {
                    files = new std::vector<FileEntry*>();
                }
...
std::vector<FileEntry*>*
FileHeader::getDatFilesWithMacAndUserID(const PlayerUID& pUID) {
    std::vector<FileEntry*>* datFiles = getValidPlayerDatFiles();
    if (datFiles == NULL) return NULL;
