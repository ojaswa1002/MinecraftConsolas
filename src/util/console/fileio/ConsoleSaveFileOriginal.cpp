                ...
                if (decompSize == 0) {
...
#endif
                    {
                        Compression::getCompression()->SetDecompressionType(
                            plat
                        ); // if this save is from another platform, set the
                        ...
                        ); // and then set the decompression back to the local
                           // machine's standard type
                    }
...
FileEntry*
ConsoleSaveFileOriginal::createFile(const ConsoleSavePath& fileName) {
    LockSaveAccess();
    ...
    if ((nNumberOfBytesToWrite & 4095) == 0) {
        if (nNumberOfBytesToWrite > 0) {
            ...
            for (uintptr_t uiCurrentChunk = uiFromEndChunk;
                ...
                // AP - use this to access the virtual memory
                VirtualMove(
                    (void*)(uiCopyStart + nNumberOfBytesToWrite),
                    (void*)uiCopyStart,
                    uiCopyEnd - uiCopyStart
                );
#else
                XMemCpy(
                    (void*)(uiCopyStart + nNumberOfBytesToWrite),
        ...
        while (true) {
            ...
            if ((writeStartOffset + buffer2Size) <= finishEndOfDataOffset) {
                ...
                assert(
                    (writeStartOffset + buffer2Size) <= finishEndOfDataOffset
                );
...
void ConsoleSaveFileOriginal::Flush(bool autosave, bool updateThumbnail) {
    ...
    if (compData == NULL) {
        ...
        Compression::getCompression()
            ->Compress(NULL, &compLength, pvSaveMem, fileSize);
#endif
    ...
    if (compData != NULL) {
...
#else
        Compression::getCompression()
            ->Compress(compData + 8, &compLength, pvSaveMem, fileSize);
#endif
        QueryPerformanceCounter(&qwNewTime);
        ...
        app.DebugPrintf(
            "Save data compressed from %d to %d\n",
            fileSize,
            compLength
        );
#endif
    ...
    ReleaseSaveAccess();
#elif (                                                                        \
    defined __PS3__ || defined __ORBIS__ || defined __PSVITA__                 \
...
void ConsoleSaveFileOriginal::DebugFlushToFile(
    ...
    swprintf(
        fileName,
        XCONTENT_MAX_FILENAME_LENGTH + 1,
...
ConsoleSaveFileOriginal::getFilesWithPrefix(const std::wstring& prefix) {
...
#if defined(__PS3__) || defined(__ORBIS__) || defined(__PSVITA__)
std::wstring
ConsoleSaveFileOriginal::getPlayerDataFilenameForLoad(const PlayerUID& pUID) {
    return header.getPlayerDataFilenameForLoad(pUID);
}
std::wstring
ConsoleSaveFileOriginal::getPlayerDataFilenameForSave(const PlayerUID& pUID) {
...
void ConsoleSaveFileOriginal::ConvertToLocalPlatform() {
    ...
    for (auto it = allFilesInSave->begin(); it < allFilesInSave->end(); ++it) {
        ...
        if (fName.compare(
            ...
            app.DebugPrintf(
                "%ls is not a region file, ignoring\n",
                fName.c_str()
            );
        }
    }
