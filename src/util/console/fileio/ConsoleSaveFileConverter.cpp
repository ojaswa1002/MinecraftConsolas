...
#include "world/level/storage/DirectoryLevelStorage.h"
...
void ConsoleSaveFileConverter::ProcessSimpleFile(
    ...
    sourceSave->readFile(
        sourceFileEntry,
        data,
        sourceFileEntry->getFileSize(),
        &numberOfBytesRead
    );

    // Write back to target
    targetSave->writeFile(
        targetFileEntry,
        data,
        numberOfBytesRead,
...
void ConsoleSaveFileConverter::ConvertSave(
    ConsoleSaveFile*  sourceSave,
    ConsoleSaveFile*  targetSave,
    ...
    if (playerFiles != NULL) {
        for (int fileIdx = 0; fileIdx < playerFiles->size(); fileIdx++) {
            ...
            PlayerUID xuid;
            ProfileManager
                .GetXUID(ProfileManager.GetPrimaryPad(), &xuid, false);
            ConsoleSavePath targetPlayerDatPath(
                L"players/" + xuid.toString() + L".dat"
            );
#else
            ConsoleSavePath targetPlayerDatPath(
                playerFiles->at(fileIdx)->data.filename
    ...
    // Overworld
    {
        ...
        if (progress)
            progress->progressStagePercentage(
                (currentProgress * 100) / progressTarget
            );
        ...
        for (int x = -halfXZSize; x < halfXZSize; ++x) {
            for (int z = -halfXZSize; z < halfXZSize; ++z) {
                ...
                if (dis) {
                    ...
                    DataOutputStream* dos =
                        targetCache
                            ._getChunkDataOutputStream(targetSave, L"", x, z);
    ...
    // Nether
    {
        ...
        for (int x = -halfXZSize; x < halfXZSize; ++x) {
            for (int z = -halfXZSize; z < halfXZSize; ++z) {
                ...
                DataInputStream* dis =
                    sourceCache
                        ._getChunkDataInputStream(sourceSave, L"DIM-1", x, z);
    ...
    // End
    {
        ...
        for (int x = -halfXZSize; x < halfXZSize; ++x) {
            for (int z = -halfXZSize; z < halfXZSize; ++z) {
                ...
                DataInputStream* dis =
                    sourceCache
                        ._getChunkDataInputStream(sourceSave, L"DIM1/", x, z);
...
#ifndef _CONTENT_PACKAGE
                wprintf(
                    L"%s is not a region file, ignoring\n",
                    fe->data.filename
                );
#endif
            }
