...
#include "C4JThread.h"
#ifdef __PSVITA__
#include "psvita/psvitaextras/PSVitaTLSStorage.h"
...
C4JThread::C4JThread(
    ...
    int err = scePthreadAttrSetaffinity(
        &m_threadAttr,
        63
    ); // set the thread affinity to all cores to start with
    assert(err == SCE_OK);
    m_oldAffinityMask = 0;
...
C4JThread::C4JThread(const char* mainThreadName) {
    ...
    int err           = sceKernelChangeThreadCpuAffinityMask(
        m_threadID,
...
#ifdef __PS3__
void C4JThread::entryPoint(std::uint64_t param) {
...
void C4JThread::Run() {
#ifdef __PS3__
    //		prio specifies the priority value of the PPU thread within the range
    // from 0 to 3071 where 0 is the highest.
...
#elif defined __PSVITA__
    int Proc = proc >> 1; // convert from 360's 3 cores * 2 hardware threads to
                          // Vita's 3 cores
...
void C4JThread::Sleep(int millisecs) {
    ...
    } else sys_timer_usleep(millisecs * 1000);
#elif defined __ORBIS__
...
DWORD C4JThread::Event::WaitForSignal(int timeoutMs) {
    ...
    std::uint32_t mode            = SYS_EVENT_FLAG_WAIT_AND;
    ...
    std::uint32_t waitMode = SCE_KERNEL_EVF_WAITMODE_AND;
...
DWORD C4JThread::EventArray::WaitForSingle(int index, int timeoutMs) {
    ...
    if (timeoutMs == INFINITE) timeoutMicrosecs = SYS_NO_TIMEOUT;
    else timeoutMicrosecs = timeoutMs * 1000;
    std::uint32_t mode = SYS_EVENT_FLAG_WAIT_AND;
    if (m_mode == e_modeAutoClear) mode |= SYS_EVENT_FLAG_WAIT_CLEAR;
    ...
    std::uint64_t resultPat;
    int           err = sceKernelWaitEventFlag(
        m_events,
        1 << index,
        waitMode,
        &resultPat,
...
DWORD C4JThread::EventArray::WaitForAny(int timeoutMs) {
    ...
    std::uint32_t mode = SYS_EVENT_FLAG_WAIT_OR;
    if (m_mode == e_modeAutoClear) mode |= SYS_EVENT_FLAG_WAIT_CLEAR;
    ...
    unsigned int bitmask = 0;
    for (int i = 0; i < m_size; i++) bitmask |= (1 << i);
    std::uint32_t waitMode = SCE_KERNEL_EVF_WAITMODE_OR;
    if (m_mode == e_modeAutoClear) {
        waitMode |= SCE_KERNEL_EVF_WAITMODE_CLEAR_PAT;
    }
    int err = sceKernelWaitEventFlag(
